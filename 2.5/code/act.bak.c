...
extern char zone1[MAX_STRING_LENGTH];
extern char zone2[MAX_STRING_LENGTH];
extern char zone3[MAX_STRING_LENGTH];
extern char zone4[MAX_STRING_LENGTH];
  ...
  if (!IS_IMMORTAL(ch) || cmd == 234) {
    ...
    for (d = descriptor_list; d; d = d->next) {
      if (!d->connected && CAN_SEE(ch, d->character) &&
        ...
        sprintf(buf, "[ POLYMORPHED ] %s %s", GET_NAME(person), person->player.title);
      } else if (GET_NAME(d->character)=="Spawn") {
        sprintf(buf, "[ CHEESE  WIZ ] %s %s", GET_NAME(person), person->player.title);
      } else if ((GET_NAME(d->character)=="DaShadow") || 
                 (GET_NAME(d->character)=="Caillin")) {
        sprintf(buf, "[ AMBASSADOR  ] %s %s", GET_NAME(person), person->player.title);
      
      } else if (GetMaxLevel(d->character)==60) {
        if (IS_SET(ch->specials.act, PLR_COLOR)) {
            sprintf(buf, "%s[GRAND  POOBAH]%s %s %s",
                ANSI_RED, ANSI_WHITE, GET_NAME(person), person->player.title);
        } else {
            sprintf(buf, "[GRAND  POOBAH] %s %s",
                GET_NAME(person), person->player.title);
        }
      } else if (GetMaxLevel(d->character)==59) {
        sprintf(buf, "[ SNEEZY LORD ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==58) {
        sprintf(buf, "[ JUNIOR LORD ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==57) {
        sprintf(buf, "[ GREATER GOD ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==56) {
        sprintf(buf, "[     GOD     ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==55) {
        sprintf(buf, "[ LESSER  GOD ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==54) {
        sprintf(buf, "[   DEMIGOD   ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==53) {
        sprintf(buf, "[    SAINT    ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==52) {
        sprintf(buf, "[   CREATOR   ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(d->character)==51) {
        sprintf(buf, "[LOW  IMMORTAL] %s %s", GET_NAME(person), person->player.title);
      } else if (HasClass(d->character,CLASS_ANTIPALADIN)) {
        sprintf(buf, "[ANTIPAL lev%2d] %s %s", person->player.level[4],
GET_NAME(person), person->player.title);
       } else if (HasClass(d->character,CLASS_PALADIN)) {
        sprintf(buf, "[PALADIN lev%2d] %s %s", person->player.level[5],
GET_NAME(person), person->player.title);
       } else if (HasClass(d->character,CLASS_MONK)) {
        sprintf(buf, "[MONK    lev%2d] %s %s", person->player.level[6],
GET_NAME(person), person->player.title);
      } else if (HasClass(d->character,CLASS_RANGER)) {
        sprintf(buf, "[RANGER  lev%2d] %s %s", person->player.level[7],
GET_NAME(person), person->player.title);
       } else {
        sprintf(buf, "[ %-2d %-2d %-2d %-2d ] %s %s", person->player.level[0], person->player.level[1], person->player.level[2], person->player.level[3], GET_NAME(person), person->player.title);
      }
      strcat(buf, "\n\r");
  ...
  send_to_char(buf, ch); 
    ...
    if (strlen(argument) == 0) {
      ...
      if (IS_NPC(person)) {
        sprintf(buf, "[ POLYMORPHED ] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(person)==60) {
        sprintf(buf, "[GRAND  POOBAH] %s %s", GET_NAME(person), person->player.title);
      } else if (GetMaxLevel(person)==59) {
        sprintf(buf, "[GRAND  WIZARD] %s %s", GET_NAME(person), person->player.title);
...
GET_NAME(person), person->player.title);
      ...
      if (arg[0] == '-') {
        if (index(arg,'?') != NULL) {
          send_to_char("[-]i=idle t=title h=hit/mana/move s=stats d=linkdead\n\r",ch);
          send_to_char("[-]g=God o=Mort [1]Mage[2]Cleric[3]War[4]Thief[5]Anti[6]Pala[7]Bard[8]Ranger\n\r", ch);
    send_to_char("--------\n\r", ch);  
        }
        for (person = character_list; person; person = person->next) {
          if (!IS_NPC(person)) {
            ...
            if (index(arg,'5') != NULL) {
              if (!HasClass(person,CLASS_ANTIPALADIN)) skip = TRUE;
            }
            if (index(arg,'6') != NULL) {
            ...
            if (!skip) {
              ...
              if ((person->desc != NULL) || (index(arg,'d') != NULL)) {
                for (l = 1; l <= strlen(arg) ; l++) {
                  ...
                  case 'h': {
                    sprintf(tempbuf,"Hit:[%-4d] Mana:[%-3d] Move:[%-3d] ",GET_HIT(person),GET_MANA(person),GET_MOVE(person));
                    strcat(buf,tempbuf);
                    break;
                  }              
                  case 's': {
...
void do_atlas(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!*name) {
    ...
    if (volume == 1) {
     page_string(ch->desc, zone1, 0);
    } else if (volume == 2) {
      page_string(ch->desc, zone2, 0);
    } else if (volume == 3) {
      page_string(ch->desc, zone3, 0); 
    } else if (volume == 4) {
      page_string(ch->desc, zone4, 0);
    } else {
...
if (GetMaxLevel(ch) > 10) {
  ...
  sprintf(buf, "You have %d items in inventory, upto %d items - %d spare item "
	  "capacity.\n\r",
  ...
  sprintf(buf, 
"Your alignment is %d (-1000 to 1000), and armor is %d (-100 to 100).\n\r",  
	  ch->specials.alignment, ch->points.armor);
  send_to_char(buf,ch);
  
  if (GetMaxLevel(ch) > 9) {
    sprintf(buf,"You have %d STR, %d INT, %d WIS, %d DEX and %d CON.\n\r",
