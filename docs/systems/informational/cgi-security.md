...
status: legacy
description: Browser-based utility security including session management, authentication, and SQL injection prevention
category: informational
keywords: [session, authentication, SQL injection, CSRF, cookies]
primary_symbols:
...
> **LEGACY SYSTEM**: The CGI tools are deprecated and not built by default. The `SNEEZY_BUILD_CGI` CMake option is OFF and marked "deprecated" in CMakeLists.txt. This documentation is preserved for reference if the tools are optionally enabled.
