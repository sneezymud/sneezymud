...
if ($port == 8900) $portname = "sneezybuilder";
else $portname = "sneezy";

        $result = runQuery("select time,players from usagelogs where port=".$port." AND time<".$graphmax." AND time>=".$graphmin." order by time desc",$portname);
...
while($line = pg_fetch_array($result)) {
        $col = ($line["time"] - $graphmin) / $segment;
...
$widthhour = $graphwidth / 7.0;

$plott = array(1,2,3,4,5,6,7);


$nowtime = ($timenow % $week) / $day;

$nowtime = $nowtime + 2.65;
if ($nowtime > 7) $nowtime -= 7.0; 
$lasthour = (int)(($timenow % $week) / $day);
...
foreach($plott as $hour) {
	...
	if ($hour >= 7) $hour -= 7;
	$which = 7-$anchor;
	if ($which == 0) $weekday = "Mon";
	if ($which == 1) $weekday = "Tue";
        if ($which == 2) $weekday = "Wed";
        if ($which == 3) $weekday = "Thu";
        if ($which == 4) $weekday = "Fri";
        if ($which == 5) $weekday = "Sat";
        if ($which == 6) $weekday = "Sun";
	...
	imagestring ($im, 0, $graphwidth - ($hour * $widthhour) + $offset2 - 10, $graphheight + $offset + 4, $weekday, $textcolor);
}
...
imagestring   ($im, 1, $graphwidth/2 + $offset2 - 20, $graphheight + $offset * 1.5 +2, "day of the week", $textcolor);

imagestring   ($im, 2, $offset2,5, "SneezyMUD usage over the past 7 days:", $textcolor);
