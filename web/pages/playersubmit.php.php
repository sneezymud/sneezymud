...
if (("" != $_POST['character']) AND ("" != $_POST['bio'])) {
	...
	$result = runQuery("select id from players order by id desc", "getting ids");
	if ($line = pg_fetch_array($result))
		$id = $line['id'] + 1;
	else
		$id = 0;

	runQuery("insert into players (id, character, title, picture, smallpic, bio, realname, email, webpage, immortal, approved) VALUES (".$id.", '".addslashed($_POST['character'])."' , '', '', '', '".addslashes($_POST['bio'])."', '".addslashes($_POST['realname'])."', '".addslashes($_POST['email'])."', '".addslashes($_POST['webpage'])."', 0, 0)");
