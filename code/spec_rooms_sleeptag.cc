...
int sleepTagControl(TBeing *tBeing, cmdTypeT tCmd, const char *tArg, TRoom *tRoom)
{
  ...
  switch (tCmd) {
    case CMD_SAY:
      tStOrig = two_arg(tStOrig, tStString, tStBuffer);

      if (strcasecmp(tStString.c_str(), "sleeptag"))
        return FALSE;
      ...
      if (!strcasecmp(tStBuffer.c_str(), "help")) {
    ...
    case CMD_UP:
      ...
      for (tThing = tBeing->stuff; tThing; ) {
        ...
        if (tObj && tObj->objVnum() == SLEEPTAG_STAFF) {
          ...
          delete tThing;
          tThing = NULL;
        } else if (tThing->stuff)
          for (tCThing = tThing->stuff; tCThing; ) {
