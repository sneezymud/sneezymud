...
TThing *findPowder(TThing *stuff){
  TThing *tt;
  TTool *powder;
  TThing *ret;
  ...
  for(tt=stuff;tt;tt=tt->nextThing){
    ...
    if(tt && tt->getStuff() && (ret=findPowder(tt->getStuff())))
      return ret;
  }
...
TThing *findShot(TThing *stuff){
  ...
  for(tt=stuff;tt;tt=tt->nextThing){
    if(tt && (Shot=dynamic_cast<TAmmo *>(tt)) &&
       Shot->getAmmoType()==AMMO_LEAD_SHOT)
      return tt;

    if(tt && tt->getStuff() && (ret=findShot(tt->getStuff())))
      return ret;
  }
