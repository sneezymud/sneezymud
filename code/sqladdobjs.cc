...
int main(int argc, char **argv)
{
  ...
  for(unsigned int t=0;t<vnums.size();t++){
    ...
    if(db_immo.fetchRow()){
      ...
      db_beta.query("insert into obj values(%s, '%s', '%s', '%s', '%s', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", db_immo.getColumn(0), db_immo.getColumn(1), db_immo.getColumn(2), db_immo.getColumn(3), db_immo.getColumn(4), db_immo.getColumn(5), db_immo.getColumn(6), db_immo.getColumn(7), db_immo.getColumn(8), db_immo.getColumn(9), db_immo.getColumn(10), db_immo.getColumn(11), db_immo.getColumn(12), db_immo.getColumn(13), db_immo.getColumn(14), db_immo.getColumn(15), db_immo.getColumn(16), db_immo.getColumn(17), db_immo.getColumn(18), db_immo.getColumn(19), db_immo.getColumn(20), db_immo.getColumn(21));
    } else {
      printf("Not found: %i\n", vnums[t]);
    }
