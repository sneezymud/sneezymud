				  ...
				  const sstring &name, int amt, bool new_id)
{
  ...
  queryqueue.push(fmt("insert into shoplogjournal (shop_nr, journal_id, customer_name, obj_name, sneezy_year, logtime, post_ref, debit, credit) values (%i, %q, '%q', '%q', %i, now(), %i, %i, 0)") % shop_nr % (new_id?"NULL":"LAST_INSERT_ID()") % customer % name % time_info.year % post_ref % amt);
}
				  ...
				  const sstring &name, int amt, bool new_id)
{
  ...
  queryqueue.push(fmt("insert into shoplogjournal (shop_nr, journal_id, customer_name, obj_name, sneezy_year, logtime, post_ref, debit, credit)values (%i, %q, '%q', '%q', %i, now(), %i, 0, %i)") % shop_nr % (new_id?"NULL":"LAST_INSERT_ID()") % customer % name % time_info.year % post_ref % amt);
}
...
void TShopOwned::COGS_add(const sstring &name, int amt, int num)
{
  ...
  if(!db.fetchRow()){
    ...
    queryqueue.push(fmt("insert into shoplogcogs (shop_nr, obj_name, count, total_cost) values (%i, '%q', %i, %i)") % shop_nr % name % num % amt);
  } else {
    queryqueue.push(fmt("update shoplogcogs set count=count+%i, total_cost=total_cost+%i where obj_name='%q' and shop_nr=%i") % num % amt % name % shop_nr);
  }
}
...
void TShopOwned::COGS_remove(const sstring &name, int num)
{
  ...
  db.query("select total_cost, count from shoplogcogs where obj_name='%q' and shop_nr=%i", name.c_str(), shop_nr);
  db.fetchRow();

  queryqueue.push(fmt("update shoplogcogs set total_cost=total_cost-(floor((total_cost/count))*%i), count=count-%i where obj_name='%q' and shop_nr=%i") % num % num % name % shop_nr);
}
