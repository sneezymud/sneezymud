...
void TBaseWeapon::setWeapType(int n)
{
  weaponType = n;
}

int TBaseWeapon::getWeapType(int n)
{
  return weaponType;
}

void TBaseWeapon::setWeapQual(int n)
{
  weaponQual = n;
}

int TBaseWeapon::getWeapQual(int n)
{
  return weaponQual;
}
...
void TBaseWeapon::assignFourValues(int x1, int x2, int x3, int)
{
  ...
  setWeapDamLvl(GET_BITS(x2, 15, 8));
  setWeapDamDev(GET_BITS(x3, 15, 8));
  setWeapType(GET_BITS(x2, 7, 8));
  setWeapQual(GET_BITS(x3, 7, 8));
}
...
void TBaseWeapon::getFourValues(int *x1, int *x2, int *x3, int *x4) const
{
  ...
  SET_BITS(y, 15, 8, getWeapDamLvl());
  SET_BITS(y, 7, 8, getWeapType());  
  SET_BITS(z, 15, 8, getWeapDamDev());
  SET_BITS(z, 7, 8, getWeapQual());
...
string TBaseWeapon::displayFourValues()
{
  ...
  sprintf(tString + strlen(tString),
	  "New weap values: Weapon type: %d Weapon quality: %d\n\r",
	  getWeapType(), getWeapQual());
