...
int shops[] = {0, 2, 4, 5, 9, 14, 36, 44, 73, 84, 85, 86, 87, 89, 90, 91, 92,
  93, 94, 95, 150, 160, 161, 184, 186, 250, -1};
...
void check_cash_balance() {
  ...
  for (int i = 0; shops[i] != -1; ++i) {
    db.query(
      "select sum(debit-credit) as sum from shoplogjournal where shop_nr=%i "
      "and post_ref=100",
      shops[i]);
    db.fetchRow();
    ...
    db.query(
      "select r.name as name from room r, shop s where s.shop_nr=%i and "
      "r.vnum=s.in_room",
      shops[i]);
    db.fetchRow();
...
void check_inventory_cogs() {
  ...
  for (int i = 0; shops[i] != -1; ++i) {
    ...
    db.query(
      "select sum(total_cost) as sum from shoplogcogs where shop_nr=%i and "
      "count>0",
      shops[i]);
    db.fetchRow();
...
void check_cogs_count() {
  ...
  for (int i = 0; shops[i] != -1; ++i) {
    ...
    db.query(
      "select count(*) as count from shoplogcogs where count < 0 and "
      "shop_nr=%i",
      shops[i]);
    db.fetchRow();
    int itemcount = convertTo<int>(db["count"]);
    db.query("select sum(count) as count from shoplogcogs where shop_nr=%i",
      shops[i]);
    db.fetchRow();
...
void check_cogs_detail(int cogs_detail) {
  ...
  db.query(
    "select obj_name, count from shoplogcogs where count < 0 and shop_nr=%i",
    cogs_detail);
  ...
  db.query(
    "select obj_name, sum(count) as count from shoplogcogs where shop_nr=%i "
    "group by obj_name",
    cogs_detail);
  while (db.fetchRow()) {
  ...
  db.query(
    "select o.short_desc as name, count(*) as count from obj o, rent r where "
    "owner_type='shop' and owner=%i and r.vnum=o.vnum group by o.vnum",
    cogs_detail);
  while (db.fetchRow()) {
  ...
  for (map<sstring, int>::iterator iter = items.begin(); iter != items.end();
       ++iter) {
    if ((*iter).second != 0)
...
int main(int argc, char* argv[]) {
  ...
  cmdline.add_options()("help", "produce help message")("all_checks,a",
    po::value<bool>(&all_checks)->zero_tokens(),
    "run all checks")("cash_balance,c",
    po::value<bool>(&cash_balance)->zero_tokens(), "check cash balances")(
    "inventory_cogs,i", po::value<bool>(&inventory_cogs)->zero_tokens(),
    "check inventory/COGS balances")("balance_sheet,b",
    po::value<bool>(&balance_sheet)->zero_tokens(), "check balance sheet")(
    "cogs_count,g", po::value<bool>(&cogs_count)->zero_tokens(),
    "check inventory/COGS item count")("cogs_detail",
    po::value<int>(&cogs_detail)->default_value(-1),
