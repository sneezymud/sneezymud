...
bool TObj::canBeMailed(sstring name) const {
  bool canMail = !isObjStat(ITEM_ATTACHED) && !isObjStat(ITEM_NORENT) &&
                 ...
                 (!dynamic_cast<const TOpenContainer*>(this) ||
                   !dynamic_cast<const TOpenContainer*>(this)->isContainerFlag(
                     CONT_TRAPPED));
...
void postmasterValue(TBeing* ch, TBeing* postmaster, const char* arg) {
  ...
  if (is_abbrev(item, sstring("talens")) ||
      is_abbrev(talen, sstring("talens"))) {
    postmaster->doTell(fname(ch->name),
      format("When sending talens, I charge an additional %d talens for "
             "handling fees.") %
        int((float)STAMP_PRICE * profit_buy * 2));
    return;
  }

  if (item == "faction") {
    postmaster->doTell(fname(ch->name),
      format("Bulk faction mail from me will cost about %d talens.") %
        int((float)FACTION_STAMP_PRICE * profit_buy));
    return;
  }
  ...
  if (item.length() > 0) {
    ...
    if (obj->isMonogrammed()) {
      postmaster->doTell(fname(ch->name),
        "This item appears to be monogrammed.  You may only mail it to its "
        "owner.");
      cost = STAMP_PRICE;
    }
    if (!obj->canBeMailed("")) {
    ...
    postmaster->doTell(fname(ch->name),
      format("Shipping %s will cost you %d talens.") % obj->getName() % cost);
    return;
  }

  postmaster->doTell(fname(ch->name),
    format("My price for regular postage is %d talens.") %
      int((float)STAMP_PRICE * profit_buy));
}
...
int postmasterGiven(TBeing* ch, TMonster* me, TObj* o) {
  ...
  if (startTag == sstring::npos) {
    me->doTell(ch->getName(),
      "Are you sure this is your token? It doesn't look right...");
    me->doGive(ch, o);
    return 0;
  }
  ...
  // raw data
  sstring fullToken =
    objName.substr(startTag, endTag - startTag + 1);  // includes [] brackets
  sstring deathDate = objName.substr(startTag + playerTag.length(),
    dueTag - startTag - playerTag.length());
  ...
  if (dueTime == 0) {
    ...
    objName.replace(dueTag + 1, endTag - dueTag - 1, dueDate.c_str(),
      dueDate.length());
    o->name = objName;

    me->doTell(ch->getName(),
      "It appears your belongings are arriving via magical transport in ten "
      "minutes.");
    me->doTell(ch->getName(),
      "In the meantime, hold on to this token and give it to me when your "
      "package has arrived.");
    me->doGive(ch, o);
    ch->doQueueSave();
    ...
    me->doTell(ch->getName(),
      "Sorry, your object hasn't materialized from voidspace yet.");
    me->doTell(ch->getName(), format("It looks like it won't be ready for "
                                     "another %i minutes and %i seconds.") %
                                minutes % seconds);
    me->doGive(ch, o);
    ...
    if (!linkbag) {
      me->doTell(ch->getName(),
        "Well that's strange; you don't have a linkbag.  Use the 'report' "
        "command for help.");
      me->doGive(ch, o);
      ch->doQueueSave();
      return 0;
    ...
    *(ch->roomp) += *linkbag;
    ...
    me->doTell(ch->getName(),
      "I'll just drop them here in the room.  Please take out your items and "
      "clean up.");
    ch->doQueueSave();
...
void TBeing::postmasterSendMail(const char* arg, TMonster* me) {
  ...
  if (sendFaction) {
    ...
    if (talen_amt + amt > getMoney()) {
      me->doTell(fname(name),
        format("Mailing %d talens plus a stamp costs a total of %d talens.") %
          talen_amt % (talen_amt + amt));
      me->doTell(fname(name), "...which I see you can't afford.");
      return;
    }
    ...
    // calculate mailing price (monogrammed items are free+letter to mail to
    // owner)
    ...
    if (amt > getMoney() && !imm) {
      me->doTell(fname(name),
        format("Mailing this item plus a stamp costs a total of %d talens.") %
          amt);
      me->doTell(fname(name), "...which I see you can't afford.");
      return;
    }
  ...
  if (!imm) {
    ...
    tso.doBuyTransaction(amt, format("mailing %s") % recipient,
      TX_BUYING_SERVICE);
  } else if (isImmortal()) {
    me->doTell(fname(name),
      "Since you're high and mighty, I'll waive the fee.");
  } else {
    me->doTell(fname(name),
      "Since you're mailing an immortal, I'll waive the fee.");
  }

  if (!desc->m_bIsClient) {
    me->doTell(fname(name),
      "Write your message, use ~ when done, or ` to cancel.");
...
void TBeing::postmasterReceiveMail(TMonster* me) {
  ...
  while (has_mail(recipient)) {
    ...
    if (robj < 0 || robj >= (signed int)obj_index.size()) {
      vlogf(LOG_BUG,
        format("postmasterReceiveMail(): No object (%d) in database!") %
          Obj::GENERIC_NOTE);
      return;
    }
    ...
    if (note->action_description.empty())
      note->action_description =
        "Mail system buggy, please report!!  Error #8.\n\r";
