...
// Check if a being's race matches the current room's sector type (home turf)
bool TBeing::homeTurf() const {
  if (!roomp)
    return FALSE;

  race_t myRace = getRace();
  sectorTypeT sector = roomp->getSectorType();

  // Define race-to-sector mappings
  switch (myRace) {
    case RACE_HUMAN:
...
bool TBeing::backgroundBonus() const {
  ...
  territoryT background = player.hometerrain;
  sectorTypeT sector = roomp->getSectorType();
  ...
  if (background == HOME_TER_HUMAN_HILL || background == HOME_TER_DWARF_HILL ||
      background == HOME_TER_HOBBIT_HILL || background == HOME_TER_GNOME_HILL ||
