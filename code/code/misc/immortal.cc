...
togEntry* togInfoT::operator[](const togTypeT i) { return toggles.at(i); }
...
togInfoT::togInfoT() : toggles(MAX_TOG_TYPES + 1, nullptr), loaded(false) {
  loaded = false;
}
...
void togInfoT::loadToggles() {
  ...
  db.query(
    "select tog_id, toggle, testcode, name, descr from globaltoggles order by "
    "name");
...
void TBeing::doChange(const char* argument) { change_hands(this, argument); }
  ...
  if (*buf && *buf2 && hasWizPower(POWER_CHANGE)) {
    if (isdigit(*buf2)) {
      if ((new_lev = convertTo<int>(buf2)) > MAX_IMMORT) {
        sendTo(
          format("The new level can't be greater than %d.\n\r") % MAX_IMMORT);
        return;
      }
    } else {
    ...
    sendTo(format("\n\r%s : Min Level : %d\n\r") % commandArray[cmd]->name %
           commandArray[cmd]->minLevel);
  } else {
    sendTo("Syntax : change <command name> <new level>\n\r");
    return;
  }
    ...
    if ((tch = get_char_room_vis(this, argument)) != 0) {
      if (tch->isImmortal() && isImmortal()) {
        switch (::number(1, 3)) {
          case 1:
            mess =
              format("Time stops for a moment as %s and %s high five.\n\r") %
              name % tch->name;
            break;
          case 2:
            mess = format(
                     "Thunder booms and lightning streaks across the heavens "
                     "as %s and %s high five.\n\r") %
                   name % tch->name;
            break;
          case 3:
...
void TBeing::doWizlock(const char* argument) {
  ...
  if (!*buf) {
    sendTo(
      "Wizlock {all | off | add <host> | rem <host> | list  | message}\n\r");
    sendTo(
      format("Global wizlock is presently %s.\n\r") % (WizLock ? "ON" : "OFF"));
  ...
  if (!strcmp(buf, "all")) {
    ...
    vlogf(LOG_MISC, format("%s has added host %s to the access denied list.") %
                      getName() % hostlist[numberhosts]);
    numberhosts++;
    return;
    ...
    if (note) {
      ...
      sendTo(
        "You need a note with what you want the message to be in you "
        "inventory.\n\r");
      return;
    }
  } else {
  ...
  if (argument.empty())
    ...
    for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
      ...
      if (ch->desc && ch->desc->ignored.isIgnored(desc))
        continue;
      if (roomp && ch->desc && (ch->canSee(this)) && ch->awake() &&
...
void TBeing::doFlag(const char* argument) {
  ...
  if (!(victim = get_pc_world(this, buf, EXACT_YES)) &&
    ...
    if ((victim->hasQuestBit(TOG_AVENGER_HUNTING) ||
      ...
      sendTo(
        "Doing this will allow them to get help on the quest and is probably a "
        "bad idea.\n\r");
#if 0
    ...
    if (IS_SET(victim->desc->account->flags, TAccount::IMMORTAL)) {
      ...
      act("$N is no longer flagged as an immortal.", false, this, 0, victim,
        TO_CHAR);
      victim->doSave(SILENT_YES);
    } else
      sendTo(
        "This flag is setup Automatically.  You Can Not simply add it.\n\r");
  } else {
    sendTo(
      "Syntax: flag <player> <\"banished\" | \"killable\" | \"solo\" | "
      "\"group\" | \"newbiehelper\" | \"nosnoop\" | \"double\" | \"triple\" | "
      "\"faction\">\n\r");
    return;
  }
}
...
void TBeing::doTrans(const char*) { sendTo("Mobs can't trans.\n\r"); }
    ...
    if (!(victim = get_pc_world(this, buf, EXACT_YES)) &&
      ...
      if (victim->hasWizPower(POWER_WIZARD) && victim->isPc()) {
        ...
        victim->sendTo(COLOR_MOBS,
          format("%s just tried to transfer you!\n\r") % getName());
        return;
      }
      if (!victim->isPc() && victim->number == -1 &&
          !hasWizPower(POWER_MEDIT_LOAD_ANYWHERE)) {
        sendTo(
          "You lack the power to load anywhere, therefore you can't transfer "
          "MEdit Mobs.\n\r");
        return;
      }
      if (!limitPowerCheck(CMD_TRANSFER,
            (victim->isPc()) ? -1 : victim->number)) {
        sendTo("You are not allowed to transfer that being.\n\r");
        return;
      }
      // used to track down: XXX trans'd me outta inn and got me killed!
      vlogf(LOG_SILENT, format("%s transferring %s from %d to %d.") %
...
int TBeing::doAt(const char* argument, bool isFarlook) {
  ...
  if (isSpammyRoom(location)) {
    sstring tStArgument(com_buf), tStString(""), tStBuffer("");
    ...
    if (!is_abbrev(tStString, "yes")) {
      sendTo(
        "That room, or the creature's room you chose, is a particular "
        "room.\n\r");
  ...
  if (buf.empty()) {
    ...
    if (desc->blockastart)
      sendTo(format("%6d - %6d: Your Primary Room Block\n\r") %
             desc->blockastart % desc->blockaend);

    if (desc->blockbstart)
      sendTo(format("%6d - %6d: Your Secondary Room Block\n\r") %
             desc->blockbstart % desc->blockbend);
  ...
  if (!*arg) {
    ...
    if (force || isdigit(*arg)) {
      ...
      if (!timeTill)
        ...
        sendTo(
          format("A shutdown has already been scheduled for %d minutes.\n\r") %
      ...
      sprintf(buf,
        "<r>******* SYSTEM MESSAGE *******\n\r%s in %d minute%s by "
        "%s.<z>\n\r<c>Use the TIME command at any point to see time until "
        "%s.<z>\n\r",
        shutdown_or_reboot().c_str(), num, (num == 1 ? "" : "s"),
        getName().c_str(), shutdown_or_reboot().c_str());
      Descriptor::worldSend(buf, this);
      ...
      sprintf(buf, "<r>System %s aborted by %s.<z>\n\r",
        shutdown_or_reboot().c_str(), getName().c_str());
      Descriptor::worldSend(buf, this);
      timeTill = 0L;
    } else {
      sendTo("Syntax : shutdown <minutes until shutdown>\n\r");
...
void TBeing::doSnoop(const char*) { sendTo("You're a mob.  No snooping.\n\r"); }
  ...
  if (victim == this) {
    ...
    if (desc->snoop.snooping) {
      if (desc->snoop.snooping->desc)
        ...
        vlogf(LOG_MISC,
          format("Caught %s snooping %s who didn't have a descriptor!") % name %
            desc->snoop.snooping->name);
...
void TBeing::doSwitch(const char*) {
  sendTo(
    "You're already a mob.  You'll have to return before you can switch.\n\r");
}

void TPerson::doSwitch(const char* argument) {
  sstring tStMobile(""), tStBuffer(""), tStArg(argument);
  TBeing* tBeing;
...
void TBeing::makeLimbTransformed(TBeing* victim, wearSlotT limb, bool paired) {
  ...
  if (paired) {
    switch (limb) {
      ...
      default:
        sendTo(
          "That limb is currently not supported for paired "
          "transformation.\n\r");
...
void TBeing::transformLimbsBack(const char* buffer, wearSlotT limb, bool cmd) {
  ...
  if (!limb) {
    ...
    if (!hasTransformedLimb() && !affectedBySpell(AFFECT_TRANSFORMED_HANDS) &&
    ...
    if (!isLimbFlags(limb, PART_TRANSFORMED) && !affectedBySpell(spell)) {
...
void TBeing::doReturn(const sstring& argument, wearSlotT limb, bool tell,
  ...
  // as that doesn't swap any stats/spell effects/discs and is supposed to
  // happen invisibly.
...
void TBeing::doForce(const char*) { sendTo("Mobs can't force.\n\r"); }

void TPerson::doForce(const char* argument) {
  Descriptor* i;
  TBeing* vict;
  char name_buf[MAX_INPUT_LENGTH], to_force[MAX_INPUT_LENGTH],
    buf[MAX_INPUT_LENGTH * 2];
  int rc;
  TThing* t;
  ...
  if (!*name_buf || !*to_force)
  ...
  else if (strcmp("all", name_buf) && strcmp("mobs", name_buf) &&
    ...
    // force all
    vlogf(LOG_MISC,
      format("%s just forced all to '%s'") % getName() % to_force);
    for (i = descriptor_list; i; i = i->next) {
      if ((vict = i->character) && (i->character != this) && !i->connected) {
        if ((GetMaxLevel() <= vict->GetMaxLevel()) &&
            dynamic_cast<TPerson*>(vict))
          continue;
        else {
    ...
    for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
      ...
      TBeing* tbt = dynamic_cast<TBeing*>(t);
      if (tbt) {
        if ((GetMaxLevel() <= tbt->GetMaxLevel()) &&
            dynamic_cast<TPerson*>(tbt))
          continue;
        else {
          sprintf(buf, "$n has forced you to '%s'.\n\r", to_force);
...
void TPerson::doDistribute(const sstring& argument) {
  ...
  if (vnum == 0 || chance == 0) {
    ...
    vlogf(LOG_MISC, format("%s distributing vnum %d to mobs, with chance %d") %
                      getName() % vnum % chance);
    ...
    vlogf(LOG_MISC,
      format(
        "%s distributed %d copies of item vnum %d to mobs, with chance %d") %
        getName() % count % vnum % chance);
    sendTo(
      format(
        "%s distributed %d copies of item vnum %d to mobs, with chance %d") %
      getName() % count % vnum % chance);
...
void TBeing::doLoad(const char*) { sendTo("You're a mob.  Go away.\n\r"); }
  ...
  if (is_abbrev(type, "mobile")) {
    ...
    if (mob_index[numx].spec == SPEC_SHOPKEEPER) {
      ...
      if (mob_index[numx].getNumber() > 0) {
        sendTo(format("Trying to create that mob when one already exists could "
                      "create problems (%s).\n\r") %
               mob_index[numx].name);
        return;
      }
    ...
    while (count--) {
      if ((obj_index[numx].getNumber() >= obj_index[numx].max_exist) &&
        ...
        sendTo(
          "Sorry, all of those items are presently in use in the game.\n\r");
        sendTo(
          "You are unable to load extraneous artifacts at your level.\n\r");
        return;
      }
    ...
    sendTo(
      "Usage: load (object|mobile|set) (number|name)\n\r       load room start "
      "[end]\n\r");
}
...
static void purge_one_room(int rnum, TRoom* rp, int* range) {
  ...
  for (StuffIter it = rp->stuff.begin(); it != rp->stuff.end();) {
    ...
    TBeing* tbt = dynamic_cast<TBeing*>(t);
    if (tbt) {
      tbt->sendTo(format("A god strikes the heavens making the %s around you "
                         "erupt into a\n\r") %
                  rp->describeGround());
      tbt->sendTo(
        "fluid fountain boiling into the ether.  All that's left is the "
        "Void.\n\r");
      tbt->doLook("", CMD_LOOK);
    }
  }
...
void TBeing::doCutlink(const char*) { return; }
...
void TMonster::purgeMe(TBeing* ch) {
  ...
  if (isShopkeeper() || spec == SPEC_SHARPENER || spec == SPEC_REPAIRMAN ||
      spec == SPEC_ATTUNER) {
    if (ch)
      ch->sendTo(
        "Boy that was close.\n\rBe glad Brutius put in a catch so you didn't "
        "purge the shopkeeper.\n\r");
    return;
  }
  delete this;
}
...
void TBeing::doPurge(const char*) { return; }
...
void TPerson::doPurge(const char* argument) {
  ...
  if (*name_buf) {  // argument supplied. destroy single object or char
    if (!strcmp(name_buf, "links") && hasWizPower(POWER_PURGE_LINKS)) {
      ...
      sendTo(
        "If there REALLY is a reason to kick linkdeads out (e.g wizlock),\n\r");
      ...
      if (is_abbrev(argument, "all")) {
        ...
        for (zone = 1; zone < zone_table.size(); zone++) {
          if (zone_table[zone].zone_value > 0 && zone_table[zone].isEmpty()) {
        ...
        sendTo(format("All excess mobs nuked.  %d mobs destroyed, %d creatures "
                      "now.\n\r") %
               (oldnum - mobCount) % mobCount);
        return;
      }
    ...
    if ((vict = get_char_room_vis(this, name_buf))) {
      if (vict->isShopkeeper()) {  // shopkeeper
        ...
        vlogf(LOG_MISC,
          format("%s just tried to purge a shopkeeper.") % getName());
        return;
      }
      if (vict->isPc() &&
      ...
      if (dynamic_cast<TMonster*>(vict) &&
        ...
        sendTo(
          "Sorry, you can't directly purge a poly'ed mob, you made them "
          "return.\n\r");
...
static void welcomeNewPlayer(const TPerson* ch) {
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (tbt->isPlayerAction(PLR_NEWBIEHELP)) {
      tbt->sendTo(COLOR_BASIC,
        format(
          "<c>Attention Newbie-Helpers: Please welcome %s to <h>!!!<1>\n\r") %
          ch->getName());
      if (count == 1)
        tbt->sendTo(COLOR_BASIC,
          format("<c>%s is the first character in %s account!<1>\n\r") %
            ch->getName() % ch->hshr());
    }
  }
}
...
void TPerson::doStart() {
  ...
  TObj* obj;
  char buf[256];
  // these tattoos are mostly used for Orcs, and should reflect 'badass
  // meanness'
  ...
  static unsigned int prompts[MAX_CLASSES] = {PROMPT_MANA, PROMPT_PIETY, 0, 0,
    PROMPT_LIFEFORCE, PROMPT_PIETY, PROMPT_MANA, 0, 0};
  ...
  desc->plr_color = PLR_COLOR_BASIC | PLR_COLOR_ALL | PLR_COLOR_BASIC |
                    PLR_COLOR_COMM | PLR_COLOR_OBJECTS | PLR_COLOR_MOBS |
  ...
  desc->prompt_d.type =
    desc->prompt_d.type & (PROMPT_CLASSIC_ANSIBAR | PROMPT_CLIENT_PROMPT);
  if (!desc->m_bIsClient) {
  ...
  if ((GetMaxLevel() > MAX_MORT) && !isPlayerAction(PLR_IMMORTAL)) {
    ...
    if (!desc->m_bIsClient)
      sendTo(
        "Setting various autobits with recommended immortal "
        "configuration.\n\r");
    setMoney(100000);
    ...
    if (!desc->m_bIsClient) {
      sendTo(
        "Setting various autobits with recommended newbie configuration.\n\r");
...
#endif
  // give a weapon
  if (hasClass(CLASS_WARRIOR) || hasClass(CLASS_RANGER) ||
  ...
  if (!desc->m_bIsClient) {
    ...
    sendTo(
      format("--> See %sHELP WEAR%s for more details.\n\r") % cyan() % norm());
    sendTo(format("Be sure to read your %snewbie guide%s...\n\r") % green() %
           norm());
    sendTo(
      "If you are feeling lost, read HELP GOTO for some quick directions.\n\r");
  }
  ...
  if (hasClass(CLASS_SHAMAN)) {
    ...
    sendTo(
      format(
        "%s**%s\tThe class you are about to play has been designed with\n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tthe most advanced of players in mind. The class is not\n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tan easy one to play in the least. Please be sure, if \n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tyou decide to dedicate time to a Shaman character, that\n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tyou can deal with playing an advanced class that would\n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tbe by most considered impossible or unplayable. Please\n\r") %
      red() % norm());
    sendTo(
      format(
        "%s**%s\tfamiliarize yourself with the help files pertaining to\n\r") %
      red() % norm());
    sendTo(format("%s**%s\tthe Shaman class before undertaking this "
                  "challenge.\n\r%s**%s\n\r") %
           red() % norm() % red() % norm());
  ...
  if (partial == RESTORE_FULL) {
    ...
    for (aff = affected; aff; aff = aff2) {
      ...
      if (aff->type == AFFECT_DISEASE) {
        diseaseStop(aff);
        affectRemove(aff);
      } else if ((aff->type == SKILL_QUIV_PALM || aff->type == SKILL_SMITE)) {
...
void TBeing::doRestore(const char* argument) {
  ...
  if (!(victim->GetMaxLevel() > MAX_MORT) &&
      !hasWizPower(POWER_RESTORE_MORTAL)) {
  ...
  if (is_abbrev(arg2, "partial"))
    ...
    pracs =
      min(pracs, (int)victim->getLevel(victim->bestClass()));  // cap at 1/lev
    victim->practices.prac[victim->bestClass()] += pracs;

    victim->sendTo(
      format("Congratulations, you've been reimbursed %d practices!\n\r") %
      pracs);
    sendTo(
      format("Reimbursing %s %d practices.\n\r") % victim->getName() % pracs);
  ...
  for (door = MIN_DIR; door < MAX_DIR; door++) {
    ...
    if ((exitdata = exitDir(door)) != NULL) {
      if (!(rp = real_roomp(exitdata->to_room))) {
        ...
        if (rp->pitchBlackDark() && !rp->isRoomFlag(ROOM_ALWAYS_LIT) &&
...
void TBeing::doWipe(const char* argument) {
  ...
  if (victim) {
    ...
    sprintf(buf + strlen(buf),
      "%s cackles in triumph as he utterly annihilates %s.\n\r",
      getName().c_str(), victim->getName().c_str());
    Descriptor::worldSend(buf, this);
...
void TPerson::doAccess(const sstring& arg) {
  ...
  static const char* access_args[] = {"passwd", "name", "description", "title",
    "level", "\n"};
  ...
  if (!arg2.empty()) {
    if (get_pc_world(this, arg1, EXACT_YES)) {
      sendTo(
        "That player is online. Better not mess with the player file.\n\r");
      return;
    }
    ...
    if ((which = old_search_block(arg2.c_str(), 0, arg2.length(), access_args,
           0)) == -1) {
    ...
    switch (which) {
      case 1:
        ...
        *(pass + 10) = '\0';
        ...
        vlogf(LOG_MISC,
          format("%s changed password on %s account") % getName() % st.aname);
        return;
      case 5:
        if (sscanf(arg.c_str(), "%d %d", &lev, &Class) != 2) {
          sendTo(
            "Syntax : access <player> level <newlevel> <class number>.\n\r");
          return;
        }
    ...
    tmpbuf = format(
               "Name : <p>%s<1>, Sex : <c>%d<1>, Screensize : <c>%d<1>, Weight "
               "<c>%.2f<1>, Height <c>%d<1>\n\r") %
             st.name % (int)st.sex % (int)st.screen % (int)st.weight %
             (int)st.height;
    buf += tmpbuf;
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    tmpbuf =
      format(
        "Last login : %s%s%s, Last Host : %s%s%s\n\rFirst Login : %s%s%s\n\r") %
      green() % tmstr % norm() % green() % st.lastHost % norm() % cyan() %
      birth_buf % norm();
    buf += tmpbuf;

    tmpbuf = format("Playing time : %d days, %d hours.\n\r") %
             (int)playing_time.day % (int)playing_time.hours;
    buf += tmpbuf;

    tmpbuf = format("User Levels: M%d C%d W%d T%d A%d D%d K%d R%d") %
             (int)st.level[0] % (int)st.level[1] % (int)st.level[2] %
             (int)st.level[3] % (int)st.level[4] % (int)st.level[5] %
             (int)st.level[6] % (int)st.level[7];
    buf += tmpbuf;
    ...
    buf +=
      "Stats  "
      ":[Str][Bra][Con][Dex][Agi][Int][Wis][Foc][Per][Cha][Kar][Spe]\n\r";

    tmpbuf = format(
               "Chosen : %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d "
               " %3d\n\r") %
             st.stats[STAT_STR] % st.stats[STAT_BRA] % st.stats[STAT_CON] %
             st.stats[STAT_DEX] % st.stats[STAT_AGI] % st.stats[STAT_INT] %
             st.stats[STAT_WIS] % st.stats[STAT_FOC] % st.stats[STAT_PER] %
             st.stats[STAT_CHA] % st.stats[STAT_KAR] % st.stats[STAT_SPE];
    buf += tmpbuf;

    db.query(
      "select sum(sob.talens) as bankmoney from shopownedbank sob, player p "
      "where sob.player_id=p.id and lower(p.name)=lower('%s')",
      st.name);
    ...
    if (!account.read(st.aname)) {
      ...
      if ((account.flags & TAccount::IMMORTAL) &&
        ...
        buf +=
          "Account flagged immortal.  Remaining Information Restricted.\n\r";
      } else {
        tmpbuf =
          format("Account name: %s%s%s, Account email address : %s%s%s\n\r") %
          cyan() % account.name % norm() % cyan() % account.email % norm();
        buf += tmpbuf;
...
void TBeing::doReplace(const sstring& argument) {
  ...
  if (is_abbrev(arg3, "today"))
    ...
    sendTo(
      "Syntax : replace <playername | accountname> <player | rent | account> "
      "<today | yesterday>\n\r");
    return;
  }
  ...
  if (get_pc_world(this, arg1, EXACT_YES)) {
    sendTo(
      "Probably don't want to backup with the person on. Ask them to log "
      "off.\n\r");
    return;
  }
  ...
  if (!(fp = fopen(buf, "r"))) {
    ...
    vlogf(LOG_FILE,
      format("%s replacing %s's %s file.") % getName() % arg1 % dir2);

    sprintf(buf, "cp -r %s/%s/%c/%s %s/%c/%s", dir, dir2, arg1[0], arg1.c_str(),
      dir2, arg1[0], arg1.c_str());
    vsystem(buf);
    ...
    if (!dontMove) {
      sendTo(
        format("Copying backup %s file from %s/%s/%c/%s to %s/%c/%s.\n\r") %
...
void TBeing::doSetsev(const char* arg) {
  ...
  char tString[512];
  Descriptor* d;
  const char* tFields[] = {"miscellaneous", "low", "file", "bug", "procedure",
    "player", "immortal", "client", "combat", "cheat", "faction", "mobile",
    "mobai", "mobresp", "object", "editor", "\n"};
  const sstring tHelp[] = {"Anything not found in the others", "L.O.W Errors",
    "File I/O Errors", "Generic Bugs", "Mobile/Object/Room Procedures",
    "Player Login/Logouts", "Immortal Info", "Client Bugs/Info", "Combat Bugs",
    "Cheat Reports/Notifications", "Faction Bugs", "Generic Mobile Bugs",
    "Mobile AI Bugs", "Mobile Response Script Bugs", "Generic Object Bugs",
    "Editor Bugs"};
  ...
  if (tMatch < 1 || tMatch > LOG_MAX) {
    if (is_abbrev(arg, "batopr") && getName() == "Batopr") {
      ...
      if ((d->severity & (1 << LOG_MAROR)))
        d->severity &= ~(1 << LOG_MAROR);
      else
        d->severity |= (1 << LOG_MAROR);
      ...
      if ((d->severity & (1 << LOG_ANGUS)))
        d->severity &= ~(1 << LOG_ANGUS);
      else
        d->severity |= (1 << LOG_ANGUS);
...
void TBeing::doInfo(const char* arg) {
  ...
  str +=
    "unlinked    : how many objects are in the obj list but not in the "
    "mud.\n\r";
  ...
  if (!*arg1) {
    ...
    if (is_abbrev(arg1, "commodity")) {
      ...
      db.query("select shop_nr from shoptype where type=%i", ITEM_RAW_MATERIAL);
      int total = 0;
      sstring name = arg2;
      ...
      while (db.fetchRow()) {
        ...
        db2.query(
          "select r.rent_id as rent_id from rent r, rent_strung rs, obj o "
          "where owner_type='shop' and owner=%i and r.rent_id=rs.rent_id and "
          "o.vnum=r.vnum and o.type=%i and rs.name like '%s%s%s'",
          shop_nr, ITEM_RAW_MATERIAL, "%", arg2, "%");

        while (db2.fetchRow()) {
          if (!tso.getKeeper())
            continue;
          TObj* t =
            tso.getKeeper()->loadItem(shop_nr, convertTo<int>(db2["rent_id"]));
          *tso.getKeeper() += *t;

          if ((commod = dynamic_cast<TCommodity*>(t)) &&
              isname(arg2, commod->name)) {
            sendTo(
              format("Shop %i: %i units of %s at %i talens per unit.\n\r") %
      ...
      sendTo(
        format("  Wiznews file accessed %d times.\n\r") % wiznews_used_num);
      sendTo(
        format("  Wizlist file accessed %d times.\n\r") % wizlist_used_num);
      ...
      while (1) {
        ...
        buf =
          format("%s[%6i] %-17s\n\r") % buf % count[li] % ItemInfo[li]->name;
        total += count[li];
        count[li] = -1;
      }
      ...
      for (TObjIter iter = object_list.begin(); iter != object_list.end();
           ++iter) {
        obj = *iter;
        if (obj->in_room == Room::NOWHERE && !obj->parent && !obj->equippedBy &&
            !obj->stuckIn && !obj->riding) {
          tbuf = format("[%6i] %-17s\n\r") % i++ % obj->getNameNOC(this);
          buf += tbuf;
        }
      }
      ...
      for (tb = character_list; tb; tb = tb->next) {
        if (tb->in_room == Room::NOWHERE && !tb->parent && !tb->equippedBy &&
            !tb->stuckIn && !tb->riding) {
          tbuf = format("[%6i] %-17s\n\r") % i++ % tb->getNameNOC(this);
          buf += tbuf;
        }
      }
      ...
      for (i = descriptor_list; i; i = i->next) {
        if (i->character) {
          ...
          strcat(buf2,
            ((!i->character->name.empty()) ? i->character->getName().c_str()
                                           : "unknown"));
          if (!i->connected)
      ...
      sendTo(
        format("  Average faction_power is: %.4f\n\r") % avg_faction_power);
      ...
      sendTo(
        format("  Beings with initialized disciplines: %d of %d (%d%c)\n\r") %
        ci % j % (ci * 100 / j) % '%');
      ...
      for (j = 0; j < MAX_IMMORT; j++) {
        long amount =
          gold_statistics[GOLD_INCOME][j] + gold_statistics[GOLD_COMM][j] +
          gold_statistics[GOLD_GAMBLE][j] + gold_statistics[GOLD_REPAIR][j] +
          gold_statistics[GOLD_SHOP][j] + gold_statistics[GOLD_DUMP][j] +
          ...
          gold_statistics[GOLD_SHOP_SYMBOL][j] + gold_statistics[GOLD_RENT][j] +
          gold_statistics[GOLD_HOSPITAL][j] + gold_statistics[GOLD_TITHE][j];
        unsigned long pos =
          gold_positive[GOLD_INCOME][j] + gold_positive[GOLD_COMM][j] +
          gold_positive[GOLD_GAMBLE][j] + gold_positive[GOLD_REPAIR][j] +
          gold_positive[GOLD_SHOP][j] + gold_positive[GOLD_DUMP][j] +
          gold_positive[GOLD_SHOP_SYMBOL][j] +
          gold_positive[GOLD_SHOP_ARMOR][j] +
          gold_positive[GOLD_SHOP_WEAPON][j] + gold_positive[GOLD_SHOP_PET][j] +
          gold_positive[GOLD_SHOP_RESPONSES][j] +
          gold_positive[GOLD_SHOP_COMPONENTS][j] +
          gold_positive[GOLD_SHOP_FOOD][j] + gold_positive[GOLD_RENT][j] +
          gold_positive[GOLD_HOSPITAL][j] + gold_positive[GOLD_TITHE][j];
        sprintf(buf2, "   %sLevel %2d:%s\n\r", cyan(), j + 1, norm());
        buf += buf2;
        sprintf(buf2, "         %sPos  : %9ld%s  (%.2f%% of total)\n\r", cyan(),
          pos, norm(), 100.0 * pos / tot_gold);
        buf += buf2;
        sprintf(buf2, "         %sNet  : %9ld%s  (%.2f%% of total)\n\r", cyan(),
          amount, norm(), 100.0 * amount / net_gold);
        buf += buf2;
        sprintf(buf2, "         %sDrain: %9ld%s  (%.2f%% of total)\n\r", cyan(),
          pos - amount, norm(), 100.0 * (pos - amount) / (tot_gold - net_gold));
        buf += buf2;

        sprintf(buf2,
          "      income     : %8ld, comm       : %8ld, gamble     : %8ld\n\r",
          gold_statistics[GOLD_INCOME][j], gold_statistics[GOLD_COMM][j],
          gold_statistics[GOLD_GAMBLE][j]);
        buf += buf2;

        sprintf(buf2,
          "      shop       : %8ld, respon shop: %8ld, repair     : %8ld\n\r",
        ...
        sprintf(buf2,
          "      armor shop : %8ld, weapon shop: %8ld, pet shop   : %8ld\n\r",
          gold_statistics[GOLD_SHOP_ARMOR][j],
          gold_statistics[GOLD_SHOP_WEAPON][j],
          gold_statistics[GOLD_SHOP_PET][j]);
        buf += buf2;
        ...
        sprintf(buf2,
          "      symbol shop: %8ld, compon shop: %8ld, food shop  : %8ld\n\r",
        ...
        sprintf(buf2,
          "      rent       : %8ld, hospit     : %8ld, tithe      : %8ld\n\r",
          gold_statistics[GOLD_RENT][j], gold_statistics[GOLD_HOSPITAL][j],
      ...
      unsigned int tTotalGold[MAX_MONEY_TYPE],
        tTotalGlobal = getPosGoldGlobal(), tTotalShops = getPosGoldShops(),
        tTotalRent = getPosGold(GOLD_RENT), tTotalBudget = getPosGoldBudget();
      int tNetGold[MAX_MONEY_TYPE],
        tNetGlobal = getNetGoldGlobal(), tNetShops = getNetGoldShops(),
        tNetBudget = getNetGoldBudget(), tNetRent = getNetGold(GOLD_RENT);
      int tTotalDrain = tTotalGlobal - tNetGlobal;

      if (!arg || !*arg) {
        char tNames[MAX_MONEY_TYPE][20] = {"X-Fer", "Income", "Repair", "Shop",
          "Commodities", "Hospital", "Gamble", "Rent", "Dump", "Tithe",
          "Shop-Symbol", "Shop-Weapon", "Shop-Armor", "Shop-Pet", "Shop-Food",
          "Shop-Components", "Shop-Responses"};
        ...
        buf += buf2;
        sprintf(buf2,
          "Modifier: Rent  : %2.2f (Factor: %6.2f%%) (Adjusted: %6.2f%%)\n\r",
          gold_modifier[GOLD_RENT].getVal(),
          100.0 * (tTotalRent - tNetRent) / tTotalDrain,
          100.0 * (tTotalRent - tNetRent) / gold_modifier[GOLD_RENT].getVal() /
            tTotalDrain);
        buf += buf2;
        sprintf(buf2, "Modifier: Equip : %2.2f (Factor: %6.2f%%)\n\r",
          tweakInfo[TWEAK_LOADRATE]->current,
          100.0 * (getPosGold(GOLD_SHOP_WEAPON) + getPosGold(GOLD_SHOP_ARMOR)) /
            tTotalGlobal);
        buf += buf2;
        buf += "\n\r";

        sprintf(buf2,
          "Economy-Total :   pos = %10u   net gold = %10d   drain = %10d\n\r",
        ...
        // Want shops to be a slight drain, so compare drain to source.
        sprintf(buf2,
          "Economy-Shop  :   pos = %10u   net gold = %10d   drain = %10d\n\r",
          tTotalShops, tNetShops, tTotalShops - tNetShops);
        buf += buf2;
        sprintf(buf2,
          "Economy-Budget:   pos = %10u   net gold = %10d   drain = %10d\n\r",
      ...
      if (arg && *arg &&
          (convertTo<int>(arg) < 1 || convertTo<int>(arg) > MAX_IMMORT))
        buf += "...Invalid Level Specified...\n\r\n\r";

      for (j = ((arg && *arg) ? (convertTo<int>(arg) - 1) : 0); j < MAX_IMMORT;
           j++) {
        if (j < 0)
          break;
        ...
        for (moneyTypeT tMoney = GOLD_INCOME; tMoney < MAX_MONEY_TYPE;
             tMoney++) {
        ...
        buf += buf2;
        sprintf(buf2, "     %sPos  : %9ld%s  (%.2f%% of total)\n\r", cyan(),
          tPosTotal, norm(), 100.0 * tPosTotal / tTotalGlobal);
        buf += buf2;
        sprintf(buf2, "     %sNet  : %9ld%s  (%.2f%% of total)\n\r", cyan(),
          tStaTotal, norm(), 100.0 * tStaTotal / tNetGlobal);
        buf += buf2;
        sprintf(buf2, "     %sDrain: %9ld%s  (%.2f%% of total)\n\r", cyan(),
          tPosTotal - tStaTotal, norm(),
        ...
        buf += buf2;
        sprintf(buf2,
          "            Shop: %8ld     Resp-Shop: %8ld      Repair: %8ld\n\r",
          gold_statistics[GOLD_SHOP][j],
          gold_statistics[GOLD_SHOP_RESPONSES][j],
          gold_statistics[GOLD_REPAIR][j]);
        buf += buf2;
        ...
        buf += buf2;
        sprintf(buf2,
          "     Symbol-Shop: %8ld     Comp-Shop: %8ld   Good-Shop: %8ld\n\r",
          gold_statistics[GOLD_SHOP_SYMBOL][j],
          gold_statistics[GOLD_SHOP_COMPONENTS][j],
          gold_statistics[GOLD_SHOP_FOOD][j]);
        buf += buf2;
        sprintf(buf2,
          "            Rent: %8ld      Hospital: %8ld       Tithe: %8ld\n\r",
          gold_statistics[GOLD_RENT][j], gold_statistics[GOLD_HOSPITAL][j],
          gold_statistics[GOLD_TITHE][j]);
        buf += buf2;
        sprintf(buf2, "            Dump: %8ld\n\r\n\r",
      ...
      sprintf(buf2, "TOTAL ECONOMY:     pos %u, net gold = %d, drain=%d\n\r",
        tot_gold, net_gold, tot_drain);
      buf += buf2;
      // shops are a little diff from normal
      // want shops to be a slight drain, so compare drain to source
      sprintf(buf2, "SHOP ECONOMY:      pos %u, net gold = %d, drain=%d\n\r",
        tot_gold_allshops, net_gold_allshops,
        tot_gold_allshops - net_gold_allshops);
      buf += buf2;
      sprintf(buf2, "BUDGET ECONOMY:    pos %u, net gold = %d, drain=%d\n\r",
        tot_gold_budget, net_gold_budget, tot_gold_budget - net_gold_budget);
      buf += buf2;

      buf += "\n\r";

      sprintf(buf2,
        "INCOME ECONOMY:    pos %u, net gold = %d  (drain: %d : %.2f%%)\n\r",
        tot_gold_income, net_gold_income, tot_gold_income - net_gold_income,
        100.0 * (tot_gold_income - net_gold_income) / tot_drain);
      buf += buf2;
      sprintf(buf2,
        "SHOPS ECONOMY:     pos %u, net gold = %d  (drain: %d : %.2f%%)\n\r",
        tot_gold_shop, net_gold_shop, tot_gold_shop - net_gold_shop,
        100.0 * (tot_gold_shop - net_gold_shop) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "FOOD SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_food, net_gold_shop_food,
        tot_gold_shop_food - net_gold_shop_food,
        100.0 * (tot_gold_shop_food - net_gold_shop_food) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "COMP SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_comp, net_gold_shop_comp,
        tot_gold_shop_comp - net_gold_shop_comp,
        100.0 * (tot_gold_shop_comp - net_gold_shop_comp) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "SYMB SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_sym, net_gold_shop_sym,
        tot_gold_shop_sym - net_gold_shop_sym,
        100.0 * (tot_gold_shop_sym - net_gold_shop_sym) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "WEAP SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_weap, net_gold_shop_weap,
        tot_gold_shop_weap - net_gold_shop_weap,
        100.0 * (tot_gold_shop_weap - net_gold_shop_weap) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "ARMR SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_arm, net_gold_shop_arm,
        tot_gold_shop_arm - net_gold_shop_arm,
        100.0 * (tot_gold_shop_arm - net_gold_shop_arm) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "PETS SHOP ECONOMY: pos %u, net gold = %d (bad drain: %d : %.2f%%)\n\r",
        tot_gold_shop_pet, net_gold_shop_pet,
        tot_gold_shop_pet - net_gold_shop_pet,
        100.0 * (tot_gold_shop_pet - net_gold_shop_pet) / tot_drain);
      buf += buf2;

      sprintf(buf2,
        "RESP SHOP ECONOMY: pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_shop_resp, net_gold_shop_resp,
        tot_gold_shop_resp - net_gold_shop_resp,
        100.0 * ((int)tot_gold_shop_resp - net_gold_shop_resp) /
          (int)tot_drain);
      buf += buf2;

      sprintf(buf2, "COMMODITY ECONOMY: pos %u, net gold = %d\n\r",
        tot_gold_comm, net_gold_comm);
      buf += buf2;
      sprintf(buf2,
        "RENT ECONOMY:      pos %u, net gold = %d (bad drain: %d : %.2f%%)\n\r",
        tot_gold_rent, net_gold_rent, tot_gold_rent - net_gold_rent,
        100.0 * ((int)tot_gold_rent - net_gold_rent) / (int)tot_drain);
      buf += buf2;
      sprintf(buf2,
        "REPAIR ECONOMY:    pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_repair, net_gold_repair, tot_gold_repair - net_gold_repair,
        100.0 * (tot_gold_repair - net_gold_repair) / tot_drain);
      buf += buf2;
      sprintf(buf2,
        "HOSPITAL ECONOMY:  pos %u, net gold = %d (drain: %d : %.2f%%)\n\r",
        tot_gold_hospital, net_gold_hospital,
        tot_gold_hospital - net_gold_hospital,
        100.0 * (tot_gold_hospital - net_gold_hospital) / tot_drain);
      buf += buf2;
      sprintf(buf2, "GAMBLE ECONOMY:    pos %u, net gold = %d\n\r",
        tot_gold_gamble, net_gold_gamble);
      buf += buf2;
      sprintf(buf2, "TITHE ECONOMY:     pos %u, net gold = %d\n\r",
        tot_gold_tithe, net_gold_tithe);
      buf += buf2;
      sprintf(buf2, "DUMP ECONOMY:      pos %u, net gold = %d\n\r",
        tot_gold_dump, net_gold_dump);
      buf += buf2;
      ...
      sprintf(buf2, "%-20.20s: %5s %6s %6s %5s %6s %7s %9s %3s\n\r",
        "SKILL:", " Uses", "   Lev", "Damage", "Victs", "    %", " Crits ",
        " Suc/Fail", "Sav");
      buf = buf2;
      spellNumT snt;
      for (snt = MIN_SPELL; snt < MAX_SKILL; snt++) {
        ...
        if (!(discArray[snt]->disc == which))
          continue;
        sprintf(buf2,
          "%-20.20s: %5d %6.2f %6.2f %5d %6.2f %3d/%-3d %4d/%-4d %3d\n\r",
          discArray[snt]->name, discArray[snt]->uses,
          (float)(discArray[snt]->uses == 0 ? 0
                                            : (float)discArray[snt]->levels /
                                                (float)discArray[snt]->uses),
          (float)(discArray[snt]->victims == 0
                    ? 0
                    : (float)discArray[snt]->damage /
                        (float)discArray[snt]->victims),
          discArray[snt]->victims,
          (float)(discArray[snt]->uses == 0 ? 0
                                            : (float)discArray[snt]->learned /
                                                (float)discArray[snt]->uses),
          discArray[snt]->crits, discArray[snt]->critf, discArray[snt]->success,
          discArray[snt]->fail, discArray[snt]->saves);
        buf += buf2;
      }
      desc->page_string(buf);
      ...
      sprintf(buf2, "DIFFICULTY: %s\tLAG: %d rounds\n\r",
        displayDifficulty(which).c_str(), discArray[which]->lag);
      buf += buf2;
      sprintf(buf2, "USES:     %d\tMOBUSES: %d\tIMMUSES: %d\n\r",
        discArray[which]->uses, discArray[which]->mobUses,
        discArray[which]->immUses);
      buf += buf2;
      sprintf(buf2, "LEVEL:    %.2f\tMOBLVL:  %.2f\tIMMLVL:  %.2f\n\r",
        discArray[which]->uses
          ? (float)discArray[which]->levels / discArray[which]->uses
          : 0,
        discArray[which]->mobUses
          ? (float)discArray[which]->mobLevels / discArray[which]->mobUses
          : 0,
        discArray[which]->immUses
          ? (float)discArray[which]->immLevels / discArray[which]->immUses
          : 0);
      buf += buf2;
      sprintf(buf2, "LEARN:    %.2f\tMOBLRN:  %.2f\tIMMLRN:  %.2f\n\r",
        discArray[which]->uses
          ? (float)discArray[which]->learned / discArray[which]->uses
          : 0,
        discArray[which]->mobUses
          ? (float)discArray[which]->mobLearned / discArray[which]->mobUses
          : 0,
        discArray[which]->immUses
          ? (float)discArray[which]->immLearned / discArray[which]->immUses
          : 0);
      buf += buf2;
      sprintf(buf2, "SUCCESS:  %d\tMOBSUCC: %d\tIMMSUCC: %d\n\r",
        discArray[which]->success, discArray[which]->mobSuccess,
        discArray[which]->immSuccess);
      buf += buf2;
      sprintf(buf2, "FAILS:    %d\tMOBFAIL: %d\tIMMFAIL: %d\n\r",
        discArray[which]->fail, discArray[which]->mobFail,
        discArray[which]->immFail);
      buf += buf2;
      sprintf(buf2, "POTIONS:  %d\tMOB-POT: %d\tIMM-POT: %d\n\r",
        discArray[which]->potSuccess, discArray[which]->potSuccessMob,
      ...
      sprintf(buf2, "SAVES:    %d\tMOBSAVE: %d\tIMMSAVE: %d\n\r",
        discArray[which]->saves, discArray[which]->mobSaves,
        discArray[which]->immSaves);
      buf += buf2;
      sprintf(buf2, "CRIT-SUC: %d\tMOBCSUC: %d\tIMMCSUC: %d\n\r",
        discArray[which]->crits, discArray[which]->mobCrits,
        discArray[which]->immCrits);
      buf += buf2;
      sprintf(buf2, "CRIT-FAI: %d\tMOBCFAI: %d\tIMMCFAI: %d\n\r",
        discArray[which]->critf, discArray[which]->mobCritf,
        discArray[which]->immCritf);
      buf += buf2;
      sprintf(buf2, "VICTIMS:  %d\tMOBVICT: %d\tIMMVICT: %d\n\r",
        discArray[which]->victims, discArray[which]->mobVictims,
        discArray[which]->immVictims);
      buf += buf2;
      sprintf(buf2, "DAMAGE:   %.2f\tMOBDAM:  %.2f\tIMMDAM:  %.2f\n\r",
        discArray[which]->victims
          ? (float)discArray[which]->damage / discArray[which]->victims
          : 0,
        discArray[which]->mobVictims
          ? (float)discArray[which]->mobDamage / discArray[which]->mobVictims
          : 0,
        discArray[which]->immVictims
          ? (float)discArray[which]->immDamage / discArray[which]->immVictims
          : 0);
      buf += buf2;
      sprintf(buf2, "FOCUS:    %.2f\n\r",
        discArray[which]->uses
          ? (float)discArray[which]->focusValue / discArray[which]->uses
          : 0);
      buf += buf2;
      sprintf(buf2,
        "Potential Victims:  %ld, Potential Damage %.2f, Potential Level "
        "%.2f\n\r",
        discArray[which]->pot_victims,
        discArray[which]->pot_victims
          ? (float)discArray[which]->pot_damage / discArray[which]->pot_victims
          : 0,
        discArray[which]->pot_victims
          ? (float)discArray[which]->pot_level / discArray[which]->pot_victims
          : 0);
      buf += buf2;

      buf += "\n\rAttempts Breakdown:\n\r";
      sprintf(buf2, "New: %d, low: %d, mid: %d, good: %d, high: %d\n\r",
        discArray[which]->newAttempts, discArray[which]->lowAttempts,
        discArray[which]->midAttempts, discArray[which]->goodAttempts,
        discArray[which]->highAttempts);
      buf += buf2;

      buf += "Failure Breakdown:\n\r";
      sprintf(buf2, "General: %d, Focus: %d, Engage: %d\n\r",
        discArray[which]->genFail, discArray[which]->focFail,
        discArray[which]->focFail);
      buf += buf2;

      buf += "Learning Info:\n\r";
      sprintf(buf2,
        "Attempts: %ld, Successes: %d, Fails: %d\n\rLearning: %d, Level: %d, "
        "Boost: %ld\n\rDisc Success: %d, Adv Success: %d\n\r",
        discArray[which]->learnAttempts, discArray[which]->learnSuccess,
        discArray[which]->learnFail, discArray[which]->learnLearn,
        discArray[which]->learnLevel, discArray[which]->learnBoost,
      ...
      for (snt = MIN_SPELL; snt < MAX_SKILL; snt++) {
        ...
        sprintf(buf2,
          "%-20.20s: %5d %6.2f %6.2f %5d %6.2f %3d/%-3d %4d/%-4d %3d\n\r",
          discArray[snt]->name, discArray[snt]->mobUses,
          (float)(discArray[snt]->mobUses == 0
                    ? 0
                    : (float)discArray[snt]->mobLevels /
                        (float)discArray[snt]->mobUses),
          (float)(discArray[snt]->mobVictims == 0
                    ? 0
                    : (float)discArray[snt]->mobDamage /
                        (float)discArray[snt]->mobVictims),
          discArray[snt]->mobVictims,
          (float)(discArray[snt]->mobUses == 0
                    ? 0
                    : (float)discArray[snt]->mobLearned /
                        (float)discArray[snt]->mobUses),
      ...
      for (snt = MIN_SPELL; snt < MAX_SKILL; snt++) {
        ...
        sprintf(buf2,
          "%-20.20s: %5d %6.2f %6.2f %5d %6.2f %3d/%-3d %4d/%-4d %3d\n\r",
          discArray[snt]->name, discArray[snt]->immUses,
          (float)(discArray[snt]->immUses == 0
                    ? 0
                    : (float)discArray[snt]->immLevels /
                        (float)discArray[snt]->immUses),
          (float)(discArray[snt]->immVictims == 0
                    ? 0
                    : (float)discArray[snt]->immDamage /
                        (float)discArray[snt]->immVictims),
          discArray[snt]->immVictims,
          (float)(discArray[snt]->immUses == 0
                    ? 0
                    : (float)discArray[snt]->immLearned /
                        (float)discArray[snt]->immUses),
...
static void TimeTravel(const char* ch) {
  ...
  if (fwrite(&h, sizeof(h), 1, fp) != 1) {
    vlogf(LOG_MISC,
      format("Cannot write updated rent file header for %s") % h.owner);
    fclose(fp);
    return;
  }
  fclose(fp);
...
#endif
    sprintf(buf, "%s has shifted game time back %d real minutes.",
      getName().c_str(), deltatime);
    doSystem(buf);
  }
}
...
void TBeing::doLog(const char* argument) {
  ...
  if (!(vict = get_char_vis_world(this, name_buf, NULL, EXACT_YES))) {
    ...
    return;
             ...
             vict->isPlayerAction(PLR_NOSNOOP)) {
    ...
    sendTo(
      "Please either post a reason for adding the log to the immortal "
      "board,\n\r");
    sendTo(
      "or add a comment to the player's account (permanent) stating your "
      "reasons.\n\r");
...
void TBeing::doHostlog(const char* argument) {
  ...
  if (!*buf) {
    ...
    vlogf(LOG_MISC, format("%s has added host %s to the hostlog list.") %
                      getName() % hostLogList[numberLogHosts]);
    numberLogHosts++;
    return;
  } else if (!strcmp(buf, "rem")) {
    if (numberLogHosts <= 0) {
...
void TBeing::doSysChecklog(const sstring& arg) {
  char *tMarkerS,  // Start
    *tMarkerE,     // End
    tString[1024], tSearch[256], tLog[256];
  unsigned int tIndex;
  ...
  if (arg.empty()) {
    sendTo(
      "Syntax: checklog \"string\" logfile\n\rSee loglist for list of "
      "logfiles.\n\r");
    return;
  }
  ...
  for (tIndex = 0; tIndex < strlen(tLog); tIndex++)
    ...
    if (tLog[tIndex] == '/' || (tLog[tIndex] == '.' && (tIndex == 0))) {
...
void TBeing::doResize(const char* arg) {
  ...
  if (dynamic_cast<TBaseClothing*>(obj)) {
    ...
    if (race_vol_constants[mapSlotToFile(slot)]) {
      if (race) {
        // if average height for each race is aleady stored someplace, i missed
        // it also, it looks like only base male height is public
        Race* targ_race = new Race((race_t)race);
        double avg_height = (double)targ_race->getBaseMaleHeight() +
                            (((double)targ_race->getMaleHtNumDice() +
                               (double)targ_race->getMaleHtDieSize()) /
                              2.0);
        new_volume =
          (int)(avg_height * race_vol_constants[mapSlotToFile(slot)]);
        feedback = format(
                     "Your target's average height is %i inches.  Item volume "
                     "is now %i cubic inches.") %
                   (int)avg_height % new_volume;
        delete targ_race;
      } else if (height) {
        // resize based on requested height
        new_volume =
          (int)((double)height * race_vol_constants[mapSlotToFile(slot)]);
        feedback = format(
                     "Your target height is %i inches.  Item volume is now %i "
                     "cubic inches.") %
                   height % new_volume;
      } else if (targ) {
        // use targ's height
        new_volume = (int)((double)targ->getHeight() *
                           race_vol_constants[mapSlotToFile(slot)]);
        feedback = format(
                     "Your target's height is %i inches.  Item volume is now "
                     "%i cubic inches.") %
                   targ->getHeight() % new_volume;
      } else {
        // shouldn't have gotten here
        vlogf(LOG_BUG,
          "Bug in TBeing::doResize - no player, race or height to resize to.");
        return;
      }
      // do the resize
      obj->setVolume(new_volume);
    } else {
      vlogf(LOG_BUG, format("Missing race_vol_constant[] while resizing %s.") %
                       obj->getName());
      return;
    }
...
void TBeing::doHeaven(const sstring& arg) {
  ...
  if (num > 100) {
    sendTo(
      "A higher power has decreed that 100 units is the most you may do at "
      "once.\n\r");
    return;
  }
...
void TBeing::doAccount(const sstring& arg) {
  ...
  if (name.empty()) {
    if (hasWizPower(POWER_ACCOUNT)) {
      sendTo(
        "Syntax: account <account name> [banished | email | double | triple | "
        "immortal | multi <n>]\n\r");
    } else {
  ...
  if (hasWizPower(POWER_ACCOUNT)) {
    ...
    if (is_abbrev(cmd, "banished")) {
      if (IS_SET(account.flags, TAccount::BANISHED)) {
        ...
        sendTo(
          format("You have unbanished the %s account.\n\r") % account.name);
        vlogf(LOG_MISC,
          format("%s unbanished account '%s'") % getName() % account.name);
      } else {
        SET_BIT(account.flags, TAccount::BANISHED);
        sendTo(
          format("You have set the %s account banished.\n\r") % account.name);
        vlogf(LOG_MISC,
          format("%s banished account '%s'") % getName() % account.name);
      }
      ...
      if (IS_SET(account.flags, TAccount::EMAIL)) {
        ...
        sendTo(format("You have un-email-banished the %s account.\n\r") %
               account.name);
        vlogf(LOG_MISC, format("%s un-email-banished account '%s'") %
        ...
        sendTo(format("You have set the %s account email-banished.\n\r") %
               account.name);
        vlogf(LOG_MISC,
          format("%s email-banished account '%s'") % getName() % account.name);
      }
      ...
      if (IS_SET(account.flags, TAccount::ALLOW_DOUBLECLASS)) {
        ...
        sendTo(
          format("You revoke the %s account's ability to double-class.\n\r") %
          account.name);
      } else {
        SET_BIT(account.flags, TAccount::ALLOW_DOUBLECLASS);
        sendTo(
          format("You grant the %s account the ability to double-class.\n\r") %
          account.name);
      }
      ...
      if (IS_SET(account.flags, TAccount::ALLOW_TRIPLECLASS)) {
        ...
        sendTo(
          format("You revoke the %s account's ability to triple-class.\n\r") %
          account.name);
      } else {
        SET_BIT(account.flags, TAccount::ALLOW_TRIPLECLASS);
        sendTo(
          format("You grant the %s account the ability to triple-class.\n\r") %
          account.name);
      }
      ...
      if (limit.empty()) {
        sendTo(format("The %s account has current multiplay limit of %i") %
               account.name % account.multiplay_limit);
        return;
      }
      ...
      try {
        ...
        sendTo(
          format(
            "Incorrect multiplay limit '%s', expected positive integer.\n\r") %
          limit);
        return;
      }
      if (new_limit < 0) {
      ...
      if ((tmp_name != "Batopr") && (tmp_name != "Lapsos") &&
          (tmp_name != "Damescena") && (tmp_name != "Peel") &&
          (tmp_name != "Jesus") && (tmp_name != "Dash") &&
          (tmp_name != "Maror") && (tmp_name != "Angus")) {
      ...
      if (IS_SET(account.flags, TAccount::IMMORTAL)) {
        ...
        sendTo(
          format("You un-flag the %s account immortal.\n\r") % account.name);
        vlogf(LOG_MISC, format("%s making account='%s' non-immortal") %
                          getName() % account.name);
      } else {
        SET_BIT(account.flags, TAccount::IMMORTAL);
        sendTo(
          format("You flag the %s account as immortal.\n\r") % account.name);
        vlogf(LOG_MISC,
          format("%s making account='%s' immortal") % getName() % account.name);
      }
      account.write(name);
      return;
    }
...
static bool verifyName(const sstring tStString) {
  ...
  TDatabase db(DB_SNEEZY);
  db.query(
    "select 1 from wizpower w, player p where p.id=w.player_id and p.name='%s' "
    "and w.wizpower=%i",
...
int TBeing::doAs(const char* arg) {
  ...
  if (IS_SET_DELETE(rc, DELETE_THIS)) {
    ...
    sendTo(
      "Please don't do things that will cause your original character to be "
      "destroyed.\n\r");
...
void TBeing::doComment(const char* argument) {
  ...
  sendTo(
    format(
      "Write your comment about %s, use ~ when done, or ` to cancel.\n\r") %
    arg);
  sendTo(
    "Please do not make stupid comments, just list things that are "
    "important.\n\r");
...
immortalTypeT TPerson::isImmortal(int lev) const {
  return (isPlayerAction(PLR_IMMORTAL) &&
           (GetMaxLevel() >= max(lev, GOD_LEVEL1)))
           ? IMMORTAL_YES
           : IMMORTAL_NO;
}
...
void TBeing::doBestow(const sstring& argument) { incorrectCommand(); }
...
void TPerson::doBestow(const sstring& argument) {
  ...
  if (!desc) {
    ...
    vlogf(LOG_MISC,
      format("How did %s get to doBestow without a desc?") % getName());
    return;
  }
  ...
  if (is_abbrev(arg1, "coins")) {
    /*** make coins ***/
    if (!(is_number(arg3) && (number_needed = convertTo<int>(arg3)) &&
          number_needed > 0)) {
      sendTo("Usage :\n\r");
      sendTo("        bestow coins <name> <number>\n\r");
      return;
    ...
    if (this == ch)
      ...
      name_buffer += format("'%s") %
                     (name_buffer.at(name_buffer.size() - 1) == 's' ? "" : "s");
    }
