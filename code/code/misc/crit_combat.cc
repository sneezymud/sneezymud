...
int TBeing::critSuccessChance(TBeing* v, TThing* weapon, wearSlotT* part_hit,
  spellNumT wtype, int* dam, int mod) {
  ...
  if (diff == 0) {
    ...
    double level_mod =
      50.0 + ((double)diff * log(absdiff / 20.0 + 1) / absdiff) * 75.0;
  ...
  if (pierceType(wtype)) {
    ...
    vlogf(LOG_BUG,
      format("unknown weapon type in critSuccessChance (%i)") % wtype);
  }
  return FALSE;
}
