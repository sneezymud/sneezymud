...
void TBeing::doNewbieEqLoad(race_t num, unsigned short num2, bool initNum) {
  ...
  if (isImmortal() || !isPc()) {
    ...
    vlogf(LOG_BUG,
      format("Something called doNewbieEqLoad when it shouldnt %s") %
        getName());
    return;
  }

  // basic races
  int hobbitGear[] = {980, 982, 105, 983, 983, 984, 984, 985, 986, 986, 987,
    989, 1001, 1009, 1009, 1010, 0};
  int dwarfGear[] = {990, 992, 105, 993, 993, 994, 994, 995, 996, 996, 997, 998,
    998, 999, 1001, 1009, 1009, 1010, 0};
  int ogreGear[] = {970, 972, 105, 973, 973, 974, 974, 975, 976, 976, 977, 978,
    978, 979, 1001, 1009, 1009, 1010, 0};
  int gnomeGear[] = {960, 962, 105, 963, 963, 964, 964, 965, 966, 966, 967, 968,
    968, 969, 1001, 1009, 1010, 0};
  int elvenGear[] = {950, 952, 105, 953, 953, 954, 954, 955, 956, 956, 957, 958,
    958, 959, 1001, 1009, 1009, 1010, 0};
  int humanGear[] = {1000, 1002, 105, 1003, 1003, 1004, 1004, 1005, 1006, 1006,
    1007, 1008, 1008, 1011, 1001, 1009, 1009, 1010, 0};

  // advanced races
  int trollGear[] = {30924, 30925, 30926, 30926, 30927, 30927, 30928, 30928,
    30929, 30930, 30931, 30931, 30932, 30932, 30933, 30934, 30935, 0};
  int birdGear[] = {44825, 44828, 44828, 44829, 44830, 44830, 44831, 44832,
    44832, 44833, 44833, 44834, 44836, 1001, 1009, 1009, 1010, 105, 0};
  int bullyGear[] = {4320, 4321, 4321, 4322, 4323, 4323, 4324, 4325, 4325, 4326,
    4326, 4327, 4328, 1001, 1009, 1009, 1010, 105, 0};
  int fishGear[] = {44767, 44768, 44768, 44769, 44770, 44770, 44771, 44772,
    44772, 44773, 44773, 44774, 44775, 44776, 44777, 44777, 44778, 105, 0};
...
// intended for newbies to get help, without having to use shout or direct tells
// to gods
void TBeing::doNewbie(const sstring& arg) {
  ...
  bool isNewbie = (!isNewbieHelper && desc &&
                   (time(0) - desc->account->birth) < NEWBIE_PURGATORY_LENGTH);

  if (!isNewbieHelper && !isNewbie) {
    sendTo(
      "You cannot use the newbie help channel unless you are a newbie or a "
      "newbie helper.\n\r");
    return;
  }
  ...
  if (!isImmortal() && !canSpeak()) {
    sendTo(
      "You are unable to speak at this time and cannot ask newbie "
      "questions.\n\r");
    return;
  }
  ...
  if (message.length() > 200) {
    sendTo(
      "Your newbie chat was too long and has been truncated for brevity.\n\r");
    message.resize(200);
  }

  sstring header = format(isNewbie ? "%sYou ask the experts: %s%%s%s"
                                   : "%sYou advise the newbies: %s%%s%s") %
  ...
  for (Descriptor* d = descriptor_list; d; d = d->next) {
    ...
    TBeing* person = (dynamic_cast<TMonster*>(d->character) && d->original)
                       ? d->original
                       : d->character;
    if (!person)
      continue;
    bool newbieHelper =
      person->isImmortal() || person->isPlayerAction(PLR_NEWBIEHELP);
    bool newbie = (!newbieHelper &&
                   (time(0) - d->account->birth) < NEWBIE_PURGATORY_LENGTH);
    ...
    sstring gmcp = format(
                     "comm.channel { \"chan\": \"newbie\", \"msg\": \"%s\", "
                     "\"player\": \"%s\" }") %
                   msg.trim().escapeJson() % title;
