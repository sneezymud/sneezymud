...
bool twitterShout(sstring from, sstring msg) {
  ...
  // manpages: curl_global_init(CURL_GLOBAL_ALL) is not required for simple
  // calls of curl - curl_easy_init will end up doing the alloc for us if needed
  // this may save us a tiny amount of time on the init call

  curl = curl_easy_init();
  curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME, "status",
    CURLFORM_COPYCONTENTS, (format("%s: %s") % from % msg).str().c_str(),
    CURLFORM_END);
...
bool twitterShout(sstring from, sstring msg) { return false; }
