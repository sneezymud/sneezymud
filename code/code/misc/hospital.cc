...
int poison_price(TBeing* ch, affectedData*, int shop_nr) {
  ...
  return (int)(500.0 * shop_index[shop_nr].getProfitBuy(nullptr, ch));
}

int syphilis_price(TBeing* ch, affectedData*, int shop_nr) {
  // get more exotic later
  return (int)(10000.0 * shop_index[shop_nr].getProfitBuy(nullptr, ch));
}
...
int limb_heal_price(TBeing* ch, wearSlotT pos, int shop_nr) {
  ...
  basenum = (int)((float)basenum * shop_index[shop_nr].getProfitBuy(nullptr, ch));
  ...
  vlogf(LOG_BUG,
    format("Bad pos (%d) in limb_heal_price for %s!") % pos % ch->getName());
  return (basenum * 10);
}
...
int limb_expel_price(TBeing* ch, wearSlotT pos, int shop_nr) {
  ...
  if (!(stuck = ch->getStuckIn(pos))) {
    vlogf(LOG_BUG, format("VERY BAD! limb_expel_price called with pos(%d) "
                          "char(%s) with no item stuck in!") %
                     pos % ch->getName());
    return (-1);
  }
  return (int)((float)stuck->expelPrice(ch, pos) *
               shop_index[shop_nr].getProfitBuy(nullptr, ch));
}

int TThing::expelPrice(const TBeing* ch, int pos) const {
  vlogf(LOG_BUG,
    format(
      "Somehow %s got something besides a weapon/arrow stuck in them pos(%d)") %
      ch->getName() % pos);
  return (1000000);
}
...
int limb_wound_price(TBeing* ch, wearSlotT pos, unsigned short int wound,
  ...
  price = (int)((float)price * shop_index[shop_nr].getProfitBuy(nullptr, ch));
  ...
  return (int)(1000000.0 * shop_index[shop_nr].getProfitBuy(nullptr, ch));
}
...
int spell_regen_price(TBeing* ch, spellNumT spell, int shop_nr) {
  ...
  return (int)((float)price * shop_index[shop_nr].getProfitBuy(nullptr, ch));
}
...
int doctorCost(int shop_nr, TBeing* ch, diseaseTypeT disease) {
  ...
  cost = (int)((float)cost * shop_index[shop_nr].getProfitBuy(nullptr, ch));
...
int doctor(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* me, TObj*) {
  ...
  if (cmd == CMD_LIST) {
    me->doTell(ch->getName(),
      "I will list out what ails you, along with a price.");
    for (i = MIN_WEAR; i < MAX_WEAR; i++) {
      ...
      if (ch->isLimbFlags(i, PART_MISSING)) {
        me->doTell(ch->getName(),
          format("%d) Your %s is missing! (%d talens)") % ++count %
            ch->describeBodySlot(i) % limb_regen_price(ch, i, shop_nr));
        continue;
      } else {
        for (j = 0; j < MAX_PARTS; j++) {
          ...
          if (ch->isLimbFlags(i, 1 << j)) {
            me->doTell(ch->getName(),
              format("%d) Your %s is %s. (%d talens)") % ++count %
                ch->describeBodySlot(i) % body_flags[j] %
                limb_wound_price(ch, i, 1 << j, shop_nr));
          }
        }
        if (ch->getCurLimbHealth(i) < ch->getMaxLimbHealth(i)) {
          double perc =
            (double)ch->getCurLimbHealth(i) / (double)ch->getMaxLimbHealth(i);
          me->doTell(ch->getName(), format("%d) Your %s is %s. (%d talens)") %
                                      ++count % ch->describeBodySlot(i) %
                                      LimbHealth(perc) %
                                      limb_heal_price(ch, i, shop_nr));
        }
        if ((stuck = ch->getStuckIn(i))) {
          me->doTell(ch->getName(),
            format("%d) You have %s stuck in your %s. (%d talens)") % ++count %
              stuck->shortDescr % ch->describeBodySlot(i) %
              limb_expel_price(ch, i, shop_nr));
        }
      }
    }
    if (ch->affected) {
      ...
      for (aff = ch->affected; aff; aff = aff->next) {
        if (aff->type == AFFECT_DISEASE) {
          if (ch->GetMaxLevel() < 12) {
            me->doTell(ch->getName(),
              "Hmm, you are just a newbie, guess I will have to take you at "
              "reduced rates.\n\r");
          }
          buf = format("%d) You have %s. (%d talens)") % ++count %
    ...
    if (!*arg || !arg) {
      me->doTell(ch->getName(),
        "What do you want to buy? Try listing to see what ails you!");
      return true;
    }
    ...
    if (!sstring(arg).isNumber()) {
      me->doTell(ch->getName(),
        "To buy a cure, type \"buy <number>\". Try listing to see what ails "
        "you!");
      return true;
    }
      ...
      if (ch->isLimbFlags(i, PART_MISSING)) {
        if (++count == bought) {
          if ((ch->getMoney()) < (cost = limb_regen_price(ch, i, shop_nr))) {
            me->doTell(ch->getName(),
              format("You don't have enough money to regenerate your %s!") %
                ch->describeBodySlot(i));
            return true;
          } else {
            if (!ch->isBodyPartAttached(i)) {
              me->doTell(ch->getName(),
                format("You can't regenerate your %s until something else is "
                       "regenerated first.") %
                  ch->describeBodySlot(i));
              return true;
            }
            ...
            if (me->getMoney() < cashCost) {
              me->doTell(ch->getName(),
                "I don't have enough money to cover my operating expenses!");
              return true;
            }

            TShopOwned tso(shop_nr, me, ch);
            tso.doBuyTransaction(cashCost,
              format("regenerating %s") % ch->describeBodySlot(i),
              TX_BUYING_SERVICE);

            buf = format(
                    "$n waves $s hands, utters many magic phrases and "
                    "regenerates $N's %s!") %
                  ch->describeBodySlot(i);
            act(buf, true, me, nullptr, ch, TO_NOTVICT);
            buf = format(
                    "$n waves $s hands, utters many magic phrases and "
                    "regenerates your %s!") %
                  ch->describeBodySlot(i);
            act(buf, true, me, nullptr, ch, TO_VICT);
            ch->setLimbFlags(i, 0);
            ch->setCurLimbHealth(i, ch->getMaxLimbHealth(i));
            if (i == WEAR_HAND_R) {
              ch->setLimbFlags(HOLD_RIGHT, 0);
              ch->setCurLimbHealth(HOLD_RIGHT,
                ch->getMaxLimbHealth(HOLD_RIGHT));
        ...
        for (j = 0; j < MAX_PARTS; j++) {
          ...
          if (ch->isLimbFlags(i, 1 << j)) {
            if (++count == bought) {
              if ((ch->getMoney()) <
                  (cost = limb_wound_price(ch, i, 1 << j, shop_nr))) {
                me->doTell(ch->getName(),
                ...
                if (me->getMoney() < cashCost) {
                  me->doTell(ch->getName(),
                    "I don't have enough money to cover my operating "
                    "expenses!");
                  return true;
                }
                ...
                TShopOwned tso(shop_nr, me, ch);
                tso.doBuyTransaction(cashCost,
                  format("mending %s") % ch->describeBodySlot(i),
                  TX_BUYING_SERVICE);
                ...
                if (i == WEAR_HAND_R) {
                  ...
                  ch->setCurLimbHealth(HOLD_LEFT,
                    ch->getMaxLimbHealth(HOLD_LEFT));
                }
                ch->doSave(SILENT_YES);
                return true;
              }
      ...
      if (ch->getCurLimbHealth(i) < ch->getMaxLimbHealth(i)) {
        if (++count == bought) {
          if ((ch->getMoney()) < (cost = limb_heal_price(ch, i, shop_nr))) {
            me->doTell(ch->getName(),
              format("You don't have enough money to heal your %s!") %
                ch->describeBodySlot(i));
            return true;
          } else {
            int cashCost = min(ch->getMoney(), cost);
            ...
            TShopOwned tso(shop_nr, me, ch);
            tso.doBuyTransaction(cashCost,
              format("healing %s") % ch->describeBodySlot(i),
              TX_BUYING_SERVICE);

            buf = format(
                    "$n waves $s hands, utters many magic phrases and touches "
                    "$N's %s!") %
                  ch->describeBodySlot(i);
            act(buf, true, me, nullptr, ch, TO_NOTVICT);
            buf = format(
      ...
      if ((stuck = ch->getStuckIn(i))) {
        if (++count == bought) {
          if ((ch->getMoney()) < (cost = limb_expel_price(ch, i, shop_nr))) {
            me->doTell(ch->getName(),
              format("You don't have enough money to expel %s from your %s!") %
                stuck->shortDescr % ch->describeBodySlot(i));
            return true;
          } else {
            int cashCost = min(ch->getMoney(), cost);
            ...
            TShopOwned tso(shop_nr, me, ch);
            tso.doBuyTransaction(cashCost,
              format("expelling %s") % ch->describeBodySlot(i),
              TX_BUYING_SERVICE);

            buf = format("$n skillfully removes $p from $N's %s!") %
                  ch->describeBodySlot(i);
            act(buf, true, me, stuck, ch, TO_NOTVICT);
            buf = format("$n skillfully removes $p from your %s!") %
                  ch->describeBodySlot(i);
            act(buf, true, me, stuck, ch, TO_VICT);
            *ch += *(ch->pulloutObj(i, true, &res));

            int rc = stuck->checkSpec(ch, CMD_OBJ_EXPELLED, "", nullptr);
        ...
        if (aff->type == AFFECT_DISEASE) {
          if (++count == bought) {
            ...
            if ((ch->getMoney()) < cost) {
              me->doTell(fname(ch->name),
                format("You don't have enough money to cure %s!") %
                  DiseaseInfo[affToDisease(*aff)].name);
              return true;
            } else {
              int cashCost = min(ch->getMoney(), cost);
              ...
              TShopOwned tso(shop_nr, me, ch);
              tso.doBuyTransaction(cashCost,
                format("curing disease %s") %
                  DiseaseInfo[affToDisease(*aff)].name,
                TX_BUYING_SERVICE);

              act(
                "$n waves $s hands, utters many magic phrases and touches $N!",
                true, me, nullptr, ch, TO_NOTVICT);
              act(
                "$n waves $s hands, utters many magic phrases and touches you!",
                true, me, nullptr, ch, TO_VICT);
          ...
          if (++count == bought) {
            ...
            if ((ch->getMoney()) < cost) {
              ...
              TShopOwned tso(shop_nr, me, ch);
              tso.doBuyTransaction(cashCost,
                format("curing blindness %i") % ch->describeBodySlot(i),
                TX_BUYING_SERVICE);
...
TMonster* getDoctor(int hospital_room, int& shop_nr) {
  ...
  TThing* t = nullptr;
  for (StuffIter it = oproom->stuff.begin();
       it != oproom->stuff.end() && (t = *it); ++it) {
...
int healing_room(TBeing*, cmdTypeT cmd, const char*, TRoom* rp) {
  ...
  for (StuffIter it = rp->stuff.begin(); it != rp->stuff.end() && (t = *it);
    ...
    if (healed->getHit() >= healed->hitLimit()) {
      if (!number(0, 10))
        healed->sendTo(
          "Since you require no healing, you are not charged for staying in "
          "the hospital.\n\r");
      continue;
    }
    ...
    if (cost > healed->getMoney()) {
      ...
      healed->sendTo(
        "You are directed toward an exit out of the hospital for being a "
        "vagrant.\n\r");
      continue;
    }
    ...
    if (!doctor) {
      if (gamePort != Config::Port::GAMMA)
        vlogf(LOG_BUG,
          format("Couldn't find doctor for shop_nr=%i!") % shop_nr);
      return false;
    }

    if (doctor->getMoney() < cost) {
      doctor->doTell(healed->getName(),
        "I don't have enough money to cover my operating expenses!");
      return true;
    }
    ...
    healed->addToHit(num);
    healed->sendTo(
      format("The charge for the healing is %d talens.\n\r") % cost);
    ...
    if ((opt >= 1) && (opt <= 3)) {
      switch (opt) {
        ...
        case 3:
          ch->setLifeforce(500);
          ch->sendTo(
            "Your spirit has been lifted and your troubles have been "
            "crucified.\n\r");
          ch->updatePos();
