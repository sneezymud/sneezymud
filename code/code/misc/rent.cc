...
struct SInnkeeperHate {
  ...
  {14323, RACE_HOBBIT, false,
    "I'm afraid our Inn is setup for those of Gnome or Hobbit stature."},
  {14323, RACE_GNOME, false,
    "I'm afraid our Inn is setup for those of Gnome or Hobbit stature."},
  {24442, RACE_GNOME, false,
    "I'm afraid our Inn is designed for those of Gnome or Hobbit stature."},
  {24442, RACE_HOBBIT, false,
    "I'm afraid our Inn is designed for those of Gnome or Hobbit stature."},
...
void handleCorrupted(const char* name, char* account) {
  ...
  sprintf(buf, "mv player/%c/%s player/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);

  // strings
  sprintf(buf, "mv player/%c/%s.strings player/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);

  // toggles
  sprintf(buf, "mv player/%c/%s.toggle player/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);

  // career
  sprintf(buf, "mv player/%c/%s.career player/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);

  // rent
  sprintf(buf, "mv rent/%c/%s rent/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);

  // followers
  sprintf(buf, "mv rent/%c/%s.fol rent/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);
  sprintf(buf, "mv rent/%c/%s.fr rent/corrupt/.", LOWER(name[0]),
    sstring(name).lower().c_str());
  vsystem(buf);
  ...
  // nuke account
  sprintf(buf, "rm account/%c/%s/%s", LOWER(account[0]),
    sstring(account).lower().c_str(), sstring(name).lower().c_str());
  vsystem(buf);
}
...
void wipePlayerFile(const char* name) {
  ...
  sprintf(buf, "player/%c/%s.strings", LOWER(name[0]),
    sstring(name).lower().c_str());
  unlink(buf);

  // nuke toggles, ignore errors
  sprintf(buf, "player/%c/%s.toggle", LOWER(name[0]),
    sstring(name).lower().c_str());
  unlink(buf);
...
void TBeing::removeRent() {
  ...
  if (!tmp) {
    ...
    vlogf(LOG_BUG,
      format("removeRent() called for a bad isPc() (%s)") % getName());
    return;
  }
  ...
  for (j = 0; j < MAX_OBJ_AFFECT; j++) {
    ...
    if (applyTypeShouldBeSpellnum(o->affected[j].location))
      item.affected[j].modifier =
        mapSpellnumToFile(spellNumT(o->affected[j].modifier));
    else
...
int ItemSaveDB::raw_write_item(TObj* o, int slot, int container, int rent_id) {
  ...
  if (rent_id == -1) {
    db.query(
      "insert into rent (owner_type, owner, vnum, slot, container, val0, val1, "
      "val2, val3, extra_flags, weight, bitvector, decay, cur_struct, "
      "max_struct, material, volume, price, depreciation) values ('%s', %i, "
      "%i, %i, %i, %i, %i, %i, %i, %i, %f, %i, %i, %i, %i, %i, %i, %i, %i)",
      owner_type.c_str(), owner, obj_index[o->getItemIndex()].virt, slot,
      container, a0, a1, a2, a3, o->getObjStat(), (float)o->getWeight(),
    ...
    if (0 == rent_id)
      vlogf(LOG_BUG,
        "Error in rent_id, value is 0 - this code should be switched to use "
        "db.lastInsertId()");
  } else {
    db.query(
      "insert into rent (rent_id, owner_type, owner, vnum, slot, container, "
      "val0, val1, val2, val3, extra_flags, weight, bitvector, decay, "
      "cur_struct, max_struct, material, volume, price, depreciation) values "
      "(%i, '%s', %i, %i, %i, %i, %i, %i, %i, %i, %i, %f, %i, %i, %i, %i, %i, "
      "%i, %i, %i)",
      rent_id, owner_type.c_str(), owner, obj_index[o->getItemIndex()].virt,
      slot, container, a0, a1, a2, a3, o->getObjStat(), (float)o->getWeight(),
      o->obj_flags.bitvector, o->obj_flags.decay_time,
      o->obj_flags.struct_points, o->obj_flags.max_struct_points,
      o->getMaterial(), o->obj_flags.volume, o->obj_flags.cost,
      o->getDepreciation());
  ...
  for (int j = 0; j < MAX_OBJ_AFFECT; j++) {
    ...
    db.query(
      "insert into rent_obj_aff (type, level, duration, renew, modifier, "
      "location, modifier2, bitvector, rent_id) values (%i, %i, %i, %i, %i, "
      "%i, %i, %i, %i)",
      mapSpellnumToFile(o->affected[j].type), o->affected[j].level,
      o->affected[j].duration, o->affected[j].renew, modifier,
      mapApplyToFile(o->affected[j].location), o->affected[j].modifier2,
      o->affected[j].bitvector, rent_id);
  }

  if (IS_SET(o->getObjStat(), ITEM_STRUNG)) {
    db.query(
      "insert into rent_strung (rent_id, name, short_desc, long_desc, "
      "action_desc) values (%i, '%s', '%s', '%s', '%s')",
      rent_id, o->name.c_str(), o->shortDescr.c_str(), o->getDescr().c_str(),
...
TObj* ItemLoad::raw_read_item() {
  ...
  if (version >= 7 || dynamic_cast<TNote*>(o)) {
    if (version < 9 && dynamic_cast<TOpenContainer*>(o)) {
      item.value[1] =
        ((item.value[1] >> 8) << 16) ^ ((item.value[1] << 24) >> 24);

      o->assignFourValues(item.value[0], item.value[1], item.value[2],
        item.value[3]);
    ...
    // deal with structure
    o->setStructPoints(
      min(item.struct_points, (int)obj_index[o->getItemIndex()].max_struct));
  ...
  // this is noe done always - just checking at this load was circumvented by
  // some DB loads
  /*if (CURRENT_RENT_VERSION > 9 &&
    convertV9MaterialToV10(o->getMaterial()) != o->getMaterial())
  {
    vlogf(LOG_OBJ, format("Object %s converting from material type %d to %d") %
  o->getName() % o->getMaterial() % convertV9MaterialToV10(o->getMaterial()));
...
TObj* ItemLoadDB::raw_read_item(int rent_id, int& slot) {
  ...
  db.query(
    "select owner_type, owner, vnum, slot, container, val0, val1, val2, val3, "
    "extra_flags, weight, bitvector, decay, cur_struct, max_struct, material, "
    "volume, price, depreciation from rent where rent_id=%i and "
    "owner_type='%s' and owner=%i",
  ...
  db.query(
    "select type, level, duration, renew, modifier, location, modifier2, "
    "bitvector from rent_obj_aff where rent_id=%i",
    rent_id);
  ...
  for (int j = 0; db.fetchRow() && j < MAX_OBJ_AFFECT; j++) {
    ...
    if (applyTypeShouldBeSpellnum(o->affected[j].location))
      o->affected[j].modifier =
        mapFileToSpellnum(convertTo<int>(db["modifier"]));
    else
  ...
  if (o->isObjStat(ITEM_STRUNG)) {
    ...
    db.query(
      "select name, short_desc, long_desc, action_desc from rent_strung where "
      "rent_id=%i",
      rent_id);
    if (db.fetchRow()) {
      if (!db["name"].empty())
...
bool ItemLoad::objToParent(signed char slot, TObj* parent, TObj* new_obj,
  TRoom* r, TBeing* ch) {
  ...
  if (slot != NORMAL_SLOT) {
    if (r)
      ...
      vlogf(LOG_BUG,
        format("%s's objects.  Invalid slot %d.") % ch->getName() % slot);
...
bool ItemLoad::objToEquipChar(unsigned char slot, TBeing* ch, TObj* new_obj,
  TRoom* r) {
  ...
  if (ch) {
    ...
    vlogf(LOG_BUG,
      format("Room %d has invalid slot #.") % ((r) ? r->number : -99));
    return false;
  }
  return true;
}
...
bool ItemLoad::objToTarg(unsigned char slot, TBeing* ch, TObj* new_obj,
  TRoom* r) {
  ...
  if (ch)
    *ch += *new_obj;
    ...
    vlogf(LOG_BUG,
      "Yikes!  An object was read with no destination in objsFromStore()!");
    return false;
  }
...
bool ItemLoad::objsFromStore(TObj* parent, int* numread, TBeing* ch, TRoom* r,
  bool corpse) {
  signed char slot;
  TObj* new_obj;
  ...
  while (!feof(fp)) {
    ...
    if (fread(&slot, sizeof(signed char), 1, fp) != 1) {
      if (r)
        ...
        vlogf(LOG_BUG,
          format(" %s's objects.  Couldn't read slot.") % ch->getName());
      else
      ...
      if (ch)
        vlogf(LOG_BUG,
          format("%s's objects.  Slot %d > MAX_WEAR.") % ch->getName() % slot);
      else if (r)
        vlogf(LOG_BUG,
          format("Room %d's objects.  Slot %d > MAX_WEAR.") % r->number % slot);
...
void ItemSave::setFile(FILE* f) { fp = f; }

void ItemLoad::setFile(FILE* f) { fp = f; }

void ItemLoad::setVersion(unsigned char v) { version = v; }
...
void ItemSave::objToStore(signed char slot, TObj* o, TBeing* ch, bool d,
  ...
  if (!o->isRentable()) {
    ...
    if (!raw_write_item(o))
      vlogf(LOG_BUG,
        format("Rent error in %s's file") % (ch ? ch->getName() : "UNKNOWN"));
...
void TRoom::loadItems() {
  ...
  if (!il.openFile(filepath)) {
    vlogf(LOG_FILE,
      format("Failed to open file '%s' in TRoom::loadItems() call.") %
        filepath);
    return;
  }
  ...
  for(StuffIter it=tThing->stuff.begin();it!=tThing->stuff.end();){
    TThing *tTemp=*(it++);
      ...
      for(StuffIter it=tThing->stuff.begin();it!=tThing->stuff.end() &&
  (tCont=*it);++it) { TNote * tNote = dynamic_cast<TNote *>(tCont);
  ...
  tNote->action_description); continue;
        }
    ...
    for(StuffIter it=tThing->stuff.begin();it!=tThing->stuff.end();){
      ...
      if (!tCont)
        vlogf(LOG_LOW, format("Storage: Unable to find rent note for: %s") %
  tString);
    }
...
void updateSavedRoom(const char* tfname) {
  ...
  if (!(fp = fopen(fileName, "r+b"))) {
    vlogf(LOG_BUG,
      format("  Error opening the room save file for room #%s") % tfname);
    return;
  }
...
void TPCorpse::removeCorpseFromList(bool updateFile) {
  ...
  if (!found && checkOnLists()) {
    vlogf(LOG_BUG, format("Error in removeCorpseList, corpse says listed but "
                          "no corpse in list (%s).") %
                     getName());
  }
  if (previousCorpse) {
  ...
  if (!otherCorpse && !fileName.empty() && pc_corpse_list) {
    for (otherCorpse = pc_corpse_list; otherCorpse;
         otherCorpse = otherCorpse->nextGlobalCorpse) {
      if (otherCorpse == this) {
        vlogf(LOG_BUG,
          format("Big error in corpse list walking %s") % fileName);
        continue;
      }
      if (!otherCorpse->fileName.empty() &&
          !otherCorpse->fileName.compare(fileName))
        break;
    }
  }
  nextCorpse = NULL;
...
void TPCorpse::addCorpseToLists() {
  ...
  if (checkOnLists())
    vlogf(LOG_BUG,
      format("Call to addCorpseToList for a corpse already on list (%s)") %
        getName());
  if (!pc_corpse_list) {
    pc_corpse_list = this;
    nextGlobalCorpse = NULL;
  ...
  for (tmpCorpse = this; tmpCorpse && tmpCorpse->previousCorpse;
       tmpCorpse = tmpCorpse->previousCorpse)
...
void TBeing::assignCorpsesToRooms() {
  ...
  if (!il.openFile(buf)) {
    ...
    // the game
    //    vlogf(LOG_FILE, format("Failed to open file '%s' in
    //    assignCorpsesToRooms() call.") % buf);
    return;
  }
  ...
  if (!il.readHeader()) {
    vlogf(LOG_BUG,
      format("Error while reading %s's corpse file header.") % getName());
    return;
  }
  ...
  for (StuffIter it = rp->stuff.begin(); it != rp->stuff.end();) {
    ...
    if (isname(name, corpse->name)) {
      ...
      corpse->obj_flags.decay_time =
        max(corpse->obj_flags.decay_time, (short int)60);
      //      corpse->obj_flags.decay_time = max(corpse->obj_flags.decay_time,
      //      MAX_PC_CORPSE_EQUIPPED_TIME);
    }
    if (!corpse->getRoomNum()) {
      ...
      if (corpse->roomp)
        --(*corpse);
      *rp2 += *corpse;
      vlogf(LOG_BUG, format("%s distributed to Storage Room (%d).") %
                       corpse->getName() % Room::STORAGE);
      sendTo(COLOR_BASIC,
        "<r>*** You had a CORPSE placed in the storage area. See a god to get "
        "it back. *** <z>\n\r");
    } else {
      ...
      if (!rp2) {
        ...
        *rp2 += *corpse;
        vlogf(LOG_BUG, format("%s distributed to Storage Room (%d).") %
                         corpse->getName() % Room::STORAGE);
        sendTo(COLOR_BASIC,
          "<r>*** You had a CORPSE placed in the storage area. See a god to "
          "get it back. ***<z>\n\r");
      } else {
        if (corpse->roomp)
          --(*corpse);
        *rp2 += *corpse;
        vlogf(LOG_BUG, format("%s distributed to %s (%d).") %
                         corpse->getName() % rp2->getName() %
                         corpse->getRoomNum());
        sendTo(COLOR_BASIC,
          "<r>*** Your CORPSE has been restored to its place in the World "
          "***.<z>\n\r");
      }
    }
...
void TPCorpse::saveCorpseToFile() {
  ...
  if (fileName.empty()) {
    vlogf(LOG_BUG,
      format("Attempt to save a corpse with no fileName (%s)") % getName());
    return;
  }
  ...
  if (!is.openFile(buf)) {
    vlogf(LOG_FILE,
      format("Failed to open file '%s' in saveCorpseToFile() call.") % buf);
  }
...
int TPerson::saveRent(bool d /*=false*/, int msgStatus /*=0*/) {
  ...
  sprintf(buf, "rent/%c/%s", LOWER(tmp->name[0]),
    sstring(tmp->name).lower().c_str());
  if (!is.openFile(buf)) {
    vlogf(LOG_BUG,
  ...
  if (msgStatus == 1 && desc) {
    vlogf(LOG_PIO,
      format("Saving %s [%d talens/%d bank/%.2f xps/%d items/%d age-mod]") %
        getName() % getMoney() % getBank() % getExp() % is.st.number % age_mod);
  } else if (msgStatus == 2 && desc) {
    vlogf(LOG_PIO,
      format("Renting %s [%d talens/%d bank/%.2f xps/%d items/%d age-mod]") %
        getName() % getMoney() % getBank() % getExp() % is.st.number % age_mod);
  }
...
void TPerson::loadRent() {
  ...
  if (!il.openFile(buf)) {
    if (should_be_logged(this)) {
      ...
      vlogf(LOG_PIO,
        format("Loading %s [%d talens/%d bank/%.2f xps/no items/%d age-mod/no "
               "rent/%d extra pracs (%d-%d)]") %
  ...
  if (name != il.st.owner)
    vlogf(LOG_BUG,
      format("  %s just got %s's objects!") % getName() % il.st.owner);
  ...
  vlogf(LOG_PIO, format("Loading %s [%d talens/%d bank/%.2f xps/%d items/%d "
                        "age-mod/%d extra pracs (%d-%d)]") %
                   getName() % getMoney() % getBank() % getExp() %
                   il.st.number % age_mod % (actual - expectedPracs()) %
...
bool check_stuff_norent(TObj* obj, TBeing* owner, TBeing* recep) {
  ...
  if (obj->isRentable() && obj->isMonogramOwner(owner, true))
    return false;
  act("$n tells you, \"Sorry!  I refuse to store $p.\"", false, recep, obj,
    owner, TO_VICT, ANSI_ORANGE);
  return true;
}
...
int receptionist(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* recep,
  TObj* o) {
  dirTypeT dir;
  roomDirData* exitp;
  TDatabase db(DB_SNEEZY);
  ...
  if (cmd == CMD_GENERIC_PULSE) {
    ...
    if (recep->spec == SPEC_RECEPTIONIST) {
      ...
      for (StuffIter it = recep->roomp->stuff.begin();
           it != recep->roomp->stuff.end() && (t = *it); ++it) {
  ...
  if (ch->isImmortal()) {
    ...
    ch->sendTo(
      "Renting will almost certainly destroy your wizfile.  If you are used "
      "to\n\r");
    ch->sendTo(
      "doing this because of mortal life then it's best to get un-used to "
      "it.\n\r");
    ch->sendTo(
      "If you Have to rent out, such as testing, then go mortal first.\n\r");
    ch->sendTo("----------\n\r");
  }
...
void printLimitedInRent(void) {
  ...
  for (i = 0; i < obj_index.size(); i++) {
    if (obj_index[i].getNumber() > 0) {
      vlogf(LOG_MISC, format("  %d - [%d] : max [%d]") % obj_index[i].virt %
                        obj_index[i].getNumber() % obj_index[i].max_exist);
...
void countAccounts(const char* arg) {
  ...
  sstring account_path =
    ((sstring)(format("account/%c/%s") % arg[0] % arg)).lower();
  if (!(dfd = opendir(account_path.c_str()))) {
    vlogf(LOG_BUG, format("bad path in countAccount (%s) from arg (%s)") %
                     account_path % arg);
    return;
  }
  ...
  while ((dp = readdir(dfd)) != NULL) {
    ...
    if (ret != 0) {
      ...
      if (errno == ENOENT) {
        ...
        vlogf(LOG_MISC,
          format("Deleting reference %s to nonexistent %s in %s's account") %
            ref_path % player_path % arg);
        if (unlink(ref_path.c_str()) != 0)
          vlogf(LOG_FILE,
            format("error in unlink (12) (%s) %d") % ref_path % errno);
      } else {
        vlogf(LOG_FILE, format("ERROR: stat() failed for %s.  errno = %d") %
                          player_path % errno);
        perror("stat");
      }
      continue;
  ...
  try {
    ...
    for (i = MIN_WEAR; i < MAX_WEAR; i++) {  // Initializing
      ...
      if (tmp != -1 && fp2_open == false) {
        char buf[256];
        sprintf(buf, "rent/%c/%s.fr", LOWER(arg[0]),
          sstring(arg).lower().c_str());
      ...
      if (tmp != -1 && (new_obj = il.raw_read_item())) {
        if (ch) {
          vlogf(LOG_SILENT, format("%s's %s rent-retrieving: (%s : %d)") % arg %
                              mob->getName() % new_obj->getName() %
                              new_obj->objVnum());
    ...
    if (ch) {
      ...
      *ch->roomp += *mob;
      ...
      for (aff = mob->affected; aff; aff = aff->next) {
        if ((aff->type == AFFECT_PET) || (aff->type == AFFECT_CHARM) ||
            (aff->type == AFFECT_THRALL) || (aff->type == AFFECT_ORPHAN_PET) ||
...
void updateRentFile(const char* who) {
  ...
  if (sstring(h.owner).lower().compare(who)) {
    vlogf(LOG_BUG,
      format("WARNING!  rent file %s holds objects for %s!") % who % h.owner);
    fclose(fp);
    return;
  }
  if (!load_char(who, &pd)) {
    vlogf(LOG_BUG,
      format("Unable to read player file for %s, so deleting rent file.") %
        h.owner);
    fclose(fp);
    wipeRentFile(who);
    // removeFollowers();
  ...
  if (pd.load_room == Room::AUTO_RENT) {  // this person was autorented
    ...
    if (!raw_save_char(who, &pd)) {
      vlogf(LOG_BUG,
        format("Error updating player-file entry for %s in updateRentFile.") %
          h.owner);
      return;
    }
...
bool TBeing::saveFollowers(bool rent_time) {
  ...
  sprintf(buf, "rent/%c/%s.fol", LOWER(tmp->name[0]),
    sstring(tmp->name).lower().c_str());
  ...
  for (f = followers; f; f = f2) {
    ...
    fprintf(fp, "%d %d %d %.1f %d %d %.1f+%d\n", mob->getClass(),
      mob->GetMaxLevel(), mob->getHitroll(), mob->getACLevel(), mob->getHit(),
      mob->points.maxHit, mob->getDamLevel(), mob->getDamPrecision());
    fprintf(fp, "%d %d %d %d %d %d\n", mob->getMoney(), (int)mob->getExp(),
      (int)mob->getMaxExp(), mob->getRace(), (int)mob->getWeight(),
      mob->getHeight());
    ...
    fprintf(fp, "%d %d %d %d\n", mapPosToFile(mob->getPosition()),
      mapPosToFile(mob->default_pos), mob->getSex(), mob->spec);
    ...
    fprintf(fp, "%d %d %d\n", mob->getMaterial(WEAR_BODY), mob->canBeSeen,
      mob->visionBonus);
    ...
    for (af = a_list, i = 0; i < MAX_AFFECT; i++) {
      if (af) {
        fprintf(fp, "%d %d %d %ld %ld %d %" PRIu64 "\n",
          mapSpellnumToFile(af->type), af->level, af->duration,
          applyTypeShouldBeSpellnum(af->location)
            ? mapSpellnumToFile(spellNumT(af->modifier))
            : af->modifier,
          af->modifier2, mapApplyToFile(af->location), af->bitvector);
        af = af->next;
      } else {
    ...
    for (i = MIN_WEAR; i < MAX_WEAR; i++) {  // Initializing
      ...
      // find the wear slot that corresponds to i in the file
      wearSlotT mapped_slot = mapFileToSlot(i);
      fprintf(fp, "%d %d", mob->getLimbFlags(mapped_slot),
      ...
      if (!obj)
        ...
        fprintf(fp, " %d\n", version);
        if (!fp2_open) {
          sprintf(buf, "rent/%c/%s.fr", LOWER(tmp->name[0]),
            sstring(tmp->name).lower().c_str());
          if (!(fp2 = fopen(buf, "w+b"))) {
            vlogf(LOG_BUG,
              format("Error opening %'s follower rent file for write.") %
                tmp->name);
          } else
            fp2_open = true;
        }
        ...
        if (rent_time) {
          vlogf(LOG_SILENT, format("%s's %s renting: (%s : %d)") % getName() %
...
bool TBeing::loadFollowers() {
  ...
  sprintf(buf, "rent/%c/%s.fol", LOWER(tmpPer->name[0]),
...
void TPerson::loadToggles() {
  ...
  sprintf(caFilebuf, "player/%c/%s.toggle", LOWER(name[0]),
    sstring(name).lower().c_str());
...
pcorpseObject::pcorpseObject() : corpse_room(0), num_corpses_room(0) {
...
void processCorpseFile(const char* cfName) {
  ...
  if (!(fp = fopen(fileName, "r+b"))) {
    vlogf(LOG_FILE,
      format("  Error opening the corpse file for corpse %s") % cfName);
    return;
  }
  if ((fread(&h, sizeof(rentHeader), 1, fp)) != 1) {
    vlogf(LOG_BUG, format("Error reading version from %s.") % fileName);
...
void processCorpseFiles() { dirwalk("corpses", processCorpseFile); }
...
void TBeing::doClone(const sstring& arg) {
  ...
  if (!il.openFile(buf)) {
    sendTo(
      "Rent file could not be opened.  Your clone stands naked before "
      "you.\n\r");
    return;
  }
  if (!il.readHeader()) {
    vlogf(LOG_BUG, format("Error while reading %s's rent file header.  Your "
  ...
  for (StuffIter it = mob->stuff.begin(); it != mob->stuff.end();) {
    ...
    if ((o = dynamic_cast<TObj*>(i))) {
      ...
      vlogf(LOG_BUG,
        format("did not add no-rent flag to %s when cloning") % i->name);
    ...
    if ((dynamic_cast<TBaseContainer*>(i))) {
      for (StuffIter itt = i->stuff.begin(); itt != i->stuff.end();) {
        ...
        if ((bo = dynamic_cast<TObj*>(j))) {
          ...
          vlogf(LOG_BUG,
            format("did not add no-rent flag to %s when cloning") % bo->name);
