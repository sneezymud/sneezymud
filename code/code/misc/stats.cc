...
int TBeing::getDexReaction() const {
  double raw = plotStat(STAT_CURRENT, STAT_DEX, -4, 6, 0);
  int    reaction = static_cast<int>(std::round(raw));
...
int TBeing::getAgiReaction() const {
  double raw = plotStat(STAT_CURRENT, STAT_AGI, -4, 6, 0);
  int    reaction = static_cast<int>(std::round(raw));
...
int TBeing::getSpeReaction() const {
  double raw = plotStat(STAT_CURRENT, STAT_SPE, -4, 6, 0);
  int    reaction = static_cast<int>(std::round(raw));
