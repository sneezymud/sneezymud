...
int TBeing::getChaReaction() const {
  return plotStat(STAT_CURRENT, STAT_CHA, -4, 6, 0);
}
...
bool TBeing::isStrong(int num, bool silent) const {
...
bool TBeing::isPerceptive(int num, bool silent) const {
...
bool TBeing::isAgile(int num, bool silent) const {
...
bool TBeing::isDextrous(int num, bool silent) const {
  if (affectedBySpell(SPELL_HASTE))
    num += 5;
  if (affectedBySpell(SPELL_CELERITE))
    num += 5;
...
bool TBeing::isTough(int num, bool silent) const {
  if (hasDisease(DISEASE_POISON))
    num -= 10;
...
bool TBeing::isBrawny(int num, bool silent) const {
  if (hasDisease(DISEASE_POISON))
    num -= 10;
  if (hasDisease(DISEASE_FLU))
    num -= 10;
...
bool TBeing::isIntelligent(int num, bool silent) const {
  if (affectedBySpell(SPELL_STUPIDITY))
    num -= 10;
  if (affectedBySpell(SKILL_SUBTERFUGE))
    num -= 10;
...
bool TBeing::isWise(int num, bool silent) const {
  if (affectedBySpell(SPELL_STUPIDITY))
    num -= 10;
  if (affectedBySpell(SKILL_SUBTERFUGE))
    num -= 10;
...
bool TBeing::isQuick(int num, bool silent) const {
  if (affectedBySpell(SPELL_ACCELERATE))
    num += 5;
  if (affectedBySpell(SPELL_HASTE))
    num += 10;
...
bool TBeing::isFocused(int num, bool silent) const {
  if (affectedBySpell(SPELL_STUPIDITY))
    num -= 10;
  if (affectedBySpell(SKILL_SUBTERFUGE))
    num -= 10;
...
bool TBeing::isCharismatic(int num, bool silent) const {
...
bool TBeing::isKarmic(int num, bool silent) const {
  if (affectedBySpell(SPELL_CURSE))
    num -= 10;
  return statSelfCheck(STAT_KAR, num);
  }

bool TBeing::isUgly(int num, bool silent) const {
  if (!isCharismatic(num, true)) {
    if (!silent)
      sendTo("You are so <y>ugly<1>! Everyone thinks so!\n\r");
    return true;
  }
  if (!silent)
...
bool TBeing::isRealUgly(int num, bool silent) const {
   return (isUgly(num - 35, silent));
  }
