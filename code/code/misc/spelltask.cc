...
/*
    Cosmo 6-10-97:
  to start a cast, do:
  int start_cast(TBeing *ch, TBeing *victim, TThing *obj, TRoom *rp, spellNumT
  spell, int difficulty, int target, char *arg, int rounds, ushort wasInRoom,
  ubyte status, int flags, int text, int nextUpdate, int offensive) ch = caster
...
void TBeing::stopCast(stopCastT messages) {
  ...
  if (target) {
    if (!(ch = target->getCaster())) {
      vlogf(LOG_BUG, format("%s doesnt have a casterList in stopCast(%s)") %
      ...
      if (target->getCaster() == this) {
        ...
        if (!ch)
          vlogf(LOG_BUG, format("%s not found in spelltasking list on %s") %
                           target->getName() % getName());
      }
    }
  }
...
int start_cast(TBeing* ch, TBeing* victim, TThing* obj, TRoom* rp,
  spellNumT spell, taskDiffT difficulty, int target, const char* arg,
  lag_t rounds, ushort wasInRoom, ubyte status, int flags, int text,
  int nextUpdate) {
  ...
  if (!ch || (ch->spelltask)) {
    ...
    // first...
    vlogf(LOG_BUG,
      format("%s got to bad place in start_cast (%d).  Tell a coder.") %
...
void cast_warn_busy(const TBeing* ch, spellNumT which) {
  ...
  if (!ch || !(ch->spelltask)) {
    vlogf(LOG_BUG,
      format("%s got to bad place in cast_warn_busy.  Tell a coder") %
...
int TBeing::cast_spell(TBeing* ch, cmdTypeT cmd, int pulse) {
  ...
  if (!ch->spelltask) {
    vlogf(LOG_BUG,
      format("Somehow %s got to cast_spell with no spelltask structure,") %
        ch->getName());
    act(
  ...
  switch (cmd) {
    case CMD_TASK_CONTINUE:
      ...
      //          to anything.  The code does a roll for 1. total fail, 2. time
      //          added or nothing then clears distraction bit. distractions
      //          will be bash, or skills or big w lloping hits
      ...
      vlogf(LOG_BUG, format("%s has a distract of %d in round %d on spell %s") %
                       ch->getName() % distract % rounds %
                       discArray[spell]->name);
#endif

      if (distract &&
          (((2 * distract) >= ::number(1, 20)) || (distract == -1))) {
        if (typ == SPELL_CASTER) {
          colorAct(COLOR_SPELLS,
        ...
        if (typ == SPELL_CASTER) {
          ...
          vlogf(LOG_BUG,
            format("Distracted(%d) distract= %d add 1 to rounds = %d") %
              ch->spelltask->distracted % distract % ch->spelltask->rounds);
#endif
          ch->spelltask->rounds++;
          rounds++;
        } else if (typ == SPELL_PRAYER) {
        ...
        ch->spelltask->distracted = 0;
      } else if (rounds && ((ch->plotStat(STAT_CURRENT, STAT_WIS, 1, 10, 6,
                              1.0)) > (::number(1, 100)))) {
        if (typ == SPELL_CASTER) {
          colorAct(COLOR_SPELLS,
      ...
      if (IS_SET(ch->spelltask->flags, CASTFLAG_CAST_INDEFINITE)) {
        ...
        counter = 1;
        // if not last or next to last round counter switch will use default
        // case
      }
      if (isPc()) {
...
// NEED a FUNCTION
          ...
          if ((ch->GetMaxLevel() < GOD_LEVEL1) && ch->desc &&
              (ch->spelltask && !ch->spelltask->component) &&
              IS_SET(discArray[spell]->comp_types, COMP_MATERIAL)) {
            sendTo(COLOR_SPELLS,
              "<r>You seem to have lost your component.<z>\n\r");
            vlogf(LOG_BUG, format("%s got to end of casting without using "
                                  "component, spell:%s (%d)") %
        ...
        default:
          vlogf(LOG_BUG,
            format("Somehow %s got sent to bad counter case in cast_spell") %
              ch->getName());
          break;
      }
      break;
...
// returns DELETE_VICT
// returns DELETE_ITEM
int TBeing::doSpellCast(TBeing* caster, TBeing* victim, TObj* o, TRoom* room,
  spellNumT which, skillUseTypeT styp) {
  const char* orgArg;
  TThing* t;
  int ok;
  int rc = 0;
  ...
  if (!ok) {
    if ((discArray[which]->targets & TAR_CHAR_WORLD) ||
        (discArray[which]->targets & TAR_CHAR_WORLD))
...
void TBeing::sendCastingMessages(bool limbs, bool silence, int round,
  ...
  if (counter == 2 || roll) {
    ...
    if (limbs) {
      ...
      switch (roll) {
        case 4:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "You continue to trace a complex pattern in the air with your "
              "hand.");
            sprintf(buf2,
              "$n continues to trace a complex magic pattern in the air.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "You continue to sing your rada song to the ancestors.");
        ...
        case 3:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "You trace another line of the magic rune in the air.");
            sprintf(buf2,
              "$n concentrates hard as $e makes a gesture in the air.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "You continue to dance and can feel the power of your "
              "ancestors.");
            sprintf(buf2,
              "$n continues to dance. You can feel a stange power eminating "
              "from $m.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "Your symbol flares with heat as you raise it towards the "
              "heavens.");
            sprintf(buf2,
              "$n's symbol flares as it is raised towards the heavens.");
          }
          break;
        case 2:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "You add another line to the magic rune you have been tracing.");
            sprintf(buf2,
              "$n adds another line to the magic rune $e has been tracing.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "You continue to sing your rada song to the ancestors.");
            ...
            sprintf(buf2,
              "$n's symbol glows with power as $e calls on its strength.");
          }
          break;
        case 1:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "The air shimmers around your hand as it follows a magic line in "
              "the air.");
            sprintf(buf2,
              "The air shimmers around $n's hand as it follows a magic line in "
              "the air.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "The ground starts to move with the power of the loa.");
            sprintf(buf2,
              "A blood red mist starts to form around $n as $e continues to "
              "dance.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "You raise your symbol towards the heavens looking for favor.");
            sprintf(buf2,
              "$n holds $s symbol towards the heaven looking for favor.");
          }
          break;
        default:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "Your gesture continues to fill the air with part of a magic "
              "rune.");
            sprintf(buf2,
              "$n's gesture fills the air with part of a magic rune.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "Your ritualistic dance and rada fill you with power.");
            sprintf(buf2, "$n's ritualistic gestures fill the air with power.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "Your symbol shines with a faint light as you raise it toward "
              "the heavenly sky.");
            sprintf(buf2,
              "$n's symbol shines with a faint light as it is raised toward "
              "the heavenly sky.");
          }
          break;
      }
      ...
      if (typ == SPELL_CASTER) {
        ...
        sprintf(buf,
          "Your symbol glows as you focus on finishing your prayer.");
        sprintf(buf2,
          "$n's symbol glows as $e focuses on finishing $s prayer.");
      }
    }
    ...
    // Then to room
    if (sendToRoom && roomp) {
      for (StuffIter it = stuff.begin(); it != stuff.end() && (temp = *it);
           ++it) {
    ...
    if (silence) {
      ...
      switch (roll) {
        case 4:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "You lyrically continue to chant the words of a gentle "
              "incantation.");
            sprintf(buf2,
              "$n lyrically continues to chant the words of a gentle "
              "incantation.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "You lyrically continue to call upon the ancestors for power "
              "through your rada song.");
        ...
        case 3:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "You continue to utter the words needed to complete your spell.");
            sprintf(buf2,
              "$n continues to utter the words needed to complete $s spell.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "You continue to sing the song needed to complete the ritual.");
            sprintf(buf2, "$n continues to sing to complete $s ritual.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "You continue to utter the words needed to complete your "
              "prayer.");
            sprintf(buf2,
              "$n continues to utter the words needed to complete $s prayer.");
          }
          break;
        case 2:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "Your words form the pattern calling on the magic to be "
              "unleashed.");
            sprintf(buf2,
              "$n words form a pattern calling on the magic to be released.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "Your song calls upon the ancestors to release their powers.");
            sprintf(buf2,
              "$n song calls upon $s ancestors to release their power.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "Your voice forms the historic words of supplication and "
              "entreaty.");
            sprintf(buf2,
              "$n gives voice to the historic words of supplication and "
              "entreaty.");
          }
          break;
        case 1:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "Your voice calls on the primordal forms and draws forth the "
              "primal magic.");
            sprintf(buf2,
              "$n's voice calls on the primordal forms and draws forth the "
              "primal magic.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "Your voice calls the ancestors and draws their power forth.");
            sprintf(buf2, "$n's voice calls to $s ancestors to do $s bidding.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "Your voice continues towards the culmination of your prayer.");
            sprintf(buf2,
              "$n's voice is heard clearly echoing the words of prayer.");
          }
          break;
        default:
          if (typ == SPELL_CASTER) {
            sprintf(buf,
              "Your voice forms the words continuing the incantation.");
            sprintf(buf2,
              "$n's voice can be heard chanting the words of the incantation.");
          } else if (typ == SPELL_DANCER) {
            sprintf(buf,
              "Your voice forms the words continuing the invokation.");
            sprintf(buf2,
              "$n's voice can be heard singing the words of ancestoral power.");
          } else if (typ == SPELL_PRAYER) {
            sprintf(buf,
              "Your faith is funnelled into the soft words of prayer you "
              "chant.");
            sprintf(buf2,
              "$n chants more of the simple words of prayer and supplication.");
          }
          break;
      }
    } else {
      ...
      if (typ == SPELL_CASTER) {
        ...
        sprintf(buf,
          "Your voice is silent as you focus on finishing your prayer.");
      }
    }
    // First do the character
    if (desc) {
