...
TAccount::TAccount() : birth(time(0)) {}

bool TAccount::read(const sstring& aname) {
  TDatabase db(DB_SNEEZY);
  db.query(
    "select multiplay_limit, account_id, email, passwd, name, birth, term, "
    "time_adjust, flags, last_logon from account where name=lower('%s')",
    aname.c_str());
  if (!db.fetchRow())
    return false;
...
bool TAccount::write(const sstring& aname) {
  ...
  if (!db.fetchRow()) {
    ...
    if (account_id == 0) {
      db.query("select account_id from account where name='%s'", aname.c_str());
      assert(db.fetchRow());
      account_id = convertTo<int>(db["account_id"]);
    }
