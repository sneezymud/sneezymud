        ...
        switch (num) {
          ...
          case MESS_RAIN_START:
            ...
            switch (ch->roomp->getSectorType()) {
              case SECT_DESERT:
                text =
                  "<k>The clouds overhead darken and look more ominous.<1>\n\r";
                break;
              default:
          ...
          case MESS_SNOW_START:
            switch (ch->roomp->getSectorType()) {
              ...
              case SECT_DESERT:
                text =
                  "<B>The cooler temperatures and worsening weather allow a "
                  "light desert rain to fall.<1>\n\r";
                ch->playsound(SOUND_RAIN_START, SOUND_TYPE_NOISE);
                break;
              case SECT_SAVANNAH:
              case SECT_VELDT:
          ...
          case MESS_LIGHTNING:
            ...
            switch (ch->roomp->getSectorType()) {
              case SECT_DESERT:
                text =
                  "<k>The clouds overhead grow dark and thunder can be heard "
                  "in the distance.<1>\n\rIt's not going to rain this time of "
                  "year though.\n\r";
          ...
          case MESS_BLIZZARD:
            switch (ch->roomp->getSectorType()) {
              ...
              case SECT_DESERT:
                snd = pickRandSound(SOUND_THUNDER_1, SOUND_THUNDER_4);
                text =
                  "<W>The desert rain intensifies and you are caught in a "
                  "lightning storm.<1>\n\r";
                ch->playsound(snd, SOUND_TYPE_NOISE);
                break;
              case SECT_SAVANNAH:
          ...
          case MESS_LIGHTNING_AWAY:
            switch (ch->roomp->getSectorType()) {
              ...
              default:
                text =
                  "<B>The lightning has gone, but it is still raining.<1>\n\r";
            }
            break;
          case MESS_BLIZZARD_AWAY:
            switch (ch->roomp->getSectorType()) {
              case SECT_JUNGLE:
                text =
                  "<B>The lightning has gone, but a jungle rain continues to "
                  "fall.<1>\n\r";
                break;
              case SECT_DESERT:
                text =
                  "<B>The lightning has gone, but a light desert rain "
                  "continues to fall.<1>\n\r";
                break;
              case SECT_SAVANNAH:
              case SECT_VELDT:
              ...
              default:
                text =
                  "<W>The blizzard is over, but it is still snowing.<1>\n\r";
            }
            break;
          default:
            vlogf(LOG_BUG,
              format("Bad num %d sent to sendWeatherMessage") % num);
            break;
        }
  ...
  switch (month) {
    ...
    case 1:
      sendToOutdoor(COLOR_NONE, "It is very cold.\n\r",
        "The chill in the air begins to lessen.\n\r");
      break;
    case 2:
    ...
    case 6:
      sendToOutdoor(COLOR_NONE, "It is warm and humid.\n\r",
        "A hot dry breeze blows from the west.\n\r");
      break;
    case 7:
      sendToOutdoor(COLOR_NONE, "It starts to get a little windy.\n\r",
        "A cool breeze blows across the humid land.\n\r");
      break;
    case 8:
      sendToOutdoor(COLOR_NONE, "The air is getting chilly.\n\r",
        "The weather is less humid nowadays.\n\r");
      break;
    case 9:
...
void TBeing::describeWeather(int room) {
  ...
  rp = real_roomp(room);
  if (!rp) {
    vlogf(LOG_BUG, format("No roomp for room %d in describeWeather for %s") %
                     room % getName());
    return;
  }
  ...
  if (wth == Weather::SNOWY)
    ...
    sendTo(COLOR_BASIC,
      "<B>The rain comes down in sheets, soaking you to the bone.<1>\n\r");
...
bool isGroundWater(sectorTypeT sector) {
  return TerrainInfo[sector] &&
         (sector == SECT_ARCTIC_MARSH || sector == SECT_ARCTIC_RIVER_SURFACE ||
           sector == SECT_ICEFLOW || sector == SECT_COLD_BEACH ||
           sector == SECT_TEMPERATE_SWAMP || sector == SECT_TEMPERATE_OCEAN ||
           sector == SECT_TEMPERATE_RIVER_SURFACE ||
           sector == SECT_TEMPERATE_BEACH || sector == SECT_TROPICAL_SWAMP ||
...
int getRoomWetness(TBeing* ch, TRoom* room, sstring& better, sstring& worse) {
  ...
  if (wetness > 0 && ch && isGroundWater(room->getSectorType())) {
    ...
    // if youre sitting, resting or lower you end up getting more wet with
    // groundwater
...
const sstring Weather::describeWet(int wetness) {
  ...
  static const sstring DescribeWet[] = {"slightly damp", "damp", "slightly wet",
    "wet", "dripping wet", "sopping wet", "drenched", "completely soaked",
    "waterlogged"};

  if (wetness <= 0)
    return DescribeDry[max(
      min((int)cElements(DescribeDry) - 1, -(wetness / 10)), 0)];
  return DescribeWet[max(
    min((int)cElements(DescribeWet) - 1, (wetness / 10) - 1), 0)];
}
...
int Weather::addWetness(TBeing* ch, int diffWet) {
  ...
  if (!wetAffect && diffWet > 0) {
    ...
    diffWet = wetAffect->modifier =
      max(0, min(WET_MAXIMUM, int(wetAffect->modifier) + diffWet));
    if (diffWet <= 0)
