  ...
  switch (message) {
    ...
    case DISEASE_PULSE:
      ...
      if (!number(0, 50)) {
        ...
        victim->sendTo(format("You feel your %s throb and the bruise turns a "
    ...
    case DISEASE_DONE:
      ...
      if (victim->getPosition() > POSITION_DEAD && victim->hasPart(i)) {
        victim->sendTo(format("The bruise on your %s fades away!\n\r") %
                       victim->describeBodySlot(i));
        sprintf(buf, "The bruise on $n's %s fades away!",
  ...
  switch (message) {
    ...
    case DISEASE_PULSE:
      ...
      if (!victim->isLimbFlags(i, PART_BANDAGED) && !::number(0, 750)) {
        // start an infection
        victim->rawInfect(i, ::number(50, 150), SILENT_NO, CHECK_IMMUNITY_YES,
          victim->GetMaxLevel());
      }
      ...
      if (!number(0, 10)) {
        if (victim->doesKnowSkill(SKILL_SNOFALTE)) {
          ...
          if ((::number(0, 99) < 40) &&
              victim->bSuccess((int)victim->getSkillValue(SKILL_SNOFALTE),
                SKILL_SNOFALTE)) {
            victim->sendTo(
              "You utilize the powers of snofalte to slow your bleeding.\n\r");
            break;
          }
        }
        victim->sendTo(format("You feel your energy drained as your blood "
                              "drips out of your %s.\n\r") %
                       victim->describeBodySlot(i));
        sprintf(buf, "$n looks stunned as blood drips from $s %s.",
          victim->describeBodySlot(i).c_str());
        ...
        // took out the damage computations since it always ended up 1 no matter
        // what slot anyhow which i think is fine since i upped chances for
        // bleeding from combat and from the disease prayer and added a chance
        // for infections
    ...
    case DISEASE_DONE:
      ...
      if (victim->getPosition() > POSITION_DEAD && victim->hasPart(i)) {
        victim->sendTo(format("Your %s clots and stops bleeding!\n\r") %
                       victim->describeBodySlot(i));
        sprintf(buf, "$n's %s clots and stops bleeding!",
          victim->describeBodySlot(i).c_str());
        act(buf, TRUE, victim, NULL, NULL, TO_ROOM);
      }
