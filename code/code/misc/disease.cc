...
void spread_affect(TBeing* ch, int chance_to_spread, bool race, bool not_race,
  ...
  if (ch->inRoom() == Room::NOCTURNAL_STORAGE)
    return;
  int spread_controller =
    1;  // to reduce rate of spreading in crowds. it gets out of hand.
  ...
  for (StuffIter it = ch->roomp->stuff.begin();
    ...
    if (v->spec == SPEC_HORSE_PESTILENCE || v->isShopkeeper() ||
  ...
  switch (::number(1, 150)) {
    ...
    case 2:
      ...
      if (getPosition() <= POSITION_SLEEPING) {
        ...
        if ((rc = reconcileDamage(this, ::number(1, 10) + 15,
               DAMAGE_SUFFOCATION)) == -1)
          return DELETE_THIS;
      }
      break;
    case 3:
...
int disease_flu(TBeing* victim, int message, affectedData*) {
  ...
  if (message == DISEASE_BEGUN) {
    ...
    if (victim->isHumanoid()) {
      act("$n starts to sweat and $s face looks pale.", true, victim, nullptr,
        nullptr, TO_ROOM);
    } else {
      act("$n looks lethargic and glassy eyed.", true, victim, nullptr, nullptr,
        TO_ROOM);
    }
    ...
    if (victim->getPosition() > POSITION_DEAD) {
      act("$n relaxes a bit.", true, victim, nullptr, nullptr, TO_ROOM);
...
int TBeing::dummyCold() {
  ...
  switch (::number(1, 100)) {
    ...
    case 10:
      ...
      act("$n's nose starts to drip and $e sniffs loudly.", true, this, 0, 0,
        TO_ROOM);
      break;
    case 11:
    case 12:
...
int disease_cold(TBeing* victim, int message, affectedData*) {
  ...
  if (message == DISEASE_PULSE) {
    ...
    act("$n doesn't look very well.", true, victim, nullptr, nullptr, TO_ROOM);
    victim->sendTo("You don't feel very well at all.\n\r");
    return false;
  } else if (message == DISEASE_DONE) {
    if (victim->getPosition() > POSITION_DEAD) {
      act("Some color has returned to $n.", true, victim, nullptr, nullptr, TO_ROOM);
...
int disease_bruised(TBeing* victim, int message, affectedData* af) {
  ...
  if (bruiseLocation < MIN_WEAR || bruiseLocation >= MAX_WEAR) {
    vlogf(LOG_BUG,
      format("disease_bruised called with bad slot: %i") % bruiseLocation);
    return false;
  }
  ...
  switch (message) {
    ...
    case DISEASE_PULSE:
      ...
      if (!victim->hasPart(bruiseLocation) ||
          !victim->isLimbFlags(bruiseLocation, PART_BRUISED)) {
        af->duration = 0;
        break;
      }
...
int disease_bleeding(TBeing* victim, int state, affectedData* af) {
  ...
  if (bleedLocation < MIN_WEAR || bleedLocation >= MAX_WEAR) {
    vlogf(LOG_BUG,
      format("disease_bleeding called with bad slot: %i") % bleedLocation);
    return false;
  }
  ...
  switch (state) {
    ...
    case DISEASE_PULSE: {
      ...
      // BLEED_STACKS% chance on every bleed tick for an unbandaged bleed to
      // become infected if victim fails constitution check. Capped at 5% from
      // bleed stacks.
      ...
      if (percentChance(realBleedChance)) {
        if (victim->doesKnowSkill(SKILL_SNOFALTE) &&
          ...
          victim->sendTo(
            "You focus your mind and forcefully prevent yourself from "
            "bleeding.\n\r");
          break;
        }

        const sstring toVict = format("Blood drips from a wound on your %s.") %
                               victim->describeBodySlot(bleedLocation);
        act(toVict, true, victim, nullptr, nullptr, TO_CHAR, ANSI_RED);

        const sstring toRoom = format("Blood drips from a wound on $n's %s.") %
                               victim->describeBodySlot(bleedLocation);
    ...
    case DISEASE_DONE:
      ...
      if (victim->getPosition() > POSITION_DEAD &&
          victim->hasPart(bleedLocation)) {
        victim->sendTo(
          format("The wound on your %s clots and stops bleeding!\n\r") %
          victim->describeBodySlot(bleedLocation));
        const sstring toRoom =
          format("The wound on $n's %s clots and stops bleeding!") %
          victim->describeBodySlot(bleedLocation);
        act(toRoom, true, victim, nullptr, nullptr, TO_ROOM);
      }
  ...
  switch (message) {
    ...
    case DISEASE_PULSE:
      ...
      if (af->duration == PERMANENT_DURATION &&
          !victim->isLimbFlags(slot, PART_GANGRENOUS)) {
        af->duration = 100;
      }
      ...
      if (!number(0, max(14, 35 - (int)((double)level / 3.2)))) {
        victim->sendTo(
          format(
            "Your %s shakes and twinges as the infection in it festers.\n\r") %
          victim->describeBodySlot(slot));
        sprintf(buf,
          "$n's %s shakes and twinges as the infection in it festers.",
    ...
    case DISEASE_DONE:
      ...
      if (victim->getPosition() > POSITION_DEAD && victim->hasPart(slot)) {
        victim->sendTo(format("Your %s stops twitching and heals!\n\r") %
                       victim->describeBodySlot(slot));
        sprintf(buf, "$n's %s stops twitching and heals!",
          victim->describeBodySlot(slot).c_str());
        act(buf, true, victim, nullptr, nullptr, TO_ROOM);
      }
...
int disease_syphilis(TBeing* ch, int message, affectedData*) {
  switch (message) {
    case DISEASE_PULSE:
      ...
      act("$n breaks out in open sores.", true, ch, 0, 0, TO_ROOM);
...
int disease_hemorrhaging(TBeing* victim, int message, affectedData*) {
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      victim->sendTo(
        "An immense pain in your chest causes you to grunt in agony!\n\r");
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      for (i = MIN_WEAR; i < MAX_WEAR; i++) {
        if (victim->validEquipSlot(i) && !(victim->equipment[i]) &&
            !number(0, 3)) {
          victim->hurtLimb(
            victim->getMaxLimbHealth((wearSlotT)i) / ::number(1, 5),
            (wearSlotT)i);
          sprintf(buf,
            "$n's %s takes on a gray-blue color as frost-bite sets in.",
            victim->describeBodySlot(i).c_str());
          act(buf, true, victim, nullptr, nullptr, TO_ROOM);
          victim->sendTo(format("Your %s feels numb and takes on a gray-blue "
                                "color.  Looks like frostbite.\n\r") %
                         victim->describeBodySlot(i));
        }
      }
      ...
      break;
    case DISEASE_BEGUN:
      act("$n looks very cold and has begun shivering involuntarily.", true,
        victim, nullptr, nullptr, TO_ROOM);
      act("Frostbite will likely set in soon.", false, victim, nullptr, nullptr,
        TO_ROOM);
      victim->sendTo(
        "You are very cold and have begun shivering involuntarily.\n\r");
      victim->sendTo("Frostbite will likely set in soon.\n\r");
      break;
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("$n still looks cold, but the shivering has stopped.", true, victim,
...
int disease_voicebox(TBeing* victim, int message, affectedData*) {
  switch (message) {
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("$n's voicebox heals and $s voice returns to $m.", true, victim, 0,
          0, TO_ROOM);
        victim->sendTo(
          "You feel your larynx mend and your voice has returned.\n\r");
      }
      break;
    default:
      break;
...
int disease_eyeball(TBeing* victim, int message, affectedData*) {
  switch (message) {
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("$n's missing eyeballs slowly regenerate.", true, victim, 0, 0,
          TO_ROOM);
        victim->sendTo(
          "You feel something growing in your empty eye sockets.\n\r");
        victim->sendTo("These new eyeballs don't seem to work fully.\n\r");
        victim->sendTo(
          "Perhaps a cleric can cure your blindness now though...\n\r");
      }
      break;
    default:
      break;
...
int disease_stomach(TBeing* victim, int message, affectedData*) {
  ...
  switch (message) {
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("$n's stomach wound slowly closes up and mends.", true, victim, 0,
          0, TO_ROOM);
        victim->sendTo(
          "Your stomach wound slowly closes over and mends itself.\n\r");
      }
      break;
    default:
      break;
  ...
  switch (message) {
    case DISEASE_PULSE:
      if (number(0, 2))
        return false;
      victim->sendTo(
        "You gasp and wheeze, you can't seem to breathe properly!\n\r");
      act(
        "$n makes gasping and wheezing sounds as the air rushes from $s "
        "punctured lung.",
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        victim->sendTo(
          "The hole in your lung seems to close up and your lungs thankfully "
          "fill with air.\n\r");
        act("The hole in $n's lung heals and $e takes a big gulp of air.", true,
          victim, 0, 0, TO_ROOM);
      }
      break;
    default:
...
int disease_suffocate(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      // narrowed this from 1-50 (at 50th) to 25-50 or so... the damage was very
      // lame
      victim->sendTo("GASP!  You can't breathe!\n\r");
      act("$n grabs $s throat and gasps, trying to breathe.", true, victim, 0,
        0, TO_ROOM);
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        ...
        act("Air returns to $n as $e takes a deep breath of air.", false,
          victim, 0, 0, TO_ROOM);
      }
      break;
    default:
...
int TTool::garottePulse(TBeing* victim, affectedData* af) {
  ...
  victim->sendTo("GASP!  You can't breathe!\n\r");
  act("$n grabs $s throat and gasps, trying to breathe.", true, victim, 0, 0,
    TO_ROOM);
  ...
  switch (message) {
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        victim->sendTo("The vaccum around you disappates.\n\r");
        victim->sendTo("You can breathe again!\n\r");
        act("Air returns to $n as $e takes a deep breath of air.", true, victim,
          0, 0, TO_ROOM);
      }
      break;
    default:
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      switch (::number(1, 5)) {
        ...
        case 2:
          ch->sendTo(
            "Your stomach churns, and the taste of bile comes to your "
            "mouth.\n\r");
          break;
        case 3:
        case 4:
          ch->sendTo(
            "You don't feel well at all.  It must have been something you "
            "ate.\n\r");
          break;
        case 5:
          ...
          if (ch->getPosition() <= POSITION_SLEEPING) {
            ...
            if ((rc = ch->reconcileDamage(ch, ::number(1, 10) + 15,
                   DAMAGE_SUFFOCATION)) == -1)
              return DELETE_THIS;
          }
          break;
      ...
      act("$n's face is pale and $e doesn't look well at all.", true, ch, 0, 0,
        TO_ROOM);
    ...
    case DISEASE_DONE:
      if (ch->getPosition() > POSITION_DEAD) {
        ch->sendTo(
          "The discomfort in your stomach and bowels seems to have cleared up, "
          "thankfully.\n\r");
      }
      break;
    default:
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      switch (::number(1, 5)) {
        ...
        case 2:
          ch->sendTo(
            "Toxins course through your body and you are overcome by "
            "cramps.\n\r");
          break;
        case 3:
        case 4:
...
int disease_drowning(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_PULSE:
      ...
      dam = ::number(1, min((int)af->level, 50));
      victim->sendTo("GASP!  You can't breathe!\n\r");
      act("$n flails about and gasps, trying to breathe.", false, victim, 0, 0,
        TO_ROOM);
      victim->addToMove(-::number(1, 10));
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        victim->sendTo(
          "The water around you evaporates.  You can breath again!\n\r");
        act("Oxygen returns to $n as $e takes a deep breath of air.", true,
          victim, 0, 0, TO_ROOM);
      }
      break;
    default:
...
int disease_herpes(TBeing*, int, affectedData*) { return false; }

int disease_broken_bone(TBeing*, int, affectedData*) { return false; }

static bool badSpreadSlot(const TBeing* ch, wearSlotT slot) {
  return (!ch->hasPart(slot) || slot == HOLD_RIGHT || slot == HOLD_LEFT);
}
...
void TBeing::bodySpread(int chance_to_spread, affectedData* af) {
  ...
  if (af->modifier == DISEASE_INFECTION) {
    // the goal here is to have the spread rate keep step with the dissipation
    // rate so that infections generally just slow down healing the older method
    // guaranteed death over time this would probably be neater if I understood
    // probability...
    int spread_em = (50000 / chance_to_spread) +
                    ::number(-(chance_to_spread / 10), (chance_to_spread / 10));
    vaf.duration = spread_em;
    if (af->duration == PERMANENT_DURATION) {
      // slightly weaken if spreading from a permanent infection (like from
      // gangrene)... too deadly...
      vaf.duration = max(10, vaf.duration - 15);
    }
    if (choice1 && !isLimbFlags(choice1, PART_INFECTED) && hasPart(choice1))
      vaf.level = choice1;
    else if (choice2 && !isLimbFlags(choice2, PART_INFECTED) &&
             hasPart(choice2))
      vaf.level = choice2;
    else
    ...
    if (!part)
      sendTo(
        format("Infection has set into your %s!\n\r") % describeBodySlot(slot));
    else
      sendTo(
        format("Infection spreads to your %s!\n\r") % describeBodySlot(slot));
    ...
    if (choice1 && !isLimbFlags(choice1, PART_LEPROSED) && hasPart(choice1))
      vaf.level = choice1;
    else if (choice2 && !isLimbFlags(choice2, PART_LEPROSED) &&
             hasPart(choice2))
      vaf.level = choice2;
    else
    ...
    sendTo(
      format(
        "Lesions begin to form on your %s and it begins to feel numb!\n\r") %
...
void TBeing::dummyLeprosy(wearSlotT part) {
  if (!::number(0, 199)) {
    sendTo(format("The lesions covering your %s thicken.\n\r") %
           describeBodySlot(part));
    act(format("The lesions covering $n's %s appear thicker.") %
          describeBodySlot(part),
  ...
  switch (message) {
    ...
    case DISEASE_BEGUN:
      if (slot) {
        ...
        act("Your skin becomes scaly and insensitive to feeling!", false,
          victim, 0, 0, TO_CHAR);
        act("$n's skin begins to look scaly!", true, victim, nullptr, nullptr,
          TO_ROOM);
      }
      break;
    case DISEASE_DONE:
      if (slot) {
        ...
        if (victim->hasPart(slot)) {
          act(format(
                "The skin of your %s softens a bit and the feeling returns.") %
                victim->describeBodySlot(slot),
            false, victim, 0, 0, TO_CHAR);
        }
      } else if (victim->getPosition() > POSITION_DEAD) {
        act("The hardened lesions on your skin disappear.", false, victim, 0, 0,
          TO_CHAR);
        act("The lesions covering $n disappear.", true, victim, nullptr, nullptr,
          TO_ROOM);
      }
      break;
  }
...
int disease_plague(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_PULSE:
      if (slot != WEAR_NOWHERE && !victim->isLimbFlags(slot, PART_LEPROSED)) {
        af->duration = 0;
        break;
      }
      vaf.type = AFFECT_DISEASE;
    ...
    case DISEASE_BEGUN:
      act(
        "Painful lumps form over your body and <o>dark, weeping sores<1> "
        "appear!",
        false, victim, 0, 0, TO_CHAR);
      act("Lumps and <o>dark splotches<1> begin to cover $n's body!", true,
        victim, nullptr, nullptr, TO_ROOM);
      if (slot != WEAR_NOWHERE)
        victim->addToLimbFlags(slot, PART_LEPROSED);
      break;
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("The festering sores and lumps covering your body disappear!",
          false, victim, 0, 0, TO_CHAR);
        act("The festering sores and lumps covering $n's body disappear!!",
...
int disease_gangrene(TBeing* victim, int message, affectedData* af) {
  // produces limb rot (non-spreading) + infection (spreads) + fever & damage
  // when on critical slots
  ...
  if (slot < MIN_WEAR || slot >= MAX_WEAR) {
    vlogf(LOG_BUG, format("disease_gangrene called with bad slot: %i on %s") %
                     slot % victim->getName());
    return false;
  }
  ...
  switch (message) {
    case DISEASE_BEGUN:
      ...
      act(format("Your %s has become <k>gangrenous<1>!") %
            victim->describeBodySlot(slot),
        false, victim, 0, 0, TO_CHAR);
      act(format("$n's %s has become <k>gangrenous<1>!") %
            victim->describeBodySlot(slot),
        true, victim, nullptr, nullptr, TO_ROOM);
      break;
    case DISEASE_PULSE:
      if (isVitalPart(slot) && victim->getCurLimbHealth(slot) <= 0) {
        ...
        // vlogf(LOG_MISC, format("%s at 0 on %s") %
        // victim->describeBodySlot(slot) % victim->getName());
      }
      ...
      if (!number(0, 10)) {
        switch (number(0, 1)) {
          case 0:
            victim->sendTo(format("You are in a world of pain as your %s "
                                  "darkens and swells.\n\r") %
                           victim->describeBodySlot(slot));
            break;
          case 1:
            act(format("Your %s discharges a <o>foul<1> <k>black<1> ichor.  "
                       "The pain!") %
                  victim->describeBodySlot(slot),
              false, victim, 0, 0, TO_CHAR);
            break;
        ...
        act("The foul stench of dying flesh surrounds $n.", false, victim, nullptr,
          nullptr, TO_ROOM);

        // do some damage if it's a critical body part
        if (isVitalPart(slot))
          if (victim->reconcileDamage(victim, ::number(1, 4), SPELL_INFECT) ==
              -1)
            return DELETE_THIS;
        ...
        if (!number(0, 1)) {
          if (!victim->isLimbFlags(slot, PART_INFECTED)) {
            victim->rawInfect(slot, PERMANENT_DURATION, SILENT_NO,
              CHECK_IMMUNITY_YES, af->modifier2);
          }
        }
        break;
      }
    ...
    case DISEASE_DONE:
      ...
      if (victim->getPosition() > POSITION_DEAD && victim->hasPart(slot)) {
        act(format("You experience profound relief as the <k>gangrene<1> in "
                   "your %s disappears!") %
              victim->describeBodySlot(slot),
          false, victim, 0, 0, TO_CHAR);
        act(format("$n's necrotic %s regains some of its health!") %
              victim->describeBodySlot(slot).c_str(),
          true, victim, nullptr, nullptr, TO_ROOM);
      }
      break;
...
int disease_scurvy(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_BEGUN:
      act("Your skin <w>pales<1> and your eyes begin to hurt.", false, victim,
        0, 0, TO_CHAR);
      act("$n pales and $s eyes become hollow and bl<r>oo<1>dsh<r>o<1>t.", true,
        victim, nullptr, nullptr, TO_ROOM);
      break;
    case DISEASE_PULSE:
      wearSlotT slot;
      ...
      switch (number(0, 500)) {
        case 0:
          ...
          buf = format(
                  "A <k>rotten<1> tooth lies here, having fallen from %s's "
                  "mouth.") %
                victim->getName();
          corpse->setDescr(buf);
          ...
          act(
            "Your gums ache and a <w>tooth<1> comes loose, falling to the $g!",
            false, victim, 0, 0, TO_CHAR);
          act("A <k>rotten<1> tooth falls from $n's mouth.", false, victim,
            corpse, nullptr, TO_ROOM);
          *victim->roomp += *corpse;
          break;
        case 1:
        case 2:
        ...
        case 4:
          ...
          if (victim->rawBruise(slot, 225, SILENT_YES, CHECK_IMMUNITY_NO)) {
            // note: bruise duration is doubled in rawBruise for all you scurvy
            // coves
            act(format("Your %s feels painfully tender and a <p>mottled<1> "
                       "<P>bruise<1> blooms.") %
                  victim->describeBodySlot(slot),
              false, victim, 0, 0, TO_CHAR);
            act(format("A mottled bruise forms on $n's %s.") %
                  victim->describeBodySlot(slot),
              true, victim, nullptr, nullptr, TO_ROOM);
          }
          break;
        ...
        case 7:
          if (!victim->isUndead()) {
            ...
            if (found) {
              ...
              act(
                format(
                  "Your %s feels painfully tender and begins to <r>bleed<1>.") %
                  victim->describeBodySlot(slot),
                false, victim, 0, 0, TO_CHAR);
              act(format("$n's %s starts to bleed.") %
                    victim->describeBodySlot(slot),
                true, victim, nullptr, nullptr, TO_ROOM);
            }
          }
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        act("Your are <c>relieved<1> of some of your aches and pains!", false,
          victim, 0, 0, TO_CHAR);
        act("Some <c>life<1> returns to $n's eyes.", true, victim, nullptr, nullptr,
          TO_ROOM);
      }
      break;
    default:
...
int disease_dysentery(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_BEGUN:
      act(
        "Something rotten in your guts causes you to double over momentarily.",
        false, victim, 0, 0, TO_CHAR);
      if (victim->isHumanoid()) {
        act("$n pales and doubles over momentarily.", true, victim, nullptr, nullptr,
          TO_ROOM);
      } else {
        act("$n winces and doubles over momentarily.", true, victim, nullptr, nullptr,
          TO_ROOM);
      }
      break;
    case DISEASE_PULSE:
      switch (number(0, 50)) {
        ...
        case 2:
          // weaken
          victim->sendTo(
            "There is a painful knot in your stomach and you feel clammy.\n\r");
        ...
        case 4:
          ...
          if (victim->isPc()) {
            if (victim->getCond(POOP) <= 0) {
              ...
              victim->sendTo(
                "You experience a hot flash and your bowels clench.\n\r");
              if (victim->isHumanoid()) {
                act("$n looks pale and sweaty.", true, victim, nullptr, nullptr,
                  TO_ROOM);
              } else {
                act("$n looks uncomfortable.", true, victim, nullptr, nullptr,
                  TO_ROOM);
              }
            } else {
              victim->sendTo(
                "Your stomach is gripped by a dreadful spasm and you cannot "
                "contain yourself.\n\r");
              act("You have <o>besmirched<1> yourself!", false, victim, nullptr,
                nullptr, TO_CHAR);
              act(
                "The air around $n is woefully befouled as $e <o>besmirches<1> "
                "$mself.",
                false, victim, nullptr, nullptr, TO_ROOM);
              victim->dropPool(
                min((int)victim->getWeight() / 10, (int)victim->getCond(POOP)),
                LIQ_POT_FILTH);
              victim->setCond(THIRST,
                max(0, ((int)(victim->getCond(THIRST) -
                              (2 * (int)victim->getCond(POOP))))));
              victim->setCond(POOP, 0);
            }
          } else {
            switch (::number(0, 1)) {
              // doesn't actually do anything to thirstless & pooless mobs, so
              ...
              case 1:
                ...
                act("You have <o>besmirched<1> yourself!", false, victim, nullptr,
                  nullptr, TO_CHAR);
                act(
                  "The air around $n is woefully befouled as $e "
                  "<o>besmirches<1> $mself.",
                  false, victim, nullptr, nullptr, TO_ROOM);
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        victim->sendTo(
          "The gnawing in your stomach ceases and you can walk normally "
          "again!\n\r");
        act("$n appears as if a weight were lifted from $s back.", true, victim,
          nullptr, nullptr, TO_ROOM);
      }
      break;
    default:
...
int disease_pneumonia(TBeing* victim, int message, affectedData* af) {
  ...
  switch (message) {
    case DISEASE_BEGUN:
      act("Your chest begins to hurt and breathing becomes difficult.", false,
        victim, 0, 0, TO_CHAR);
      act("$n looks <w>pale<1> and $s breathing becomes labored.", true, victim,
        nullptr, nullptr, TO_ROOM);
      break;
    case DISEASE_PULSE:
      ...
      switch (number(0, 49)) {
        ...
        case 1:
          ...
          act("$n begins to pant in short, sharp breaths.", true, victim, 0, 0,
            TO_ROOM);
        ...
        case 3:
          // cough blood: damage
          act("You cough violently, bringing up <G>phelgm<1> and <r>blood<1>.",
            false, victim, 0, 0, TO_CHAR);
          act(
            "$n erupts into a hoarse, hacking cough.  It does not sound good.",
            true, victim, 0, 0, TO_ROOM);
          if (victim->reconcileDamage(victim, ::number(2, 4), SPELL_DISEASE) ==
              -1)
            return DELETE_THIS;
          break;
        case 4:
          ...
          if (dam <= 0)
            return false;
          victim->sendTo(
            "Your lungs burn with pain and you cannot breathe!\n\r");
          act("$n's face <b>darkens<1> as $e struggles to draw a breath.", true,
            victim, 0, 0, TO_ROOM);
          victim->addToMove(-::number(0,
            (10 * ((100 - victim->getImmunity(IMMUNE_SUFFOCATION)) / 100))));
          if (victim->reconcileDamage(victim, dam, DAMAGE_SUFFOCATION) == -1)
            return DELETE_THIS;
          break;
        default:
    ...
    case DISEASE_DONE:
      if (victim->getPosition() > POSITION_DEAD) {
        victim->sendTo(
          "Your lungs clear and you regain the strength to draw breath!\n\r");
        act("$n loses some of $s pallor.", true, victim, nullptr, nullptr, TO_ROOM);
      }
      break;
    default:
      // do nothing
...
//
// applies&removes an AGI penalty manually, as the affect code can't handle
// adding that kind of stuff to a disease affect
int disease_extreme_pain(TBeing* vict, int stage, affectedData* aff) {
  switch (stage) {
    case DISEASE_BEGUN:
      ...
      act("$n is rendered senseless by the pain of $s wounds.", false, vict,
        nullptr, nullptr, TO_ROOM);
      act("You are rendered senseless by the pain of your wounds.", false, vict,
        nullptr, nullptr, TO_CHAR);
    ...
    case DISEASE_PULSE:
      switch (number(0, 30)) {
        case 0:
          act("$n writhes in extreme agony.", false, vict, nullptr, nullptr,
            TO_ROOM);
          act("You cannot help but writhe wretchedly on the $g!", false, vict,
            nullptr, nullptr, TO_CHAR);
          break;
        case 1:
          act("$n groans piteously.", false, vict, nullptr, nullptr, TO_ROOM);
          act("A deep groan forces itself from your throat.", false, vict,
            nullptr, nullptr, TO_CHAR);
          break;
        case 2:
          act("$n shrieks in raw torment!", false, vict, nullptr, nullptr,
            TO_ROOM);
          act("You scream your throat raw from the torment!", false, vict,
            nullptr, nullptr, TO_CHAR);
          break;
        case 3:
          act("$n twitches and jerks helplessly.", false, vict, nullptr,
            nullptr, TO_ROOM);
          act("Your limbs dance uncontrollably!", false, vict, nullptr, nullptr,
            TO_CHAR);
          break;
        case 4:
        default:
    ...
    case DISEASE_DONE:
      act("$n gasps in relief as the pain recedes.", false, vict, nullptr,
        nullptr, TO_ROOM);
      act("The pain dissipates and you regain control of yourself.", false,
