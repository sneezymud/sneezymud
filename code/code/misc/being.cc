...
int TBeing::chiMe(TBeing* tLunatic) {
  int bKnown = tLunatic->getSkillValue(SKILL_CHI), tDamage, tMana;
  ...
  if (tLunatic == this) {
    ...
    if (bSuccess(bKnown, SKILL_CHI)) {
      ...
      act(
        "You close your eyes and concentrate for a moment, then begin "
        "radiating an intense <R>heat<1> from your body.",
      ...
      act("You are unable to focus your mind.", TRUE, this, NULL, NULL,
        TO_CHAR);
  ...
  if (this->isImmortal() || this->inGroup(*tLunatic) ||
      tLunatic->noHarmCheck(this))
    return FALSE;

  if (tLunatic->bSuccess(bKnown, SKILL_CHI)) {
    tDamage = getSkillDam(this, SKILL_CHI, tLunatic->getSkillLevel(SKILL_CHI),
    ...
    act("You unleash your chi upon $N!", FALSE, tLunatic, NULL, this, TO_CHAR);
    ...
    if (affectedBySpell(SKILL_CHI)) {
      ...
      act(
        "A bright <W>aura<1> flares up around $N, deflecting $n's chi attack "
      ...
      if (this->reconcileDamage(tLunatic, ::number((tDamage / 2), tDamage),
            SKILL_CHI) == -1)
        return (DELETE_THIS | RET_STOP_PARSING);
    } else {
      act("...$N screws up $S face in agony.", TRUE, tLunatic, NULL, this,
        TO_CHAR);
      act("$n exerts $s <r>chi force<1> on you, causing extreme pain.", TRUE,
        tLunatic, NULL, this, TO_VICT);
      act("$N screws up $S face in agony.", TRUE, tLunatic, NULL, this,
        TO_NOTVICT);
    ...
    act("You fail to harm $N with your <r>blast of chi<z>!", FALSE, tLunatic,
      NULL, this, TO_CHAR);
    act("You escape $n's attempt to chi you!", TRUE, tLunatic, NULL, this,
      TO_VICT);
    act("$N avoids $n's attempt to chi them!", TRUE, tLunatic, NULL, this,
      TO_NOTVICT);
  }
