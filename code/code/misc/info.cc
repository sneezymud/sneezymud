...
sstring TBeing::describeAffects(TBeing* ch, showMeT showme) const {
  ...
  for (aff = ch->affected; aff; aff = af2) {
    ...
    switch (aff->type) {
      ...
      case SKILL_SAP:
        if (show) {
          // modifier2 stores the limb information
          if (aff->modifier2 > 0) {
            str +=
              format(
                "Your %s is paralyzed from being sapped. Duration: %s\n\r") %
              describeBodySlot(static_cast<wearSlotT>(aff->modifier2)) %
              describeDuration(this, aff->duration);
          } else {
            str +=
              format("You are recovering from being sapped. Duration: %s\n\r") %
              describeDuration(this, aff->duration);
          }
        }
        break;
