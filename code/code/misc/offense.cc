...
int TBeing::frostEngulfed() {
  ...
  if (affectedBySpell(AFFECT_WET) && !isImmune(IMMUNE_COLD, WEAR_BODY)) {
    ...
    if (!hasDisease(DISEASE_FROSTBITE)) {
      ...
      sendTo(
        "You feel the onset of frostbite as your skin takes on a bluish "
        "tint!\n\r");
      act("$n's wet body begins to freeze in the intense cold!", TRUE, this, 0,
        0, TO_ROOM);
      act("$n's skin takes on a bluish tint as frostbite sets in.", TRUE, this,
        0, 0, TO_ROOM);
