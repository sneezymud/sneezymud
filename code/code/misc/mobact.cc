  ...
  for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
    ...
    if (tbt->fight() && IS_SET(tbt->specials.act, ACT_PROTECTEE)) {
      ...
      if ((IS_SET(specials.act, ACT_PROTECTOR) &&
          ...
          (getRace() == tbt->getRace())) {
#endif
      if (IS_SET(specials.act, ACT_PROTECTOR) && isSameFaction(tbt) &&
          canSee(tbt->fight())) {
    ...
    if (found) {
      for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
        ...
        if ((isSameFaction(tbt->fight())) &&
          ...
          if (isSameFaction(tbt) && (getRace() == tbt->getRace())) {
            continue;
          } else if (canSee(tbt->fight())) {
            doSay("I will come to thy Aid!");
            if (getPosition() < POSITION_STANDING)
      ...
      act("$n watches the fight closely with some concern.", false, this, nullptr,
        nullptr, TO_ROOM);
      return false;
    } else {
      return false;
      ...
      if (tmp->fight() == master) {
        if (hasClass(CLASS_WARRIOR)) {
          ...
          if (!master->isPc() || IS_SET(specials.act, ACT_GUARDIAN)) {
            if (!isDumbAnimal())
              act("$n hollers, \"I'll save you master!\"", false, this, 0, 0,
                TO_ROOM);
            else
              aiGrowl(tmp);
            ...
            if (!::number(0, 2)) {
              ...
              act("$n tries to rescue you but fails.", false, this, 0, master,
                TO_VICT);
            }
...
static bool legalMobMovement(TMonster* mob, dirTypeT door) {
  ...
  if (!exit_ok(exitp, &rp) || rp->isRoomFlag(ROOM_DEATH) ||
...
int TMonster::mobileWander(dirTypeT door) {
  ...
  if ((getPosition() < POSITION_STANDING) ||
      ...
      (rp->isRoomFlag(ROOM_NO_MOB) && !isPolice()) || !(rp2 = roomp) ||
      master || !legalMobMovement(this, door))
    return 0;
  ...
  if ((rp->isWaterSector() &&
      ...
      (rp->isVertSector() && !(isFlying() || raceHasNaturalClimb())) ||
      (rp->isAirSector() && !(isFlying()))) {
    if (!canFly())
      return 0;
  ...
  if (!::number(0, 4) && hasClass(CLASS_CLERIC) && persist >= 30 && !fight() &&
    ...
    if (roomp && hunted->roomp &&
      ...
      if (doesKnowSkill(SPELL_SUMMON) && getSkillValue(SPELL_SUMMON) > 33 &&
          !isNotPowerful(hunted, getSkillLevel(SPELL_SUMMON), SPELL_SUMMON,
            SILENT_YES)) {
        act(
          "$n utters the words, 'Set it free and if it returns, it was meant "
          "to be.'",
      ...
      if (doesKnowSkill(SPELL_ASTRAL_WALK) &&
        ...
        act("$n utters the words, 'You can run, but you can't hide!'", true,
          this, 0, 0, TO_ROOM);
  ...
  if (persist <= 0) {
    if (sameRoom(*hunted)) {
      if (hunted == fight()) {
        ...
        hunt_dist = 0;
        return false;
      } else if ((Hates(hunted, nullptr) || hunted->fight()) && canSee(hunted)) {
        rc = targetFound();
        if (IS_SET_DELETE(rc, DELETE_THIS))
          return DELETE_THIS;
        return 1;
    ...
    for (count = 0; count < amt; count++) {
      ...
      for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end(); ++it)
        if (dynamic_cast<TGas*>(*it) &&
            dynamic_cast<TGas*>(*it)->getType() == GAS_MUSK) {
          count++;
          break;
        }
      res = dirTrack(hunted);
      ...
      if (spec == SPEC_ARCHER && (next_room = clearpath(in_room, res)) &&
        ...
        for (j = 0; j < bows.size(); j++) {
          ...
          if (!bow->stuff.empty())
            break;
          if (!bow)
            vlogf(LOG_BUG, "mobact.cc: bow is null somehow");
        ...
        if (bow && (ammo || !bow->stuff.empty())) {
          if (archer(nullptr, CMD_GENERIC_PULSE, nullptr, this, nullptr))
            break;
        }
      }
      ...
      if (res != DIR_NONE) {
        ...
        if (sameRoom(*hunted)) {
          if (Hates(hunted, nullptr) || hunted->fight()) {
            rc = targetFound();
            if (IS_SET_DELETE(rc, DELETE_THIS))
              return DELETE_THIS;
            return true;
...
// DELETE_VICT = ch
int TObj::scavengeMe(TBeing*, TObj**) { return false; }
...
// why do we need to rearrange inventory?
  ...
  // best_o = nullptr here, check room for goodies
  ...
  if (best_o->parent) {
    // arbitrarily cast it to a corpse, non-corpses will be nullptr
    ...
    if (corpse) {
      vlogf(LOG_MOB_AI,
        format("Mob superScavenger: %s looting %s from %s in room (%d)") %
          this->name % best_o->name % corpse->name %
          (roomp ? roomp->in_room : 0));
  ...
  if (best_o) {
    vlogf(LOG_MOB_AI,
      format("Mob superScavenger: %s picking up %s in room (%d)") % this->name %
        best_o->name % (roomp ? roomp->in_room : 0));
    strcpy(buf, best_o->name.c_str());
    strcpy(buf, add_bars(buf).c_str());
    rc = doGet(buf);
    if (IS_SET_DELETE(rc, DELETE_THIS))
...
int TMonster::senseWimps() {
  ...
  // if we are being used as a cheap tank (by exploiting
  // zombie/leprosy/cityguard), lets turn the tables we don't want this to
  // happen if you're helping defend a city though, so skip if in cities
  if (roomp && isAffected(AFF_AGGRESSOR) && !inGrimhaven() && !inAmber() &&
      !inLogrus() && !inBrightmoon() &&
      !fight()->isPet(PETTYPE_PET | PETTYPE_CHARM | PETTYPE_THRALL) &&
      !fight()->isPc()) {
    bool beingUsed = false;
    // if a PC using me to tank?
    for (StuffIter it = roomp->stuff.begin();
         !beingUsed && it != roomp->stuff.end() && (t = *it); ++it) {
  ...
  for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
    ...
    if (!::number(0, 31)) {
      if (tmp_victim->fight()) {
        ...
        if (tmp_victim->isPc() && tmp_victim->task &&
            (tmp_victim->getStat(STAT_CURRENT, STAT_FOC) >=
              ::number(-10, 270))) {
          act("You feel nudged but you stay focused on your task.", true, this,
            nullptr, tmp_victim, TO_VICT);
          continue;
        } else if (!tmp_victim->isPc() &&
                   ::number(1, 100) <
                     min(99,
                       max(GetMaxLevel(), tmp_victim->GetMaxLevel()) - 1)) {
          // added to reduce chance of brawling as mob level increases
          // % chance of avoiding brawl is equal to brawler or brawlee level
          // (whichever is higher)
          continue;
          ...
          act("Suddenly, $N finds $Mself involved in the brawl.", true, this, 0,
            tmp_victim, TO_NOTVICT);
          act("Suddenly, you find yourself involved in the brawl.", true, this,
            0, tmp_victim, TO_VICT);
          vlogf(LOG_MOB_AI, format("Brawl! %s (%d) brawls %s (%d) in %d") %
                              tmp_victim->name % tmp_victim->GetMaxLevel() %
                              this->name % this->GetMaxLevel() %
                              (roomp ? roomp->in_room : 0));
    ...
    // to.  most of its arbitrary, ie. a mounted person _should_ be more
    // powerful in reality, but otherwise the master (a PC) would rarely be hit
  ...
  if (tbt && (!::number(0, 1))) {
    act("$n realizes that $N is just an innocent mount.", true, this, 0,
      fight(), TO_ROOM);
    if ((wimp = tbt)->fight())
      wimp->stopFighting();

    stopFighting();
    act("$n lashes into $N, the mount's master!", true, this, 0, wimp,
      TO_NOTVICT);
    act("$n lashes into you, the mount's master!", true, this, 0, wimp,
      TO_VICT);
    setCharFighting(wimp);
    return true;
  }
  ...
  if (wimp->fight()) {
    ...
    if (wimp->isPc() && wimp->task &&
      ...
      act("You feel nudged but you stay focused on your task.", true, this,
        nullptr, wimp, TO_VICT);
      return false;
    } else if (!wimp->isPc() && ::number(1, 65) < max(int(GetMaxLevel()),
                                                    wimp->GetMaxLevel() - 1)) {
      // added to reduce chance of brawling as mob level increases
      // % chance of avoiding brawl is equal to brawler or brawlee level
      // (whichever is higher)
      ...
      act("You are nearly pulled into the brawl, but manage to avoid it.", true,
        this, 0, wimp, TO_VICT);
      return false;
    } else {
      act("Suddenly, $N finds $Mself involved in the brawl.", true, this, 0,
        wimp, TO_NOTVICT);
      act("Suddenly, you find yourself involved in the brawl.", true, this, 0,
        wimp, TO_VICT);
      vlogf(LOG_MOB_AI, format("Brawl! %s (%d) lured to %s (%d) in (%d)") %
                          wimp->name % wimp->GetMaxLevel() % this->name %
                          this->GetMaxLevel() % (roomp ? roomp->in_room : 0));
      if (getPosition() == POSITION_RESTING ||
          getPosition() == POSITION_SITTING)
        doStand();
...
static bool mobBSlamCheck(TMonster& mob, TBeing& vict) {
  ...
  // weight
  return (
    (compareWeights(vict.getTotalWeight(true), mob.carryWeightLimit()) != -1) &&
    // dex
...
int TMonster::monkMove(TBeing& vict) {
  ...
  if (getPosition() <= POSITION_SITTING) {
    ...
    switch (num) {
      case 1:
        if ((this->attackers > 2) && (getMove() >= 80) &&
            (getPosition() == POSITION_STANDING) &&
            (doesKnowSkill(SKILL_HURL)) && (getSkillValue(SKILL_HURL) > 66)) {
          int i;
          ...
          for (i = 0; i < 20; i++) {
            ...
            if (this->canGo(hurlDir)) {
              vlogf(LOG_ANGUS, format("monkMove: %s hurling %s, dir= %i") %
                                 name % vict.name % hurlDir);
      ...
      case 2:
        if ((GetMaxLevel() > 75) && (doesKnowSkill(SKILL_BONEBREAK)) &&
            (getMove() >= 80) &&
      ...
      case 3:
        if (!vict.hasClass(CLASS_MONK) &&
            (vict.getPosition() == POSITION_STANDING) &&
            (getPosition() == POSITION_STANDING) &&
            (doesKnowSkill(SKILL_SHOULDER_THROW)) && (getMove() >= 80)) {
      ...
      case 5:
        if ((getPosition() >= POSITION_STANDING) &&
            (!vict.affectedBySpell(SKILL_CHI)) && (getMana() > 200)) {
          return doChi("", &vict);
          break;
        }
      default:
...
// this goes with disarm at the end
  ...
  if (::number(0, 1)) {
    ...
    if (hasHands() && (getPosition() >= POSITION_CRAWLING) && prim &&
        prim->isPierceWeapon()) {
      return doStab("", &vict);
    }
  } else {
    if (getSkillValue(SKILL_DISARM_THIEF) > 33 &&
        canDisarm(&vict, SILENT_YES) && (getPosition() >= POSITION_CRAWLING) &&
        !vict.affectedBySpell(SPELL_FUMBLE)) {
      return doDisarm("", &vict);
    }
  }
...
static bool faerieFireCheck(TBeing& ch, TBeing& vict, spellNumT spell) {
  if (!vict.affectedBySpell(spell) && !::number(0, 9) &&
    ...
    act("$n utters the words, 'Tingle all over!'", true, &ch, 0, 0, TO_ROOM);
    return true;
  }
  return false;
}
...
static spellNumT get_mage_spell(TMonster& ch, TBeing& vict, bool& on_me) {
  ...
  discNumT good_discs[] = {DISC_AIR, DISC_ALCHEMY, DISC_EARTH, DISC_FIRE,
    DISC_SORCERY, DISC_SPIRIT, DISC_WATER, DISC_NONE};
  ...
  if (ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
    ...
    if (!::number(0, 30) && !ch.pissed() && (ch.getHit() < ch.hitLimit() / 8) &&
      ...
      act("$n utters the words, 'Departing is such Sweet Sorrow!'", false, &ch,
        0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
    ...
    if (ch.attackers >= 2 && vict.isPc() && !::number(0, 30)) {
  ...
  if (best_disc == DISC_MAGE) {
    ...
    if (ch.attackers >= 2 && ::number(0, ch.attackers - 1)) {
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Burn, Acid Burn!'", true, &ch, 0, 0,
          TO_ROOM);
        return spell;
      }
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Frozen Death!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_SAND_BLAST;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Stone Wash!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_TORNADO;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'There's no place like home!'", true, &ch, 0,
          0, TO_ROOM);
        return spell;
      }
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Napalm!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_COLOR_SPRAY;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Magic Rainbow Colors!'", true, &ch, 0, 0,
          TO_ROOM);
        return spell;
      }
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Mr. Clean!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_ARCTIC_BLAST;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'York Peppermint Patty!'", true, &ch, 0, 0,
          TO_ROOM);
        return spell;
      }
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Gravel Travel!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
    }
    ...
    if (!::number(0, 9) && ch.doesKnowSkill(SPELL_DISPEL_MAGIC) &&
      ...
      act("$n utters the words, 'Back to the basics!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 9) && (cutoff < discArray[spell]->start) &&
        ch.getSkillValue(spell) > 33) {
      act("$n utters the words, 'Cold Shoulder!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 3) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Destroying Arrow!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_GRANITE_FISTS;
    if (!::number(0, 3) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Pumice Pummel!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_MYSTIC_DARTS;
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell)) {
      act("$n utters the words, 'Missiles, come forth!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (ch.hasHands() && !::number(0, 1) &&
      ...
      act("$n utters the words, 'Fire, Fire Everywhere!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = SPELL_GUSHER;
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell)) {
      act("$n utters the words, 'Jack and Jill Time!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell)) {
      act("$n utters the words, 'David and Goliath!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = SPELL_GUST;
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell)) {
      act("$n utters the words, 'Summer Breeze!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Catch your Breath!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 3) && !vict.affectedBySpell(SPELL_IMMOBILIZE) &&
        (cutoff < discArray[spell]->start) && ch.doesKnowSkill(spell) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Freeze!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_GUST;
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
    ...
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell)) {
      act("$n utters the words, 'Crack and Break!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (ch.attackers >= 2 && ::number(0, ch.attackers - 1)) {
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Krakatoa!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_SAND_BLAST;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Flocks of Rocks!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_GRANITE_FISTS;
    if (!::number(0, 3) && (cutoff < discArray[spell]->start) &&
    ...
    if (ch.attackers >= 2 && ::number(0, ch.attackers - 1)) {
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Snowball's chance!'", true, &ch, 0, 0,
          TO_ROOM);
        return spell;
      }
      spell = SPELL_FIREBALL;
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Pillar of Flame!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_FLAMING_SWORD;
    if (!::number(0, 3) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Blade of Fire!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_HANDS_OF_FLAME;
    if (!::number(0, 1) && (cutoff < discArray[spell]->start) &&
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Feel my WRATH!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Jankity JANK!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_BLAST_OF_FURY;
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
    ...
    spell = SPELL_FUMBLE;
    if ((cutoff < discArray[spell]->start) && ch.doesKnowSkill(spell) &&
        (vict.heldInPrimHand() || vict.heldInSecHand())) {
      act("$n utters the words, 'Butter Fingers!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (ch.attackers >= 2 && ::number(0, ch.attackers - 1)) {
      ...
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
          ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Surf's up!'", true, &ch, 0, 0, TO_ROOM);
        return spell;
      }
      spell = SPELL_ICE_STORM;
      if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
    ...
    if (!::number(0, 8) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Mirror Mirror!'", true, &ch, 0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
    ...
    if (!::number(0, 6) && (cutoff < discArray[spell]->start) &&
        ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Sleep with the Fishies!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = SPELL_ICY_GRIP;
...
int TMonster::dynamicComponentLoader(spellNumT spell, int loadrate) {
  ...
  for (comp = 0;
       (comp < CompInfo.size()) && (spell != CompInfo[comp].spell_num); comp++)
    ;
  ...
  if (!(tcomp = findComponent(spell))) {
    if (::number(1, 100) <= loadrate) {
      tcomp = dynamic_cast<TComponent*>(
        read_object(CompInfo[comp].comp_num, VIRTUAL));
      if (!tcomp)
        return false;
...
int TMonster::mageMove(TBeing& vict) {
  ...
  if (getPosition() < POSITION_STANDING) {
    vlogf(LOG_BUG, format("%s was not standing in mageMove vs %s.") %
                     getName() % vict.getName());
    return false;
  }
...
int TMonster::shamanMove(TBeing& vict) {
  ...
  if (getPosition() < POSITION_STANDING) {
    vlogf(LOG_BUG, format("%s was not standing in shamanMove vs %s.") %
                     getName() % vict.getName());
    return false;
  }
  ...
  if (on_me) {
    ...
    vlogf(LOG_BUG, format("Shaman Mob casting (1) spell %d on self with "
                          "possibly bad target flags for spell") %
                     spell);
    return doDiscipline(spell, name);
  } else {
    return doDiscipline(spell, vict.name);
...
static spellNumT get_cleric_heal_spell(TMonster& ch, TBeing& targ) {
  // best to worst
  spellNumT heals[] = {SPELL_HEAL_FULL, SPELL_HEAL_CRITICAL, SPELL_HEAL_SERIOUS,
    SPELL_HEAL_LIGHT};
  int count = 4, i;
  wearSlotT slot;
...
static spellNumT get_cleric_spell(TMonster& ch, TBeing& vict, bool& on_me) {
  ...
  for (j = 0; j < 6; j++) {
    ...
    if (!ch.getDiscipline(best_disc))
      return TYPE_UNDEFINED;
    if (ch.getDiscipline(i) && (ch.getDiscipline(i)->getLearnedness() >
                                 ch.getDiscipline(best_disc)->getLearnedness()))
      best_disc = i;
  }
  ...
  if (ch.doesKnowSkill(spell) && (ch.getSkillValue(spell) > 33)) {
    ...
    if (!::number(0, 30) && !ch.pissed() && (ch.getHit() < ch.hitLimit() / 8) &&
        ...
        !ch.affectedBySpell(AFFECT_TEST_FIGHT_MOB)) {
      act("$n sing-songs 'Na na na na, na na na na, hey hey hey, goodbye!'",
        false, &ch, 0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
  ...
  if (best_disc == DISC_CLERIC) {
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33) &&
        ch.canParalyzeLimb(&vict, SILENT_YES)) {
      act("$n utters the words, 'Be Still!'", true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    spell = SPELL_BLINDNESS;
    if (!::number(0, 6) && !vict.affectedBySpell(spell) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters, \"Darkness...take over the land!!!\"", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    spell = SPELL_HEAL;
    if (!::number(0, 6) && (ch.getHit() < ch.hitLimit()) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act(
        "$n beseeches, \"Oh lord, unleash pestilence upon this unrighteous "
        "one!\"",
        true, &ch, 0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Pain and Suffering'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = SPELL_FLAMESTRIKE;
    if (!::number(0, 4) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act("$n biblethumps, \"Hellfire and Lava unleashed!\"", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && (ch.getHit() < ch.hitLimit()) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'I feel much better now'", true, &ch, 0, 0,
        TO_ROOM);
      on_me = true;
      return spell;
    }
    spell = ch.getSkillNum(SPELL_RAIN_BRIMSTONE);
    if (!::number(0, 1) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters, \"Let's see how this smells!\"", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = ch.getSkillNum(SPELL_BLESS);
    if (!::number(0, 6) && !ch.affectedBySpell(SPELL_BLESS) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
    ...
    if (!::number(0, 6) && !ch.affectedBySpell(SPELL_ARMOR) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'This will only hurt a little...'", true, &ch,
        0, 0, TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && (ch.getHit() < ch.hitLimit()) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'I feel better now'", true, &ch, 0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
    ...
    if (ch.attackers >= 2 && ::number(0, ch.attackers - 1)) {
      ...
      if (!::number(0, 6) && !ch.roomp->isWaterSector() &&
          !ch.roomp->isUnderwaterSector() && ch.doesKnowSkill(spell) &&
          (cutoff < discArray[spell]->start) &&
          (ch.getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Crumble and die!'", true, &ch, 0, 0,
          TO_ROOM);
        return spell;
      }
    }
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      act("$n utters, \"How about a little salt for your wounds!\"", true, &ch,
        0, 0, TO_ROOM);
      return spell;
    }
    spell = ch.getSkillNum(SPELL_CALL_LIGHTNING);
    if (!::number(0, 6) && vict.outside() &&
        ((Weather::getWeather(*vict.roomp) == Weather::RAINY) ||
          (Weather::getWeather(*vict.roomp) == Weather::LIGHTNING)) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'Come forth lightning!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    ...
    if (!::number(0, 6) && ch.doesKnowSkill(spell) &&
      ...
      act("$n utters the words, 'Sticks and Stones!'", true, &ch, 0, 0,
        TO_ROOM);
      return spell;
    }
    spell = ch.getSkillNum(SPELL_HARM_CRITICAL);
    ...
    if (!::number(0, 6) && (ch.getHit() < ch.hitLimit()) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'I feel REAL good'", true, &ch, 0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
    spell = SPELL_HEAL;
    if (!::number(0, 6) && (ch.getHit() < ch.hitLimit()) &&
        ch.doesKnowSkill(spell) && (cutoff < discArray[spell]->start) &&
        (ch.getSkillValue(spell) > 33)) {
      act("$n utters the words, 'JAMES BROWN'", true, &ch, 0, 0, TO_ROOM);
      on_me = true;
      return spell;
    }
    spell = SPELL_HEAL_CRITICAL;
    ...
    if (!::number(0, 6) && !ch.affectedBySpell(spell) &&
        !ch.isAffected(AFF_SANCTUARY) && ch.doesKnowSkill(spell) &&
        (cutoff < discArray[spell]->start) && (ch.getSkillValue(spell) > 33)) {
      switch (::number(1, 3)) {
        case 1:
          act("$n utters, \"Safe Haven!\"", true, &ch, 0, 0, TO_ROOM);
        case 2:
        case 3:
        default:
          act("$n utters, \"White aura, surround me!\"", true, &ch, 0, 0,
            TO_ROOM);
          break;
      }
...
int TMonster::clerMove(TBeing& vict) {
  bool on_me;
  spellNumT spell = TYPE_UNDEFINED;
  TBeing *tank = nullptr, *k, *t = nullptr;
  followData* f;
  ...
  if (eitherArmHurt() || checkSoundproof() ||
      nomagic("No-magic room prevents spell cast."))
    return false;
  ...
  if (!isAffected(AFF_CHARM) && (tank = vict.fight()) && tank != this &&
      isFriend(*tank)) {
    ...
    if (spell != TYPE_UNDEFINED && t) {
      //      vlogf(LOG_MISC, format("%s HEALING %s, spell %i") %  name %
      //      t->name % spell);
      return doDiscipline(spell, t->name);
    }
  } else if (isAffected(AFF_CHARM) && (tank = vict.fight()) && tank != this &&
             tank == master) {
    spell = get_cleric_heal_spell(*this, *(t = tank));
    if (spell != TYPE_UNDEFINED && t) {
      //      vlogf(LOG_MISC, format("%s HEALING %s, spell %i") %  name %
...
int TMonster::scavenge() {
  ...
  if (!roomp->stuff.empty() && !::number(0, 5)) {
    ...
    if (best_obj) {
      ...
      --(*best_obj);
      *this += *best_obj;
      vlogf(LOG_MOB_AI, format("Mob scavenge: %s picking up %s in room (%d)") %
                          this->name % best_obj->name %
                          (roomp ? roomp->in_room : 0));
...
int TMonster::notFightingMove(int pulse) {
  ...
  if (!(pulse % (9 * Pulse::MOBACT)) && !spelltask) {
    rc = defendSelf(pulse);
    if (IS_SET_DELETE(rc, DELETE_THIS)) {
      return DELETE_THIS;
    } else if (rc)
    ...
    for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
      ...
      if (t != this && tmp_ch && !tmp_ch->isPc() && canSee(tmp_ch) &&
          isFriend(*tmp_ch) && !tmp_ch->isDumbAnimal()) {
  ...
  if (!(pulse % (9 * Pulse::MOBACT)) && !spelltask && !task && canFly() &&
      race->isWinged() && race->isFeathered() &&
      !isAffected(AFF_FLIGHTWORTHY)) {
    sstring self = "";
    doPreen(self);
  }
...
int TMonster::mobileActivity(int pulse) {
  TBeing *tmp_ch = nullptr, *vict = nullptr;
  affectedData *af, *af2;
  int iHeight;
  int rc;
  TThing* t = nullptr;
  ...
  if (isAquatic() && !IS_SET(specials.act, ACT_IMMORTAL) &&
      (specials.zone != 1) && (mobVnum() >= 0) && !roomp->isWaterSector() &&
      !roomp->isUnderwaterSector() && !inImperia() &&
      getRace() != RACE_FISHMAN) {
    vlogf(LOG_MISC,
      format("Fish (%s), found out of water in %s (%d)! Destroying!") %
  ...
  for (af = affected; af; af = af2) {
    ...
    if (af->type == SPELL_PLAGUE_LOCUSTS) {
      if (!master || !fight() || !sameRoom(*master)) {
        act("$n circles the area once and then dissipates.", true, this, 0, 0,
          TO_ROOM);
        return DELETE_THIS;
      }
    }
    ...
    if (af->type == SKILL_TRANSFIX) {
      if (!::number(0, 5)) {
        act("$n looks glazed-eyed and content.", true, this, nullptr, nullptr,
          TO_ROOM);
        return true;
      }
    }
  ...
  if ((getPosition() < POSITION_CRAWLING) &&
    ...
    if (!(pulse % (36 * Pulse::MOBACT)) && !riding && !fight() &&
        (getHit() >= hitLimit()) &&
        !isPet(PETTYPE_PET | PETTYPE_CHARM | PETTYPE_THRALL)) {
  ...
  } else if ((default_pos <= POSITION_CRAWLING) && !fight() &&
             (getPosition() > POSITION_CRAWLING) &&
             !IS_SET(specials.act, ACT_HUNTING) && (getHit() >= hitLimit()) &&
  ...
  if (!(pulse % (2 * Pulse::MOBACT))) {
    ...
    if (hasClass(CLASS_WARRIOR) && isAffected(AFF_GROUP) && (vict = fight()) &&
        (tmp_ch = vict->fight()) && tmp_ch != this && inGroup(*tmp_ch) &&
      ...
      if (!::number(0, 2)) {
        ...
        act("$n tries to rescue you but fails.", false, this, 0, tmp_ch,
          TO_VICT);
        act("$n tries to rescue $N but fails.", true, this, 0, tmp_ch,
          TO_NOTVICT);
      }
      addSkillLag(getSkillNum(SKILL_RESCUE), 0);
      return true;
  ...
  if (isShopkeeper() && !(pulse % (200 * Pulse::MOBACT)) &&
      !Config::NoSpecials()) {
  ...
  if (spec && !(pulse % (50 * Pulse::MOBACT)) && !::number(0, 1) &&
      !Config::NoSpecials()) {
    rc = checkSpec(this, CMD_MOB_ALIGN_PULSE, "", nullptr);
    if (IS_SET_DELETE(rc, DELETE_THIS) || IS_SET_DELETE(rc, DELETE_VICT))
      return DELETE_THIS;
    else if (rc)
      return true;
  ...
  if (spec && !(pulse % (2 * Pulse::MOBACT)) && !Config::NoSpecials()) {
    rc = checkSpec(this, CMD_GENERIC_PULSE, "", nullptr);
    if (IS_SET_DELETE(rc, DELETE_VICT) || IS_SET_DELETE(rc, DELETE_THIS))
      return DELETE_THIS;
    else if (rc)
      return true;
  ...
  if (tmp_ch) {
    ...
    if (!::number(0, 74) &&
        !tmp_ch->isLucky(levelLuckModifier(GetMaxLevel()))) {
    ...
    if (getHit() > (hitLimit() / 2)) {
      ...
      if (IS_SET(specials.act, ACT_AFRAID)) {
        ...
        if (getHit() == (hitLimit() * 4) / 5 && !eitherLegHurt() &&
            !eitherArmHurt() && !affectedBySpell(SPELL_FEAR) &&
          ...
          if (fears.clist) {
            ...
            while (fears.clist && safety < 10) {
              if (fears.clist &&
                  ...
                  !IS_SET(specials.act, ACT_HUNTING)) {
                remFeared(tmp_ch, nullptr);
                setHunting(tmp_ch);
                addHated(tmp_ch);
              }
              safety++;  // this is just to make sure we dont go into an
                         // infinite loops here
            }
            return true;
          }
...
// ch will be nullptr for mobpulse=true
...
int TMonster::factionAggroCheck() {
  ...
  if ((!isCult() && !isBrother()) || (isCult() && !inLogrus()) ||
      ...
      if (((isCult() && (tmp_ch->isBrother() || tmp_ch->isSnake())) ||
        ...
        if (isCult() && tmp_ch->isBrother()) {
          ...
          switch (::number(0, 3)) {
            ...
            case 1:
              doSay(
                "You are either with us or against us.  You will take your "
                "choice to the grave!");
              break;
            case 2:
              doSay(
                "The streets of Logrus will run red with the blood of our "
                "enemies!");
              doAction("", CMD_SCREAM);
              break;
            case 3:
          ...
          switch (::number(0, 3)) {
            case 0:
              doSay(
                "Get thee back to the festering hole whence thou came, "
                "Logrite!");
...
int TMonster::aggroCheck(bool mobpulse) {
  ...
  TRoom *rp1 = nullptr, *rp2 = nullptr;

  if (!(rp1 = roomp) || !(rp2 = real_roomp(brtRoom)))
    return false;
  if (IS_SET(specials.act, ACT_STAY_ZONE) &&
      rp1->getZoneNum() != rp2->getZoneNum())
    hasWandered = true;
  ...
  if ((aggro() && (getHit() >= (hitLimit() / 2)))) {
    ...
    whichtarget = 0;
    if (numtargets)
      whichtarget = ::number(1, numtargets);
    // randomly choose one of the targets we found
    numtargets = 0;
    if (!mobpulse)
      whichtarget = -1;  // no randomization since we're attacking on roomenter
    for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
      t = *(it++);
      tmp_ch = dynamic_cast<TBeing*>(t);
      if (!tmp_ch || !tmp_ch->isPc())
      ...
      if (canSee(tmp_ch) && (getPosition() >= POSITION_STANDING)) {
        ...
        numtargets++;
        if (numtargets != whichtarget && whichtarget != -1)
          continue;
        ...
        if (((tmp_ch->plotStat(STAT_CURRENT, STAT_KAR, 0, 100, 50)) <=
          ...
          if (!isDumbAnimal() && ::number(0, 9)) {
            ...
            // a level 48 PC.
            if ((double)(tmp_ch->GetMaxLevel()) >
                ((double)GetMaxLevel() * 1.5)) {
              if (!::number(0, 6) || !mobpulse) {
                act("$n considers killing $N, but thinks better of it.", true,
                  this, nullptr, tmp_ch, TO_NOTVICT, nullptr);
                act("$n considers killing you, but thinks better of it.", true,
                  this, nullptr, tmp_ch, TO_VICT, nullptr);
                act("You consider killing $N, but thinks better of it.", true,
                  this, nullptr, tmp_ch, TO_CHAR, nullptr);
              }
          ...
          if (hasWandered && ::number(0, 29)) {
            // don't attack people less than 2/3rds my level if i'm somewhere
            // i'm not 'expected' to be
            if ((double)(tmp_ch->GetMaxLevel()) <
                ((double)GetMaxLevel() * 0.67)) {
              if (!::number(0, 4) || !mobpulse) {
                act(
                  "$n considers killing $N, but decides not to waste $s time.",
                  true, this, nullptr, tmp_ch, TO_NOTVICT, nullptr);
                act(
                  "$n considers killing you, but decides not to waste $s time.",
                  true, this, nullptr, tmp_ch, TO_VICT, nullptr);
                act(
                  "You consider killing $N, but decides not to waste your "
                  "time.",
                  true, this, nullptr, tmp_ch, TO_CHAR, nullptr);
              }
              return false;
            }
          }
...
int TMonster::assistFriend() {
  TBeing *tbeTarg = nullptr, *tmp_ch = nullptr;
  TBeing* final = nullptr;
  int rc;
  ...
  for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
    ...
    if (isFriend(*tmp_ch)) {
        ...
        if (!tmp_ch->fight()->isAnimal()) {
          if (tmp_ch->fight()->isPolice()) {
            ...
            for (StuffIter it = roomp->stuff.begin();
              ...
              if (tbtm && !tbtm->isPc()) {
                if (tbtm->Hates(tbtm->fight(), nullptr))
                  tbtm->remHated(tbtm->fight(), nullptr);
                tbtm->DA(10);
              }
              tbto->stopFighting();
            }
      ...
      if ((tbeTarg = tmp_ch->findAnAttacker())) {
        ...
        if (!tbeTarg->isPc() &&
            ::number(1, 100) <
              min(99, max(GetMaxLevel(), tbeTarg->GetMaxLevel()) - 1)) {
          // added to reduce chance of brawling as mob level increases
          // % chance of avoiding brawl is equal to brawler or brawlee level
          // (whichever is higher)
          continue;
  ...
  if (final) {
    ...
    vlogf(LOG_MOB_AI, format("Brawl! %s (%d) attacks %s (%d) in %d") %
                        this->name % this->GetMaxLevel() % final->name %
                        final->GetMaxLevel() % (roomp ? roomp->in_room : 0));
    rc = hit(final);
    if (IS_SET_DELETE(rc, DELETE_VICT)) {
      delete final;
      final = nullptr;
...
int TMonster::findABetterWeapon() {
  ...
  if ((best->equippedBy != this) && (best->parent != this)) {
    vlogf(LOG_MOB_AI, format("Mob weapon grab: %s picking up %s in room (%d)") %
                        this->name % best->name % (roomp ? roomp->in_room : 0));
...
int TMonster::defendOther(TBeing& targ) {
  ...
  if (hasClass(CLASS_SHAMAN)) {
    if (!found) {
      ...
      if (!targ.affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n sings the words, 'Eluagga Xypomine!'", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
    if (found) {
      if (IS_SET(discArray[spell]->targets, TAR_CHAR_ROOM)) {
        ...
        vlogf(LOG_BUG, format("Shaman Mob casting (2) spell %d on other with "
                              "possibly bad target flags for spell") %
                         spell);
  ...
  if (hasClass(CLASS_MAGE)) {
    if (!found) {
      ...
      if (!targ.affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Protection Correction!'", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
    ...
    if (found) {
      if (IS_SET(discArray[spell]->targets, TAR_CHAR_ROOM)) {
        ...
        vlogf(LOG_BUG, format("Mob casting (2) spell %d on other with possibly "
                              "bad target flags for spell") %
                         spell);
        rc = doDiscipline(spell, "");
      }
    ...
    if (!found) {
      ...
      if (!targ.affectedBySpell(SPELL_BLESS) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters, \"What a Rush!\"", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    }
    if (!found) {
      spell = getSkillNum(SPELL_ARMOR);
      if (!targ.affectedBySpell(SPELL_ARMOR) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Abrazak'", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    }
#if 0
    ...
    if (found) {
      if (IS_SET(discArray[spell]->targets, TAR_CHAR_ROOM)) {
        ...
        vlogf(LOG_BUG, format("Mob invoking (2) prayer %d on other with "
                              "possibly bad target flags for spell") %
                         spell);
        rc = doDiscipline(spell, "");
      }
    ...
    if (found) {
      if (IS_SET(discArray[spell]->targets, TAR_CHAR_ROOM)) {
        ...
        vlogf(LOG_BUG, format("Mob invoking (3) prayer %d on other with "
                              "possibly bad target flags for spell") %
                         spell);
        rc = doDiscipline(spell, "");
      }
    ...
    if (!found) {
      ...
      if (!targ.affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters, \"I'm a lumberjack, and I'm OK...\"", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
    ...
    if (found) {
      if (IS_SET(discArray[spell]->targets, TAR_CHAR_ROOM)) {
        ...
        vlogf(LOG_BUG, format("Mob casting (3) spell %d on other with possibly "
                              "bad target flags for spell") %
                         spell);
        if (spell == SKILL_BARKSKIN)
          rc = doBarkskin(targ.name.c_str());
...
int TMonster::defendSelf(int) {
  ...
  if (hasClass(CLASS_SHAMAN)) {
    if (!found) {
      spell = SPELL_SHIELD_OF_MISTS;
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n sings the words, 'Eluagga Xypomine!'", true, this, 0, 0,
          TO_ROOM);
        found = true;
    ...
    if (!found) {
      ...
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Big Prick!'", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    }
    if (!found) {
      ...
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the invokation, 'You Sneeggy Bastidge!'", true, this, 0,
          0, TO_ROOM);
        found = true;
      }
    }
    if (!found) {
      ...
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act(
          "$n utters the invokation, 'I can see clearly now the rain is gone!'",
          true, this, 0, 0, TO_ROOM);
        found = true;
      }
    }
        ...
        switch (spellnum) {
          case 1:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 35)) {
              act("$n sings the rada, 'Xebec Kamala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 2:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 40)) {
              act("$n sings the rada, 'Xebec Romula!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 3:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 45)) {
              act("$n sings the rada, 'Xebec Pumula!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 4:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 50)) {
              act("$n sings the rada, 'Xebec Tamala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 5:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 55)) {
              act("$n sings the rada, 'Xebec Oakala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 6:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 60)) {
              act("$n sings the rada, 'Xebec Rokala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          case 7:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 65)) {
              act("$n sings the rada, 'Xebec Metala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
          ...
          default:
            ...
            if (doesKnowSkill(spell) && (getSkillValue(spell) > 70)) {
              act("$n sings the rada, 'Xebec Diala!'", true, this, 0, 0,
                TO_ROOM);
              found = true;
            }
            break;
    ...
    if (!found) {
      ...
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the invokation, 'Legba de' Cheval!'", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
  ...
  if (hasClass(CLASS_MAGE)) {
    ...
    if (!found) {
      ...
      if (!affectedBySpell(SPELL_STONE_SKIN) &&
          !affectedBySpell(SPELL_FLAMING_FLESH) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        if (spell == SPELL_STONE_SKIN) {
          act("$n utters the words, 'I Rock!'", true, this, 0, 0, TO_ROOM);
        } else {
          act("$n utters the words, 'HOT DAMN!  I'm on fire!'", true, this, 0,
            0, TO_ROOM);
        }
        found = true;
      }
    ...
    if (!found) {
      ...
      if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Mirror Mirror!'", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    }
    if (!found && specials.hunting && Hates(specials.hunting, nullptr)) {
      // we're hunting down some bastard!  Let's be real nasty about it
      if (!found) {
        spell = SPELL_DISPEL_MAGIC;
        if (affectedBySpell(SPELL_CALM) && doesKnowSkill(spell) &&
            (getSkillValue(spell) > 66)) {
          act("$n utters the words, 'Back to the basics!'", true, this, 0, 0,
            TO_ROOM);
          found = true;
        }
      }
      if (!found) {
        spell = SPELL_FLY;
        if (!isAffected(AFF_FLYING) && doesKnowSkill(spell) &&
            (getSkillValue(spell) > 60)) {
          act("$n utters the words, 'Whoa, I feel light headed!'", true, this,
            0, 0, TO_ROOM);
          found = true;
        }
      }
      if (!found) {
        ...
        if (!affectedBySpell(spell) && doesKnowSkill(spell) &&
            (getSkillValue(spell) > 60)) {
          act("$n utters the words, 'Time to get moving!'", true, this, 0, 0,
            TO_ROOM);
          found = true;
        }
      }
      if (!found) {
        ...
        if (!isAffected(AFF_DETECT_INVISIBLE) && doesKnowSkill(spell) &&
            (getSkillValue(spell) > 60)) {
      ...
      if (!found) {
        ...
        if (!isAffected(AFF_INVISIBLE) && doesKnowSkill(spell) &&
            (getSkillValue(spell) > 33)) {
          act("$n utters the words, 'All my troubles will disappear!'", true,
            this, 0, 0, TO_ROOM);
          found = true;
        }
      }
    ...
    if (!found) {
      spell = getSkillNum(SPELL_BLESS);
      if (!affectedBySpell(SPELL_BLESS) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters, \"What a Rush!\"", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    ...
    if (!found) {
      spell = getSkillNum(SPELL_ARMOR);
      if (!affectedBySpell(SPELL_ARMOR) && doesKnowSkill(spell) &&
          (getSkillValue(spell) > 33)) {
        act("$n utters the words, 'Abrazak'", true, this, 0, 0, TO_ROOM);
        found = true;
      }
    ...
    if (!found) {
      ...
      if (!affectedBySpell(spell) && !isAffected(AFF_SANCTUARY) &&
          doesKnowSkill(spell) && (getSkillValue(spell) > 33)) {
        act("$n utters, \"White aura, surround me!\"", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
    ...
    if (!found) {
      ...
      if (spell != TYPE_UNDEFINED) {
        act("$n utters the words, 'I feel better now'", true, this, 0, 0,
          TO_ROOM);
        found = true;
      }
    }
    ...
    if (found) {
      ...
      if (IS_SET(discArray[spell]->targets,
        ...
        vlogf(LOG_BUG, format("Mob casting (5) spell %d on self with possibly "
                              "bad target flags for spell") %
                         spell);
        if (spell == SKILL_BARKSKIN)
          rc = doBarkskin(name.c_str());
...
int TMonster::doHatefulStuff() {
  ...
  if ((tmp_ch = findAHatee()) && canSee(tmp_ch)) {
    ...
    if (tmp_ch->isPc() &&
      ...
      if (checkPeaceful("You ask your mortal enemy to step outside to settle "
                        "matters.\n\r")) {
        if (!::number(0, 4)) {
          if (canSpeak())
            act(
              ...
              "settle this?'",
              true, this, 0, tmp_ch, TO_ROOM);
          else
            act("$n points and motions to the nearest exit.", true, this, 0,
              tmp_ch, TO_ROOM);
          TThing* t;
          for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();) {
            t = *(it++);
        ...
        if (canSpeak()) {
          if (hates.sex == SEX_MALE && tmp_ch->getSex() == SEX_MALE)
            ...
            act("$n screams, 'Die, dwarven scum!!!!'", true, this, 0, 0,
              TO_ROOM);
          else if (hates.race == RACE_ELVEN && tmp_ch->getRace() == RACE_ELVEN)
            act("$n screams, 'Death to all elves!!!!'", true, this, 0, 0,
              TO_ROOM);
