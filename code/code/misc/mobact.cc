...
int TMonster::protectionStuff() {
  ...
  if (found && IS_SET(specials.act, ACT_PROTECTOR)) {
    ...
    if (found) {
      ...
      act("$n watches the fight closely with some concern.", FALSE, this, NULL,
        NULL, TO_ROOM);
      return FALSE;
    } else {
      return FALSE;
...
int TMonster::mobileActivity(int pulse) {
  ...
  for (af = affected; af; af = af2) {
    ...
    if (af->type == SKILL_TRANSFIX) {
      if (!::number(0, 5)) {
        act("$n looks glazed-eyed and content.", TRUE, this, NULL, NULL,
          TO_ROOM);
        return TRUE;
      }
    }
