...
int findRepairMaterials(unsigned int shop_nr, TBeing* repair, TBeing* buyer,
  ubyte mat, int& mats_needed, bool purchase) {
  int mat_price = 0;
  ...
  db.query(
    "select r.rent_id from rent r, obj o where r.owner=%i and "
    "r.owner_type='shop' and o.vnum=r.vnum and o.type=%i and r.material=%i",
    commod_shop, ITEM_RAW_MATERIAL, mat);
...
int TObj::repairPrice(TBeing* repair, TBeing* buyer, depreciationTypeT dep_done,
  bool purchase, int* pmatCost) const {
  unsigned int shop_nr = 0;
  ...
  mat_price += findRepairMaterials(shop_nr, repair, buyer, getMaterial(),
    mats_needed, purchase);

  // 200% base cost for materials that we can't purchase
  mat_price += (int)(mats_needed * material_nums[getMaterial()].price * 2);

  //    vlogf(LOG_PEEL, format("gsp=%i, perc_repaired=%f, price=%i,
  //    mats_needed=%i, mat_price=%i") % gsp % perc_repaired % price %
  //    mats_needed % mat_price);
...
static int repair_time(TBeing* keeper, const TObj* o) {
  ...
  // player repair is 4 + success(struct) + fail(struct) (okay this is loose,
  // since fails will require another success) where success is skillcheck >
  // (rand(1,101)-dexbonus*3) and 101 always fails so if we say mob repairs are
  // skilled at around 65% and their dexbonus is 0 it should be 4 + struct / 0.3
  // as the average amount of ticks to repair the object because the approximate
  // average here is 2 * (successrate - .5)  (we ignore the 1/101 chance of
  // failure)
  iTime = 4 + (structs / 0.3);
  iTime *=
    (Pulse::MOBACT / Pulse::ONE_SECOND);  // seconds per player repair pulse

  // adjust this time by a const to represent the lameness of paying for
...
static void save_repairman_file(TBeing* repair, TBeing* buyer, TObj* o,
  int iTime, int) {
  ...
  if (!buyer->isPc()) {
    vlogf(LOG_BUG,
      format(
        "Non-PC (%s) got into save_repairman_file() somehow!!! BUG BRUTIUS!!") %
        buyer->getName());
    return;
  }
  ...
  if (!(fp = fopen(buf, "w"))) {
    ...
    if (mkdir(buf2, 0770)) {
      ...
      vlogf(LOG_BUG,
        format("Created a repair directory for %s") % repair->getName());
    }
...
static int getRepairItem(TBeing* repair, TBeing* buyer, int ticket,
  TNote* obj) {
  char buf[80];
  long tmp, cur_time;
  ...
  if (!(fixed_obj = loadRepairItem(repair, ticket, tmp, tmp_cost, version))) {
    repair->doSay(
      "Either that item has been lost or I must have considered the item "
      "abandoned and got rid of it, sorry.");
    vlogf(LOG_FILE,
  ...
  if (cur_time < tmp) {
    ...
    repair->doTell(fname(buyer->getName()),
  ...
  if (tmp_cost > buyer->getMoney()) {
    sprintf(buf, "%s I don't repair items for free! No money, no item!",
      buyer->getName().c_str());
    repair->doSay(buf);
...
static bool will_not_repair(TBeing* ch, TMonster* repair, TObj* obj,
  ...
  if (obj->isObjStat(ITEM_CHARRED)) {
    if (!silent) {
      repair->doTell(fname(ch->name),
        "I can repair this, but it is very badly fire-damaged.");
    }
  }
  if (obj->isObjStat(ITEM_RUSTY)) {
    if (!silent) {
      repair->doTell(fname(ch->name),
        "This is a little rusty, but I can polish it up.");
    }
  }
  if (obj_index[obj->getItemIndex()].getNumber() >
    ...
    if (!silent) {
      repair->doTell(fname(ch->name),
        "Someone has put out a contract to reclaim that item.  It's just too "
...
void repairman_value(const char* arg, TMonster* repair, TBeing* buyer) {
  ...
  if (!*arg) {
    repair->doTell(fname(buyer->name),
      "Can you be a little more specific about what you want to value....\n\r");
    return;
  }
  ...
  if (is_abbrev(arg, "all.damaged")) {
    ...
    if (!iCostForAll)
      repair->doTell(fname(buyer->name),
        format(
          "%s, You don't have anything I can repair in your inventory...") %
          buyer->getName());
    else
      repair->doTell(fname(buyer->name),
        format(
          "It will cost a total of %d talens to repair all the listed items.") %
          iCostForAll);
  ...
  int singleMatCost = 0;
  int singleRepairCost =
    valued->repairPrice(repair, buyer, DEPRECIATION_NO, false, &singleMatCost);
  const char* costPlural = singleMatCost != 1 ? "s" : "";
  repair->doTell(fname(buyer->name),
    format("It'll cost you %d talens to repair %s to a status of %s.") %
      singleRepairCost % valued->getName() %
      valued->equip_condition(valued->maxFix(repair, DEPRECIATION_NO)));
  repair->doTell(fname(buyer->name),
    format("%d talen%s of that cost is for raw materials.") % singleMatCost %
      costPlural);
  ...
  repair->doTell(fname(buyer->name),
    format("I can have it ready by %s.") % ready);
  repair->doTell(fname(buyer->name),
    format("That's %s.") % secsToString(when_ready - ct));
}
...
int repairman_give(const char* arg, TMonster* repair, TBeing* buyer) {
  ...
  if (convertTo<int>(obj_name) && (obj_name[1] != '.') &&
      (obj_name[2] != '.')) {
...
static TObj* make_ticket(TMonster* repair, TBeing* buyer, TObj* repaired,
  time_t when_ready, int tick_num) {
  TObj* tmp_obj;
...
void TObj::giveToRepair(TMonster* repair, TBeing* buyer, int* found) {
  ...
  *found = false;
  ...
  repair_number++;
  repair->doTell(fname(buyer->name),
    "Payment is due when you pick your item up.");
  repair->doTell(fname(buyer->name),
    format("Here is your ticket, %s") % buyer->getName());
  repair->doTell(fname(buyer->name),
    "If you lose the ticket, it might be hard to reclaim your item.");
  ...
  *buyer += *ticket;
  ...
  // vlogf(LOG_DASH, format("%s repairing %s - str %d/%d, lev %d.  Repair time:
  // %s.") %
...
int repairman(TBeing* buyer, cmdTypeT cmd, const char* arg, TMonster* repair,
  TObj* o) {
  ...
  switch (cmd) {
    ...
    case CMD_REMOVE:
      ...
      if (is_abbrev(buf, "all.damaged")) {
        act("$N helps you decide what equipment you own that $E can fix.", 0,
          buyer, 0, repair, TO_CHAR);
        for (j = MIN_WEAR; j < MAX_WEAR; j++) {
          if ((t = buyer->equipment[j]) && (o = dynamic_cast<TObj*>(t)) &&
...
void TNote::noteMe(TMonster* repair, TBeing* buyer, TObj* repaired,
  time_t when_ready, int tick_num) {
  sstring buf;

  swapToStrung();

  setDescr(((sstring)(format("A receipt ticket from %s's shop lies here.") %
                      repair->getName()))
             .c_str());

  shortDescr =
    ((sstring)(format("a small ticket marked number %d") % tick_num)).c_str();
  ...
  setTimeAdj(buyer->desc && buyer->desc->account
               ? buyer->desc->account->time_adjust
               : 0);
...
void processRepairFile(const char* name) {
  ...
  if (!(fp = fopen(name, "r"))) {
    vlogf(LOG_BUG,
      format(
        "Error reading repairman_file %s for limited item count.  Point 1!") %
        name);
    return;
  }
  if (fread(&then, sizeof(then), 1, fp) != 1) {
    vlogf(LOG_BUG,
      format(
        "Error reading repairman_file %s for limited item count.  Point 2!") %
        name);
    fclose(fp);
    return;
  }
  ...
  if (fread(&cost, sizeof(cost), 1, fp) != 1) {
    vlogf(LOG_BUG,
      format(
        "Error reading repairman_file %s for limited item count.  Point 3!") %
        name);
    fclose(fp);
    return;
  }
  if (fread(&version, sizeof(version), 1, fp) != 1) {
    vlogf(LOG_BUG,
      format(
        "Error reading repairman_file %s for limited item count.  Point 3b!") %
        name);
    fclose(fp);
    return;
  }
  if (fread(&item, sizeof(item), 1, fp) != 1) {
    vlogf(LOG_BUG,
      format(
        "Error reading repairman_file %s for limited item count.  Point 4!") %
        name);
    fclose(fp);
    return;
  }
...
int repairCrystal(TBeing* ch, TObj* o) {
  if (!ch->doesKnowSkill(SKILL_BLACKSMITHING_ADVANCED) &&
      !ch->doesKnowSkill(SKILL_REPAIR_THIEF)) {
    ch->sendTo(
