...
static void TBeingLoad(TBeing* ch, int vnum) {
  ...
  // mod for imm
  mob->loadResponses(vnum, ch->getPlayerID());
...
static void TBeingSave(TBeing* ch, TMonster* mob, int vnum) {
  ...
  db.query("delete from mob where player_id=%i and vnum=%i", ch->getPlayerID(),
    vnum);
  db.query(
...
static void mlist(TPerson* ch, bool zone = false) {
  ...
  if (zone) {
    db.query(
      "select vnum, name, short_desc from mob where player_id=%i and vnum>%i "
      "and vnum<=%i order by vnum",
      ch->getPlayerID(), zone_table[ch->roomp->getZone()->zone_nr - 1].top,
      ch->roomp->getZone()->top);
  } else {
    db.query(
      "select vnum, name, short_desc from mob where player_id=%i order by vnum",
      ch->getPlayerID());
  }
