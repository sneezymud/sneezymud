...
TCorporation::TCorporation(int c) : corp_id(c) {}

TCorporation::~TCorporation() {}

void TCorporation::corpLog(const sstring& name, const sstring& action,
  int talens) {
  TDatabase db(DB_SNEEZY);
...
int TCorporation::getMoney() {
  ...
  db.query(
    "select talens from shopownedcorpbank b, corporation c where c.corp_id=%i "
    "and c.corp_id=b.corp_id and b.shop_nr=c.bank",
    corp_id);
...
void TCorporation::setMoney(int g) {
  ...
  db.query(
    "update shopownedcorpbank scb, corporation c set scb.talens=%i where "
    "scb.corp_id=c.corp_id and c.corp_id=%i and scb.shop_nr=c.bank",
    max(0, g), corp_id);
}

int TCorporation::getCorpID() { return corp_id; }

int TCorporation::getAssets() {
  TDatabase db(DB_SNEEZY);

  db.query(
    "select sum(price) as price from rent where owner_type='shop' and owner in "
    "(select shop_nr from shopowned where corp_id=%i)",
    corp_id);
  db.fetchRow();
