...
void TMonster::aiTarget(TBeing* vict) {
  if (!canSee(vict) || !awake() || UtilMobProc(this) || !sameRoom(*vict))
    return;
...
int TBeing::socialLimbBad(TBeing* mob, cmdTypeT cmd) {
  ...
  */
  if (!mob->canUseLimb(WEAR_HAND_L) || !mob->canUseLimb(WEAR_HAND_R) ||
      !mob->canUseLimb(WEAR_ARM_L) || !mob->canUseLimb(WEAR_ARM_R)) {
    switch (cmd) {
      case CMD_DANCE:
      case CMD_SHAKE:
...
int TMonster::aiSocialSwitch(TBeing* doer, TBeing* other, cmdTypeT cmd,
  aiTarg cond) {
  int rc = 0;
  ...
  switch (cmd) {
    ...
    default:
      vlogf(LOG_MOB_AI,
        format("doAction mob_AI loop called with cmd of %d and type %d") % cmd %
          cond);
      US(1);
