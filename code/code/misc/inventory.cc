...
void TThing::logMe(const TBeing* ch, const char* cmdbuf) const {
  vlogf(LOG_SILENT, format("%s%s%s %s.") % (ch ? ch->getName() : "") %
                      (ch ? " " : "") % cmdbuf % getName());
}

void TObj::logMe(const TBeing* ch, const char* cmdbuf) const {
  vlogf(LOG_SILENT, format("%s%s%s %s. (max: %d, cur: %d)") %
                      (ch ? ch->getName() : "") % (ch ? " " : "") % cmdbuf %
                      getName() % obj_index[getItemIndex()].max_exist %
...
#else
  else if (cmd == CMD_LOAD || cmd == CMD_RENT || cmd == CMD_WEST ||
           cmd == CMD_PURGE || cmd == CMD_NE)
    return;
#endif
  else
        ...
        for (StuffIter it = stuff.begin(); it != stuff.end() && i < num;) {
          ...
          if (obj && isname(tmp, obj->name)) {
            ...
            if (!sub) {
              TObj* tmpobj;
              int bits =
                generic_find(arg2, FIND_CHAR_ROOM, this, &horse, &tmpobj);
              if (bits)
                if (horse->isRideable() && horse->equipment[WEAR_BACK]) {
                  TBaseContainer* saddlebag =
                    dynamic_cast<TBaseContainer*>(horse->equipment[WEAR_BACK]);
...
static int genericGiveDrop(TBeing* ch, TObj* obj) {
  ...
  act("You point towards the $g as you start to drop $p.", 0, ch, obj, 0,
    TO_CHAR);
  act("$n points towards the $g as $e starts to drop $p.", 0, ch, obj, 0,
    TO_ROOM);
  rc = ch->doDrop("", obj);
  if (IS_SET_DELETE(rc, DELETE_THIS))
    return DELETE_THIS;
...
int TBeing::doGive(const sstring& oarg, giveTypeT flags) {
  ...
  if (is_number(obj_name) || obj_name.lower() == "all") {
    amount =
      (obj_name.lower() == "all") ? getMoney() : convertTo<int>(obj_name);
    ...
    if ((vict->getMoney() > 500000) && (amount > 100000))
      vlogf(LOG_MISC, format("%s gave %d talens to %s.") % getName() % amount %
                        vict->getName());
    ...
    if (!vict->isPc()) {
      ...
      if (vict->spec && dynamic_cast<TMonster*>(vict)->isShopkeeper()) {
        ...
        for (shop_nr = 0; (shop_nr < shop_index.size()) &&
                          (shop_index[shop_nr].keeper != (vict)->number);
             shop_nr++)
          ;

        shoplog(shop_nr, this, dynamic_cast<TMonster*>(vict), "talens", amount,
          "giving");
        TShopOwned tso(shop_nr, dynamic_cast<TMonster*>(vict), this);
        int corp_cash = tso.doReserve();
        tso.journalize(getName(), "talens", TX_GIVING_TALENS, amount, 0,
          corp_cash, 0);
      }
    ...
    while (num != 0) {
      ...
      if (obj == last_given) {
        sendTo(
          "For some reason, you decide to cease giving further things "
          "away.\n\r");
        break;
      }
      last_given = obj;
      ...
      if ((flags != GIVE_FLAG_IGN_DEX_NOTEXT) && !badWeight && !badVol) {
        ...
        *vict += *obj;
        ...
        if (badWeight && badVol) {
          ...
          if (flags == GIVE_FLAG_DROP_ON_FAIL) {
            doSay(
              "This is too big for you to carry! I'll just drop it here for "
              "you!");
            rc = genericGiveDrop(this, obj);
            if (IS_SET_DELETE(rc, DELETE_THIS))
              return DELETE_THIS;
...
// returns 2 : same host on 2 players, playing time differential extreme and
// karma
      ...
      if (ch->desc->host == other->desc->host) {
        if ((ch->getStat(STAT_CHOSEN, STAT_CHA) <= -10) &&
          ...
          if ((ch->player.time->played * 4) < other->player.time->played) {
            return 2;
          } else if ((other->player.time->played * 4) <
...
float TBeing::carryWeightLimit() const {
  float num;
  num =
    plotStat(STAT_CURRENT, STAT_STR, (float)30.0, (float)1920.0, (float)495.0);
  if (isFourLegged())
    num *= 2.0;
...
bool TObj::canDrop() const { return (!isObjStat(ITEM_NODROP)); }
...
void TCardDeck::putMoneyInto(TBeing* ch, int amount) {
  ch->sendTo(
    "You should go to the casino if you want to put money into cards.\n\r");
}
  ...
  if (GetMaxLevel() <= MAX_MORT) {
    ...
    for (StuffIter it = stuff.begin(); it != stuff.end();) {
      ...
      if (tobj && tobj->isObjStat(ITEM_NEWBIE) && tobj->stuff.empty() &&
        ...
        if (!ok)
          vlogf(LOG_BUG, format("%s had objects going to room somehow.  "
                                "Investigate immediately.  Tell Batopr!") %
                           getName());
...
int TBeing::doDonate(const char* argument) {
  ...
  while (num != 0) {
    ...
    TThing* t;
    for (StuffIter it = t_o->stuff.begin(); it != t_o->stuff.end() && (t = *it);
         ++it) {
      TObj* tobj = dynamic_cast<TObj*>(t);
      if (!tobj)
        continue;
    ...
    if (o && (o->obj_flags.cost <= 0 || o->isObjStat(ITEM_NEWBIE) ||
      ...
      for (StuffIter it = t_o->stuff.begin();
           it != t_o->stuff.end() && (t = *it); ++it)
        logItem(t, CMD_DONATE);

      --(*t_o);
      thing_to_room(t_o, Room::DONATION);
      Descriptor::worldSend(
        format("<Y>%s<z> just donated %s.\n") % name % t_o->getName(), this);
      sprintf(buf,
        "A small portal appears for an instant, dropping %s in the room.\n\r",
        t_o->getName().c_str());
