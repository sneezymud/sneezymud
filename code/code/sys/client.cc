...
void Descriptor::clientf(const sstring& msg) {
  ...
  if (!msg.empty() &&
      (m_bIsClient || IS_SET(prompt_d.type, PROMPT_CLIENT_PROMPT)))
...
void TRoom::clientf(const sstring& msg) {
  ...
  if (!msg.empty()) {
    for (StuffIter it = stuff.begin(); it != stuff.end() && (t = *it); ++it) {
      if (t->isPc() && t->desc &&
          (t->desc->m_bIsClient ||
            IS_SET(t->desc->prompt_d.type, PROMPT_CLIENT_PROMPT)))
...
void Descriptor::send_client_inventory() {
  ...
  for (StuffIter it = ch->stuff.begin(); it != ch->stuff.end() && (t = *it);
    ...
    if (!tobj)
      continue;
    clientf(format("%d|%d|%s|%d") % CLIENT_INVENTORY % ADD % tobj->getName() %
            tobj->itemType());
  }
}
...
void Descriptor::send_client_prompt(int, int update) {
  ...
  *c_name = *cond = *t_name = *t_cond = '\0';
  ...
  if (ch->fight() && ch->awake() && ch->sameRoom(*ch->fight())) {
    ratio =
      min(10, max(0, ((ch->fight()->getHit() * 9) / ch->fight()->hitLimit())));
    ...
    clientf(format("%d|%s|%s|%s|%s") % CLIENT_FIGHT % c_name % cond % t_name %
            t_cond);
  }
  if ((update & CHANGED_MANA) || (update & CHANGED_PIETY) ||
      (update & CHANGED_LIFEFORCE)) {
  ...
  if (update & CHANGED_HP)
    clientf(
      format("%d|%d|%d") % CLIENT_HITPOINT % ch->getHit() % ch->hitLimit());
  ...
  if (update & CHANGED_ROOM) {
    if (m_bIsClient)
      clientf(format("%d|%d") % CLIENT_ROOM %
      ...
      for (iClass = MAGE_LEVEL_IND; iClass < MAX_CLASSES; iClass++) {
        if (ch->getLevel(iClass)) {
          if (ch->getLevel(iClass) < MAX_MORT) {
            ...
            clientf(format("%d|%d|%d|%s") % CLIENT_TONEXTLEVEL % iClass %
                    (ch->getLevel(iClass) + 1) % StTemp);
          }
  ...
  if (update & CHANGED_COND)
    clientf(format("%d|%d|%d") % CLIENT_COND % ch->getCond(FULL) %
            ch->getCond(THIRST));
...
void Descriptor::send_client_exits() {
  ...
  for (door = MIN_DIR; door < MAX_DIR; door++) {
    if ((exitdata = ch->exitDir(door))) {
      if ((exitdata->to_room != Room::NOWHERE) &&
          (!(exitdata->condition & EXIT_CLOSED) || ch->isImmortal()))
        SET_BIT(bits, (1 << door));
  ...
  if (m_bIsClient)
    ...
    clientf(
      format("%d|%d|%d|%d") % CLIENT_EXITS % bits % cbits % ch->isImmortal());
}
  ...
  switch (type) {
    case CLIENT_INIT:
      ...
      if (WizLock) {
        ...
        clientf(format("%d|The mud is presently Wizlocked.|%d") % CLIENT_ERROR %
                ERR_NOT_ALLOWED);
        if (!lockmess.empty())
          clientf(lockmess);
        outputProcessing();
      ...
      if (vers <= BAD_VERSION) {
        clientf(format("%d|Your client is an old version. The latest version "
                       "is %d. Please upgrade! You can upgrade from "
        ...
        clientf(format("%d|You client is an old version. You can continue "
                       ...
                       "recommended. The latest version is %d and can be "
                       "received from http://sneezy.saw.net/client.|%d") %
                CLIENT_ERROR % CURRENT_VERSION % 7);  // ERR_CUR_VERSION);
        outputProcessing();
      }
    ...
    case CLIENT_ROOM: {
      ...
      for (door = MIN_DIR; door < MAX_DIR; door++) {
        if ((exitdata = character->exitDir(door)))
          sprintf(tmpBuf + strlen(tmpBuf), "|1|%d|%d|%s|%s", exitdata->to_room,
            exitdata->door_type, exitdata->keyword.c_str(),
            exitdata->description.c_str());
        else
      ...
      if (rp->ex_description) {
        for (exptr = rp->ex_description; exptr; exptr = exptr->next) {
          sprintf(tmpBuf2 + strlen(tmpBuf2), "|%s|%s", exptr->keyword.c_str(),
            exptr->description.c_str());
        }
      }
      sstring sb = format("%d|%s|%s|%d|%d|%d%s%s") % CLIENT_CURRENTROOM %
                   rp->name.c_str() % rp->getDescr().c_str() %
                   rp->getSectorType() % rp->getMoblim() % rp->getRoomHeight() %
                   tmpBuf % tmpBuf2;
    ...
    case CLIENT_ROOMNAME:
      ...
      if (!character->isImmortal()) {
        character->sendTo(
          "You cannot do this as a mortal. If this is a client bug report to "
          "Brutius. Otherwise stop trying to hack the client because the "
          "server double checks everything. Thanks. Brutius.\n\r");
        break;
      }
      strcpy(buf, nextToken('|', 255, str2).c_str());
    ...
    case CLIENT_BUG: {
      ...
      character->sendTo(
        "Thanks for the report. It will be looked at soon!\n\r");
      character->sendTo(
        "If necessary, a mudmail will be sent to you to inform you of any "
        "changes or possible explanations.\n\r");
      break;
    }
    ...
    case CLIENT_WHO:
      ...
      for (person = character_list; person; person = person->next) {
        if (person->isPc() && person->polyed == POLY_TYPE_NONE) {
          if (dynamic_cast<TPerson*>(person) && character &&
              (character->GetMaxLevel() >= person->getInvisLevel())) {
            if (person->isLinkdead() && character->isImmortal())
              clientf(format("%d|[%s]|%d|%d|%d") % CLIENT_WHO %
                      person->getName() % ADD % person->GetMaxLevel() % notify);
            else {
              if (person->isPlayerAction(PLR_ANONYMOUS) &&
                  !character->isImmortal())
                clientf(format("%d|%s|%d|0|%d") % CLIENT_WHO %
                        person->getName() % ADD % notify);
              else
                clientf(format("%d|%s|%d|%d|%d") % CLIENT_WHO %
                        person->getName() % ADD % person->GetMaxLevel() %
                        notify);
            }
          }
        }
    ...
    case CLIENT_CONNECTED:
      ...
      for (ch = character_list; ch; ch = ch->next) {
        if ((boost::iequals(character->getName(), ch->getName()) && !ch->desc &&
              !dynamic_cast<TMonster*>(ch)) ||
            (dynamic_cast<TMonster*>(ch) && ch->orig &&
              boost::iequals(character->getName(), ch->orig->getName()))) {
          ...
          *rp += *character;
          ...
          if (should_be_logged(character)) {
            if (IS_SET(account->flags, TAccount::IMMORTAL))
              vlogf(LOG_PIO, format("%s[*masked*] has reconnected (client)  "
                                    "(account: *masked*).") %
                               ch->getName());
            else
              vlogf(LOG_PIO,
    ...
    case CLIENT_NORMAL:
      if (character) {
        ...
        character->sendTo(format("Setting attack mode to %snormal%s\n\r") %
                          character->redBold() % character->norm());
    ...
    case CLIENT_NEWACCOUNT: {
      ...
      if (illegalEmail(email, this, SILENT_YES)) {
        clientf(format("%d|The email address you entered failed validity "
      ...
      if (!*timezone || (convertTo<int>(timezone) > 23) ||
          (convertTo<int>(timezone) < -23)) {
        clientf(
          format(
...
int Descriptor::client_nanny(char* arg) {
  ...
  if (!*pwd) {
    clientf(format("%d|No account %s exists! Please reenter account name, or "
      ...
      *rp += *character;
      ...
      if (should_be_logged(character)) {
        if (IS_SET(account->flags, TAccount::IMMORTAL)) {
          vlogf(LOG_PIO,
            format(
              "%s[*masked*] has reconnected (client 2)  (account: *masked*).") %
              character->getName());
        } else {
          vlogf(LOG_PIO,
  ...
  if (should_be_logged(character)) {
    if (IS_SET(account->flags, TAccount::IMMORTAL)) {
      vlogf(LOG_PIO,
        format("%s[*masked*] has connected (client)  (account: *masked*).") %
          character->getName());
    } else {
    ...
    if (d->character) {
      ...
      if (d->character->isImmortal())
        d->output.push(CommPtr(new WhoListComm(getName(), false, GetMaxLevel(),
          getTimer(), isLinkdead(), prof, title)));
      else
        d->output.push(CommPtr(
          new WhoListComm(getName(), false, -1, -1, false, prof, title)));
        ...
        if (d->character->isImmortal()) {
          ...
          if (d->character->canSeeWho(this)) {
            if (isPlayerAction(PLR_ANONYMOUS)) {
              d->output.push(CommPtr(
                new WhoListComm(getName(), true, -1, -1, false, prof, title)));
            } else {
              d->output.push(CommPtr(new WhoListComm(getName(), true,
                GetMaxLevel(), -1, false, prof, title)));
            }
          }
        }
      }
...
int Descriptor::clientCreateChar(char* arg) {
  ...
  if (ch->chosenStats.values[STAT_STR] + ch->chosenStats.values[STAT_BRA] +
      ch->chosenStats.values[STAT_CON] + ch->chosenStats.values[STAT_DEX] +
      ch->chosenStats.values[STAT_AGI] + ch->chosenStats.values[STAT_INT] +
      ch->chosenStats.values[STAT_WIS] + ch->chosenStats.values[STAT_FOC] +
      ch->chosenStats.values[STAT_PER] + ch->chosenStats.values[STAT_CHA] +
      ch->chosenStats.values[STAT_KAR] + ch->chosenStats.values[STAT_SPE]) {
