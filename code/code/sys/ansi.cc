...
bool TBeing::hasColor() const { return (color() || ansi()); }

bool TBeing::hasColorVt() const { return (color() || ansi() || vt100()); }
  ...
  if (hasColor()) {
    switch (desc->plr_colorSub) {
      ...
      default:
        return "";
        vlogf(LOG_BUG,
          format("Problem in color substituting (%s)") % getName());
        break;
    }
    return buf;
  } else if (hasColorVt()) {
...
const char* TBeing::highlight(char* s) const { return (hasColorVt() ? s : ""); }
...
bool Descriptor::hasColorVt() const {
  return (
    (plr_act & PLR_COLOR) || (plr_act & PLR_ANSI) || (plr_act & PLR_VT100));
}
...
bool TBeing::color() const { return isPlayerAction(PLR_COLOR); }

bool TBeing::ansi() const { return isPlayerAction(PLR_ANSI); }

bool TBeing::vt100() const { return isPlayerAction(PLR_VT100); }
