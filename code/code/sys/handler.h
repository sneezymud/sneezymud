...
TThing* get_thing_in_equip(TBeing*, const char*, equipmentData equipment,
  wearSlotT* j, bool, int* count);
TThing* get_thing_stuck_in_vis(TBeing*, const char*, wearSlotT* j, int* count,
  TBeing*);
...
extern TThing* searchLinkedList(const sstring& name, StuffList list,
  thingTypeT x = TYPETHING);
