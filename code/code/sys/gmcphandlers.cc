...
namespace {
  ...
  void handleCoreHello(const sstring& s, Descriptor& d) {
  ...
  void handleDiscord(const sstring& s, Descriptor& d) {
    sstring discordInfoCmd(
      "External.Discord.Info { \"inviteurl\": "
      "\"https://discord.gg/TMz8gMBDXA\", \"applicationid\": \"SneezyMUD\" }");
    d.sendGmcp(discordInfoCmd, true);
  }

  void handleRemember(const sstring& s, Descriptor& d) {
  ...
  void handleRememberPlayer(const sstring& s, Descriptor& d) {
    sstring arg = s.dropWord();
  ...
  void handleRetrieve(const sstring& s, Descriptor& d) {
  ...
  void handleGmcpCommand(const sstring& s, Descriptor* d) {
    assert(d);
    ...
    if (s == "request sectors") {
      ...
      for (int i = 0; i < MAX_SECTOR_TYPES; i++) {
        str += format(", { \"id\" : %d, \"name\" : \"%s\", \"color\" : %d }") %
               i % TerrainInfo[i]->name % TerrainInfo[i]->color;
      }
      ...
      sstring area =
        format(
          ...
          \"flags\": \"quiet\" }") %
        roomp->getZone()->zone_nr % roomp->getZone()->name;
      d->sendGmcp(area, true);
    } else if ((it = commandHandlers.find(s.word(0))) !=
               commandHandlers.end()) {
      it->second(s, *d);
    } else
...
sstring handleTelnetOpts(sstring& s, Descriptor* d) {
  ...
  // Ugliest hack ever: clients don't bother to negotiate for GMCP. Therefore,
  // if they ever send any Telnet commands, assume they can handle subchannels.
  d->gmcp = true;
  ...
  if (cmd == will || cmd == do_ || cmd == wont || cmd == dont) {
    ...
    if (cmd == will && arg == GMCP) {  // let's have a lil' GMCP
      ...
      result = "\xff\0xfc\xc9";
               ...
               arg == GMCP) {  // I can handle GMCP should you wish
      // IAC WILL GMCP
      d->gmcp = true;
      result = "\xff\0xfb\xc9";
      ...
      // IAC DONT ...
      vlogf(LOG_MISC,
        format("Telnet: Unsupported protocol request: IAC WILL 0x%2x") %
          static_cast<int>(arg));
      result = format("\xff\xfe%c") % arg;
    } else if (cmd == do_) {  // Anything else is unsupported
      // IAC WONT ...
      vlogf(LOG_MISC,
        format("Telnet: Unsupported protocol request: IAC DO 0x%02x") %
          static_cast<int>(arg));
      result = format("\xff\xfd%c") % arg;
    } else {
      if (arg == GMCP)
        d->gmcp = false;
      // vlogf(LOG_MISC, format("Telnet: Unsupported IAC DONT/WONT: 0x%02x") %
      // static_cast<int>(arg));
    }

    // so that it won't get into general processing
    s.erase(iac_pos, 3);
    return result + handleTelnetOpts(s,
                      d);  // also handle other commands in the same string
  } else if (cmd == sb) {
    ...
    if (end == sstring::npos) {
      vlogf(LOG_MISC,
        format("Telnet: Truncated IAC SB 0x%02x") % static_cast<int>(arg));
      return "";
    }
