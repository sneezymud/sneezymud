  ...
  public:
    ...
    void testSimple() {
      ...
      TS_ASSERT_EQUALS(testPerson->desc->output.front()->getComm(),
        "You start riding Testone.\n\r");

      TS_ASSERT(!testPerson2->desc->output.empty());
      TS_ASSERT_EQUALS(testPerson2->desc->output.front()->getComm(),
        "Test starts riding Testone.\n\r");

      TS_ASSERT(!testHorse->desc->output.empty());
      TS_ASSERT_EQUALS(testHorse->desc->output.front()->getComm(),
        "Test hops on your back!\n\r");
    }
