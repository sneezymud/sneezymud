...
class AllocateDeallocateTPersonDescriptorAndDb : public CxxTest::TestSuite
{
 public:
  ...
  void testSimple()
  {
    ...
    Descriptor* testDesc = new Descriptor(new TSocket()); // Descriptor deallocates the socket
    ...
    // whoever thought it's a neat idea to use operator-- to signify removing item from room?
    *testRoom += *testPerson;
    --(*testPerson);
