...
class CommTest : public CxxTest::TestSuite {
  public:
    ...
    void testUncategorizedComm() {
      testPerson->desc->output.push(
        CommPtr(new UncategorizedComm(testString[0])));
    ...
    void testColoredComm() {
      testPerson->desc->output.push(
        CommPtr(new UncategorizedComm("this is <r>a<1> test")));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();

      TS_ASSERT_EQUALS(comm->getComm(), "this is <r>a<1> test");
    }

    void testAnsiColoredComm() {
      testPerson->desc->output.push(CommPtr(new UncategorizedComm(
        format("this is %sa%s test") % ANSI_RED % ANSI_NORMAL)));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();
    ...
    void testSystemLogComm() {
      testPerson->desc->output.push(
        CommPtr(new SystemLogComm(time(0), LOG_PIO, testString[1])));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();
    ...
    void testTellFromComm() {
      testPerson->desc->output.push(CommPtr(new TellFromComm("Deirdre", "Peel",
        format("%s, %s, %s") % testString[0] % testString[1] % testString[2],
        true, false)));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();
    ...
    void testTellToComm() {
      ...
      TS_ASSERT_EQUALS(comm->getComm(),
        (format("<G>You tell Deirdre<z>, \"%s, %s, %s\"\n\r") % testString[0] %
          testString[1] % testString[2])
          .str());
    }

    void testWhoListRemoveComm() {
      testPerson->desc->output.push(
        CommPtr(new WhoListComm("Peel", false, 60, 100, true, "The Freshmaker",
          "Peel keeps her warm but they never kiss.")));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();
    ...
    void testWhoListAddComm() {
      testPerson->desc->output.push(
        CommPtr(new WhoListComm("Peel", false, 60, 100, false, "The Freshmaker",
          "Peel keeps her warm but they never kiss.")));
      CommPtr comm = testPerson->desc->output.front();
      testPerson->desc->output.pop();
    ...
    void testPagedOutput() {
      testPerson->desc->output.push(
        CommPtr(new UncategorizedComm(testString[0])));
      testPerson->desc->output.push(
        CommPtr(new UncategorizedComm(testString[1])));
      testPerson->desc->output.push(
        CommPtr(new UncategorizedComm(testString[2])));
    ...
    void testSoundComm() {
      CommPtr c;
      auto sound = CommPtr(
        new SoundComm("sound", "", "cackle.wav", "socials", -1, -1, -1, -1));
      ...
      auto sound2 = CommPtr(new SoundComm("sound",
        "http://sneezymud.org/sounds/", "Off", "", -1, -1, -1, -1));
      testPerson->sendTo(sound2);
      c = testPerson->desc->output.front();
      testPerson->desc->output.pop();

      TS_ASSERT_EQUALS(c->getComm(),
        "!!SOUND(Off U=http://sneezymud.org/sounds/)\n\r");
    }
};
