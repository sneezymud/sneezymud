      ...
      for (int i = 0; i < (int)Garble::Garble::SPEECH_MAX; ++i) {
        for (int j = 0; j < (int)Garble::TYPE_MAX; ++j) {
          ...
          TSM_ASSERT_THROWS_NOTHING(
            format("Garble::SPEECHTYPE = %i, Garble::TYPE = %i") % i % j,
            testPerson->garble(testPerson, testString[0], (Garble::SPEECHTYPE)i,
