...
spellNumT mapWeaponT(
  weaponT w);  // i'm an idiot and can't figure out how to link the sneezy
               // global version of this routine
...
void delExtra(Cgicc cgi, int account_id) {
  ...
  db.query("delete from objextra where vnum=%s and owner='%s' and name='%s'",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
...
void makeNewExtra(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into objextra (vnum, owner, name, description) values (%s, '%s', "
    "'%s', '')",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("name"))).c_str());
}
...
void makeNewAffect(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into objaffect (vnum, owner, type, mod1, mod2) values (%s, '%s', "
    "%s, 0, 0)",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("type"))).c_str());
}
...
void makeNewObj(Cgicc cgi, int account_id, bool power_load) {
  ...
  db_sneezy.query(
    "select name, short_desc, long_desc, action_desc, type, action_flag, "
    "wear_flag, val0, val1, val2, val3, weight, price, can_be_seen, spec_proc, "
    "max_exist, max_struct, cur_struct, decay, volume, material from obj where "
    "vnum=%s",
    (**(cgi.getElement("template"))).c_str());
  db_sneezy.fetchRow();

  db.query(
    "insert into obj (owner, vnum, name, short_desc, long_desc, action_desc, "
    "type, action_flag, wear_flag, val0, val1, val2, val3, weight, price, "
    "can_be_seen, spec_proc, max_exist, max_struct, cur_struct, decay, volume, "
    "material) values ('%s', %s, '%s', '%s', '%s', '%s', %s, %s, %s, %s, %s, "
    "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    (**(cgi.getElement("owner"))).c_str(), (**(cgi.getElement("vnum"))).c_str(),
    db_sneezy["name"].c_str(), db_sneezy["short_desc"].c_str(),
    db_sneezy["long_desc"].c_str(), db_sneezy["action_desc"].c_str(),
    db_sneezy["type"].c_str(), db_sneezy["action_flag"].c_str(),
    db_sneezy["wear_flag"].c_str(), db_sneezy["val0"].c_str(),
    db_sneezy["val1"].c_str(), db_sneezy["val2"].c_str(),
    db_sneezy["val3"].c_str(), db_sneezy["weight"].c_str(),
    db_sneezy["price"].c_str(), db_sneezy["can_be_seen"].c_str(),
    db_sneezy["spec_proc"].c_str(), db_sneezy["max_exist"].c_str(),
    db_sneezy["max_struct"].c_str(), db_sneezy["cur_struct"].c_str(),
    db_sneezy["decay"].c_str(), db_sneezy["volume"].c_str(),
    db_sneezy["material"].c_str());

  db_sneezy.query("select vnum, name, description from objextra where vnum=%s",
    (**(cgi.getElement("template"))).c_str());
  ...
  while (db_sneezy.fetchRow()) {
    db.query(
      ...
      "'%s', '%s')",
      (**(cgi.getElement("vnum"))).c_str(),
      (**(cgi.getElement("owner"))).c_str(), db_sneezy["name"].c_str(),
      db_sneezy["description"].c_str());
  }

  db_sneezy.query("select vnum, type, mod1, mod2 from objaffect where vnum=%s",
    (**(cgi.getElement("template"))).c_str());
  ...
  while (db_sneezy.fetchRow()) {
    db.query(
      ...
      "%s, %s, %s)",
      (**(cgi.getElement("vnum"))).c_str(),
      (**(cgi.getElement("owner"))).c_str(), db_sneezy["type"].c_str(),
      db_sneezy["mod1"].c_str(), db_sneezy["mod2"].c_str());
  }
}
...
void saveExtra(Cgicc cgi, int account_id) {
  ...
  db.query(
    ...
    "'%s', '%s')",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("name"))).c_str(), buf.c_str());
...
void saveAffect(Cgicc cgi, int account_id) {
  ...
  db.query(
    ...
    "%i, %s, %s)",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    apply_file, (**(cgi.getElement("mod1"))).c_str(),
...
void saveObj(Cgicc cgi, int account_id) {
  ...
  db.query(
    ...
    "type, action_flag, wear_flag, val0, val1, val2, val3, weight, price, "
    "can_be_seen, spec_proc, max_exist, max_struct, cur_struct, decay, volume, "
    "material) values ('%s', %s, '%s', '%s', '%s', '%s', %s, %i, %i, %s, %s, "
    "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    (**(cgi.getElement("owner"))).c_str(), (**(cgi.getElement("vnum"))).c_str(),
    (**(cgi.getElement("name"))).c_str(),
    ...
    (**(cgi.getElement("type"))).c_str(), action_flag, wear_flag,
    (**(cgi.getElement("val0"))).c_str(), (**(cgi.getElement("val1"))).c_str(),
    (**(cgi.getElement("val2"))).c_str(), (**(cgi.getElement("val3"))).c_str(),
...
void sendShowExtra(int account_id, int vnum) {
  ...
  db.query(
    "select owner from obj where lower(owner) in (%r) and vnum=%i group by "
    "owner",
  ...
  cout
    << "<button name=state value=showaffect type=submit>edit affects</button>";
  ...
  db.query(
    "select owner, vnum, name, description from objextra where vnum=%i and "
    "owner in (%r) order by name",
    vnum, getPlayerNames(account_id).c_str());

  while (db.fetchRow()) {
    cout << "<form action=\"objeditor.cgi\" method=post name=saveextra>"
         << endl;
    ...
    cout << format(
              "<tr><td>%s</td><td><input type=text size=127 name='%s' "
              "value='%s'></td></tr>\n") %
              "vnum" % "vnum" % db["vnum"];
    cout << format(
              "<tr><td>%s</td><td><input type=text size=127 name='%s' "
    ...
    cout << format(
              "<tr><td>%s</td><td><textarea name=description cols=90 "
              "rows=5>%s</textarea></td></tr>\n") %
              "description" % buf;
...
sstring getItemTypeForm(int selected) {
  ...
  for (it = m.begin(); it != m.end(); ++it) {
    buf += format("<option value=%i %s>%s</option>\n") % (*it).second %
           (((*it).second == selected) ? "selected" : "") % (*it).first;
  }
  buf += "</select>\n";
...
void sendShowAffect(int account_id, int vnum) {
  ...
  db.query(
    "select owner, vnum, type, mod1, mod2 from objaffect where vnum=%i and "
    "owner in (%r) order by type",
    vnum, getPlayerNames(account_id).c_str());

  while (db.fetchRow()) {
    cout << "<form action=\"objeditor.cgi\" method=post name=saveaffect>"
         << endl;
...
sstring getProcForm(int selected, bool wizard) {
  ...
  for (it = m.begin(); it != m.end(); ++it) {
    if (objSpecials[(*it).second].assignable || (*it).second == selected ||
        wizard) {
      buf += format("<option value=%i %s>%s</option>\n") % (*it).second %
             (((*it).second == selected) ? "selected" : "") % (*it).first;
    }
...
void sendShowObj(int account_id, int vnum, bool wizard) {
  ...
  db.query(
    "select owner, vnum, name, short_desc, long_desc, action_desc, type, "
    "action_flag, wear_flag, val0, val1, val2, val3, weight, price, "
    "can_be_seen, spec_proc, max_exist, max_struct, cur_struct, decay, volume, "
    "material from obj where vnum=%i and owner in (%r)",
    vnum, getPlayerNames(account_id).c_str());
  db.fetchRow();

  cout << "<form method=post action=objeditor.cgi>" << endl;
  cout << "<table width=100%><tr>";
  cout << "<td align=left><button name=state value=logout "
          "type=submit>logout</button></td>";
  cout << "<td align=left><button name=state value=main type=submit>object "
          "list</button></td>";
  cout << "<td align=left><button name=state value=showextra type=submit>edit "
          "extras</button></td>";
  cout << "<td align=left><button name=state value=showaffect type=submit>edit "
          "affects</button></td>";
  cout << "<td width=100% align=right><button name=state value=delobj "
  ...
  for (int i = 0; i < MAX_OBJ_STAT; ++i) {
    cout << format("<td><input type=checkbox %s name='%s'> %s") %
              ((action_flag & (1 << i)) ? "checked" : "") % extra_bits[i] %
              extra_bits[i];
  ...
  for (unsigned int i = 0; i < MAX_ITEM_WEARS; ++i) {
    cout << format("<td><input type=checkbox %s name='%s'> %s</td>") %
              ((wear_flag & (1 << i)) ? "checked" : "") % wear_bits[i] %
              wear_bits[i];
  ...
  if (convertTo<int>(db["type"]) == 5) {
    ...
    cout << format(
      "  document.saveobj.val0.value = 1 * "
      "(val_check(document.saveobj.sharp_cur.value, 'sharp_cur')) + "
      "(val_check(document.saveobj.sharp_max.value, 'sharp_max') << 8);\n");
    cout << format("}\n");
    cout << format("\n");
    cout << format("function compute_weap_x1(){\n");
    cout << format(
      "  document.saveobj.val1.value = 1 * "
      "(val_check(document.saveobj.dam_lev.value, 'dam_lev') * 4) + "
      "(val_check(document.saveobj.dam_dev.value, 'dam_dev') << 8);\n");
    cout << format("  weap_dam()\n");
    cout << format("}\n");
    cout << format("\n");
    cout << format("function compute_weap_x2(){\n");
    cout << format(
      "  document.saveobj.val2.value = 1 * "
      "(val_check(document.saveobj.weap_type_1.value, 'weap_type_1')) + "
      "(val_check(document.saveobj.weap_freq_1.value, 'weap_freq_1') << 8) + "
      "(val_check(document.saveobj.weap_type_2.value, 'weap_type_2') << 16) + "
      "(val_check(document.saveobj.weap_freq_2.value, 'weap_freq_2') << "
      "24);\n");
    cout << format("}\n");
    cout << format("\n");
    cout << format("function compute_weap_x3(){\n");
    cout << format(
      "  document.saveobj.val3.value = 1 * "
      "(val_check(document.saveobj.weap_type_3.value, 'weap_type_3')) + "
      "(val_check(document.saveobj.weap_freq_3.value, 'weap_freq_3') << 8);\n");
    cout << format("}\n");
    cout << format("function weap_dam(){\n");
    cout << format(
      "  base = val_check(document.saveobj.dam_lev.value, 'dam_lev') * "
      "1.75;\n");
    cout << format(
      "  flux = parseInt(base * val_check(document.saveobj.dam_dev.value, "
      "'dam_dev') / 10);\n");
    cout << format(
      "  document.saveobj.weap_dam.value = parseInt(base - flux) + ' - ' + "
      "parseInt(base + flux) + ' avg of ' + parseInt(base);\n");
    cout << format("}\n");
    cout << format("</script>\n") << endl;

    // wtf is up with javascript? multiplied values by 1 so it knows not to use
    ...
    cout << format(
              "<tr><td>%s<br>Bit # <input type=text size=12 name='%s' "
              "value='%s' style='border:0'></td>") %
              ItemInfo[convertTo<int>(db["type"])]->val0_info % "val0" %
              db["val0"]
         << endl;
    cout << format(
              "<td>Current sharpness <input type='text' size='15' "
              "maxlength='3' name='sharp_cur' value='%i' "
              "onchange='compute_weap_x0()'>\n<br>Maximum sharpness <input "
              "type='text' size='15' maxlength='3' name='sharp_max' value='%i' "
              "onchange='compute_weap_x0()'></td></tr>\n") %
              sharp_cur % sharp_max
         << endl;
    ...
    cout
      << format(
           "<tr><td>%s<br>Bit # <input type=text size=12 name='%s' value='%s' "
           "style='border:0'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=text "
           "size=24 name='weap_dam' value='' readonly "
           "style='border:0'></td>\n") %
           ItemInfo[convertTo<int>(db["type"])]->val1_info % "val1" %
           db["val1"];
    cout << format(
              "<td>Damage level <input type='text' size='15' maxlength='4' "
              "name='dam_lev' value='%i' onchange='compute_weap_x1()'> (no <b "
              "style='color:red'>NOT</b> multiply by 4 here)\n<br>Damage "
              "deviation <input type='text' size='15' maxlength='3' "
              "name='dam_dev' value='%i' "
              "onchange='compute_weap_x1()'></td></tr>\n") %
              dam_lev % dam_dev;
    ...
    cout << format(
              "<tr><td>%s<br>Bit # <input type=text size=12 name='%s' "
              "value='%s' style='border:0'></td>") %
              ItemInfo[convertTo<int>(db["type"])]->val2_info % "val2" %
              db["val2"]
         << endl;

    cout << format(
              "<td>Type 1 <select type='select' name='weap_type_1' "
              "onchange='compute_weap_x2()'><option value='0'>None</option>")
         << endl;
    for (wt = weaponT(WEAPON_TYPE_NONE + 1); wt < WEAPON_TYPE_MAX;
         wt = weaponT(wt + 1)) {
      cout << format("<option value='%i'%s>%s</option>") % (int)wt %
                (((int)wt == weap_type_1) ? " selected" : "") %
                attack_hit_text[(int)((mapWeaponT(wt) - TYPE_MIN_HIT))].singular
           << endl;
    }
    cout << format(
              "</select>&nbsp;&nbsp;&nbsp;Frequency 1 <input type='text' "
              "size='5' maxlength='3' name='weap_freq_1' value='%i' "
              "onchange='compute_weap_x2()'> %") %
              weap_freq_1
         << endl;

    cout << format(
              "<br>Type 2 <select type='select' name='weap_type_2' "
              "onchange='compute_weap_x2()'><option value='0'>None</option>")
         << endl;
    for (wt = weaponT(WEAPON_TYPE_NONE + 1); wt < WEAPON_TYPE_MAX;
         wt = weaponT(wt + 1)) {
      cout << format("<option value='%i'%s>%s</option>") % (int)wt %
                (((int)wt == weap_type_2) ? " selected" : "") %
                attack_hit_text[(mapWeaponT(wt) - TYPE_MIN_HIT)].singular
           << endl;
    }
    cout << format(
              "</select>&nbsp;&nbsp;&nbsp;Frequency 2 <input type='text' "
              "size='5' maxlength='3' name='weap_freq_2' value='%i' "
              "onchange='compute_weap_x2()'> %</td></tr>") %
              weap_freq_2
         << endl;
    ...
    cout << format(
              "<tr><td>%s<br>Bit # <input type=text size=12 name='%s' "
              "value='%s' style='border:0'></td>\n") %
              ItemInfo[convertTo<int>(db["type"])]->val3_info % "val3" %
              db["val3"]
         << endl;
    cout << format(
      "<td>Type 3 <select type='select' name='weap_type_3' "
      "onchange='compute_weap_x3()'><option value='0'>None</option>");
    for (wt = weaponT(WEAPON_TYPE_NONE + 1); wt < WEAPON_TYPE_MAX;
         wt = weaponT(wt + 1)) {
      cout << format("<option value='%i'%s>%s</option>") % (int)wt %
                (((int)wt == weap_type_3) ? " selected" : "") %
                attack_hit_text[(mapWeaponT(wt) - TYPE_MIN_HIT)].singular
           << endl;
    }
    cout << format(
              "</select>&nbsp;&nbsp;&nbsp;Frequency 3 <input type='text' "
              "size='5' maxlength='3' name='weap_freq_3' value='%i' "
              "onchange='compute_weap_x3()'> %</td></tr>") %
              weap_freq_3
         << endl;
...
void sendObjlist(int account_id) {
  ...
  db.query(
    "select owner, max(vnum)+1 as nvnum from obj where lower(owner) in (%r) "
    "group by owner",
  ...
  if (db.fetchRow())
    ...
    TDatabase db_sneezy(DB_SNEEZY);
    db_sneezy.query(
      "select p.name as name from wizpower w, account a, player p where "
      "p.id=w.player_id and p.account_id=a.account_id and a.account_id=%i and "
      "w.wizpower=%i",
      account_id, mapWizPowerToFile(POWER_BUILDER));
    db_sneezy.fetchRow();
  ...
  cout << "<table border=1>";
  cout << "<tr><td>vnum</td><td>name</td><td>short_desc</td><td>extras</"
          "td><td>affects</td></tr>";

  db.query(
    "select vnum, name, short_desc from obj o where lower(owner) in (%r) order "
    "by vnum asc",
    getPlayerNames(account_id).c_str());

  db_affects.query(
    "select vnum, count(*) as count from objaffect where lower(owner) in (%r) "
    "group by vnum order by vnum asc",
    getPlayerNames(account_id).c_str());
  db_affects.fetchRow();

  db_extras.query(
    "select vnum, count(*) as count from objextra where lower(owner) in (%r) "
    "group by vnum order by vnum asc",
    getPlayerNames(account_id).c_str());
  db_extras.fetchRow();
  ...
  while (db.fetchRow()) {
    ...
    while (convertTo<int>(db_affects["vnum"]) < convertTo<int>(db["vnum"]))
      if (!db_affects.fetchRow())
        break;
    while (convertTo<int>(db_extras["vnum"]) < convertTo<int>(db["vnum"]))
    ...
    cout << "<tr><td>"
         << "<a href=javascript:pickobj('" << db["vnum"];
