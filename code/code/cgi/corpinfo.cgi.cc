...
void sendCorporationInfo(int corp_id) {
  ...
  db.query(
    ...
    "count(s.shop_nr) as shops, bank, r.name as bankname, sob.corp_id as "
    "bankowner from shop sbank, room r, shopowned sob, corporation c left "
    "outer join shopownedcorpbank b on (c.corp_id=b.corp_id), shopowned so, "
    "shop s where sob.shop_nr=c.bank and c.corp_id=so.corp_id and c.corp_id=%i "
    "and so.shop_nr=s.shop_nr and r.vnum=sbank.in_room and "
    "sbank.shop_nr=c.bank group by c.corp_id, c.name, b.talens, c.bank, "
    "sob.corp_id, r.name order by c.corp_id",
    corp_id);
  ...
  cout << (format("<tr><td>%-3i</td><td>%s</td></tr>") % corp_id % db["name"]);
  ...
  cout << (format("<tr><td>Total value</td><td>%12s</td></tr>") %
           (format("%i") % (banktalens + gold + value + (shopcount * 1000000)))
             .comify());
  ...
  // shops
  db.query(
    "select s.shop_nr, r.name, s.gold from shop s, shopowned so, room r where "
    "s.shop_nr=so.shop_nr and so.corp_id=%i and r.vnum=s.in_room order by "
    "s.gold desc",
    corp_id);

  cout
    << ("<tr><td>The following shops are owned by this "
        "corporation:</td><td></td></tr>");
...
void sendCorporationsList() {
  ...
  for (it = m.begin(); it != m.end(); ++it) {
    cout << (format(
               "<tr><td>%i</td><td><a "
               "href=\"corpinfo.cgi?corp_id=%i\">%s</a></td><td>%s</td></tr>") %
             corp_list[(int)((*it).second)].corp_id %
             corp_list[(int)((*it).second)].corp_id %
             corp_list[(int)((*it).second)].name % talenDisplay((*it).first))
              .c_str();
    cout << endl;
  }
  cout << "</table>";
