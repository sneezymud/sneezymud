...
where \
  ...
  db.query(
    "select sl.shop_nr, sl.name, sl.action, sl.item, sl.talens, sl.shoptalens, "
    "sl.shopvalue, sl.logtime, sl.itemcount from shoplog sl, shopowned so "
    "where sl.shop_nr=so.shop_nr and sl.shop_nr=%i order by sl.logtime desc",
    shop_nr);
  cout << "shop_nr, name, action, item, talens, shoptalens, shopvalue, "
          "logtime, itemcount\n";
...
where \
  ...
  cout << "<td>shop_nr</td><td>name</td><td>action</td><td>item</"
          "td><td>talens</td><td>shoptalens</td><td>shopvalue</td><td>logtime</"
          "td><td>itemcount</td></tr>"
       << endl;
...
where \
  ...
  db.query(
    "select sl.shop_nr, sl.name, sl.action, sl.item, sl.talens, sl.shoptalens, "
    "sl.shopvalue, sl.logtime, sl.itemcount from shoplogarchive sl, shopowned "
    "so where sl.shop_nr=so.shop_nr and sl.shop_nr=%i order by sl.logtime desc",
    shop_nr);
  cout << "shop_nr, name, action, item, talens, shoptalens, shopvalue, "
          "logtime, itemcount\n";
...
order by r.name",
  ...
  while (db.fetchRow()) {
    ...
    cout << "<td>" << ((access & SHOPACCESS_OWNER) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_INFO) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_RATES) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_GIVE) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_SELL) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_ACCESS) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_LOGS) ? "true" : "false")
         << "</td>";
    cout << "<td>" << ((access & SHOPACCESS_DIVIDEND) ? "true" : "false")
         << "</td>";
  }
  cout << "</table>" << endl;
  ...
  /////////// shop ratios
  db.query(
    "select o.short_desc, sor.profit_buy, sor.profit_sell, sor.max_num from "
    "obj o, shopownedratios sor where sor.shop_nr=%i and o.vnum=sor.obj_nr",
    shop_nr);
...
void sendCorpShopList(int account_id) {
  ...
  db.query(
    "select c.name as corpname, p.name as pname from player p, corporation c, "
    "corpaccess ca where p.account_id=%i and c.corp_id=ca.corp_id and "
    "ca.player_id=p.id order by c.name",
    account_id);
  ...
  db.query(
    "select distinct so.shop_nr, r.name as shopname, c.name as corpname, "
    "c.corp_id from corporation c, room r, shop s, shopowned so, corpaccess "
    "ca, player p where p.account_id=%i and ca.player_id=p.id and "
    "so.corp_id=ca.corp_id and s.shop_nr=so.shop_nr and s.in_room=r.vnum and "
    "c.corp_id=ca.corp_id order by c.name, r.name",
    account_id);

  cout << "Shops you have access to via corporations you belong to:<br>";
  cout << "<table border=1>" << endl;
  cout << "<tr><td>Shop</td><td>Player(s)</td><td>Corporation</td><td>Logs</"
          "td><td>Export Logs</td><td>Archived Logs</td><td>Export Archived "
          "Logs</td></tr>";
...
void sendAccessShopList(int account_id) {
  ...
  db.query(
    "select p.name as pname, r.name as shopname, soa.access  from "
    "shopownedaccess soa, room r, shop s, shopowned so, player p where "
    "p.account_id=%i and s.shop_nr=so.shop_nr and s.in_room=r.vnum and "
    "lower(p.name)=lower(soa.name) and soa.shop_nr=s.shop_nr order by r.name",
    account_id);

  std::map<sstring, sstring> name_list;

  while (db.fetchRow()) {
    name_list[db["shopname"]] +=
      format("%s (%s) ") % db["pname"] % db["access"];
  }

  db.query(
    "select distinct so.shop_nr, r.name as shopname, c.name as corpname, "
    "c.corp_id from shopownedaccess soa, corporation c, room r, shop s, "
    "shopowned so, player p where p.account_id=%i and so.corp_id=c.corp_id and "
    "s.shop_nr=so.shop_nr and s.in_room=r.vnum and "
    "lower(p.name)=lower(soa.name) and soa.shop_nr=s.shop_nr order by c.name, "
    "r.name",
    account_id);
  ...
  cout << "<table border=1>" << endl;
  cout << "<tr><td>Shop</td><td>Player(s) "
          "(Access)</td><td>Corporation</td><td>Logs</td><td>Export "
          "logs</td></tr>";
