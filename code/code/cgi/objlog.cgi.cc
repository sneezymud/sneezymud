...
void print_form() {
  ...
  db.query(
    "select SUBSTR(LOWER(i.name), 6) as object_type from itemtypes i order by "
    "i.name");
  ...
  cout << "Notes:<br>" << endl;
  cout << "1.  Leaving all fields blank will list all entries in the log.<br>"
       << endl;
  cout << "2.  The object name field should use the % wildcard character.  "
          "Example: %shield%<br>"
       << endl;
  cout << "3.  The format for date filters is:  YYYY-MM-DD HH24:MI:SS<br>"
       << endl;
  cout << "4.  The script ignores the object name and type filters if the VNum "
          "field is used.<br>"
       << endl;
}
...
int main(int argc, char** argv) {
  ...
  if (start == cgi.getElements().end()) {
    ...
    // my_query = "SELECT l.vnum, i.name AS objtype, o.name,
    // l.loadtime::TIMESTAMP(0), l.objcount FROM objlog l, obj o, itemtypes i
    // WHERE l.vnum = o.vnum AND o.\"type\" = i.\"type\"";
    my_query =
      "SELECT l.vnum, SUBSTR(LOWER(i.name), 6) AS objtype, o.name, "
      "DATE_FORMAT(l.loadtime, GET_FORMAT(DATETIME, 'ISO')) AS loadtime, "
      "l.objcount FROM objlog l LEFT OUTER JOIN obj o USING (vnum) LEFT OUTER "
      "JOIN itemtypes i USING (type) WHERE 1 = 1";
    if ((**object_type) != "ALL") {
      my_query += " AND LOWER(i.name) = LOWER(CONCAT('ITEM_' ,'" +
                  (**object_type) + "'))";
    }
    if (!(**sdate).empty()) {
      my_query += " AND l.loadtime >= STR_TO_DATE('" + (**sdate) +
                  "', GET_FORMAT(DATETIME, 'ISO'))";
    }
    if (!(**edate).empty()) {
      my_query += " AND l.loadtime <= STR_TO_DATE('" + (**edate) +
                  "', GET_FORMAT(DATETIME, 'ISO'))";
    }
    if ((**vnum).empty() && (**name).empty()) {
      ...
      my_query +=
        format(" AND l.vnum = %i ORDER BY l.loadtime") % convertTo<int>(**vnum);
      db.query(my_query.c_str());
    }
    ...
    while (db.fetchRow()) {
      ...
      cout << "    <td align=right>" << stripColorCodes(db["name"]) << "</td>"
           << endl;
