...
bool TSession::hasWizPower(wizPowerT wp) {
  ...
  db.query(
    "select 1 from wizpower w, player p where w.player_id=p.id and "
    "p.account_id=%i and w.wizpower=%i",
    account_id, mapWizPowerToFile(wp));
...
void TSession::sendLoginPage(sstring url) {
  ...
  /* killing this for now. hopefully i can get IE to cooperate with the various
    tools if((env.getUserAgent().find("MSIE")) != string::npos){ cout <<
    HTTPRedirectHeader("http://www.mozilla.com/en-US/firefox/switch.html"); cout
    << endl; cout << html() << head() << title("Broken Browser") << endl; cout
    << head() << body() << endl; cout << "Microsoft's Internet Explorer does not
    conform to web standards.  Please use a proper browser.<p><hr><p>" << endl;
      cout << body() << endl;
      cout << html() << endl;
      return;
    }
  */
  ...
  cout << "<tr><td colspan=\"2\"><input type=\"submit\" name=\"Login\" "
          "value=\"Login\" /></td></tr>"
       << endl;
  cout << "</table>" << endl;
  cout << "</form>" << endl;
...
void TSession::logout() {
  TDatabase db(DB_SNEEZY);
  db.query("delete from cgisession where session_id='%s'", session_id.c_str());
  cookieduration = 0;
}
...
sstring escape_html(sstring content) {
  unsigned int loc;
  for (loc = content.find("&", 0); loc != sstring::npos;
       loc = content.find("&", loc + 1))
