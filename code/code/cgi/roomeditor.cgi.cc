...
sstring getNextVnum(int account_id) {
  ...
  db.query(
    "select max(vnum)+1 as nvnum from room where block=1 and lower(owner) in "
    "(%r) group by owner",
...
void delExtra(Cgicc cgi, int account_id) {
  ...
  db.query(
    "delete from roomextra where vnum=%s and owner='%s' and block=1 and "
    "name='%s'",
...
void delExit(Cgicc cgi, int account_id) {
  ...
  db.query(
    "delete from roomexit where vnum=%s and owner='%s' and block=1 and "
    "direction=%s",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
...
void makeNewExtra(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into roomextra (vnum, owner, block, name, description) values (%s, "
    "'%s', 1, '%s', '')",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("name"))).c_str());
}
...
void makeNewExit(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into roomexit (vnum, owner, block, direction, name, description, "
    "type, condition_flag, lock_difficulty, weight, key_num, destination) "
    "values (%s, '%s', 1, %s, '', '', 0, 0, -1, -1, -1, 0)",
    (**(cgi.getElement("vnum"))).c_str(), (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("direction"))).c_str());
}

void makeNewRoom(Cgicc cgi, int account_id, sstring vnum,
  sstring templatevnum) {
  TDatabase db(DB_IMMORTAL);
  ...
  if (templatevnum.empty() || templatevnum == "0") {
    ...
    db_sneezy.query(
      "select vnum, x, y, z, name, description, room_flag, sector, teletime, "
      "teletarg, telelook, river_speed, river_dir, capacity, height, spec from "
      "room where vnum=%s and block=1 and owner in (%r)",
      templatevnum.c_str(), getPlayerNames(account_id).c_str());
  }
  db_sneezy.fetchRow();

  db.query(
    "insert into room (owner, vnum, block, x, y, z, name, description, "
    "room_flag, sector, teletime, teletarg, telelook, river_speed, river_dir, "
    "capacity, height, spec) values ('%s', %s, 1, %s, %s, %s, '%s', '%s', %s, "
    "%s, %s, %s, %s, %s, %s, %s, %s, %s)",
    (**(cgi.getElement("owner"))).c_str(), vnum.c_str(), db_sneezy["x"].c_str(),
    db_sneezy["y"].c_str(), db_sneezy["z"].c_str(), db_sneezy["name"].c_str(),
    db_sneezy["description"].c_str(), db_sneezy["room_flag"].c_str(),
    db_sneezy["sector"].c_str(), db_sneezy["teletime"].c_str(),
    db_sneezy["teletarg"].c_str(), db_sneezy["telelook"].c_str(),
    db_sneezy["river_speed"].c_str(), db_sneezy["river_dir"].c_str(),
    db_sneezy["capacity"].c_str(), db_sneezy["height"].c_str(),
    db_sneezy["spec"].c_str());

  if (templatevnum.empty()) {
    db_sneezy.query(
      "select vnum, name, description from roomextra where vnum=0");
  } else {
    db_sneezy.query(
      "select vnum, name, description from roomextra where vnum=%s and block=1 "
      "and owner in (%r)",
      templatevnum.c_str(), getPlayerNames(account_id).c_str());
  }
  ...
  while (db_sneezy.fetchRow()) {
    db.query(
      "insert into roomextra (vnum, owner, block, name, description) values "
      "(%s, '%s', 1, '%s', '%s')",
      vnum.c_str(), (**(cgi.getElement("owner"))).c_str(),
      db_sneezy["name"].c_str(), db_sneezy["description"].c_str());
  }
}
...
void saveExtra(Cgicc cgi, int account_id) {
  ...
  db.query(
    "delete from roomextra where owner='%s' and vnum=%s and block=1 and "
    "name='%s'",
...
void saveExit(Cgicc cgi, int account_id) {
  ...
  if ((**(cgi.getElement("destination"))) == "new") {
    ...
    makeNewRoom(cgi, account_id, destination, (**(cgi.getElement("vnum"))));
    cout << "Created new destination room " << destination << ".<br>";
  }

  db.query(
    "delete from roomexit where owner='%s' and vnum=%s and block=1 and "
    "direction=%s",
    (**(cgi.getElement("owner"))).c_str(), (**(cgi.getElement("vnum"))).c_str(),
    (**(cgi.getElement("direction"))).c_str());
  ...
  db.query(
    ...
    (**(cgi.getElement("key_num"))).c_str(), destination.c_str());

  cout << "Saved for direction " << (**(cgi.getElement("direction")))
       << ".<br>";
  ...
  db.query(
    "delete from roomexit where owner='%s' and vnum=%s and block=1 and "
    "direction=%i",
    (**(cgi.getElement("owner"))).c_str(), destination.c_str(),
    rev_dir(convertTo<int>((**(cgi.getElement("direction"))))));

  db.query(
    "insert into roomexit (vnum, owner, block, direction, name, description, "
    "type, condition_flag, lock_difficulty, weight, key_num, destination) "
    "values (%s, '%s', 1, %i, '%s', '%s', %s, %s, %s, %s, %s, %s)",
    destination.c_str(), (**(cgi.getElement("owner"))).c_str(),
    rev_dir(convertTo<int>((**(cgi.getElement("direction"))))),
    (**(cgi.getElement("name"))).c_str(),
    (**(cgi.getElement("description"))).c_str(),
    (**(cgi.getElement("type"))).c_str(),
...
void saveRoom(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into room (owner, vnum, block, x, y, z, name, description, "
    "room_flag, sector, teletime, teletarg, telelook, river_speed, river_dir, "
    "capacity, height, spec) values ('%s', %s, 1, %s, %s, %s, '%s', '%s', %i, "
    "%i, %s, %s, %s, %s, %s, %s, %s, %s)",
    (**(cgi.getElement("owner"))).c_str(), (**(cgi.getElement("vnum"))).c_str(),
    (**(cgi.getElement("x"))).c_str(), (**(cgi.getElement("y"))).c_str(),
    (**(cgi.getElement("z"))).c_str(), (**(cgi.getElement("name"))).c_str(),
    (**(cgi.getElement("description"))).c_str(), room_flag, terrain_file,
...
void sendShowExtra(int account_id, int vnum) {
  ...
  db.query(
    "select owner from room where lower(owner) in (%r) and vnum=%i and block=1 "
    "group by owner",
  ...
  db.query(
    "select owner, vnum, name, description from roomextra where vnum=%i and "
    "block=1 and owner in (%r) order by name",
    vnum, getPlayerNames(account_id).c_str());

  while (db.fetchRow()) {
    cout << "<form action=\"roomeditor.cgi\" method=post name=saveextra>"
         << endl;
...
sstring getTypeForm(int selected) {
  ...
  for (int i = 0; i < MAX_DOOR_TYPES; ++i) {
    buf += format("<option value=%i %s>%s</option>\n") % i %
           ((i == selected) ? "selected" : "") % door_types[i];
  }
  buf += "</select>\n";
...
sstring getDestinationForm(int selected, int account_id) {
  ...
  db.query("select vnum, name from room where block=1 and owner in (%r)",
    getPlayerNames(account_id).c_str());
  ...
  while (db.fetchRow()) {
    buf += format("<option bgcolor=black value=%s %s>%s - %s</option>\n") %
           db["vnum"] %
           ((convertTo<int>(db["vnum"]) == selected) ? "selected" : "") %
...
void sendShowExit(int account_id, int vnum) {
  ...
  db.query(
    "select owner from room where lower(owner) in (%r) and block=1 and vnum=%i "
    "group by owner",
    getPlayerNames(account_id).c_str(), vnum);
  db.fetchRow();
  ...
  db.query(
    "select owner, vnum, direction, name, description, type, condition_flag, "
    "lock_difficulty, weight, key_num, destination from roomexit where vnum=%i "
    "and block=1 and owner in (%r) order by type",
    vnum, getPlayerNames(account_id).c_str());

  while (db.fetchRow()) {
    cout << "<form action=\"roomeditor.cgi\" method=post name=saveexit>"
         << endl;
...
sstring getProcForm(int selected, bool wizard) {
  ...
  for (it = m.begin(); it != m.end(); ++it) {
    if (roomSpecials[(*it).second].assignable || (*it).second == selected ||
        wizard) {
      buf += format("<option value=%i %s>%s</option>\n") % (*it).second %
             (((*it).second == selected) ? "selected" : "") % (*it).first;
    }
...
void sendShowRoom(int account_id, int vnum, bool wizard) {
  ...
  db.query(
    "select owner, vnum, x, y, z, name, description, room_flag, sector, "
    "teletime, teletarg, telelook, river_speed, river_dir, capacity, height, "
    "spec from room where block=1 and vnum=%i and owner in (%r)",
    vnum, getPlayerNames(account_id).c_str());
  db.fetchRow();
  ...
  cout << "<td align=left><button name=state value=main type=submit>room "
          "list</button></td>";
  cout << "<td align=left><button name=state value=showextra type=submit>edit "
          "extras</button></td>";
  cout << "<td align=left><button name=state value=showexit type=submit>edit "
          "exits</button></td>";
  cout << "<td width=100% align=right><button name=state value=delroom "
  ...
  db_exits.query(
    "select direction, destination from roomexit where vnum=%i and owner in "
    "(%r) and block=1 and destination in (select vnum from room where owner in "
    "(%r) and block=1)",
    vnum, getPlayerNames(account_id).c_str(),
    getPlayerNames(account_id).c_str());
  int exits[MAX_DIR];
  ...
  while (db_exits.fetchRow()) {
    exits[convertTo<int>(db_exits["direction"])] =
      convertTo<int>(db_exits["destination"]);
  }
  ...
  cout << "<td><button name=vnum value=" << exits[DIR_NORTH]
       << ((exits[DIR_NORTH] == -1) ? " disabled> " : "> ") << dirs[DIR_NORTH]
       << "</button></td>";
  ...
  cout << "<td><button name=vnum value=" << exits[DIR_UP]
       << ((exits[DIR_UP] == -1) ? " disabled> " : "> ") << dirs[DIR_UP]
       << "</button></td></tr>";

  cout << "<tr><td align=right><button name=vnum value=" << exits[DIR_WEST]
       << ((exits[DIR_WEST] == -1) ? " disabled> " : "> ") << dirs[DIR_WEST]
       << "</button></td>" << endl;

  cout << "<td></td>" << endl;

  cout << "<td><button name=vnum value=" << exits[DIR_EAST]
       << ((exits[DIR_EAST] == -1) ? " disabled> " : "> ") << dirs[DIR_EAST]
       << "</button></td></tr>";
  ...
  cout << "<td><button name=vnum value=" << exits[DIR_SOUTH]
       << ((exits[DIR_SOUTH] == -1) ? " disabled> " : "> ") << dirs[DIR_SOUTH]
       << "</button></td>";
  ...
  cout << "<td><button name=vnum value=" << exits[DIR_DOWN]
       << ((exits[DIR_DOWN] == -1) ? " disabled> " : "> ") << dirs[DIR_DOWN]
       << "</button></td></tr>";
  ...
  for (int i = 0; i < MAX_ROOM_BITS; ++i) {
    cout << format("<td><input type=checkbox %s name='%s'> %s") %
              ((room_flag & (1 << i)) ? "checked" : "") % room_bits[i] %
              room_bits[i];
...
void sendRoomlist(int account_id) {
  ...
  db.query(
    "select owner, max(vnum)+1 as nvnum from room where block=1 and "
    "lower(owner) in (%r) group by owner",
    getPlayerNames(account_id).c_str());
  ...
  db.query(
    "select vnum, name, sector from room r where block=1 and lower(owner) in "
    "(%r) order by vnum asc",
    getPlayerNames(account_id).c_str());

  db_exits.query(
    "select vnum, count(*) as count from roomexit where block=1 and "
    "lower(owner) in (%r) group by vnum order by vnum asc",
    getPlayerNames(account_id).c_str());
  db_exits.fetchRow();

  db_extras.query(
    "select vnum, count(*) as count from roomextra where block=1 and "
    "lower(owner) in (%r) group by vnum order by vnum asc",
    getPlayerNames(account_id).c_str());
  db_extras.fetchRow();
  ...
  while (db.fetchRow()) {
    ...
    while (convertTo<int>(db_exits["vnum"]) < convertTo<int>(db["vnum"]))
    ...
    cout << "<tr><td>"
         << "<a href=javascript:pickroom('" << db["vnum"];
