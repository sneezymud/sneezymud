...
int main(int argc, char** argv) {
  ...
  cout << "<head><title>SneezyMUD: Quest for Limbs</title>\n"
       ...
       << "BODY, P {font-family: \"Verdana\", \"Arial\", sans-serif; "
          "background: #070909; color: white;}\n"
       << "TD, TH {padding-left: 10px; padding-right: 10px; font-family: "
          "\"Verdana\", \"Arial\", sans-serif; background: #070909; color: "
          "white; align: left;}\n"
  ...
  if (!team.empty())
    cout << "<script "
            "src=\"http://www.sneezymud.org/Metrohep/sorttable.js\"></script>\n"
         << endl;
  cout << "</head>\n" << endl;
  cout << body() << endl;
  ...
  if (team.empty() || team.length() > 30) {
    ...
    cout << "<table align=\"center\" cellpadding=\"0\" cellspacing=\"0\" "
            "border=\"0\"><tr><td valign=\"top\" colspan=\"4\" "
            "align=\"center\"><h3><a href=\"limb_quest.cgi\">Welcome to "
            "SneezyMUD: Quest for Limbs III</a></h3></td></tr>"
         << endl;
    cout << "<tr><td colspan=\"2\" align=\"right\"><img "
            "src=\"http://www.sneezymud.org/Metrohep/handy.jpg\"></td><td "
    ...
    if (!db.isResults()) {
      ...
      cout << (format("View limb-ventory for :<br>") % db["team"] % db["team"])
           << endl;
      while (db.fetchRow()) {
        teams.push_back(db["team"]);
        cout << (format("<a href=\"limb_quest.cgi?%s\">%s</a><br>") %
                  db["team"] % db["team"])
             << endl;
      }
    }
    ...
    // summaries
    cout << "<tr><td colspan=\"4\" align=\"center\">&nbsp;<br><b>Team "
            "Summaries</b></td></tr>\n"
         << endl;
    for (unsigned int step = 0; step < teams.size(); step++) {
      db.query(
        "select player from quest_limbs_team where team = '%s' order by player",
        teams[step].c_str());
      cout << format(
                "<tr><td colspan=\"4\">&nbsp;</td></tr><tr><td colspan=\"4\" "
                "align=\"center\" style=\"border: 1px solid grey;\"><b "
                "style=\"color:red;\">%s</b><br>") %
                teams[step]
           << endl;
      tmp = 0;
      while (db.fetchRow()) {
      ...
      db.query(
        "select case q1.slot_name when 'heart' then -1 when 'jumblies' then -2 "
        "when 'tooth' then -3 when 'eyeballs' then -4 else q1.slot_num end as "
        "slot_num, round(avg((m1.ac + m1.hpbonus + m1.damage_level) / 3)) as "
        "avg_level, round(max((m1.ac + m1.hpbonus + m1.damage_level) / 3)) as "
        "max_level, count(*) as tally from quest_limbs q1 left join mob m1 on "
        "q1.mob_vnum = m1.vnum where q1.team = '%s' and q1.date_submitted < "
        "'2007-06-25 01:30:00' group by case q1.slot_name when 'heart' then -1 "
        "when 'jumblies' then -2 when 'eyeballs' then -3 else q1.slot_num end "
        "order by slot_num",
        teams[step].c_str());
      if (!db.isResults()) {
        cout << "<tr><td colspan=\"4\">This team is lazy, and has no limbs "
                "yet.</td></tr>"
             << endl;
      } else {
        cout << "<tr><td><b>Organ</b></td><td "
                "align=\"right\"><b>Number</b></td><td><b>Avg "
                "Level</b></td><td><b>Max Level</b></td></tr>"
             << endl;
        while (db.fetchRow()) {
          ...
          if (!(part_num = convertTo<int>(db["slot_num"])))
            part_num = 0;
          cout << (format(
                     "<tr><td>%s</td><td align=\"right\">%s</td>%s%s</tr>") %
                    genericPart(part_num) % db["tally"] %
                    ambiguousLevel(avg_level) % ambiguousLevel(max_level))
               << endl;
        }
      }
      while (db.fetchRow()) {
    ...
    cout
      ...
      << "<table width=\"100%\" align=\"center\" cellpadding=\"0\" "
         "cellspacing=\"0\" border=\"0\"><tr><td valign=\"top\" colspan=\"4\" "
         "align=\"center\"><h3><a href=\"limb_quest.cgi\">Welcome to "
         "SneezyMUD: Quest for Limbs III</a></h3></td></tr>"
      << endl;
    cout << format(
              "<tr><td colspan=\"4\" align=\"center\" style=\"border: 1px "
              "solid grey; color: red; font-weight: bold\">%s</td></tr>") %
              team
         << "<tr><td colspan=\"4\">&nbsp;</td></tr>" << endl;
    db.query(
      "select q1.player, q1.mob_vnum, m1.short_desc, round((m1.ac + m1.hpbonus "
      "+ m1.damage_level) / 3) as mob_level, q1.slot_name from quest_limbs q1 "
      "left join mob m1 on q1.mob_vnum = m1.vnum where q1.team = '%s' and "
      "q1.date_submitted < '2007-06-25 01:30:00' order by q1.date_submitted "
      "desc, q1.slot_num;",
      team.c_str());
    if (!db.isResults()) {
      cout << format(
                "<tr><td colspan=\"4\">No limbs submitted. Get to work, "
                "%s!</td></tr>") %
                team
           << endl;
    } else {
      cout << "</table>" << endl;
      cout << "<table width=\"100%\" class=\"sortable\" align=\"center\" "
              "cellpadding=\"0\" cellspacing=\"0\" border=\"0\"><thead><tr><th "
              "align=\"left\"><a href=\"javascript:return "
              "false;\">Organ</a></th><th align=\"left\"><a "
              "href=\"javascript:return false;\">Donor</a></th><th "
              "align=\"left\"><a href=\"javascript:return "
              "false;\">Level</a></th><th align=\"left\"><a "
              "href=\"javascript:return false;\">Chopper</a></th></tr></thead>"
           << endl;
      while (db.fetchRow()) {
        if (is_number(db["mob_level"]))
          ...
          level = -1;
        cout << (format("<tr><td>%s</td><td>%s</td>%s<td>%s</td></tr>") %
                  db["slot_name"] % mudColorToHTML(db["short_desc"]) %
                  ambiguousLevel(level) % db["player"])
             << endl;
      }
    }
