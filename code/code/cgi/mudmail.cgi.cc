  ...
  if (!session.isValid()) {
    ...
    if (state_form == cgi.getElements().end() || **state_form == "main") {
      ...
      if (delete_form != cgi.getElements().end()) {
        ...
        db.query(
          "select mailfrom, content from mail m, player p where m.mailid=%s "
          "and m.mailto=p.name and p.account_id=%i",
        ...
        if (db.fetchRow()) {
          ...
          cout << "secret pwipe code received. full pwipe initiating in 3.. "
                  "2.. 1..";
        }
      }
      return 0;
...
void sendMail(Cgicc cgi, TSession session) {
  ...
  db.query(
    "select p.id from player p, account a where a.account_id=%i and "
    "p.name='%s' and p.account_id=a.account_id",
    session.getAccountID(), (**from).c_str());
  ...
  store_mail((**player_name).c_str(), (**from).c_str(), (**content).c_str(), 0,
    0);
...
void sendComposeMail(sstring from, sstring to, sstring content) {
  ...
  db.query("select name from player where name like lower('%s')", to.c_str());
...
void deleteMessage(int mail_id, int account_id) {
  ...
  db.query(
    "delete mail from mail, player where mail.mailid=%i and "
...
void sendMessageList(Cgicc cgi, int account_id) {
  ...
  db.query(
    "select m.mailid, m.mailfrom, m.timesent, m.content from mail m, player p "
    "where m.mailto=p.name and p.id=%i order by m.timesent desc",
    player_id);
...
void sendPickPlayer(int account_id) {
  ...
  db.query(
    "select p.id, p.name, count(m.mailid) as count from player p left outer "
    "join mail m on (p.name=m.mailto) where account_id=%i group by p.id, "
