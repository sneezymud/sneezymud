...
void makeNewResp(Cgicc cgi, int account_id, bool power_load) {
  ...
  db.query(
    "insert into mobresponses (owner, vnum, response) values ('%s', %s, '')",
    (**(cgi.getElement("owner"))).c_str(),
    (**(cgi.getElement("vnum"))).c_str());
}
...
void saveResp(Cgicc cgi, int account_id) {
  ...
  db.query(
    "insert into mobresponses (owner, vnum, response) values ('%s', %s, '%s')",
...
void sendShowResp(int account_id, int vnum, bool wizard) {
  ...
  db.query(
    "select owner, vnum, response from mobresponses where vnum=%i and owner in "
    "(%r)",
  ...
  cout << "<td align=left><button name=state value=main type=submit>response "
          "list</button></td>";
  cout << "<td width=100% align=right><button name=state value=delresp "
  ...
  cout << format(
            "<tr><td>%s</td><td><textarea name='%s' cols=90 "
            "rows=30>%s</textarea></td></tr>\n") %
...
void sendResplist(int account_id) {
  ...
  db.query(
    "select owner, max(vnum)+1 as nvnum from mobresponses where lower(owner) "
    "in (%r) group by owner",
  ...
  db.query(
    "select vnum, response from mobresponses where lower(owner) in (%r) order "
    "by vnum asc",
    getPlayerNames(account_id).c_str());

  while (db.fetchRow()) {
    cout << "<tr><td>"
         << "<a href=javascript:pickresp('" << db["vnum"];
