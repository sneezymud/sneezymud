...
TArrow* TBeing::autoGetAmmoQuiver(TBow* bow, TQuiver* quiver) {
  ...
  for (StuffIter it = quiver->stuff.begin();
       it != quiver->stuff.end() && (i = *it); ++it) {
    if ((temp = dynamic_cast<TArrow*>(i)) &&
        bow->getArrowType() == temp->getArrowType() && canSee(i)) {
      ammo = temp;
      break;
    }
  }
...
int archer(TBeing*, cmdTypeT cmd, const char*, TMonster* ch, TObj*) {
  ...
  for (j = 0; j < bows.size(); j++) {
    ...
    if (!bow->stuff.empty())
      break;
    if (!bow)
      vlogf(LOG_BUG, "spec_mobs_archer.cc: archer: bow is null somehow");
  ...
  for (i = MIN_DIR; i <= (MAX_DIR - 1); i++) {
    ...
    for (range = 1; range <= MAX_RANGE; range++) {
      ...
      buf =
        format("%s %d.%s %d") % directions[i][0] % numsimilar % temp % range;
      ...
      if (!(ch->doShoot(buf.c_str())))
        vlogf(LOG_BUG, format("spec_mobs_archer.cc: archer: error shooting bow "
                              "with arguments: %s") %
                         buf);
