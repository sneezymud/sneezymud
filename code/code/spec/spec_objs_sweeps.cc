...
int sweepsScratch(TBeing* ch, cmdTypeT cmd, const char* arg, TObj* o, TObj*) {
  ...
  if (newtile) {  // delete tile and load item
    ...
    sstring buf6 =
      format("an <b>obsidian tile<z> inscribed with <Y>%s<z>") % buf3;
...
int sweepsSplitJoin(TBeing* ch, cmdTypeT cmd, const char* arg, TObj* o, TObj*) {
  ...
  if (cmd == CMD_COMBINE) {
    ...
    sstring buf6 =
      format("an <b>obsidian tile<z> inscribed with <Y>%s<z>") % name2;
    tile2->swapToStrung();
    tile2->name = buf5;
    ...
    if (!left) {
      ...
      vlogf(LOG_BUG,
        "Somehow got to end of splitting tiles in spec_objs_sweeps.cc with "
        "both hands full.");
    }
  }
