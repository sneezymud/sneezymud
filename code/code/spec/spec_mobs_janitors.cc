...
bool okForJanitor(TMonster* myself, TObj* obj) {
  ...
  if ((myself->mobVnum() == Mob::SWEEPER ||
        myself->mobVnum() == Mob::Mob::SWEEPER2) &&
      corpse)
    return false;
...
int garbageConvoy(TBeing*, cmdTypeT cmd, const char*, TMonster* myself, TObj*) {
  ...
  sstring hunt_text_stateT[] = {"doing nothing", "going to cs from gh surplus",
    "waiting for trolley to bm", "going to bm dump", "going to bm fountain",
    ...
    "going to gh dump", "going to cs from gh dump"};
  ...
  class hunt_struct {
    public:
      ...
      hunt_struct() : cur_pos(0), cur_path(0), state(STATE_TO_CS) {}
      ~hunt_struct() {}
  } * job;
  ...
  switch (job->state) {
    case STATE_NONE:
      ...
      break;
    case STATE_TO_CS:
      if (!myself->walk_path(garbage_convoy_path[job->cur_path],
            job->cur_pos)) {
        if (cart->stuff.empty()) {
          switch (::number(0, 3)) {
            case 0:
      ...
      if (myself->inRoom() == Room::TROLLEY) {
        ...
        if (exitp->to_room == 1303) {
          rc = myself->goDirection(
            garbage_convoy_path[job->cur_path][job->cur_pos + 1].direction);
          if (IS_SET_DELETE(rc, DELETE_THIS)) {
            return DELETE_THIS;
          }
...
int fruitScavenger(TBeing*, cmdTypeT cmd, const char*, TMonster* myself,
  ...
  for (StuffIter it = myself->roomp->stuff.begin();
    ...
    if (tree && tree->getType() == seek_tree) {
      ...
      for (StuffIter it = tree->stuff.begin();
  ...
  for (use_dir = MIN_DIR; use_dir < MAX_DIR; use_dir++) {
    if (myself->roomp->exitDir(use_dir) &&
        !IS_SET(myself->roomp->exitDir(use_dir)->condition, EXIT_CLOSED)) {
