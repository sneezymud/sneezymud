...
// Resize and engrave for worn items
int tailor(TBeing *ch, cmdTypeT cmd, const char *arg, TMonster *me, TObj *o)
{
  ...
  switch (cmd) {
    ...
    case CMD_GENERIC_PULSE:
      if (!(job = (reg_struct *) me->act_ptr)) {
        vlogf(LOG_PROC,"TAILOR PROC ERROR: terminating (hopefully)");
        return FALSE;   
      }
      if (!job->char_name || !job->obj_name)
        return FALSE;
      ...
      if (job->wait > 0) {
        ...
        if (!job->wait) {
          ...
          if (!(ts = searchLinkedList(job->obj_name, me->stuff)) ||
              !(final = dynamic_cast<TObj *>(ts))) {
            me->doSay("Ack, I lost the item somehow! Tell a god immediately!  ");
            vlogf(LOG_PROC,format("tailor lost his tailor item (%s)") %
                (final ? final->name : ""));
            return FALSE;
          }
          ...
          if (!(final_pers = get_char_room(job->char_name, me->in_room))) {
            me->doSay("Hmm, I seem to have lost the person I was tailoring for.");
            rc = me->doDrop(job->obj_name, NULL);
            if (IS_SET_DELETE(rc, DELETE_THIS))
              return DELETE_THIS;
            return FALSE;
          ...
          sprintf(buf, "$n sews part of %s's %s.", job->char_name, job->obj_name);
          act(buf, FALSE, me, NULL, ch, TO_ROOM);
          return FALSE;
        }
      }
    ...
    case CMD_VALUE: {
      ...
      if (!valued->action_description.empty()) {
        me->doTell(ch->getName(), "This item has already been engraved and therefore I can't customize it.");
        return TRUE;
      }
      if (obj_index[valued->getItemIndex()].max_exist <= 10) {
      ...
      if (valued->obj_flags.decay_time >= 0) {
        me->doTell(ch->getName(), "Sorry, but this item won't last long enough to bother with a customize!");
        return TRUE;
      }

      cost = engraveCost(valued, ch, find_shop_nr(me->number)) * 2;
    ...
    case CMD_MOB_GIVEN_ITEM:
      // prohibit polys and charms from engraving 
      if (dynamic_cast<TMonster *>(ch)) {
        me->doTell(fname(ch->name), "I don't customize for beasts.");
        return TRUE;
      }
      if (!(item = o)) {
        me->doTell(ch->getName(), "You don't have that item!");
      ...
      cost = engraveCost(item, ch, find_shop_nr(me->number)) * 2;
	...
	ch->saveChar(Room::AUTO_RENT);
        ...
        sprintf(buf, "Sorry, %s, but you'll have to wait while I customize %s's item.", ch->getName().c_str(), job->char_name);
        me->doSay(buf);
        strcpy(buf, item->name.c_str());
        strcpy(buf, add_bars(buf).c_str());
        sprintf(buf + strlen(buf), " %s", fname(ch->name).c_str());
...
int blacksmith(TBeing *ch, cmdTypeT cmd, const char *arg, TMonster *me, TObj *o)
{
  ...
  switch (cmd) {
    ...
    case CMD_GENERIC_CREATED:
      if (!(me->act_ptr = new reg_struct())) {
        perror("failed new of blacksmith.");
        exit(0);
      }
      return FALSE;
    case CMD_MOB_MOVED_INTO_ROOM:
    ...
    case CMD_GENERIC_PULSE:
      if (!(job = (reg_struct *) me->act_ptr)) {
        vlogf(LOG_PROC,"BLACKSMITH PROC ERROR: terminating (hopefully)");
        return FALSE;   
      }
      if (!job->char_name || !job->obj_name)
        return FALSE;
      ...
      if (job->wait > 0) {
        ...
        if (!job->wait) {
          ...
          if (!(ts = searchLinkedList(job->obj_name, me->stuff)) ||
              !(final = dynamic_cast<TObj *>(ts))) {
            me->doSay("Ack, I lost the item somehow! Tell a god immediately!  ");
            vlogf(LOG_PROC,format("blacksmith lost his blacksmith item (%s)") %
                (final ? final->name : ""));
            return FALSE;
          }
          ...
          if (!(final_pers = get_char_room(job->char_name, me->in_room))) {
            me->doSay("Hmm, I seem to have lost the person I was blacksmithing for.");
            rc = me->doDrop(job->obj_name, NULL);
            if (IS_SET_DELETE(rc, DELETE_THIS))
              return DELETE_THIS;
            return FALSE;
          ...
          sprintf(buf, "$n hammers part of %s's %s.", job->char_name, job->obj_name);
          act(buf, FALSE, me, NULL, ch, TO_ROOM);
          return FALSE;
        }
      }
