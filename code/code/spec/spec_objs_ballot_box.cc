  ...
  if (cmd == CMD_LIST) {
    ...
    if (arg.empty()) {
      ...
      ch->sendTo(COLOR_BASIC,
        format("You have a look at %s...\n\r") % o->getName());

      db.query(
        "select poll_id, descr from poll where status='open' order by poll_id");

      if (db.fetchRow()) {
        ch->sendTo(COLOR_BASIC, "These polls are open:\n\r");
        do {
          ch->sendTo(COLOR_BASIC, format("%-2i| <r>%s<1>\n\r") %
                                    convertTo<int>(db["poll_id"]) %
                                    db["descr"]);
        } while (db.fetchRow());
        ch->sendTo("\n\r");
      }
      ...
      db.query(
        "select poll_id,descr from poll where status='closed' order by "
        "poll_id");
      ...
      if (status == "open") {
        ...
        while (db.fetchRow()) {
          ch->sendTo(COLOR_BASIC, format("%-2i| <b>%s<1>\n\r") %
                                    convertTo<int>(db["option_id"]) %
                                    db["descr"]);
        }
      } else if (status == "closed") {
        db.query(
          "select pv.option_id as option_id, po.descr as descr, "
          "count(pv.option_id) as count from poll_vote pv, poll_option po "
          "where pv.poll_id=%i and pv.poll_id=po.poll_id and "
          "pv.option_id=po.option_id group by pv.option_id, po.descr order by "
          "count desc",
          which);
        ...
        while (db.fetchRow()) {
          ch->sendTo(COLOR_BASIC, format("%-2i| <b>%s<1> (%i votes)\n\r") %
                                    convertTo<int>(db["option_id"]) %
                                    db["descr"] % convertTo<int>(db["count"]));
        }
      }
    }
    return true;
	...
	return true;
      ...
      if (tmp == "add") {
        ...
        db.query("insert into poll_option values (%i, %i, '%s')", option_id,
          poll_id, arg.c_str());
        ch->sendTo("Done.\n\r");
      } else if (tmp == "remove") {
        ...
        db.query("delete from poll_option where option_id=%i", option_id);
        ch->sendTo("Done.\n\r");
      } else {
