...
sstring getPlayerName(int id) {
  ...
  if (!db.fetchRow()) {
    vlogf(LOG_BUG,
      format("Couldn't find player name for %i in getPlayerName()!") % id);
    return "unknown";
  }
...
void endAuction(int ticket, int bidder, int seller) {
  ...
  db.query(
    "select s.keeper as keeper, r.name as name from room r, shop s, "
    "shopownedauction soa where r.vnum=s.in_room and s.shop_nr=soa.shop_nr and "
    "soa.ticket=%i",
    ticket);
  ...
  il.openFile(
    format("mobdata/auction/%d/%d") % convertTo<int>(db["keeper"]) % ticket);
  il.readVersion();
...
void procUpdateAuction::run(const TPulse&) const {
  ...
  db.query(
    "select shop_nr, ticket, bidder, seller from shopownedauction where "
    "days=1");
...
sstring listItem(int ticket, TObj* obj, int bid, int buyout, int days) {
  ...
  buf = format("[%3i] %38s %15s %15s\n\r") % ticket % obj->getName() %
        obj->equip_condition(-1) % "fit";
...
void auctionList(TBeing* ch, TMonster* myself) {
  ...
  db.query(
    "select ticket, current_bid, days, buyout from shopownedauction where "
    "shop_nr=%i and seller=%i",
  ...
  if (db.fetchRow()) {
    ...
    do {
      ...
      ch->sendTo(COLOR_BASIC,
        listItem(ticket, obj, convertTo<int>(db["current_bid"]),
          convertTo<int>(db["buyout"]), convertTo<int>(db["days"])));
  ...
  db.query(
    "select ticket, current_bid from shopownedauction where shop_nr=%i and "
    "bidder=%i and days <= 0",
    shop_nr, ch->getPlayerID());
  ...
  if (db.fetchRow()) {
    ...
    myself->doTell(ch->getName(),
      "I don't have anything up for auction right now");
}
...
void auctionSell(TBeing* ch, TMonster* myself, sstring arg) {
  ...
  if (min_bid <= 0 || days <= 0 || (buyout && buyout < min_bid)) {
    myself->doTell(ch->getName(),
      "Usage: sell <item> <minimum bid> <mud days to run auction> <buyout "
      "bid>");
    myself->doTell(ch->getName(), "");
    myself->doTell(ch->getName(),
  ...
  if (!db.fetchRow()) {
    ...
    myself->doTell(ch->getName(),
      format(
        "You need to have a bank account at %s in order to sell items here.") %
        tr->getName());
    return;
  }
  ...
  db.query(
    "insert into shopownedauction (shop_nr, ticket, current_bid, max_bid, "
    "bidder, buyout, days, seller) values (%i, %i, %i, %i, %i, %i, %i, %i)",
    shop_nr, ticket, min_bid, min_bid, ch->getPlayerID(), buyout, days,
    ch->getPlayerID());
  ...
  myself->doTell(ch->getName(),
    "Your item has been placed on the auction block.");
}
...
void auctionBuy(TBeing* ch, TMonster* myself, sstring arg) {
  ...
  db.query(
    "select current_bid, bidder, days from shopownedauction where shop_nr=%i "
    "and ticket=%i",
    shop_nr, ticket);
  ...
  if (days > 0) {
    ...
    db.query(
      ...
      "shop_nr=%i",
      (bid - fee), ch->getPlayerID(), corp.getBank());
    shoplog(corp.getBank(), ch, myself, myself->getName(), (bid - fee),
      "auction");
...
void auctionBid(TBeing* ch, TMonster* myself, sstring arg) {
  ...
  db.query(
    "select seller, bidder, current_bid, max_bid, buyout from shopownedauction "
    "where ticket=%i",
    ticket);
  ...
  if (my_bid >= buyout) {
    ...
    db.query(
      "update shopownedauction set days=0, current_bid=%i, bidder=%i where "
      "ticket=%i",
    ...
    myself->doTell(ch->getName(),
      "Your bid must be greater than your previous max bid if you want to "
      "increase it.");
  } else if (bidder == ch->getPlayerID()) {
    myself->doTell(ch->getName(), "You're already the high bidder!");
    myself->doTell(ch->getName(), "Your max bid has been increased.");
    db.query("update shopownedauction set max_bid=%i where ticket=%i", my_bid,
      ticket);
    shoplog(shop_nr, ch, myself, "max bid increased", 0,
      format("%i talens") % my_bid);
    ...
    db.query("update shopownedauction set current_bid=%i where ticket=%i",
      my_bid + 1, ticket);
    shoplog(shop_nr, ch, myself, "outbidded", 0,
      format("%i talens") % (my_bid + 1));
  } else {
    myself->doTell(ch->getName(), "Congratulations, you're the high bidder.");
    db.query(
      "update shopownedauction set current_bid=%i, bidder=%i, max_bid=%i where "
      "ticket=%i",
      (max_bid + 1), ch->getPlayerID(), my_bid, ticket);
    shoplog(shop_nr, ch, myself, "made bid", 0,
      format("%i talens") % (max_bid + 1));
  }
}

int auctioneer(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself,
  TObj*) {
#if 0
  TDatabase db(DB_SNEEZY);
  int shop_nr;
