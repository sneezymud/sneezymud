...
int bloodDrain(TBeing* vict, cmdTypeT cmd, const char*, TObj* o, TObj*) {
  ...
  if (cmd == CMD_OBJ_HIT && vict && percentChance(20)) {
    ...
    act(
      "$p <1><r>pulses and oozes blood as it draws <y>life essence <r> from<1> "
      "$n.",
      false, vict, o, nullptr, TO_ROOM);

    act(
      "$p <1><r>pulses and oozes blood as it draws your very <y>life essence "
      "<r> from you<1>.",
    ...
    if (dam > 8) {
      ...
      act("$n <r> looks aghast as $s flesh parts and begins to leak blood.<1>",
        false, vict, nullptr, nullptr, TO_ROOM);
