...
// Summary of this weapon proc
// - lightning damage proc
// - 1.75% chance of wither limb, with a white lightning theme (withers the limb due to electric damage)
// - Flavor text proc of arching blue electricity from the weapon

int lightningboltWeapon(TBeing *vict, cmdTypeT cmd, const char *arg, TObj *o, TObj *)
{
char limb[80];
...
{ 
if (!::number(0,150)){
act("<w>Crackles of <b>blue electricity<z> from $n's $o arch into the $g.<1>",
0, ch, o, 0, TO_ROOM);

act("<w>Your $o crackles with <b>blue electricity<z> arching into the $g.<1>",
0, ch, o, 0, TO_CHAR);
...
// Witherlimb proc + 10-20 damage if successful (max once/fight)
...
// Accounting for electricity immunity since this is electricity-based wither limb
if (vict->isImmune(IMMUNE_ELECTRIC, slot))
return false;
...
sprintf(limb, "%s", vict->describeBodySlot(slot).c_str());\
buf = format("Your $p flashes with a blinding white light and emits a bolt of lightning!");

act(buf, FALSE, ch, o, vict, TO_CHAR, ANSI_WHITE);
buf = format("$n's $p flashes with a blinding white light and emits a bolt of lightning!");

act(buf, FALSE, ch, o, vict, TO_NOTVICT, ANSI_WHITE);
buf = format("$n's $p flashes with a blinding white light and emits a bolt of lightning!");

act(buf, FALSE, ch, o, vict, TO_VICT, ANSI_WHITE);

buf = format("You look down in horror and disbelief, as $n's $p turns your %s into ash!") % limb;

act(buf, FALSE, ch, o, vict, TO_VICT, ANSI_GREY);

buf = format("The pain overloads your senses, as your %s is turned into ash!") % limb;

act(buf, FALSE, vict, NULL, NULL, TO_CHAR, ANSI_GREY);

buf = format("Your $p flashes bright white as a bolt of lightning turns $N's %s into ash!") % limb;

act(buf, FALSE, ch, o, vict, TO_CHAR, ANSI_WHITE);

buf = format("$n flashes bright white light as a bolt of lightning turns $N's %s into ash!.") % limb;

act(buf, FALSE, ch, o, vict, TO_NOTVICT, ANSI_WHITE);

buf = format("When your vision returns, you're horrified to see that $n's %s is completely gone!") % limb;

act(buf, FALSE, vict, NULL, NULL, TO_ROOM, ANSI_GREY);

vict->dropWeapon(slot);

dam = ::number(10,20);

rc = vict->reconcileDamage(vict, dam, DAMAGE_ELECTRIC);
...
act("A blue bolt of electricity leaps from your $p and shocks $N.", 
0, ch, o, vict, TO_CHAR, ANSI_BLUE);

act("A blue bolt of electricity leaps from $n's $p and shocks $N.", 
0, ch, o, vict, TO_ROOM, ANSI_BLUE);

act("A blue bolt of electricity from $n's $p shocks your flesh.", 
0, vict, o, 0, TO_CHAR, ANSI_BLUE);
