...
void bounty_hunt_struct::reset() {
  ...
  //  (hunted_victim ? hunted_victim : "NULL"));
  delete[] hunted_item;
  hunted_item = NULL;
...
int bounty_hunter(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself,
    ...
    if ((cmd == CMD_RENT) || (cmd == CMD_DROP) || (cmd == CMD_GIVE)) {
      ...
      vlogf(LOG_DASH,
        format("hunter flee ch->name: %s, job->vict: %s, isname? %s") %
          ch->name % (job && job->hunted_victim ? job->hunted_victim : "NULL") %
      ...
      if (!myself->act_ptr) {
        myself->act_ptr = new bounty_hunt_struct(NULL, NULL, 9,
  ...
  //	(job->hunted_item ? job->hunted_item : "NULL") % (job->hunted_victim ?
  // job->hunted_victim : "NULL") % 	(job->singletarg ? "TRUE" : "FALSE"));
  if (job->hunted_item) {
    if (job->singletarg)
      temp_obj = findHuntedItem(myself, job->hunted_item, job->noneBeyond,
        job->hunted_victim);
      ...
      if (targ) {
        ...
        switch (job->num_chances) {
          ...
          case 9:
            ...
            (job->num_chances)--;
            if (!targ->isPc())
              act("$n falls down laughing at $N.", FALSE, targ, NULL, myself,
                TO_ROOM);
            break;
          case -50:
            // in arena zone
            ...
            if (tmtarg) {
              vlogf(LOG_PROC,
                format("REPO: hunter %s doing monster repo to %s for %s") %
                  myself->getName() % targ->getName() % temp_obj->getName());
              act("$n puts $N in a sleeper hold.", FALSE, myself, NULL, targ,
                TO_ROOM);
