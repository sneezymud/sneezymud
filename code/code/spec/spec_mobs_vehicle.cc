...
int trolleyBoatCaptain(TBeing*, cmdTypeT cmd, const char*, TMonster* myself,
  ...
  if (trolley->in_room == *job) {
    ...
    if (*job == 100) {
      myself->doSay(
        "Grimhaven stop, Grimhaven.  Trolley will be departing for Brightmoon "
        "shortly.");
      ...
      *job = 1303;
      ...
      myself->doSay(
        "If you're not heading for Grimhaven, then you'd better get off now.");
  ...
  switch (::number(0, 80)) {
    ...
    case 1:
      myself->doSay(
        "Keep your limbs inside the trolley please, unless you want to lose "
        "them.");
      break;
    case 2:
...
int casinoElevatorGuard(TBeing* ch, cmdTypeT cmd, const char*, TMonster* myself,
  ...
  for (StuffIter it = myself->roomp->stuff.begin();
    ...
    if ((elevator = dynamic_cast<TVehicle*>(*it)) &&
        elevator->objVnum() == 2360) {
...
class ship_masters {
  ...
  public:
    ...
    ship_masters() {
      ...
      captains.insert(
        std::make_pair(19000, 19077));  // captain matho & the tequila sunrise
      captains.insert(std::make_pair(15375,
        15375));  // viking norseman & the viking ship of shopping
    }
    int may_control(TMonster* captain, TBeing* ersatz_master) {
      ...
      db.query(
        ...
        "else 0 end as privileges from ship_master where captain_vnum =  %i "
        "and (account_id = %i or player_id = %i)",
        ersatz_master->desc->account->account_id, ersatz_master->desc->playerID,
        captain->mobVnum(), ersatz_master->desc->account->account_id,
        ersatz_master->desc->playerID);
      if (!db.fetchRow()) {
        return 0;
      } else {
...
int shipCaptain(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself,
  ...
  TDatabase db(DB_SNEEZY);
  int privileges = 0;  // 0 = none, 1 = delegated (minimal control), 2 = full
                       // control of captain
    ...
    if (argument.word(1) == "destination") {
      if (argument.word(2).empty()) {
        ...
        db.query(
          ...
          "room r1 on r1.vnum = d1.room where d1.vnum = %i",
          myself->mobVnum());
        if (!db.fetchRow()) {
        ...
        myself->doSay(
          format("Aye aye, this 'ere be <W>%s<1>.") % argument.word(2));
        db.query("delete from ship_destinations where vnum=%i and name='%s'",
          myself->mobVnum(), argument.word(2).c_str());
        db.query(
          "insert into ship_destinations (vnum, name, room) values (%i, '%s', "
          "%i)",
          myself->mobVnum(), argument.word(2).c_str(), vehicle->in_room);
      } else {
        myself->doSay(
          "I shall not alter the charts for the likes of ye, missy.");
      }
      ...
      if (argument.word(2).empty()) {
        ...
        db.query(
          "select d1.name, r1.name as aka from ship_destinations d1 left join "
          "room r1 on r1.vnum = d1.room where d1.vnum = %i and d1.room = %i",
          myself->mobVnum(), job->room[job->cur]);
        sstring buf = "a";
      ...
      myself->doSay(
        "Sail here, sail there, stop here, for the love o' me beard make up "
        "yer mind!");
      myself->doDrive("stop");
      for (int i = 0; i < 10; ++i)
        job->room[i] = 0;
      ...
      if (privileges == 2) {
        ...
        myself->doEmote(
          format("begins untangling %s salt encrusted beard.") %
          myself
            ->hshr());  // a female captain would, of course, untangle HER beard
      } else {
      ...
      if (!argument.word(2).empty()) {
        ...
        if (delegate->desc) {
          db.query(
            "delete from ship_master where captain_vnum = %i and player_id = "
            "%i and account_id is null",
            myself->mobVnum(), delegate->desc->playerID);
          db.query(
            "insert into ship_master (captain_vnum, player_id) select %i, %i",
            myself->mobVnum(), delegate->desc->playerID);
          myself->doSay(
            format("Aye, <W>%s<1> may set us upon any known course.") %
            delegate->name);
        } else {
        ...
        // list delegates
        db.query(
          "select p1.name from ship_master s1 join player p1 on s1.player_id = "
          "p1.id and captain_vnum = %i and s1.player_id is not null",
          myself->mobVnum());
        if (!db.fetchRow()) {
          myself->doSay(
        ...
        do {
          myself->doSay(
            format(
              "<W>%s<1> may bid for passage to any of our fine destinations.") %
            db["name"].cap());
        } while (db.fetchRow());
      }
  ...
  if (boat->in_room == job->room[job->cur]) {
    ...
    // move onto next location or stop if none specified (or restart loop if
    // "cruising")
    myself->doDrive("stop");
  ...
  switch (::number(0, 200)) {
    ...
    case 4:
      myself->doSay(
        "Aye, powder monkey, get back below decks where ye belong!");
      break;
    case 5:
