...
int calcInterest(int amt, time_t granted, int term, float rate,
  float def_charge) {
  time_info_data due;
...
int loanShark(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* me,
  TObj* o) {
  ...
  if (cmd == CMD_LIST) {
    ...
    if (sstring(arg) == "loans" && tso.hasAccess(SHOPACCESS_INFO)) {
      db.query(
        "select name, amt, granted_time, term, rate, default_charge from "
        "player p, shopownedloans l where p.id=l.player_id order by "
        "granted_time");

      while (db.fetchRow()) {
        due = whenDue(convertTo<int>(db["granted_time"]),
          convertTo<int>(db["term"]));

        amt = calcInterest(convertTo<int>(db["amt"]),
          convertTo<int>(db["granted_time"]), convertTo<int>(db["term"]),
          convertTo<float>(db["rate"]), convertTo<float>(db["default_charge"]));
        ...
        me->doTell(ch->getName(),
          format("%s for %i talens at %.2f%c due %s of %s, %d P.S") %
            ...
            numberAsString(due.day) % month_name[due.month] % due.year);
        me->doTell(ch->getName(),
          format("%s currently owes %i talens.") % db["name"] % amt);
      }
    } else if (sstring(arg) == "repo" && tso.hasAccess(SHOPACCESS_INFO)) {
      me->doTell(ch->getName(),
        "I have the following options for collecting on loans.");
      me->doTell(ch->getName(), format("1.[1%c]) I can do a direct transfer "
                                       "from the debtors bank account.") %
                                  '%');
      me->doTell(ch->getName(), format("2.[1%c]) I can do a direct transfer "
                                       "from the debtors corporate account.") %
                                  '%');
      me->doTell(ch->getName(),
        format("3.[3%c]) I can do a direct transfer from the debtors shops.") %
          '%');
      me->doTell(ch->getName(),
        format("4.[5%c]) I can repossess items from the debtors shops and give "
               "them to you.") %
          '%');
      me->doTell(ch->getName(),
        format("5.[7%c]) I can repossess entire shops if need be.") % '%');
      me->doTell(ch->getName(), format("6.[9%c]) I can send out a collection "
                                       "agent to collect the debt by force.") %
                                  '%');
      me->doTell(ch->getName(),
        "The cost of the collection method is a percent of the debt, as "
        "listed.");
      me->doTell(ch->getName(),
        "You will be charged even if the collection isn't successful.");
      me->doTell(ch->getName(),
        "Shops won't be repossessed unless the debt exceeds the value of the "
        "shop.");
      me->doTell(ch->getName(),
        "To use one of these collection methods, do 'buy repo <number> "
        "<playername>");
    } else {
      db.query(
      ...
      if (db.fetchRow()) {
        ...
        me->doTell(ch->getName(), format("You have a loan for %i talens, due "
                                         "on the %s day of %s, Year %d P.S.") %
                                    amt % numberAsString(due.day) %
                                    month_name[due.month] % due.year);
        ...
        me->doTell(ch->getName(),
          format("With interest, you owe %i talens.") % amt);
      } else {
        me->doTell(ch->getName(),
  ...
  if (cmd == CMD_BUY) {
    if (sstring(arg).word(0) == "repo" && tso.hasAccess(SHOPACCESS_OWNER)) {
      ...
      if (loanamt > amt || loanamt <= 0) {
        me->doTell(ch->getName(),
          format("You can't take out a loan for that much.  The most I can "
                 "give you is %i.") %
            amt);
        return true;
      }
      ...
      if (amt > me->getMoney()) {
        me->doTell(ch->getName(),
          "At the moment, I don't have the necessary capital to extend a loan "
          "to you.");
        return true;
      }
      ...
      me->doTell(ch->getName(),
        format("There you go.  Remember, I need the money back, plus interest, "
               "within %i years.") %
          term);
      me->doTell(ch->getName(),
        "Do 'list' again at anytime to see how much you owe with interest "
        "included.");
      me->doTell(ch->getName(),
        "You can give me talens at any time to make a payment on your loan.");

      shoplog(shop_nr, ch, me, "talens", -amt, "loaning");
    } else {
      me->doTell(ch->getName(),
        "If you want to take out the loan, do \"buy loan <amount>\".");
    }
  }
  ...
  if (cmd == CMD_MOB_GIVEN_COINS) {
    ...
    if (db.fetchRow()) {
      ...
      if (coins >= amt) {
        me->doTell(ch->getName(),
          "Alright, everything appears to be in order here.  Consider your "
          "loan paid off!");
        db.query("delete from shopownedloans where player_id=%i",
          ch->getPlayerID());
        ...
        me->doTell(ch->getName(),
          format("Thanks for the payment.  You paid down the principle by %i "
                 "talens, the rest went to interest.") %
            (int)(perc * coins));
