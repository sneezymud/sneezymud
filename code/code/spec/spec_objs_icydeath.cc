...
int ColdHit(TBeing* ch, TBeing* vict, TObj* obj, int level) {
  ...
  */
  ...
  if (dam <= 11) {
    act("$p becomes covered with ice and freezes $n.", false, vict, obj, ch,
      TO_ROOM, ANSI_CYAN);
    act("$p becomes covered with ice and freezes you.", false, vict, obj, ch,
      TO_CHAR, ANSI_CYAN);
  } else {
...
int ColdShroud(TBeing* ch, TObj* obj, int level) {
  ...
  act("A frigid wind emerges from $p and envelopes $N.", false, ch, obj, ch,
    TO_ROOM, ANSI_BLUE);
  act("A frigid wind emerges from $p and envelopes you.", false, ch, obj, ch,
    TO_CHAR, ANSI_BLUE);
  ...
  if (ch->getImmunity(IMMUNE_COLD) >= 100) {
    ...
    act("The air around $N coallesces into a layer of protective frost.", false,
      ch, obj, ch, TO_ROOM, ANSI_CYAN);
    act("The air around you coallesces into a layer of protective frost.",
      false, ch, obj, ch, TO_CHAR, ANSI_CYAN);
  }
  return true;
}
...
int coldBlast(TBeing* ch, TBeing* vict, TObj* obj, int level,
  ...
  dam =
    dam * (1 + coldimmunity / 100);  // Increase damage based on cold immunity
  dam = dam * (level / 50);          // Scale with level

  act("A savage winter storm erupts from $p!", false, ch, obj, vict, TO_CHAR,
    ANSI_WHITE_BOLD);
  act("A savage winter storm erupts from $p!", false, ch, obj, vict, TO_ROOM,
    ANSI_WHITE_BOLD);

  act("Howling winds pummel into $N's body with intense fury!", false, ch, obj,
    vict, TO_ROOM, ANSI_BLUE_BOLD);
  act("Howling winds pummel into your body with intense fury!", false, ch, obj,
    vict, TO_VICT, ANSI_BLUE_BOLD);
  act("Howling winds pummel into $N's body with intense fury!", false, ch, obj,
    vict, TO_CHAR, ANSI_BLUE_BOLD);
  ...
  if (manaSpent >= 300) {
    ...
    act("Wind slams into $N, knocking them to the ground!", false, ch, obj,
      vict, TO_ROOM, ANSI_CYAN);
    act("Wind slams into you, knocking you to the ground!", false, ch, obj,
      vict, TO_VICT, ANSI_CYAN);
    act("Wind slams into $N, knocking them to the ground!", false, ch, obj,
    ...
    if (manaSpent >= 400) {
      ...
      act("The storm rages around $N, freezing $M to the core!", false, ch, obj,
        vict, TO_ROOM, ANSI_CYAN_BOLD);
      act("The storm rages around you, freezing you to the core!", false, ch,
        obj, vict, TO_VICT, ANSI_CYAN_BOLD);
      act("The storm rages around $N, freezing $M to the core!", false, ch, obj,
        vict, TO_CHAR, ANSI_CYAN_BOLD);
    }
    ...
    if (manaSpent >= 500) {
      ...
      for (wearSlotT limb = MIN_WEAR; limb < MAX_WEAR;
           limb = static_cast<wearSlotT>(limb + 1)) {
      ...
      while (numLimbs > 0 && !validLimbs.empty()) {
        ...
        sstring buf =
          format("The icy winds tear into $N's %s with shards of ice!") %
          vict->describeBodySlot(limb);
        act(buf, false, ch, obj, vict, TO_ROOM, ANSI_BLUE_BOLD);

        buf = format("The icy winds tear into your %s with shards of ice!") %
              vict->describeBodySlot(limb);
        act(buf, false, ch, obj, vict, TO_VICT, ANSI_BLUE_BOLD);

        buf = format("The icy winds tear into $N's %s with shards of ice!") %
              vict->describeBodySlot(limb);
  ...
  act(
    "Jagged shards of ice explode from within $n's body, impaling them in a "
    "bloody spray.",
...
int coldThief(TBeing* ch, TBeing* vict, TObj* obj, int level) {
  ...
  act("<W>$p turns bright white and freezes down $N's spine!<z>", false, ch,
    obj, vict, TO_CHAR);
  act("<W>$p turns bright white and freezes down $N's spine!<z>", false, ch,
    obj, vict, TO_ROOM);
  act("<W>$p turns bright white and freezes down your spine!<z>", false, ch,
    obj, vict, TO_VICT);
  ...
  if (vict->slotChance(limb) && !vict->equipment[limb] &&
    ...
    buf =
      format(
        "$p freezes as it pierces your %s, leaving behind a jagged icicle!") %
      vict->describeBodySlot(limb);
    act(buf, false, ch, obj, vict, TO_VICT, ANSI_BLUE_BOLD);
...
int icyDeath(TBeing* vict, cmdTypeT cmd, const char* arg, TObj* obj, TObj*) {
  ...
  if (cmd == (CMD_SAY) || cmd == (CMD_SAY2)) {
    ...
    if (buf == "winter" && buf2 == "cometh") {
      if (!vict) {
        act("You cannot use $p's powers unless you are fighting.", TRUE, ch,
          obj, NULL, TO_CHAR, NULL);
        return FALSE;
      }
