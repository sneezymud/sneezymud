...
void procBankInterest::run(const TPulse&) const {
  ...
  db.query(
    "update shopownedbank set earned_interest=0 where earned_interest is null");
  db.query(
    "update shopownedcorpbank set earned_interest=0 where earned_interest is "
    "null");
  ...
  db.query(
    "select s1.shop_nr, s1.keeper from shop s1 join mob m1 on s1.keeper = "
    "m1.vnum where spec_proc = %i",
    SPEC_BANKER);
  while (db.fetchRow()) {
    ...
    in.query("select player_id, talens from shopownedbank where shop_nr=%i",
      shop_nr);
    while (in.fetchRow())
      player_gain[convertTo<int>(in["player_id"])] =
    ...
    // calculate interest
    in.query(
      "update shopownedbank set earned_interest=earned_interest + (talens * "
      "(%f / 365.0)) where shop_nr=%i",
      profit_sell, shop_nr);

    // doll out earned interest that isn't fractional
    in.query(
      "update shopownedbank set talens=talens + truncate(earned_interest,0), "
      "earned_interest=earned_interest - truncate(earned_interest,0) where "
      "shop_nr=%i",
      shop_nr);

    // calculate interest
    in.query(
      "update shopownedcorpbank set earned_interest=earned_interest + (talens "
      "* (%f / 365.0)) where shop_nr=%i",
      profit_sell, shop_nr);

    // doll out earned interest that isn't fractional
    in.query(
      "update shopownedcorpbank set talens=talens + "
      "truncate(earned_interest,0), earned_interest=earned_interest - "
    ...
    // log player gains
    in.query(
      "select p.name as name, sob.player_id as player_id, sob.talens as talens "
      "from shopownedbank sob, player p where shop_nr=%i and "
      "sob.player_id=p.id",
      shop_nr);
    while (in.fetchRow()) {
      if ((convertTo<int>(in["talens"]) -
            player_gain[convertTo<int>(in["player_id"])]) != 0) {
        int amt = convertTo<int>(in["talens"]) -
                  player_gain[convertTo<int>(in["player_id"])];
    ...
    // log corporate gains
    in.query(
      "select c.name, sob.corp_id, sob.talens from shopownedcorpbank sob, "
      "corporation c where c.corp_id=sob.corp_id and sob.shop_nr=%i",
      shop_nr);
    while (in.fetchRow()) {
      if ((convertTo<int>(in["talens"]) -
            corp_gain[convertTo<int>(in["corp_id"])]) != 0) {
        int amt = convertTo<int>(in["talens"]) -
                  corp_gain[convertTo<int>(in["corp_id"])];
...
int bankWithdraw(TBeing* ch, TMonster* myself, TMonster* teller, int shop_nr,
  int money) {
  ...
  db.query(
    "update shopownedbank set talens=talens-%i where player_id=%i and "
    "shop_nr=%i",
...
int bankDeposit(TBeing* ch, TMonster* myself, TMonster* teller, int shop_nr,
  ...
  if (!db.fetchRow()) {
    teller->doTell(ch->getName(),
      "You really should not see me, this is an error...");
    vlogf(LOG_BUG,
      format(
...
int bankBuyAccount(TBeing* ch, TMonster* myself, TMonster* teller, int shop_nr,
  ...
  db.query(
    "insert into shopownedbank (player_id, shop_nr, talens) values (%i, %i, 0)",
    ch->getPlayerID(), shop_nr);
...
int centralBanker(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself,
  ...
  if (cmd == CMD_LIST) {
    db.query(
      "select s.shop_nr, r.name from room r, shop s, shopownedcentralbank socb "
      "where socb.centralbank=%i and s.shop_nr=socb.bank and r.vnum=s.in_room",
      shop_nr);

    while (db.fetchRow()) {
      db2.query(
        "select (sb.c+sbc.c) as c, (sb.t+sbc.t) as t from (select count(*) as "
        "c, sum(talens) as t from shopownedbank where shop_nr=%i) sb, (select "
        "count(*) as c, sum(talens) as t from shopownedcorpbank where "
        "shop_nr=%i) sbc",
        db["shop_nr"].c_str(), db["shop_nr"].c_str());

      if (db2.fetchRow()) {
        buf += format("<c>%s - %s accounts, %s in deposits.<1>\n\r") %
               db["name"] % db2["c"] % talenDisplay(convertTo<int>(db2["t"]));
      }
    }
...
int banker(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself, TObj*) {
  ...
  if (cmd == CMD_LIST) {
    ...
    db.query(
      "select p.name, b.talens from player p, shopownedbank b where "
      "b.shop_nr=%i and b.player_id=p.id union select c.name, b.talens from "
      "corporation c, shopownedcorpbank b where b.shop_nr=c.bank and "
      "b.shop_nr=%i and b.corp_id=c.corp_id order by talens desc",
      shop_nr, shop_nr);
    ...
    db.query(
      "select (sb.c+sbc.c) as c, (sb.t+sbc.t) as t from (select count(*) as c, "
      "sum(talens) as t from shopownedbank where shop_nr=%i) sb, (select "
      "count(*) as c, sum(talens) as t from shopownedcorpbank where "
      "shop_nr=%i) sbc",
      shop_nr, shop_nr);
    ...
    if (db.fetchRow()) {
      buf += format("%i total accounts, %s talens.\n\r") %
             convertTo<int>(db["c"]) % talenDisplay(convertTo<int>(db["t"]));
    }
