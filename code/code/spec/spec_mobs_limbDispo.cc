...
int limbDispo(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* mob, TObj*) {
  ...
  if (partname.find("[q]") != sstring::npos) {
    ...
    // assuming the tooth generating code was updated in crit_combat.cc at
    // start/finish of quest
    sstring slot_num = "";
      ...
      if (!team.empty()) {
        if (samaritan) {
          ...
          if (team.compare(db["caddy_team"]))
            ...
            mob->doWhisper(format("%s I'll make sure your gang, <o>%s<1>, gets "
                                  "the nod for this one.") %
                           ch->name % team);
        } else {
          mob->doWhisper(format("%s I'll make sure your gang, <o>%s<1>, gets "
                                "the nod for this one.") %
  ...
  switch (::number(1, 11)) {
    ...
    case 5:
      resp =
        "Trying to hide the evidence, eh?  Don't worry, your secret is safe "
        "with me.";
      break;
    case 6:
