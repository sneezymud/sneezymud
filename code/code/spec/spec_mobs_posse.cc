...
int grimhavenPosse(TBeing* ch, cmdTypeT cmd, const char*, TMonster* myself,
  ...
  switch (job->state) {
    ...
    case STATE_RETURN_OFFICE:  // go back to office
      if (head_guard_path[job->cur_path][(job->cur_pos + 1)].direction ==
          -1) {  // XXX: TODO: buffer overflow here
        ...
        act("$n grows weary of searching for criminals.", 0, myself, 0, 0,
          TO_ROOM);
        act("$n goes back to his office.", 0, myself, 0, 0, TO_ROOM);
        ...
        rc = myself->goDirection(
          head_guard_path[job->cur_path][job->cur_pos + 1].direction);
        if (IS_SET_DELETE(rc, DELETE_THIS)) {
          return DELETE_THIS;
        } else if (rc) {
          job->cur_pos++;
            ...
            switch (::number(0, 4)) {
              case 0:
                if (dynamic_cast<TMonster*>(job->criminal))
                  act("$n struggles to get free, but $N holds him tightly.", 0,
                    job->criminal, 0, myself, TO_ROOM);
                break;
              case 1:
                if (dynamic_cast<TMonster*>(job->criminal))
                  act(
                    "$n looks at you pleadingly, with a panicked look in his "
                    "eye.",
                    0, job->criminal, 0, 0, TO_ROOM);
                act("$n jerks him around roughly.", 0, myself, 0, 0, TO_ROOM);
                break;
              case 2:
                myself->doSay(
                  "Clear the way, guards coming through, we've got a "
                  "criminal.");
                break;
              case 3:
                if (dynamic_cast<TMonster*>(job->criminal)) {
                  act(
                    "$n whispers to you, \"Please you've got to help me!  I "
                    "didn't do it, it's all a big setup!\"",
                    0, job->criminal, 0, 0, TO_ROOM);
                }
                act("$n casts a reproving look your way.", 0, myself, 0, 0,
                  TO_ROOM);
                break;
            }
          }
                ...
                for (i = 0; i < 3; ++i) {
                  if (tmons->mobVnum() == criminals[i]) {
                    ...
                    break;
                         ...
                         tb->hasClass(CLASS_THIEF) && tb->GetMaxLevel() >= 8 &&
                         myself->canSee(tb) && !::number(0, 3)) {
                job->criminal = tb;
                job->state = STATE_ARREST_FAST;
                break;
              }
            }
          } else if (job->state == STATE_BOOK_UM && !::number(0, 2) &&
                     job->criminal) {
            switch (::number(0, 4)) {
              case 0:
                if (dynamic_cast<TMonster*>(job->criminal))
    ...
    case STATE_ARREST_FAST:  // arrest the criminal
      ...
      switch (job->arrest_state) {
        case 0:
          act("$n glares at $N intently for a moment.", 0, myself, 0,
            job->criminal, TO_ROOM);
          myself->doSay(
            "You're coming with us, we've got some questions to ask you.");
          job->arrest_state = 1;
          break;
        case 1:
          if (dynamic_cast<TMonster*>(job->criminal)) {
            job->criminal->doSay("Wh-what?  I didn't do anything.");
            act("$n looks about innocently.", 0, job->criminal, 0, 0, TO_ROOM);
          }
          job->arrest_state = 2;
          break;
        case 2:
          if (dynamic_cast<TMonster*>(job->criminal)) {
            act(
              "$n meets $N's cold hard eyes for a moment and then looks away.",
              0, job->criminal, 0, myself, TO_ROOM);
            act("$n panics and attempts to flee!", 0, job->criminal, 0, 0,
              TO_ROOM);
          }
          act(
            "With lightning fast reflexes, $n sticks a foot out and trips $N!",
