...
int tudy(TBeing*, cmdTypeT cmd, const char*, TMonster* tudy, TObj*) {
  ...
  if (haskey1 && haskey2) {
    act("$n quickly unlocks his shackles.", 0, tudy, 0, 0, TO_ROOM);
    act("$n looks to the heavens and as large wings sprout from his back.", 0,
      tudy, 0, 0, TO_ROOM);
    ...
    *tudy->roomp += *newtudy;
    ...
    for (StuffIter it = newtudy->roomp->stuff.begin();
         it != newtudy->roomp->stuff.end() && (t = *it); ++it) {
