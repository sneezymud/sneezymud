  ...
  for (unsigned int i = 0; i < corp_list.size(); ++i) {
    m.insert(std::pair<int, sstring>(corp_list[i].rank,
      format("%-2i| <r>%-38s<1> | %6s talens  %6s in assets\n\r") %
        ...
        talenDisplay(corp_list[i].gold) % talenDisplay(corp_list[i].assets)));
  }
...
void corpLogs(TBeing* ch, TMonster* me, sstring arg, sstring corp_arg) {
    ...
    if (corp_arg.empty()) {
      ...
      if (db.fetchRow()) {
        me->doTell(ch->getName(),
          "You must specify the ID of the corporation you wish to deposit the "
          "money for.");
        return;
      }
    } else {
      if (convertTo<int>(corp_arg) == 0) {
        me->doTell(ch->getName(),
          "You must specify the ID of the corporation you wish look at the "
          "logs of.");
        return;
      }
  ...
  if (arg == "daily") {
    db.query(
      "select name, action, sum(talens) as talens, round(avg(corptalens)) as "
      "corptalens, concat(extract(year from logtime),'-',extract(month from "
      "logtime),'-',extract(day from logtime)) as logtime from corplog where  "
      "corp_id = %i group by concat(extract(year from "
      "logtime),'-',extract(month from logtime),'-',extract(day from "
      "logtime)), name, action order by concat(extract(year from "
      "logtime),'-',extract(month from logtime),'-',extract(day from logtime)) "
      "desc, talens desc",
      corp_id);
    ...
    while (db.fetchRow()) {
      ...
      buf = format("%s%16.16s  %20.20s %10s %10s  Total: %s<1>\n\r") %
            color_code % db["logtime"] % db["name"] % db["action"] %
            db["talens"] % db["corptalens"];
      sb += buf;
    }
  } else {
    db.query(
      "select name, action, talens, corptalens, logtime from corplog where "
      "corp_id = %i order by logtime desc",
      corp_id);

    while (db.fetchRow()) {
      buf = format("%16.16s  %20.20s %10s %10s  Total: %s\n\r") %
            db["logtime"] % db["name"] % db["action"] % db["talens"] %
            db["corptalens"];
      sb += buf;
    }
  }
...
void corpSummary(TBeing* ch, TMonster* me, int corp_id) {
  ...
  if (!corp_id) {
    me->doTell(ch->getName(),
      "I don't have any information for that corporation.");
    return;
  }
  ...
  if (!db.fetchRow()) {
    ...
    db.query(
      "select c.name, 0 as gold, b.talens as banktalens, 0 as shops, bank, "
      "sob.corp_id as bankowner from shopowned sob, corporation c left outer "
      "join shopownedcorpbank b on(c.corp_id=b.corp_id) where "
      "sob.shop_nr=c.bank and c.corp_id=%i group by c.corp_id, c.name, "
      "b.talens, c.bank, sob.corp_id order by c.corp_id",
      corp_id);
  ...
  // shops
  db.query(
    "select s.shop_nr, s.in_room, so.gold from shop s, shopowned so where "
    "s.shop_nr=so.shop_nr and so.corp_id=%i order by so.gold desc",
    corp_id);

  ch->sendTo(COLOR_BASIC,
    "The following shops are owned by this corporation:\n\r");

  while (db.fetchRow()) {
    if ((tr = real_roomp(convertTo<int>(db["in_room"])))) {
      gold = convertTo<int>(db["gold"]);
      ch->sendTo(COLOR_BASIC, format("%-3s| <r>%s<1> with %s talens.\n\r") %
                                db["shop_nr"] % tr->getName() %
                                talenDisplay(gold));
    }
  }
}
...
void corpWithdraw(TBeing* ch, TMonster* me, int gold, sstring arg) {
  ...
  if (arg.empty()) {
    ...
    if (db.fetchRow()) {
      me->doTell(ch->getName(),
        "You must specify the ID of the corporation you wish to withdraw the "
        "money from.");
      return;
    }
  } else {
  ...
  if (tmp < gold) {
    me->doTell(ch->getName(),
      format("Your corporation only has %i talens.") % tmp);
    return;
  }
  ...
  if (bank_amt < gold) {
    me->doTell(ch->getName(),
      "I'm afraid your bank has overextended itself, and doesn't have your "
