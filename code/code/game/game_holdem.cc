...
int HoldemGame::handValue(HoldemPlayer* hp) {
  ...
  for (i = 0; i < 15; ++i) {
    ...
    if ((++straight) >= 5) {
      ...
      for (int j = 0; j < 4; ++j) {
        if (rank[j][i] && rank[j][i - 1] && rank[j][i - 2] && rank[j][i - 3] &&
            rank[j][i - 4]) {
          // straight flush
          score[8] = i;
          if (i == 14) {
...
void HoldemGame::showdown(TBeing* ch) {
  ...
  if (playerHandCount() < 2) {
    ...
    for (unsigned int p = 0; p < winners.size(); ++p) {
      buf =
        format("$n %s with %s!") % (winners.size() > 1 ? "ties" : "wins") % msg;
      act(buf, false, players[winners[p]]->ch, 0, 0, TO_ROOM);
      buf =
        format("You %s with %s!") % (winners.size() > 1 ? "tie" : "win") % msg;
  ...
  for (i = 0; i < MAX_HOLDEM_PLAYERS; ++i) {
    if (players[i] && players[i]->ch) {
      for (StuffIter it = players[i]->ch->stuff.begin();
           it != players[i]->ch->stuff.end(); ++it) {
  ...
  act("The button moves to $n.", false, players[button]->ch, 0, 0, TO_ROOM);
  act("The button moves to you.", false, players[button]->ch, 0, 0, TO_CHAR);

  // reinitialize players
  for (i = 0; i < MAX_HOLDEM_PLAYERS; ++i) {
    if (players[i]) {
      players[i]->hand[0] = nullptr;
      players[i]->hand[1] = nullptr;
    }
  }
    ...
    for (int j = 0; j < MAX_HOLDEM_PLAYERS; ++j) {
      if (players[i] && players[j] && players[i]->name == players[j]->name) {
        if ((++count) > 1) {
          ...
          //	  vlogf(LOG_PEEL, format("duplicate entry, removing player %s") %
          // players[i]->name);
          delete players[i];
        }
      }
...
int HoldemGame::nextBetter(int b) {
  ...
  for (int i = (b + 1) % MAX_HOLDEM_PLAYERS; i != firstbetter;
       i = (i + 1) % MAX_HOLDEM_PLAYERS) {
    if (players[i] && players[i]->ch && players[i]->hand[0] &&
        !players[i]->allin) {
      return i;
    }
  }
...
int HoldemGame::nextButton() {
  for (int i = (button + 1) % MAX_HOLDEM_PLAYERS; i != button;
       i = (i + 1) % MAX_HOLDEM_PLAYERS) {
...
int HoldemGame::exitGame(const TBeing* ch) {
  ...
  if (playerHandCount() == 1) {
    ...
    if ((tmp = nextBetter(better)) != -1) {
      ...
      players[better]->ch->sendTo(COLOR_BASIC,
        "You can <c>raise<1>, <c>fold<1> or <c>call<1>.\n\r");
    } else {
...
void HoldemGame::peek(const TBeing* ch) {
  ...
  if (last_bet) {
    ch->sendTo(COLOR_BASIC,
      format("You peek at the pot and see: %s. [%i]\n\r\n\r") %
  ...
  if (ch->isImmortal() && ch->getName() == "Peel" && !ch->checkHoldem()) {
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      if (!community[i]) {
        ...
        ch->sendTo(COLOR_BASIC,
          format("Community draw: %s\n\r") % community[i]->getName());
      }
    }

    for (int i = 0; i < MAX_HOLDEM_PLAYERS; ++i) {
      if (players[i] && players[i]->hand[0] && players[i]->hand[1] &&
          players[i]->name != ch->name) {
        ch->sendTo(COLOR_BASIC,
...
void HoldemGame::call(TBeing* ch) {
  ...
  if (players[better]->allin) {
    if (chipl.size() == 0) {
      ...
      buf =
        format("$n goes all in %s! [%i]") % chipl[0]->getName() % chipl.size();
      act(buf, false, ch, 0, 0, TO_ROOM);
      buf =
        format("You go all in %s! [%i]") % chipl[0]->getName() % chipl.size();
      act(buf, false, ch, 0, 0, TO_CHAR);
      bet += chipl[0]->obj_flags.cost * chipl.size();
    }
  } else {
    buf = format("$n calls with %s. [%i]") % chipl[0]->getName() % chipl.size();
    act(buf, false, ch, 0, 0, TO_ROOM);
    buf = format("You call with %s. [%i]") % chipl[0]->getName() % chipl.size();
    act(buf, false, ch, 0, 0, TO_CHAR);
    bet += chipl[0]->obj_flags.cost * chipl.size();
  }
...
void HoldemGame::raise(TBeing* ch, const sstring& arg) {
  ...
  buf = format("$n raises with %s. [%i]") % chipl[0]->getName() % chipl.size();
  act(buf, false, ch, 0, 0, TO_ROOM);
  buf = format("You raise with %s. [%i]") % chipl[0]->getName() % chipl.size();
  act(buf, false, ch, 0, 0, TO_CHAR);
...
void HoldemGame::flop(TBeing* ch) {
  ...
  if (players[better]->hand[0] == nullptr)
...
void HoldemGame::fold(TBeing* ch) {
  ...
  players[better]->hand[0] = nullptr;
  players[better]->hand[1] = nullptr;
...
void HoldemGame::Bet(TBeing* ch, const sstring& arg) {
  ...
  for (int i = better; i != -1; i = nextPlayer(i)) {
    ...
    act("$n is dealt two cards facedown.", false, players[i]->ch, 0, 0,
      TO_ROOM);
