...
void TBeing::doPlay(const char* arg) {
  ...
  if (!*game) {
    ...
    if (!checkSlots()) {
      sendTo(
        "Do you have a gambling problem?  There's no slot machine here!\n\r");
      return;
    }
    ...
    if (!*options) {
      ...
      if (!strcmp(options, "1")) {
        if ((chip = find_chip(this, CHIP_100))) {
          ...
          sendTo(
            "You search your pockets for chips but come up empty-handed.\n\r");
          return;
        }
    ...
    if (!*options) {
      ...
      sendTo(
        "In order to place a bet, you must first use the bet command to "
        "determine\n\r");
      sendTo(
        "the amount of the bets you will be making.Unfortunately, all your "
        "bets must\n\r");
      sendTo(
        "remain constant throughout the dice roll. You can always bet on new "
        "things,\n\r");
      sendTo(
        "but your bet must be the same. You can change the amount you are "
        "betting \n\r");
      sendTo("only after a point roll has been changed.\n\r\n\r");
      sendTo(
        "Type help craps for more help on this. It is most probably confusing "
        "at\n\r");
