...
static int spinMiss(TBeing* caster, TBeing* victim, spinMissT type) {
  ...
  if (type == TYPE_DEX) {
    act("$N deftly avoids your attempt at spinning $M.", FALSE, caster, 0,
      victim, TO_CHAR);
    act("You deftly avoid $n's attempt at spinning you.", FALSE, caster, 0,
      victim, TO_VICT);
    act("$N deftly avoids $n's attempt at spinning $M.", FALSE, caster, 0,
      victim, TO_NOTVICT);
  } else if (type == TYPE_DEFENSE) {
    act("$N is too fast and avoids your attempt at spinning $M.", FALSE, caster,
      0, victim, TO_CHAR);
    ...
    act("$N deftly counters your attempt at spinning $M.", FALSE, caster, 0,
      victim, TO_CHAR, ANSI_RED);
    act("You trip and land on the $g.", FALSE, caster, 0, victim, TO_CHAR,
      ANSI_RED);
    act("You deftly counter $n's attempt at spinning you.", FALSE, caster, 0,
      victim, TO_VICT);
    act("You stick out your foot and trip $m to the $g.", FALSE, caster, 0,
      victim, TO_VICT);
    act("$N deftly counters $n's attempt at spinning $M.", FALSE, caster, 0,
      victim, TO_NOTVICT);
    act("$N sticks out $S foot tripping $n to the $g.", FALSE, caster, 0,
      victim, TO_NOTVICT);
    ...
    act("$n tries to spin $N, but ends up falling down.", FALSE, caster, 0,
      victim, TO_NOTVICT);
    act("You try to spin $N, but end up falling on your face.", FALSE, caster,
      0, victim, TO_CHAR);
    act("$n fails to spin you, and tumbles to the $g.", FALSE, caster, 0,
      victim, TO_VICT);
...
static int spinHit(TBeing* caster, TBeing* victim) {
  ...
  if (!victim->riding) {
    act("$n grabs $N's arm and spins $M!", FALSE, caster, 0, victim,
      TO_NOTVICT);
    act("Now dizzy, $N trips and falls to the $g.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    act("You grab $N's arm and spin $M HARD!", FALSE, caster, 0, victim,
      TO_CHAR);
    act("Now dizzy $N trips and falls to the $g.", FALSE, caster, 0, victim,
      TO_CHAR);
    act("$n grabs you by the arm and spins you violently.", FALSE, caster, 0,
      victim, TO_VICT);
    act(
    ...
    act("$n grabs $N's arm and rips $M off of $S $o!", FALSE, caster,
      victim->riding, victim, TO_NOTVICT);
    act("$N slams head first into the $g.", FALSE, caster, victim->riding,
      victim, TO_NOTVICT);
    act("You grab $N's arm and pull $M off of $S $o!", FALSE, caster,
      victim->riding, victim, TO_CHAR);
    act("$N slams head first into the $g.", FALSE, caster, victim->riding,
      victim, TO_CHAR);
    act("$n suddenly grabs your arm and gives a hard yank!", FALSE, caster,
      victim->riding, victim, TO_VICT);
    act("Suddenly, the $g rushes upward as you fall off of your $o.", FALSE,
      caster, victim->riding, victim, TO_VICT, ANSI_RED);
    act("OOFFF!! Yuck, dirt tastes AWFUL!", FALSE, caster, victim->riding,
  ...
  int dam = caster->getSkillDam(victim, SKILL_SPIN,
    caster->getSkillLevel(SKILL_SPIN), caster->getAdvLearning(SKILL_SPIN));
