...
{ 
  ...
  int dam = getSkillDam(victim, SKILL_SLAM, skillLevel, getAdvLearning(SKILL_SLAM));
  auto *weapon = dynamic_cast<TBaseWeapon *>(heldInPrimHand());  
  ...
  if (getCombatMode() == ATTACK_BERSERK) {
    dam *= 2;
    act("$n slams $N with $s weapon in a <R>berserk fury<z>, inflicting considerable damage!", FALSE, this, 0, victim, TO_NOTVICT);
    act("You slam your weapon into $N in a <R>berserk fury<z>, inflicting considerable damage!", FALSE, this, 0, victim, TO_CHAR);
    act("$n slams $s weapon into you with a <r>berserker's <z>zeal!", FALSE, this, 0, victim, TO_VICT);

  } else {
    act("$n slams $N with $s weapon, inflicting considerable damage!", FALSE, this, 0, victim, TO_NOTVICT);
    act("You slam your weapon into $N, inflicting considerable damage!", FALSE, this, 0, victim, TO_CHAR);
...
int TBeing::slamFail(TBeing *victim)
{
  if (victim->getPosition() > POSITION_DEAD) {
    act("$n's attempt at slamming $N's with $s his weapon fails to make contact.", FALSE, this, 0, victim, TO_NOTVICT);
    act("Your attempt at slamming $N with your weapon fails to make contact.", FALSE, this, 0, victim, TO_CHAR);
    act("$n attempts to slam you with $s weapon but comes up short.", FALSE, this, 0, victim, TO_VICT);
  }
