...
static int kneestrikeMiss(TBeing* c, TBeing* v, int type) {
  switch (type) {
    case 0: {  // normal
      act("$n tries to plant a knee on $N, but fails.", FALSE, c, 0, v,
        TO_NOTVICT);
      act("$N moves out of the way and you miss your knee strike.", FALSE, c, 0,
        v, TO_CHAR);
      act("$n tries to hit you with $s knee, but you dodge it.", FALSE, c, 0, v,
        TO_VICT);
    } break;
    case 1: {  // monk counter move
      ...
      int dam =
        c->getSkillDam(v, SKILL_KNEESTRIKE, c->getSkillLevel(SKILL_KNEESTRIKE),
          c->getAdvLearning(SKILL_KNEESTRIKE)) /
        2;
      ...
      if ((item = dynamic_cast<TObj*>(v->equipment[v->getPrimaryFoot()])))
        if (item->isSpiked() || item->isObjStat(ITEM_SPIKED)) {
          act("The spikes on your $o sink into $N's side.", FALSE, v, item, c,
            TO_CHAR);
          act("The spikes on $n's $o sink into $N's side.", FALSE, v, item, c,
            TO_NOTVICT);
      ...
      case 1:{ // monk counter move
        act("$n tries to plant a knee on $N, but $E nimbly leaps out of the
      way.", FALSE, c, 0, v, TO_NOTVICT); act("$N nimbly leaps out of the way
      and you miss your knee strike.", FALSE, c, 0, v, TO_CHAR); act("$n tries
      to hit you with $s knee, but you nimbly leap out of the way.", FALSE, c,
      0, v, TO_VICT); } break;
      */
  }
...
static int kneestrikeHit(TBeing* c, TBeing* victim) {
  ...
  int spikeddam = 0;
  int dam = c->getSkillDam(victim, SKILL_KNEESTRIKE,
    c->getSkillLevel(SKILL_KNEESTRIKE), c->getAdvLearning(SKILL_KNEESTRIKE));
  ...
  switch (crit) {
    ...
    case CRIT_S_DOUBLE:
      dam = (int)(dam * 1.5);
      for (i = 0; i < 3; ++i)
        act(crit_msg[2][i], FALSE, c, 0, victim, msg_tgt[i]);
      break;
    case CRIT_S_NONE:
      break;
  }
...
// why the fuck are we doing critFail inside the success case?
  ...
  if (caster_hgt < victim->getPartMinHeight(ITEM_WEAR_FEET)) {
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[0][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[1][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[2][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[3][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[4][i], FALSE, c, 0, victim, msg_tgt[i]);
  } else if (caster_hgt >= victim->getPartMinHeight(ITEM_WEAR_ARMS) &&
    ...
    for (i = 1; i < 3; ++i)
      act(hit_msg[5][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[9][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[8][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[7][i], FALSE, c, 0, victim, msg_tgt[i]);
    ...
    for (i = 0; i < 3; ++i)
      act(hit_msg[6][i], FALSE, c, 0, victim, msg_tgt[i]);
  } else {
    // target solar plexus
    pos = WEAR_BODY;
    dam_type = DAMAGE_KNEESTRIKE_SOLAR;
