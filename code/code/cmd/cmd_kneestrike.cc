...
int TBeing::kneestrikeMiss(TBeing *v, int type)
{
  switch(type){
    case 0:{ // normal
      act("$n tries to plant a knee on $N, but fails.", FALSE, this, 0, v, TO_NOTVICT);
      act("$N moves out of the way and you miss your knee strike.", FALSE, this, 0, v, TO_CHAR);
      act("$n tries to hit you with $s knee, but you dodge it.", FALSE, this, 0, v, TO_VICT);
    } break;
    case 1:{ // monk counter move
      act("$N grabs your knee, and hits you in the side with a powerful roundhouse kick.", FALSE, this, 0, v, TO_CHAR);
      act("You grab $n's knee, and use the opening to hit $m in the side with a powerful roundhouse kick.", FALSE, this, 0, v, TO_VICT);
      act("$N grabs $n's knee, and hit $m in the side with a powerful roundhouse kick.", FALSE, this, 0, v, TO_NOTVICT);

      // do 1/2 the dam we would have done to caster
      // yes, I mean to use caster below since vict doesn't have this skill...
      int dam = getSkillDam(v, SKILL_KNEESTRIKE, getSkillLevel(SKILL_KNEESTRIKE), getAdvLearning(SKILL_KNEESTRIKE)) / 2;
      ...
      if ((item = dynamic_cast<TObj *>(v->equipment[v->getPrimaryFoot()]))) {
        if (item->isSpiked() || item->isObjStat(ITEM_SPIKED)) {
	        ...
	        act("The spikes on your $o sink into $N's side.", FALSE, v, item, this, TO_CHAR);
	        act("The spikes on $n's $o sink into $N's side.", FALSE, v, item, this, TO_NOTVICT);
	        act("The spikes on $n's $o sink into your side, OW!", FALSE, v, item, this, TO_VICT);
...
int TBeing::kneestrikeHit(TBeing *victim)
{
  ...
  int spikeddam=0;
  int dam = getSkillDam(victim, SKILL_KNEESTRIKE, getSkillLevel(SKILL_KNEESTRIKE), getAdvLearning(SKILL_KNEESTRIKE));
  ...
  switch(crit){
    ...
    case CRIT_S_DOUBLE:
      dam=(int)(dam*1.5);
      for(i=0;i<3;++i) act(crit_msg[2][i], FALSE, this, 0, victim, msg_tgt[i]);
      break;
    case CRIT_S_NONE:
      break;
  }
  ...
  if (caster_hgt < victim->getPartMinHeight(ITEM_WEAR_FEET)) {
    ...
    for(i=0;i<3;++i) act(hit_msg[0][i], FALSE, this, 0, victim, msg_tgt[i]);
    ...
    for(i=0;i<3;++i) act(hit_msg[1][i], FALSE, this, 0, victim, msg_tgt[i]);
    ...
    for(i=0;i<3;++i) act(hit_msg[2][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else if (caster_hgt < victim->getPartMinHeight(ITEM_WEAR_BODY)) {
    // target knee
    pos = (::number(0,1) ? WEAR_LEG_L : WEAR_LEG_R);
    dam_type = DAMAGE_KNEESTRIKE_KNEE;
    
    for(i=0;i<3;++i) act(hit_msg[3][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else if (caster_hgt < victim->getPartMinHeight(ITEM_WEAR_ARMS)) {
    // target thigh
    pos = (::number(0,1) ? WEAR_LEG_L : WEAR_LEG_R);
    dam_type = DAMAGE_KNEESTRIKE_THIGH;

    for(i=0;i<3;++i) act(hit_msg[4][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else if (caster_hgt >= victim->getPartMinHeight(ITEM_WEAR_ARMS) &&
    ...
    for(i=1;i<3;++i) act(hit_msg[5][i], FALSE, this, 0, victim, msg_tgt[i]);

    if (victim->getSex() == SEX_MALE) {
      act("You jerk your knee into $N's crotch.  Felt like you got both of them.", FALSE, this, 0, victim, TO_CHAR);
    ...
    for(i=0;i<3;++i) act(hit_msg[9][i], FALSE, this, 0, victim, msg_tgt[i]);
    ...
    for(i=0;i<3;++i) act(hit_msg[8][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else if (victim_hgt < getPartMinHeight(ITEM_WEAR_BODY)) {
    // target chin
    pos = WEAR_HEAD;
    dam_type = DAMAGE_KNEESTRIKE_CHIN;

    for(i=0;i<3;++i) act(hit_msg[7][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else if (victim_hgt < getPartMinHeight(ITEM_WEAR_ARMS)) {
    // target solar plexus
    pos = WEAR_BODY;
    dam_type = DAMAGE_KNEESTRIKE_SOLAR;

    for(i=0;i<3;++i) act(hit_msg[6][i], FALSE, this, 0, victim, msg_tgt[i]);
  } else {
  // target solar plexus
    pos = WEAR_BODY;
    dam_type = DAMAGE_KNEESTRIKE_SOLAR;
