...
static int disarm(TBeing* caster, TBeing* victim, spellNumT skill) {
  ...
  if (victim->heldInPrimHand())
    obj =
      dynamic_cast<TObj*>(victim->equipment[worn = victim->getPrimaryHold()]);
  if (!obj && victim->heldInSecHand())
    obj =
      dynamic_cast<TObj*>(victim->equipment[worn = victim->getSecondaryHold()]);
  ...
  // attacker to crit
  bool retained = victim->doesKnowSkill(SKILL_WEAPON_RETENTION) &&
                  victim->bSuccess(SKILL_WEAPON_RETENTION);
  int trancePower =
    (int)(((float)victim->getSkillValue(SKILL_TRANCE_OF_BLADES) * 0.70) + 30);
  bool isTranced = victim->doesKnowSkill(SKILL_TRANCE_OF_BLADES) &&
                   victim->task && victim->task->task == TASK_TRANCE_OF_BLADES;
  bool tranceBack =
    isTranced && victim->bSuccess(trancePower, SKILL_TRANCE_OF_BLADES);
...
int TBeing::doDisarm(sstring argument, TThing* v) {
  ...
  if (!victim) {
    ...
    return disarmTrap(argument.c_str(), to) &
           DELETE_THIS;  // only return 0 or delete
  }
