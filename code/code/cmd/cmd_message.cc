...
const char* messageCommandTypes[] = {"title",  //  1
  "purge",                                     //  2
  "purge-target",                              //  3
  "rload",                                     //  4
  "load-object",                               //  5
...
void TBeing::doMessage(const char* tArg) {
  ...
  if (tStCommand.empty())
    ...
    if (tValue < 1 || tValue >= MSG_TYPE_MAX)
      ...
      sendTo(
        "All sstrings have a hard limit of 250 characters, please use less "
        "than you did.\n\r");
    else {
      if (tStString.empty()) {
      ...
      if ((messageCommandSwitches[tValue][1] & MSG_REQ_GNAME) && !isNamed &&
          (tStString.find("<n>") == sstring::npos)) {
        sendTo(
          format("This type requires your name.  Either use %s or <n>\n\r") %
          getNameNOC(this));
        return;
      }
      ...
      if (tStString.find("$") != sstring::npos)
        sendTo(
          "You used $ in your sstring, this will be replaced with -, "
          "sorry.\n\r");
    }
  }
}
...
sstring TMessages::getDefaultMessage(messageTypeT tValue, TBeing* tChar) {
  switch (tValue) {
    ...
    case MSG_PURGE:  // Purge
      return "<n> gestures... You are surrounded by thousands of tiny "
             "scrubbing bubbles!";
      break;
...
void TMessages::operator()(messageTypeT tValue, sstring tStString) {
  ...
  switch (tValue) {
    ...
    case MSG_MAX:
      vlogf(LOG_BUG,
        format(
          "TMessages::operator()(int, sstring) got invalid tValue.  [%d]") %
          tValue);
  }
}
...
void findAndReplace(sstring& tStOrig, sstring tStArg, sstring tStNew) {
  while ((tStOrig.find(tStArg.c_str()) != sstring::npos))
    tStOrig.replace(tStOrig.find(tStArg.c_str()), tStArg.length(),
      tStNew.c_str());
}

sstring TMessages::operator()(messageTypeT tValue, TThing* tThing,
  const char* tString, bool sendFiltered) const {
  sstring tMessage("");
  TBeing* tBeing = dynamic_cast<TBeing*>(tThing);
  int tSexP = (!tPlayer ? 2
                        : (tPlayer->getSex() == SEX_FEMALE
                              ? 0
                              : (tPlayer->getSex() == SEX_MALE ? 1 : 2)));
  int tSexM = (!tBeing ? 2
                       : (tBeing->getSex() == SEX_FEMALE
                             ? 0
                             : (tBeing->getSex() == SEX_MALE ? 1 : 2)));

  const char* sexTypes[][3] = {{"her", "him", "it"}, {"she", "he", "it"},
    {"her", "his", "its"}};
  ...
  if (sendFiltered) {
    ...
    if (tPlayer)
      findAndReplace(tMessage, "<n>",
        (!tPlayer->getName().empty() ? tPlayer->getName() : "ERROR"));
    else
      findAndReplace(tMessage, "<n>", "Someone");

    if (tThing)
      findAndReplace(tMessage, "<N>",
        (!tThing->getName().empty() ? tThing->getName() : "ERROR"));
    else
      findAndReplace(tMessage, "<N>", "Someone");
  } else {
...
const sstring& TMessages::operator[](messageTypeT tValue) const {
  switch (tValue) {
    case MSG_IMM_TITLE:  // Immortal Title
    ...
    default:
      vlogf(LOG_BUG,
        format("TMessages::operator[](int) got invalid tValue.  [%d]") %
          tValue);
      {
...
void TMessages::savedown() {
  ...
  if (!tPlayer || !tPlayer->desc || !tPlayer->isPc() || tPlayer->isLinkdead() ||
