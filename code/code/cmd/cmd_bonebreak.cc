...
int bonebreakHit(TBeing* c, TBeing* victim) {
  ...
  switch (critSuccess(c, SKILL_BONEBREAK)) {
    case CRIT_S_KILL:
      ...
      sprintf(buf,
        "$n yells out a mighty warcry and rips your %s completely off!", limb);
      act(buf, FALSE, c, 0, victim, TO_VICT);
      sprintf(buf,
        "$n yells out a mighty warcry and rips $N's %s completely off!", limb);
    ...
    default:
      ...
      act(buf, FALSE, c, 0, victim, TO_VICT);
      sprintf(buf, "You hear a muffled SNAP as $n grabs $N's %s and breaks it!",
        limb);
      act(buf, FALSE, c, 0, victim, TO_NOTVICT);
...
int bonebreak(TBeing* caster, TBeing* victim) {
  ...
  if (victim->attackers > 4) {
    caster->sendTo(
      "You can't get close enough to them to break their bones!\n\r");
    return FALSE;
  }
  ...
  if (caster->bSuccess(bKnown + percent, SKILL_BONEBREAK) &&
      ...
      i != GUARANTEED_FAILURE && percent < bKnown &&
