...
static int charge(TBeing* ch, TBeing* vict) {
  ...
  if (vict->awake() && (!ch->specialAttack(vict, SKILL_CHARGE) ||
  ...
  // vlogf(LOG_MISC, format("Charge Damage Formula [%s][%.2f / %.2f|%d / %s]") %
  // ch->getName() %
  //      newDam % crossValue % initialDamage % (didCrit ? "Critical" :
  //      "Normal"));
  //  }
#endif
