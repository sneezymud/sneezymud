...
static void showStatsTo(const Descriptor *d, const TBeing *ch, bool hidden_stuff) {
  ...
  if (playing_time.day) playing_time.hours += playing_time.day * 24;
  snprintf(time_buf, sizeof(time_buf), "%d hour%s, %d minute%s and %d second%s", playing_time.hours,
           ((playing_time.hours == 1) ? "" : "s"), playing_time.minutes,
           ((playing_time.minutes == 1) ? "" : "s"), playing_time.seconds,
  ...
  if (d == ch->desc)
    ...
    snprintf(buffer, sizeof(buffer), "%s (L%d:%d) has", victim->getName().c_str(),
             victim->GetMaxLevel(), victim->GetMaxLevel() * 50 / 3);
  str += buffer;
  ...
  snprintf(buffer, sizeof(buffer),
           ...
           d->session.kills, ch->norm(), ch->cyan(), d->session.groupKills, ch->norm(), ch->cyan(),
           d->session.xp, ch->norm());
  str += buffer;
  ...
  if (hidden_stuff) {
    ...
    str += buffer;
    snprintf(buffer, sizeof(buffer),
             "Combat   :            hits swings   rnds   hit%%  ComDam SklDam  Mod  Pot\n\r");
    str += buffer;
    ...
    for (i = 0; i < MAX_ATTACK_MODE_TYPE; i++) {
      ...
      snprintf(
          ...
          "%s%-9.9s%s: inflict: %s%6u%s %s%6u%s %s%6u%s (%s%4.1f%%%s) %s%6d%s %s%6d%s %s%4d%s %s%4.1f%s\n\r",
          ch->cyan(), attack_modes[i], ch->norm(), ch->cyan(), d->session.hits[i], ch->norm(),
          ch->cyan(), d->session.swings[i], ch->norm(), ch->cyan(), d->session.rounds[i],
          ch->norm(), ch->cyan(),
          (d->session.swings[i] ? 100.0 * d->session.hits[i] / d->session.swings[i] : 0.0),
          ch->norm(), ch->cyan(), d->session.combat_dam_done[i], ch->norm(), ch->cyan(),
          d->session.skill_dam_done[i], ch->norm(), ch->cyan(),
          (d->session.swings[i] ? d->session.mod_done[i] / (int)d->session.swings[i] : 0),
          ch->norm(), ch->cyan(),
          (d->session.hits[i] ? (float)d->session.potential_dam_done[i] / d->session.hits[i] : 0),
          ch->norm());
      str += buffer;

      snprintf(
          buffer, sizeof(buffer),
          "  %sL%6.2f%s: receive: %s%6u%s %s%6u%s %s%6u%s (%s%4.1f%%%s) %s%6d%s %s%6d%s %s%4d%s %s%4.1f%s\n\r",
          ch->cyan(),
          (d->session.swings_received[i]
               ? (float)d->session.level_attacked[i] / d->session.swings_received[i]
               : 0.0),
          ch->norm(), ch->cyan(), d->session.hits_received[i], ch->norm(), ch->cyan(),
          d->session.swings_received[i], ch->norm(), ch->cyan(), d->session.rounds_received[i],
          ch->norm(), ch->cyan(),
          (d->session.swings_received[i]
               ? 100.0 * d->session.hits_received[i] / d->session.swings_received[i]
               : 0.0),
          ch->norm(), ch->cyan(), d->session.combat_dam_received[i], ch->norm(), ch->cyan(),
          d->session.skill_dam_received[i], ch->norm(), ch->cyan(),
          (d->session.swings_received[i]
               ? d->session.mod_received[i] / (int)d->session.swings_received[i]
               : 0),
          ch->norm(), ch->cyan(),
          (d->session.hits_received[i]
               ? (float)d->session.potential_dam_received[i] / d->session.hits_received[i]
               : 0),
          ch->norm());
      str += buffer;
    }
  ...
  if (ch->desc == d)
    ...
    snprintf(buffer, sizeof(buffer), "\n\rIn %s's career:\n\r%s has", victim->getName().c_str(),
             victim->getName().c_str());
  str += buffer;
  GameTime::realTimePassed((time(0) - victim->player.time->logon) + victim->player.time->played, 0,
                           &playing_time);

  snprintf(buffer, sizeof(buffer), " been playing for %s%d%s days and %s%d%s hours.\n\r",
           ch->purple(), playing_time.day, ch->norm(), ch->purple(), playing_time.hours,
           ch->norm());
  str += buffer;

  snprintf(buffer, sizeof(buffer),
           "Creatures killed : Alone  %s%u%s, Group %s%u%s. Exp Gained : %s%.2f%s\n\r", ch->cyan(),
           d->career.kills, ch->norm(), ch->cyan(), d->career.group_kills, ch->norm(), ch->cyan(),
           d->career.exp, ch->norm());
  str += buffer;
  ...
  if (hidden_stuff) {
    ...
    for (i = 0; i < MAX_ATTACK_MODE_TYPE; i++) {
      ...
      snprintf(buffer, sizeof(buffer),
               ...
               attack_modes[i], ch->norm(), ch->cyan(), d->career.hits[i], ch->norm(), ch->cyan(),
               d->career.swings[i], ch->norm(), ch->cyan(),
               (d->career.swings[i] ? 100.0 * d->career.hits[i] / d->career.swings[i] : 0.0),
    ...
    for (i = 0; i < MAX_ATTACK_MODE_TYPE; i++) {
      if (d->career.dam_received[i])
        snprintf(buf3, sizeof(buf3), "%5.3f:1.0",
                 (float)(((float)d->career.dam_done[i]) / ((float)d->career.dam_received[i])));
      else
        snprintf(buf3, sizeof(buf3), "0.0:1.0");
      snprintf(buffer, sizeof(buffer), "%s%-9.9s%s: (%s%5.2f%%%s)     %s%-10s%s\n\r", ch->cyan(),
               attack_modes[i], ch->norm(), ch->cyan(),
  ...
  snprintf(buffer, sizeof(buffer),
           ...
           d->career.deaths, ch->norm(), ch->cyan(), d->career.arena_loss, ch->norm(), ch->cyan(),
           d->career.arena_victs, ch->norm());
  str += buffer;
  ...
  snprintf(
      buffer, sizeof(buffer), "Pets owned: %d of an average level of %5.3f\n\r",
      d->career.pets_bought,
      ((float)d->career.pet_levels_bought / (float)max((unsigned int)1, d->career.pets_bought)));
  str += buffer;
  ...
  snprintf(buffer, sizeof(buffer),
           "Combat inflicted : crit-hits %s%u%s, crit-misses %s%u%s, crit-kills %s%u%s\n\r",
           ch->cyan(), d->career.crit_hits, ch->norm(), ch->cyan(), d->career.crit_misses,
           ch->norm(), ch->cyan(), d->career.crit_kills, ch->norm());
  str += buffer;
  snprintf(buffer, sizeof(buffer), "Combat suffered  : crit-hits %s%u%s, crit-kills %s%u%s\n\r",
           ch->cyan(), d->career.crit_hits_suff, ch->norm(), ch->cyan(), d->career.crit_kills_suff,
           ch->norm());
  str += buffer;
...
void TBeing::doAttribute(const char *arg) {
  ...
  if (isImmortal()) {
    ...
    if (!victim) victim = get_pc_world(this, cmdbuf, EXACT_NO);
  ...
  if (is_abbrev(cmdbuf, "personal")) {
    ...
    if (!age()->month && !age()->day) sendTo(" It's your birthday today.\n\r");
...
#else
      sendTo(format("You are allied to %s.\n\r") % FactionInfo[getFaction()].faction_name);
#endif
    }
    ...
    if (buf != "") sendTo(format("Your character traits are: %s\n\r") % buf);
    ...
    if (GetMaxLevel() > 10) {
      if (!isImmortal()) {
        ...
        sendTo(
            format(
                "You have %.1f lbs of equipment, and can carry up to %.1f lbs - %.1f spare lbs.\n\r") %
            getCarriedWeight() % carryWeightLimit() % (carryWeightLimit() - getCarriedWeight()));
        sendTo(
            format(
                "You have %d volume in inventory.\n\rYou can carry up to %d volume - You have %d spare volume capacity.\n\r") %
            getCarriedVolume() % carryVolumeLimit() % (carryVolumeLimit() - getCarriedVolume()));
      }
    }
    ...
    if (GetMaxLevel() >= 5) sendTo(describeImmunities(this, 100));
      ...
      switch (getPosition()) {
        ...
        case POSITION_MOUNTED:
          ...
          if (tbr && tbr->horseMaster() == this) {
            ...
            sendTo(COLOR_MOBS, format("You are here, also riding on %s's %s%s.\n\r") %
    ...
    if (spelltask) {
      ...
      if (discArray[which]->minPiety) sendTo("You are praying.\n\r");
      if (discArray[which]->minMana) sendTo("You are casting a spell.\n\r");
      if (discArray[which]->minLifeforce) sendTo("You are invoking a ritual.\n\r");
    }
    sendTo(format("You are in %s%s%s attack mode.\n\r") % cyan() % attack_modes[getCombatMode()] %
           norm());

    if (getWimpy())
      sendTo(format("You are in wimpy mode, and will flee at %d hit points.\n\r") % getWimpy());
    if (IS_SET(desc->account->flags, TAccount::BOSS)) sendTo("Your account is in boss-mode.\n\r");
    ...
    for (i = MIN_DRUG; i < MAX_DRUG; ++i) {
      if (desc->drugs[i].total_consumed > 0) {
        ...
        sendTo(format("You first used %s on the %s day of %s, Year %d P.S.\n\r") %
               ...
               month_name[desc->drugs[i].first_use.month] % desc->drugs[i].first_use.year);
        sendTo(format("You last used %s on the %s day of %s, Year %d P.S.\n\r") %
               drugTypes[i].name % numberAsString(desc->drugs[i].last_use.day + 1) %
               month_name[desc->drugs[i].last_use.month] % desc->drugs[i].last_use.year);
        sendTo(format("You have consumed %i units of %s.\n\r") % desc->drugs[i].total_consumed %
               drugTypes[i].name);
