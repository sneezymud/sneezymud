...
void TBaseCup::lookObj(TBeing* ch, int) const {
  ...
  if (getMaxDrinkUnits() / 128) {
    ch->sendTo(COLOR_OBJECTS,
      format("%s has a capacity of %d gallon%s, %d fluid ounce%s.\n\r") %
        sstring(ch->pers(this)).cap() % (getMaxDrinkUnits() / 128) %
        (getMaxDrinkUnits() / 128 == 1 ? "" : "s") %
        (getMaxDrinkUnits() % 128) %
        (getMaxDrinkUnits() % 128 == 1 ? "" : "s"));
  } else {
    ch->sendTo(COLOR_OBJECTS,
      format("%s has a capacity of %d fluid ounce%s.\n\r") %
        sstring(ch->pers(this)).cap() % (getMaxDrinkUnits() % 128) %
        (getMaxDrinkUnits() % 128 == 1 ? "" : "s"));
  }
  if (getDrinkUnits() <= 0 || !getMaxDrinkUnits())
    ...
    ch->sendTo(COLOR_OBJECTS,
      format("It's %sfull of a %s%s liquid.\n\r") % fullness[temp] %
...
void TBeing::lookInObj(sstring arg2, TThing* specific, unsigned int bits,
  ...
  if (!(arg2.empty()) || specific) {
    if (specific) {
      if (!dynamic_cast<TObj*>(specific)) {
        ...
        if ((bits == FIND_OBJ_ROOM) && riding && tmpO->parent) {
          sendTo(
            format("You can't look into items on the %s while mounted!\n\r") %
    ...
    bits = generic_find(arg2.c_str(),
      FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP, this, &tmp_char, &o);
    if (bits) {
...
void TBeing::lookAtBeing(TThing* specific) {
  ...
  if (bIsSpying)  // Let thieves have a chance of learning spy when looking at
                  // someone.
    bSuccess(SKILL_SPY);
...
void TBeing::lookingAtObj(TThing* specific) {
  ...
  if (tmpObj->ex_description) {
    if ((tmp_desc = tmpObj->ex_description->findExtraDesc(tmp))) {
      ...
      if (tmpObj->riding)
        sendTo(COLOR_OBJECTS, format("%s is on %s.") % tmpObj->getName() %
                                tmpObj->riding->getName());
...
void TBeing::doLook(const sstring& argument, cmdTypeT cmd, TThing* specific) {
  ...
  if (getPosition() < POSITION_SLEEPING) {
    ...
    return;
  } else if (roomp->pitchBlackDark() && !isImmortal() && (visionBonus <= 0) &&
  ...
  switch (keyword_no) {
    ...
    case 7: {
      if (!(arg2.empty()) || specific) {
        ...
        // In inventory
        if (!found) {
          for (StuffIter it = stuff.begin(); it != stuff.end() && (t = *it);
               ++it) {
            if (!canSee(t))
              continue;
            if (t->ex_description) {
              ...
              for (t2 = t->rider; t2; t2 = t2->nextRider) {
                ...
                if (t2->ex_description &&
                  ...
                  sendTo(COLOR_OBJECTS,
                    format("%s is on %s.") % t2->getName() % t->getName());
                  return;
                }
