...
int TBeing::stabSuccess(TBeing* victim) {
  ...
  if (!victim->isLimbFlags(limb, PART_MISSING) && !victim->isUndead() &&
    ...
    sstring buf =
      format("You puncture $N's %s with your $o, leaving a bloody gash!") %
      victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_CHAR);

    buf = format("$n punctures $N's %s with their $o, leaving a bloody gash!") %
          victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_ROOM);

    buf =
      format("$n punctures your %s with their $o, leaving a bloody gash!!") %
      victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_VICT);
  } else if (!victim->isLimbFlags(limb, PART_MISSING) &&
             victim->isLimbFlags(limb, IMMUNE_BLEED)) {
    sstring buf =
      format("You puncture $N's %s with your $o, leaving a gaping hole!") %
      victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_CHAR);

    buf = format("$n punctures $N's %s with their $o, leaving a gaping hole!") %
          victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_ROOM);

    buf =
      format("$n punctures your %s with their $o, leaving a gaping hole!!") %
      victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_VICT);

  } else {
    sstring buf =
      format("You stab $N's %s with your $o, leaving a big wound!") %
      victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_CHAR);

    buf = format("$n stabs $N's %s with their $o, leaving a big wound!") %
          victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_ROOM);

    buf = format("$n stabs your %s with their $o, leaving a big wound!!") %
          victim->describeBodySlot(limb);
    act(buf, false, this, weapon, victim, TO_VICT);
  }
  ...
  if (!victim->isLimbFlags(limb, PART_MISSING) &&
    ...
    if (victim->getCurLimbHealth(limb) <= victim->getMaxLimbHealth(limb) / 2) {
      ...
      if (limb == WEAR_NECK || limb == WEAR_BODY || limb == WEAR_BACK ||
          limb == WEAR_WAIST ||
          (limb == WEAR_HEAD && victim->hasDisease(DISEASE_EYEBALL))) {
        // Apply normal damage without special effects
        spellNumT damageType =
          weapon->isPierceWeapon() ? DAMAGE_IMPALE : DAMAGE_NORMAL;
        if (!victim->isLimbFlags(limb, IMMUNE_BLEED) && !victim->isUndead()) {
          int duration = (this->GetMaxLevel() * 3 + 200);
          victim->rawBleed(limb, duration, SILENT_YES, CHECK_IMMUNITY_NO);
          sstring buf =
            format(
              "You puncture $N's %s with your $o, leaving a bloody gash!") %
            victim->describeBodySlot(limb);
          act(buf, false, this, weapon, victim, TO_CHAR);
          buf =
            format(
              "$n punctures $N's %s with their $o, leaving a bloody gash!") %
            victim->describeBodySlot(limb);
          act(buf, false, this, weapon, victim, TO_ROOM);
        }
        ...
        sstring buf = format("You slice $N's %s right off!") %
                      victim->describeBodySlot(limb);
        act(buf, false, this, weapon, victim, TO_CHAR);

        buf = format("$n slices your %s right off!") %
              victim->describeBodySlot(limb);
        act(buf, false, this, 0, victim, TO_VICT);

        buf = format("$n slices $N's %s right off!") %
              victim->describeBodySlot(limb);
  ...
  if (weapon->isPoisoned()) {
    ...
    sstring buf = format("Your stab to $N's %s infects it!") %
                  victim->describeBodySlot(limb);
    act(buf, false, this, NULL, victim, TO_CHAR);

    buf = format("Your %s gets infected from $n's stab!") %
          victim->describeBodySlot(limb);
    act(buf, false, this, NULL, victim, TO_VICT);

    buf = format("$N's %s gets infected from $n's stab!") %
          victim->describeBodySlot(limb);
