...
int TBed::getMinPosUse() const { return min_pos_use; }

void TBed::setMinPosUse(int n) { min_pos_use = n; }

int TBed::getMaxUsers() const { return max_users; }

void TBed::setMaxUsers(int n) { max_users = n; }

int TBed::getMaxSize() const { return max_size; }

void TBed::setMaxSize(int n) { max_size = n; }

int TBed::getSeatHeight() const { return seat_height; }

void TBed::setSeatHeight(int n) { seat_height = n; }

int TBed::getRegen() const { return regen; }

void TBed::setRegen(int n) { regen = n; }
...
int TBed::putSomethingInto(TBeing* ch, TThing* tThing) {
  ...
  if (!(corpse = dynamic_cast<TBaseCorpse*>(tThing)) || this->parent) {
    ...
    case CHANGE_BED_VALUE1:
      switch (update_num) {
        ...
        case 2:
          ch->sendTo(VT_HOMECLR);
          ch->sendTo(
            "Enter new min. position (0 = sleeping, 1 = resting, 2 = "
            "sitting).\n\r--> ");
    ...
    case CHANGE_BED_MINPOS:
      if (type != ENTER_CHECK) {
        if ((update_num > 3) || (update_num < 0)) {
          ...
          ch->sendTo(
            "0 = sleeping, 1 = resting, 2 = sitting, 3 = can't be used.\n\r");
          return;
        }
    ...
    *gain = max(*gain, 0);
    ...
    if (!ch->awake()) {
      if (ch->getPosition() == POSITION_SLEEPING) {
        if (!silent)
          ch->sendTo(
            "You are much too uncomfortable to continue to sleep well.\n\r");
  ...
  if (ch->getHit() < ch->hitLimit()) {
    ...
    if (!silent)
      ch->sendTo(COLOR_OBJECTS,
        format("Your rest on %s rejuvenates you.\n\r") % getName());
  }
}
...
int TBed::mobPulseBed(TMonster* mob, short int occurrence) {
  ...
  mob->doWake("");
  mob->doStand();
  if (mob->default_pos == POSITION_SITTING || (getMinPosUse() == 2)) {
    mob->doSit(fullName);
  } else if (mob->default_pos == POSITION_RESTING || (getMinPosUse() == 1)) {
    mob->doRest(fullName);
...
int TBed::getRiderHeight() const { return getSeatHeight(); }
...
sstring TBed::statObjInfo() const {
  ...
  sprintf(buf + strlen(buf),
    "Max Designed Size: %d inches, Seat height: %d inches\n\rExtra Regen: %d%%",
...
void TBed::lowCheck() {
  if (canWear(ITEM_WEAR_TAKE) && getVolume() <= 3000 && getWeight() <= 10 &&
      getRegen() >= 3)
    vlogf(LOG_LOW,
      format("Portable bed (%s) with excessive regen rates!") % getName());
