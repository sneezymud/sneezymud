...
TArmorWand::TArmorWand(const TArmorWand& a) : TArmor(a), TWand(a) {}
...
int TArmorWand::suggestedPrice() const {
  // (int)(10.0 * getWeight() * material_nums[getMaterial()].price) will get
  // added twice, so remove one
...
sstring TArmorWand::getNameForShow(bool useColor, bool useName,
  const TBeing* ch) const {
  char buf[256];

  // put both show names together:  name (level) [spell]
  sprintf(buf, "%s [%s]", TArmor::getNameForShow(useColor, useName, ch).c_str(),
    (getSpell() >= 0
