...
TPlant::TPlant(const TPlant& a) : TExpandableContainer(a) {}
...
TPlant::~TPlant() {
  if (this->getVerminated())
    vlogf(LOG_MISC,
      format("%s is dying: lifetime yield of %i, %i lost to vermin.") %
        this->getName() % this->getYield() % this->getVerminated());
  else
    vlogf(LOG_MISC, format("%s is dying: lifetime yield of %i.") %
                      this->getName() % this->getYield());
}
...
sstring TPlant::statObjInfo() const {
  char buf[256];
  if (this->getVerminated())
    sprintf(buf, "Type: %i Age: %i Total Yield: %i To Vermin: %i", getType(),
      getAge(), getYield(), getVerminated());
  else
    sprintf(buf, "Type: %i Age: %i Total Yield: %i", getType(), getAge(),
      getYield());
  sstring a(buf);
  return a;
}
...
void TPlant::updateDesc() {
  ...
  const char* plantkeywords[] = {"mound dirt", "sprout tiny", "plant small %s",
    "plant %s", "plant withered %s"};

  const char* plantname[] = {"a small mound of <o>dirt<1>", "a tiny sprout",
    "a small %s", "a %s", "an old, withered %s"};

  const char* plantdesc[] = {"A small mound of <o>dirt<1> is here.",
    "A tiny sprout is growing here.", "A small %s is here.", "A %s is here.",
    "An old, withered %s is here."};

  const char* planttypes[] = {"<r>tomato<1> <g>plant<1>", "<r>red<1> rose bush",
    "<r>apple<1> tree", "<W>white<1> rose bush", "<Y>yellow<1> rose bush",
  ...
  (tcount=*it);++it) { tree = dynamic_cast<TPlant *>(tcount); if(tree &&
  tree->getType() == getType()) {
          // looking for same type of trees
          // let's only count trees with stuff in them, to avoid over-population
  of vermin if (tree->getStuff())
            ++tree_count;
        }
      }
      if (tree_count > 1 && ::number(1, 100000) < (int) ((tree_count / 16) *
  200)) {
      ...
      int mob_vnum = Mob::CANDY_HEART_EATER; // sugar-toothed weasel
        ...
        if (!mob) {
          // vlogf(LOG_PROC, format("Failed attempt to spawn a fruit scavenger
  (%i) in room %i.") % mob_vnum % in_room); return;
        }
        *roomp += *mob;
        colorAct(COLOR_MOBS,
                ((mob->ex_description &&
