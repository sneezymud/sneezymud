...
TWindow::TWindow(const TWindow& a) : TSeeThru(a) {}
...
void TWindow::lookObj(TBeing* ch, int) const { ch->windowLook(this); }

void TWindow::showMe(TBeing* ch) const { ch->windowLook(this); }

// don't display windows
void TWindow::show_me_mult_to_char(TBeing* ch, showModeT mode,
  unsigned int) const {
...
void TBeing::windowLook(const TWindow* w) {
  ...
  if (!(target = real_roomp(w->getTarget(&isRandom)))) {
    ...
    vlogf(LOG_BUG, format("%s [%d] points to non existant room %d") %
                     w->shortDescr % obj_index[w->getItemIndex()].virt %
                     w->getTarget());
    return;
  }
  if (isRandom == -1)
