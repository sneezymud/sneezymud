  ...
  if (c == MAX_CURRENCY) {
    for (int i = 0; i < MAX_CURRENCY; ++i)
      total += (int)((float)currency[i] *
                     currencyInfo[(currencyTypeT)i]->getExchangeRate(
                       CURRENCY_GRIMHAVEN));
  } else {
    total = currency[c];
  }
...
TMoneypouch::TMoneypouch() : TExpandableContainer() {}

TMoneypouch::TMoneypouch(const TMoneypouch& a) : TExpandableContainer(a) {}
  ...
  for (currencyTypeT c = MIN_CURRENCY; c < MAX_CURRENCY; c++) {
    if (getMoney(c) > 0) {
      buf += format("%ss inside: %i\n\r") % currencyInfo[c]->getName().cap() %
             getMoney(c);
      total += (int)((float)getMoney(c) *
                     currencyInfo[c]->getExchangeRate(CURRENCY_GRIMHAVEN));
    }
  }
...
bool TMoneypouch::objectRepair(TBeing* ch, TMonster* repair,
  silentTypeT silent) {
  if (!silent) {
