...
sstring TSymbol::statObjInfo() const {
  ...
  sprintf(buf, "Symbol Faction: %d (%s)\n\r", getSymbolFaction(),
    getSymbolFaction() == -1 ? "not attuned"
                             : FactionInfo[getSymbolFaction()].faction_name);
  a += buf;
...
void TSymbol::setSymbolCurStrength(int r) { strength = r; }

void TSymbol::addToSymbolCurStrength(int r) { strength += r; }

int TSymbol::getSymbolMaxStrength() const { return max_strength; }

void TSymbol::setSymbolMaxStrength(int r) { max_strength = r; }

void TSymbol::addToSymbolMaxStrength(int r) { max_strength += r; }

factionTypeT TSymbol::getSymbolFaction() const { return faction; }

void TSymbol::setSymbolFaction(factionTypeT r) { faction = r; }
...
void TSymbol::lowCheck() {
  ...
  if (getSymbolMaxStrength() < getSymbolCurStrength())
    vlogf(LOG_LOW,
      format("symbol (%s) has lower max strength then current.") % getName());
  for (i = 0; i < MAX_OBJ_AFFECT; i++) {
    if (affected[i].location == APPLY_ARMOR) {
      vlogf(LOG_LOW, format("symbol (%s) had armor, bad!") % getName());
    }
  }
  ...
  if (ap != obj_flags.cost && obj_flags.cost >= 0 && ap) {
    ...
    vlogf(LOG_LOW,
      format("symbol (%s:%d) has a bad price (%d).  should be (%d)") %
...
bool TSymbol::lowCheckSlots(silentTypeT silent) {
  ...
  if (value != 0) {
    if (!silent)
      vlogf(LOG_LOW,
        format("symbol (%s) with bad wear slots: %d") % getName() % value);
    return true;
  }
  return false;
}
...
void TSymbol::objMenu(const TBeing* ch) const {
  ...
  ch->sendTo(format("%sSymbol Level:%s %.1f") % ch->purple() % ch->norm() %
             getSymbolLevel());
}
...
double TSymbol::objLevel() const { return getSymbolLevel(); }
...
sstring TSymbol::getNameForShow(bool useColor, bool useName,
