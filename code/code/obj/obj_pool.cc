...
void TPool::overFlow() {
  ...
  if (index >= 8) {
    ...
    for (unsigned int i = 0; i < rooms.size(); ++i) {
      ...
      sendrpf(COLOR_BASIC, rooms[i], "Some %s flows in from the %s.\n\r",
        liquidInfo[getDrinkType()]->name, dirs[rev_dir(roomdirs[i])]);
    }
...
bool TPool::willMerge(TMergeable* tm) {
  ...
  if (!(pool = dynamic_cast<TPool*>(tm)) || pool == this ||
      pool->getDrinkType() != getDrinkType()) {
    return false;
  }
...
void TPool::updateDesc() {
  ...
  const char* poolname[] = {"a few drops of %s", "a tiny puddle of %s",
    "a small puddle of %s", "a puddle of %s", "a fair sized puddle of %s",
    "a big pool of %s", "a large pool of %s", "a huge pool of %s",
    "a massive pool of %s", "a tremendously huge pool of %s",
...
int TRoom::dropPool(int amt, liqTypeT liq) {
  ...
  if (!pool) {
    ...
    if (robj < 0 || robj >= (signed int)obj_index.size()) {
      vlogf(LOG_BUG,
        format("dropPool(): No object (%d) in database!") % Obj::GENERIC_POOL);
      return false;
    }
...
TPool::TPool() : TBaseCup() {}

TPool::TPool(const TPool& a) : TBaseCup(a) {}
