...
TCookware::TCookware(const TCookware& a) : TOpenContainer(a) {}
...
sstring TCookware::statObjInfo() const { return TOpenContainer::statObjInfo(); }
...
void TCookware::pourMeIntoDrink2(TBeing* ch, TBaseCup* from) {
  ...
  if (robj < 0 || robj >= (signed int)obj_index.size()) {
    vlogf(LOG_BUG,
      format("TCookware::pourMeIntoDrink2(): No object (%d) in database!") %
        Obj::GENERIC_POOL);
    return;
  }
