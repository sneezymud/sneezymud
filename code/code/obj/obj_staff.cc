...
void TStaff::setMaxCharges(int n) { maxCharges = n; }

int TStaff::getMaxCharges() const { return maxCharges; }

void TStaff::addToMaxCharges(int n) { maxCharges += n; }

void TStaff::setCurCharges(int n) { curCharges = n; }

int TStaff::getCurCharges() const { return curCharges; }

void TStaff::addToCurCharges(int n) { curCharges += n; }

void TStaff::setSpell(spellNumT n) { spell = n; }

spellNumT TStaff::getSpell() const { return spell; }
...
void TStaff::lowCheck() {
  ...
  if (curspell > TYPE_UNDEFINED && discArray[curspell]->targets & TAR_IGNORE) {
    ...
    //    vlogf(LOG_LOW, "Staff (%s : %d) had possible-area-spell that shouldn't
    //    be on objs (%s : %d)" , getName(), objVnum(),
    //    discArray[curspell]->name, curspell);
  }
