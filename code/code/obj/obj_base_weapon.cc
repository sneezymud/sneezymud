...
int TBaseWeapon::poisonWeaponWeapon(TBeing* ch, TThing* poison) {
  ...
  // Handle "remove" command (poison is nullptr, water already validated by caller)
  if (!poison) {
    if (isPoisoned()) {
      this->poison = (liqTypeT)-1;
  ...
  // Check for water container attempt
...
void TBaseWeapon::applyPoison(TBeing* vict) {
  ...
  if ((ch = dynamic_cast<TBeing*>(equippedBy))) {
    ...
    if (ch->doesKnowSkill(SKILL_POISON_WEAPON)) {
      int bKnown = ch->getSkillValue(SKILL_POISON_WEAPON);
      int preserveChance = (bKnown / 2) + (int)(ch->getStatMod(STAT_DEX));
      if (ch->bSuccess(bKnown, SKILL_POISON_WEAPON) && percentChance(preserveChance)) {
