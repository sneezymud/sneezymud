...
void TBeing::doShuffle(const sstring& arg) {
  ...
  if (!(dynamic_cast<TCardDeck*>(deck))) {
    sendTo(
      "Try as you might, you just don't have the dexterity to shuffle "
      "that.\n\r");
    return;
  }
...
TCardDeck::TCardDeck() : TExpandableContainer() {}

TCardDeck::TCardDeck(const TCardDeck& a) : TExpandableContainer(a) {}
