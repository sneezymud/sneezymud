...
unsigned int TBaseCup::getDrinkConFlags() const { return drinkFlags; }

void TBaseCup::setDrinkConFlags(unsigned int r) { drinkFlags = r; }

bool TBaseCup::isDrinkConFlag(unsigned int r) const {
  return ((drinkFlags & r) != 0);
}

void TBaseCup::addDrinkConFlags(unsigned int r) { drinkFlags |= r; }

void TBaseCup::remDrinkConFlags(unsigned int r) { drinkFlags &= ~r; }

int TBaseCup::getMaxDrinkUnits() const { return maxDrinks; }

void TBaseCup::setMaxDrinkUnits(int n) { maxDrinks = n; }

void TBaseCup::addToMaxDrinkUnits(int n) { maxDrinks += n; }

int TBaseCup::getDrinkUnits() const { return curDrinks; }
...
liqTypeT TBaseCup::getDrinkType() const { return liquidType; }
...
int TBaseCup::getLiqDrunk() const { return liquidInfo[liquidType]->drunk; }

int TBaseCup::getLiqHunger() const { return liquidInfo[liquidType]->hunger; }

int TBaseCup::getLiqThirst() const { return liquidInfo[liquidType]->thirst; }
...
void TBaseCup::lowCheck() {
  if (getMaxDrinkUnits() < getDrinkUnits())
    vlogf(LOG_LOW,
      format("drinkcon %s  maxdrinks < current drinks.") % getName());
  if (isDrinkConFlag(DRINK_PERM) && canWear(ITEM_WEAR_TAKE))
    vlogf(LOG_LOW,
      format("drinkcon %s  takeable and permanent container.") % getName());
...
void TBaseCup::assignFourValues(int x1, int x2, int x3, int x4) {
  ...
  if (x3 < MIN_DRINK_TYPES || x3 >= MAX_DRINK_TYPES) {
    vlogf(LOG_OBJ,
      format("Bad parm (%d) for drink type on %s") % x3 % getName());
    x3 = MIN_DRINK_TYPES;
  }
...
sstring TBaseCup::statObjInfo() const {
  ...
  sprintf(buf,
    "Max-contains : %d     Contains : %d\n\rPoisoned : %s   Permanent : %s   "
    "Spillable : %s   Frozen : %s\n\rLiquid : %s (%i)",
    ...
    if (getDrinkUnits() > 0) {  // has liquid
      ...
      if (!tSilent)
        ch->sendTo(COLOR_OBJECTS,
          format("The desert heat causes some of your %s to evaporate.\n\r") %
            liquidInfo[getDrinkType()]->name);
    }
  }
}

int TBaseCup::quaffMe(TBeing* ch) { return drinkMe(ch); }

void TBaseCup::tasteMe(TBeing* ch) { sipMe(ch); }
...
int TBaseCup::getReducedVolume(const TThing*) const { return getTotalVolume(); }
...
void TBaseCup::updateDesc() {
  ...
  if (found) {
    ...
    name = stripColorCodes(newname);
    shortDescr = short_desc;
    setDescr(long_desc);
  }
}
