...
TScroll::TScroll(const TScroll& a) : TMagicItem(a) {
  int i;
  for (i = 0; i < 3; i++) {
    spells[i] = a.spells[i];
  }
...
sstring TScroll::statObjInfo() const {
  ...
  sprintf(buf, "Level:   %d        Learnedness:   %d\n\rSpells : %s, %s, %s",
    ...
    (getSpell(0) == TYPE_UNDEFINED
        ? "No Spell"
        : (discArray[getSpell(0)] ? discArray[getSpell(0)]->name
                                  : "BOGUS SPELL.  Bug this")),
    (getSpell(1) == TYPE_UNDEFINED
        ? "No Spell"
        : (discArray[getSpell(1)] ? discArray[getSpell(1)]->name
                                  : "BOGUS SPELL.  Bug this")),
    (getSpell(2) == TYPE_UNDEFINED
        ? "No Spell"
        : (discArray[getSpell(2)] ? discArray[getSpell(2)]->name
                                  : "BOGUS SPELL.  Bug this")));
