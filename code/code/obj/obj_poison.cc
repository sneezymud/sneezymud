...
TPoison::TPoison(const TPoison& a) : TBaseCup(a) {}
...
int TPoison::sellPrice(int, int shop_nr, float, const TBeing* ch) {
  ...
  price = (int)(getDrinkUnits() * cost_per *
                shop_index[shop_nr].getProfitSell(this, ch));
...
int TPoison::shopPrice(int num, int shop_nr, float, const TBeing* ch) const {
  ...
  cost_per = liquidInfo[getDrinkType()]->price;
  price = (int)(num * cost_per * getDrinkUnits() *
                shop_index[shop_nr].getProfitBuy(this, ch));
  price = max(1, price);
