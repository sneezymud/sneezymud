...
int TObj::boardHandler(TBeing*, cmdTypeT cmd, const char*) {
  if (cmd != CMD_GENERIC_DESTROYED) {
    vlogf(LOG_PROC,
      format("board handler with non-board (%s) cmd=%d") % getName() % cmd);
  } else {
...
int TBoard::readPost(TBeing* ch, const char* arg) {
  ...
  db.query(
    "select post_num, date_format(date_posted, '%%b %%e %%H:%%i %%Y') as "
    "date_posted, subject, author, post from board_message where board_vnum = "
    "%i and date_removed is null and post_num = %i",
    objVnum(), post_num);
    ...
    if (ch->desc) {
      if (ch->desc->m_bIsClient) {
        sstring sb =
          format(
            "Message %d : [%s] %s (%s)\n\r\n\r%s\n\rEnd of message %d.\n\r") %
          post_num % db["date_posted"] % db["subject"] % db["author"] %
          db["post"] % post_num;
...
int TBoard::lookBoard(TBeing* ch, const char* arg) {
  ...
  sbuf1 = format(
            ...
            "<#>%s.\n\r") %
          ch->green() % ch->norm() % ch->green() % ch->norm() % ch->green() %
          ch->norm();
  sbuf1 +=
    format(
      "To view the latest message first, %sLOOK <BOARD> REVERSE%s.\n\r\n\r") %
    ch->green() % ch->norm();
  ...
  while (db.fetchRow()) {
    sbuf1 += format("%s%-2d%s : %s[%s] %s%s %s(%s)%s\n\r") % ch->cyan() %
             convertTo<int>(db["post_num"]) % ch->norm() % ch->green() %
             db["date_posted"] % ch->purple() % db["subject"] % ch->orange() %
             db["author"] % ch->norm();
    num++;
  }
...
void TNote::postMe(TBeing* ch, const char* arg2, TBoard* b) {
  ...
  TDatabase db(DB_SNEEZY);
  db.query(
    "insert board_message (board_vnum, subject, author, post, post_num) select "
    "%i, '%s', '%s', '%s', count(*) + 1 from board_message where board_vnum = "
...
int TBoard::postCount() const {
  TDatabase db(DB_SNEEZY);
  db.query(
    "select count(*) as post_count from board_message where board_vnum = %i "
...
int TBoard::removeFromBoard(TBeing* ch, const char* arg) {
  ...
  TDatabase db(DB_SNEEZY);
  db.query(
    "select author, post from board_message where board_vnum = %i and post_num "
    "= %i and date_removed is null",
    objVnum(), post_num);
  if (db.fetchRow()) {
    // only the original author, a board police wiz or a faction power wanker at
    // a faction board can remove a note
    if (db["author"] != ch->getName() && !ch->hasWizPower(POWER_BOARD_POLICE) &&
        !(objVnum() == FACT_BOARD_SERPENT &&
          ch->getFactionAuthority(FACT_SNAKE, 0)) &&
        !(objVnum() == FACT_BOARD_BROTHER &&
      ...
      *ch += *note;
      ...
      TDatabase db(DB_SNEEZY);
      db.query(
        "update board_message set date_removed = now(), post_num = null where "
        "board_vnum = %i and post_num = %i and date_removed is null",
        objVnum(), post_num);
      db.query(
...
void TBoard::assignFourValues(int x1, int, int, int) { setBoardLevel(x1); }
...
int TBoard::getBoardLevel() const { return board_level; }

void TBoard::setBoardLevel(int n) { board_level = n; }

TBoard::TBoard() : TObj(), board_level(0) {}

TBoard::TBoard(const TBoard& a) : TObj(a), board_level(a.board_level) {}
