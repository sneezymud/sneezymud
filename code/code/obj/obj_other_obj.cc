...
TOtherObj::TOtherObj(const TOtherObj& a) : TObj(a) {}
...
void TOtherObj::lowCheck() {
  ...
  if (!getVolume() && canWear(ITEM_WEAR_TAKE))
    vlogf(LOG_LOW,
      format("other item (%s:%d) had 0 volume.") % getName() % objVnum());
  ...
  if (canWear(ITEM_WEAR_TAKE)) {
    ...
    if ((max_exist < 10 && ((int)getWeight() < lgt)) ||
        (max_exist >= 10 && ((int)getWeight() < 2 * lgt))) {
      vlogf(LOG_LOW,
        format("other item (%s:%d) had bad light-to-weight ratio.") %
          getName() % objVnum());
      setWeight(2 * lgt);
    }
  }
