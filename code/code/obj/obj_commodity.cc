...
bool TCommodity::willMerge(TMergeable* tm) {
  ...
  if (!(tCommod = dynamic_cast<TCommodity*>(tm)) || tCommod == this ||
      tCommod->getMaterial() != getMaterial() || disableMerge ||
...
bool TCommodity::splitMe(TBeing* ch, const sstring& tString) {
  ...
  if (tCount >= numUnits()) {
    ch->sendTo(
      format("Units must be between 1 and %d.\n\r") % (numUnits() - 1));
    return true;
  }
...
TCommodity::TCommodity() : TMergeable() { disableMerge = false; }

TCommodity::TCommodity(const TCommodity& a) : TMergeable(a) {
  disableMerge = false;
}
...
float TCommodity::demandCurvePrice(int num, float price, int mat,
  ...
  for (int i = 1; i <= abs(num); ++i) {
    ...
    if (num >= 0) {
      if ((total_units - i) <= 0)  // edge case: small values of total_units,
                                   // large values of num can return infinite
        total_price += max_commod_price;
      else
        total_price += (multiplier * log(total_units - i) + max_commod_price);
    } else {
      // if(!(total_units-i))
      //   total_price+=0; // Don't set price floor - this causes edge cases
      //   when total_units == 1 and num == -1
      // else
      total_price += (multiplier * log(total_units + i) + max_commod_price);
    }
  }
...
int TCommodity::sellPrice(int num, int shop_nr,
  float total_units /*hack to save db lookups*/, const TBeing* ch) {
  float price;
...
float TCommodity::shopPriceFloat(int num, int shop_nr, float,
...
int TCommodity::buyMe(TBeing* ch, TMonster* keeper, int num, int shop_nr) {
  ...
  if (num > (int)(numUnits())) {
    ...
    keeper->doTell(ch->getName(),
      format("I don't have that much %s.  Here's the %d that I do have.") %
        fname(name) % num);
  }
  ...
  if (num) {
    ...
    *ch += *obj2;
    ...
    vlogf(LOG_BUG, format("Bogus num %d in buyMe commodity at %d.  wgt=%.2f") %
...
bool TCommodity::sellMeCheck(TBeing* ch, TMonster* keeper, int num,
  int total_inventory /* hack to save db lookups*/) const {
  sstring buf;
...
int TCommodity::sellMe(TBeing* ch, TMonster* keeper, int shop_nr, int) {
  ...
  db.query(
    "select rent_id, weight*10 as units from rent where owner_type='shop' and "
    "owner=%i and material=%i and vnum=%i",
...
int TCommodity::sellCommod(TBeing* ch, TMonster* keeper, int shop_nr,
  TThing* bag) {
  int rc;
...
void TCommodity::valueMe(TBeing* ch, TMonster* keeper, int shop_nr, int) {
  ...
  db.query("select rent_id from rent where owner_type='shop' and owner=%i and
  material=%i and vnum=%i", shop_nr, getMaterial(), Obj::GENERIC_COMMODITY);
  ...
  *keeper += *obj2;
  ...
  keeper->doTell(ch->getName(),
    format("Hmm, I'd give you %d talens for your %i units of that.") % price %
      numUnits());

  // delete obj2;
}

const sstring TCommodity::shopList(const TBeing* ch, const sstring& arg,
  int min_amt, int max_amt, int, int shop_nr, int k, unsigned long int) const {
  char buf[256];

  float cost = shopPriceFloat(1, shop_nr, -1, ch);

  sprintf(buf,
    "[%2d] COMMODITY: %-20.20s  : %5d units    %.2f talens (per unit)\n\r",
    k + 1, material_nums[getMaterial()].mat_name, (int)(numUnits()), cost);
  ...
  /* specific item and specific cost */
    ...
    return buf;
  else if (arg.empty() && cost >= min_amt &&
           cost <= max_amt) /* specific cost */
    return buf;
  else
    return "";
}

int TCommodity::numUnits() const { return (int)(10.0 * getWeight()); }
...
void TCommodity::updateDesc() {
  ...
  const char* metalname[] = {"bit", "nugget", "ingot", "sovereign", "rod",
    "bar", "pile"};

  const char* mineralname[] = {"tiny piece", "small piece", "piece",
    "large piece", "huge piece", "gigantic piece", "massive piece"};

  const char* miscname[] = {"bit", "tiny pile", "small pile", "pile",
    "big pile", "large pile", "huge pile"};
  ...
  if (isMineral()) {
    sprintf(buf, "%s",
      ((sstring)(format("commodity %s %s") % mineralname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    name = buf;

    sprintf(buf, "%s",
      ((sstring)(format("a %s of rough %s") % mineralname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    shortDescr = buf;

    sprintf(buf, "%s",
      ((sstring)(format("A %s of rough %s has been left here.  What luck!") %
    ...
    sprintf(buf, "%s",
      ((sstring)(format("commodity %s %s") % metalname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    name = buf;

    sprintf(buf, "%s",
      ((sstring)(format("a %s of %s") % metalname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    shortDescr = buf;

    sprintf(buf, "%s",
      ((sstring)(format("A %s of %s has been left here.  What luck!") %
                 metalname[sizeindex] % material_nums[getMaterial()].mat_name))
        .c_str());
    setDescr(buf);
  } else {
    sprintf(buf, "%s",
      ((sstring)(format("commodity %s %s") % miscname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    name = buf;

    sprintf(buf, "%s",
      ((sstring)(format("a %s of %s") % miscname[sizeindex] %
                 material_nums[getMaterial()].mat_name))
        .c_str());
    shortDescr = buf;
    ...
    sprintf(buf, "%s",
      ((sstring)(format("A %s of %s has been left here.  What luck!") %
                 miscname[sizeindex] % material_nums[getMaterial()].mat_name))
        .c_str());
    setDescr(buf);
  }
