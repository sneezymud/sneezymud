...
TLight::TLight(const TLight& a) : TBaseLight(a), lit(a.lit) {}
...
void TLight::setLit(bool n) { lit = n; }

bool TLight::isLit() const { return lit; }
...
void TLight::unequipMe(TBeing* ch) { genericExtinguish(ch); }
...
sstring TLight::statObjInfo() const {
  ...
  sprintf(buf, "Light: %s (%d), Max fuel: %s%d, Fuel left: %d, Lit? : %s",
    describe_light(getLightAmt()), getLightAmt(),
...
int TLight::getMe(TBeing* ch, TThing*) {
  ...
  if (isLit()) {
    ...
    if (!ch->heldInSecHand() && ch->canUseLimb(ch->getSecondaryHold()) &&
        ...
        (compareWeights(getWeight(), ch->maxWieldWeight(this, HAND_TYPE_SEC)) !=
          -1)) {
      --(*this);
...
void TLight::describeObjectSpecifics(const TBeing* ch) const {
  ...
  if (getMaxBurn() < 0)
    ...
    ch->sendTo(COLOR_OBJECTS,
      format("%s looks refuelable.\n\r") % sstring(getName()).cap());
    ch->sendTo(COLOR_OBJECTS,
      format("You can tell that %s has %s fuel left.\n\r") %
...
void TLight::lightMe(TBeing* ch, silentTypeT silent) {
  ...
  if (iLit == true) {
    ...
    vlogf(LOG_BUG,
      format("%s had problems lighting an object.") % ch->getName());
    return;
  }
}
