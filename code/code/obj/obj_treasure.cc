...
void TTreasure::assignFourValues(int x1, int, int, int) {
  ...
  // so i'm using an accessor specifically for the immortal exchange coin serial
  // #s here
  setSerialNumber(x1);
}
    ...
    if (!getSerialNumber()) {
      ...
      if (db.fetchRow()) {
        ...
        if (db["date_redeemed"].length() > 0)
          a +=
            "<R>Note: Reedemed coins should no longer be in play.\n\r      "
            "This is possibly a duplicated object.<1>\n\r";
      } else {
        a = format(
              "\n\r<R>Invalid Immortal Exchange coin: unregistered serial "
              "number %i.<1>\n\r") %
            getSerialNumber();
      }
    }
...
void TTreasure::lowCheck() {
  if (objVnum() == Obj::IMMORTAL_EXCHANGE_COIN) {
    if (getSerialNumber()) {
      TDatabase db(DB_SNEEZY);
      db.query(
        "select k_coin from immortal_exchange_coin where k_coin = %i and "
        "date_redeemed is not null",
        getSerialNumber());
      if (db.rowCount() > 0) {
        sstring buffer;
        if (roomp) {
        ...
        } else if (parent && dynamic_cast<TBaseContainer*>(parent) &&
                   parent->roomp) {
          buffer = format(" in %s in room %i") % parent->getName() %
                   parent->roomp->in_room;
        } else if (parent && parent->parent &&
                   dynamic_cast<TBeing*>(parent->parent)) {
          buffer = format(" in %s on %s") % parent->getName() %
                   parent->parent->getName();
        } else if (parent && parent->parent && parent->parent->roomp) {
          buffer = format(" in %s on %s in %i") % parent->getName() %
                   parent->parent->getName() % parent->parent->roomp->in_room;
        } else {
          buffer = "";
        }
        vlogf(LOG_LOW,
          format("Calling all godly wrath: We've got a counterfeit Immortal "
                 "Exchange coin%s, serial #%s.") %
