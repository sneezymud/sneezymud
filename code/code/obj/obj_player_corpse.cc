...
void TPCorpse::setRoomNum(int n) { corpse_in_room = n; }

int TPCorpse::getRoomNum() const { return corpse_in_room; }

void TPCorpse::setNumInRoom(int n) { num_corpses_in_room = n; }

int TPCorpse::getNumInRoom() const { return num_corpses_in_room; }

void TPCorpse::addToNumInRoom(int n) { num_corpses_in_room += n; }

void TPCorpse::setOwner(const sstring& Name) { fileName = Name; }

sstring TPCorpse::getOwner() const {
  sstring buf = name;

  if (fileName.empty() && buf.word(0) == "corpse" && buf.word(2) == "pcorpse") {
...
void TPCorpse::clearOwner() { fileName.erase(); }
