...
void TDrug::addToMaxFuel(int n) { maxFuel += n; }

void TDrug::setMaxFuel(int n) { maxFuel = n; }

int TDrug::getMaxFuel() const { return maxFuel; }

void TDrug::addToCurFuel(int n) { curFuel += n; }

void TDrug::setCurFuel(int n) { curFuel = n; }

int TDrug::getCurFuel() const { return curFuel; }

void TDrug::setDrugType(drugTypeT n) { drugType = n; }

drugTypeT TDrug::getDrugType() const { return drugType; }
...
affectedData* findDrugAffect(TBeing* ch, int drug, int type) {
  ...
  for (hjp = ch->affected; hjp; hjp = hjp->next) {
    if (hjp->type == AFFECT_DRUG && hjp->modifier2 == drug &&
        hjp->location == type)
      return hjp;
  }
  return NULL;
  ...
  switch (drug) {
      ...
      if (!(affptr = findDrugAffect(ch, DRUG_PIPEWEED, APPLY_SPE))) {
        ...
        reapplyDrugAffect(ch, affptr, aff.modifier,
          istick ? affptr->duration
                 : min(affptr->duration + duration1hit, aff.duration));
      }
    ...
    case DRUG_FROGSLIME: {
      const static sstring buzzes[6] = {
        "You notice that the edges of the room are sharper.  You can see "
        "minute details.",  // 0
        "You notice that the edges of the room are sharper.  You can see "
        "minute details.",          // 1
        "Your tongue feels numb.",  // 2
        "With your tongue you can taste all of the colors in the room.  "
        "Your vision is unsurpassed!",  // 3
        "You notice all of the colors in the room are pulsing and "
        "throbbing in rhythm.",  // 4
        "You feel like you can hear the throbbing of the colors in this "
        "room!"  // 5
      };
      ...
      if (!istick && consumed > 0) {
        const static applyTypeT locations[5] = {APPLY_NONE, APPLY_GARBLE,
          APPLY_GARBLE, APPLY_GARBLE, APPLY_GARBLE};
        const static long modifiers[5] = {0, Garble::TYPE_FROGTALK,
          Garble::TYPE_FROGTALK, Garble::TYPE_FROGTALK, Garble::TYPE_FROGTALK};
        const static uint64_t bitvectors[5] = {0, 0, AFF_SENSE_LIFE,
          AFF_SENSE_LIFE, AFF_SENSE_LIFE};
  ...
  switch (drug) {
    case DRUG_PIPEWEED:
      ...
      if (severity < 20 && ch->getRace() != RACE_HOBBIT) {
        ...
        ch->sendTo(
          ...
          drugTypes[drug].name);
        ch->sendTo(
          "You've got a splitting headache and you feel very very tired.\n\r");
    ...
    case DRUG_FROGSLIME: {
      ...
      for (StuffIter it = ch->roomp->stuff.begin();
           it != ch->roomp->stuff.end() && (target = *it); ++it)
        ;
      if (!target)
        target = ch;
...
int TBeing::doSmoke(const char* argument) {
  ...
  if (desc) {
    if (!desc->drugs[tdc->getDrugType()].total_consumed) {
      desc->drugs[tdc->getDrugType()].first_use.seconds =
        GameTime::getSeconds();
      desc->drugs[tdc->getDrugType()].first_use.minutes =
        GameTime::getMinutes();
