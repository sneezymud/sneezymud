...
int main(int argc, char** argv) {
  ...
  while (++i) {
    ...
    if (val["DATATYPE"] == "obj") {
      ...
      db.query("delete from obj where vnum=%s", val["vnum"].c_str());
      db.query("delete from objextra where vnum=%s", val["vnum"].c_str());
      db.query("delete from objaffect where vnum=%s", val["vnum"].c_str());

      db.query(
        "insert into obj "
        "(vnum,name,short_desc,long_desc,action_desc,type,action_flag,wear_"
        "flag,val0,val1,val2,val3,weight,price,can_be_seen,spec_proc,max_exist,"
        "max_struct,cur_struct,decay,volume,material) values "
        "(%s,'%s','%s','%s','%s',%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, "
        "%s, %s, %s, %s, %s, %s)",
        val["vnum"].c_str(), val["name"].c_str(), val["short_desc"].c_str(),
        val["long_desc"].c_str(), val["action_desc"].c_str(),
        val["type"].c_str(), val["action_flag"].c_str(),
        val["wear_flag"].c_str(), val["val0"].c_str(), val["val1"].c_str(),
        val["val2"].c_str(), val["val3"].c_str(), val["weight"].c_str(),
        val["price"].c_str(), val["can_be_seen"].c_str(),
        val["spec_proc"].c_str(), val["max_exist"].c_str(),
        val["max_struct"].c_str(), val["cur_struct"].c_str(),
        val["decay"].c_str(), val["volume"].c_str(), val["material"].c_str());

    } else if (val["DATATYPE"] == "objextra") {
      printf("replacing objextra %s\n", val["vnum"].c_str());

      db.query(
        "insert into objextra (vnum, name, description) values (%s,'%s','%s')",
        val["vnum"].c_str(), val["name"].c_str(), val["description"].c_str());

    } else if (val["DATATYPE"] == "objaffect") {
      printf("replacing objaffect %s\n", val["vnum"].c_str());

      db.query(
        "insert into objaffect (vnum,type,mod1,mod2) values (%s,%s,%s,%s)",
        val["vnum"].c_str(), val["type"].c_str(), val["mod1"].c_str(),
        val["mod2"].c_str());
    }
    printf("\n");
  }
