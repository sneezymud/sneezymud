...
NODE* read_room(TDatabase* db, TDatabase* dbexits) {
  ...
  do {
    ...
    tmp->idirs[convertTo<int>((*dbexits)["direction"])] =
      convertTo<int>((*dbexits)["destination"]);
...
void remove_one_way_exits(bool quiet = false, bool checkrooms_p = false) {
  ...
  if (!quiet)
    printf("Couldn't find %i rooms.  Removed %i one way exits.", notfound,
      removed);
}
...
int coordinates(bool quiet = false) {
  ...
  if (!quiet) {
    printf("Coordinate iteration %4i (%i rooms left)\r", ++count, roomsleft);
    fflush(stdout);
  }
...
void check_rooms(int MAXROOMS) {
  ...
  for (int i = 0; i < MAXROOMS; ++i) {
    for (int j = i + 1; j < MAXROOMS; ++j) {
      if (nodes[i] && nodes[j] && nodes[i]->done && nodes[j]->done &&
          nodes[i]->num != nodes[j]->num && nodes[i]->x == nodes[j]->x &&
          nodes[i]->y == nodes[j]->y && nodes[i]->z == nodes[j]->z) {
        printf("%i and %i have same coords (%i, %i, %i)\n", nodes[i]->num,
          nodes[j]->num, nodes[i]->x, nodes[i]->y, nodes[i]->z);
        ++count;
      }
    }
  }
      ...
      for (int j = 0; j < 10; ++j) {
        if (nodes[i]->pdirs[j]) {
          if ((!nodes[i]->pdirs[j]->pdirs[rev_dir(static_cast<dirTypeT>(j))] ||
                (nodes[i]
                    ->pdirs[j]
                    ->pdirs[rev_dir(static_cast<dirTypeT>(j))]
                    ->num != nodes[i]->num)) &&
              !done[i]) {
            printf("Room %i has a one way exit %i (%i) to room %i\n",
              nodes[i]->num, j, rev_dir(static_cast<dirTypeT>(j)),
              nodes[i]->pdirs[j]->num);
    ...
    if (nodes[i]) {
      for (int j = 0; j < 10; ++j) {
        if (nodes[i]->pdirs[j] && !done[i]) {
          ...
          if (dx > 1 || dy > 1 || dz > 1) {
            printf(
              "Room %i (%i, %i, %i) has a misaligned link to room %i (%i, %i, "
              "%i)\n",
              nodes[i]->num, nodes[i]->x, nodes[i]->y, nodes[i]->z,
              nodes[i]->pdirs[j]->num, nodes[i]->pdirs[j]->x,
              nodes[i]->pdirs[j]->y, nodes[i]->pdirs[j]->z);
            done[i] = true;
          }
        }
      }
...
void createmap(int MINLEVEL, int MAXLEVEL, int SCALEBY, sstring outputfile,
  bool sideways, FILE* logf, bool gradient_exits) {
  NODE* t;
  int minx = 0, maxx = 0, miny = 0, maxy = 0, mapsize = 0, mapwidth, mapheight,
      loc;
  ...
  for (t = head; t; t = t->next) {
    ...
    for (i = 0; i < 10; ++i) {
      ...
      if (t->pdirs[i]) {
        switch (i) {
          case 0:
            newloc = (mapwidth * (celly + 1)) + cellx;
            break;
          case 1:
            newloc = (mapwidth * (celly)) + cellx + 1;
            break;
          case 2:
            newloc = (mapwidth * (celly - 1)) + cellx;
            break;
          ...
          case 5:
            break;
          case 6:
            newloc = (mapwidth * (celly + 1)) + cellx + 1;
            break;
          case 7:
            newloc = (mapwidth * (celly + 1)) + cellx - 1;
            break;
        ...
        if (newloc != loc) {
          int new_r = (sector_colors[t->sector][0] +
                        sector_colors[t->pdirs[i]->sector][0]) /
                      2;
          int new_g = (sector_colors[t->sector][1] +
                        sector_colors[t->pdirs[i]->sector][1]) /
                      2;
          int new_b = (sector_colors[t->sector][2] +
                        sector_colors[t->pdirs[i]->sector][2]) /
                      2;
  ...
  fclose(out);
  sprintf(buff, "convert -size %ix%i -depth 8 rgb:imageout tmp-%s", mapwidth,
    mapheight, outputfile.c_str());
  printf("%s\n", buff);
  system(buff);
  sprintf(buff, "convert -flip -bordercolor black -border 10x10 tmp-%s %s",
...
void usage() {
  ...
  printf(
    "  -p              - color code rooms by popularity instead of sector\n");
  printf(
    "  -l <file>       - specify a log file to use for popularity parsing\n");
