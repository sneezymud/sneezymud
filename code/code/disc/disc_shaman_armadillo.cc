...
int thornflesh(TBeing *caster, int level, short bKnown)
{
  ...
  aff.duration = caster->durationModify(SPELL_THORNFLESH, max(min(level/10, 5), 1) * Pulse::UPDATES_PER_MUDHOUR);
  aff.modifier = 0;
...
int aqualung(TBeing * caster, TBeing * victim, int level, short bKnown)
{
  ...
  if (caster->bSuccess(bKnown, SPELL_AQUALUNG)) {
    ...
    aff.level = level;
    aff.duration = caster->durationModify(SPELL_AQUALUNG, 8 * Pulse::UPDATES_PER_MUDHOUR);
    aff.modifier = 0;
...
TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, SPELL_SHADOW_WALK)) {
    ...
    aff.level = level;
    aff.duration = caster->durationModify(SPELL_SHADOW_WALK, 18 * Pulse::UPDATES_PER_MUDHOUR);
    aff.modifier = -40;
