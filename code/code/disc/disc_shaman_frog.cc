  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SKILL_TRANSFORM_LIMB)) {
    ...
    switch (critFail(caster, SKILL_TRANSFORM_LIMB)) {
      case CRIT_F_HITOTHER:
        ...
        caster->rawBleed(limb, (level * 3) + 100, SILENT_NO,
          CHECK_IMMUNITY_YES);
...
int stormySkies(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  int dam = caster->getSkillDam(victim, SPELL_STORMY_SKIES,
    caster->getSkillLevel(SPELL_STORMY_SKIES),
    caster->getAdvLearning(SPELL_STORMY_SKIES));
...
int stormySkies(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = stormySkies(caster, victim, obj->getMagicLevel(),
...
int aquaticBlast(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, SPELL_AQUATIC_BLAST)) {
    ...
    if ((critSuccess(caster, SPELL_AQUATIC_BLAST) ||
          ...
          critSuccess(caster, SPELL_AQUATIC_BLAST)) &&
        !caster->isNotPowerful(victim, level, SPELL_AQUATIC_BLAST,
          SILENT_YES)) {
...
int castAquaticBlast(TBeing* caster, TBeing* victim) {
  ...
  ret = aquaticBlast(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_AQUATIC_BLAST));
...
int aquaticBlast(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = aquaticBlast(caster, victim, obj->getMagicLevel(),
...
int shapeShift(TBeing* caster, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_SHAPESHIFT)) {
    switch (critSuccess(caster, SPELL_SHAPESHIFT)) {
      ...
      case CRIT_S_TRIPLE:
      case CRIT_S_DOUBLE:
        duration = caster->durationModify(SPELL_SHAPESHIFT,
    ...
    *caster->roomp += *mob;
    ...
    //    caster->spellAffectJoin(mob, caster->affected, AVG_DUR_NO,
    //    AVG_EFF_YES);
...
int deathWave(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  char buf[256];
  sstring bBuf;
...
int deathWave(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = deathWave(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), 0);
  if (IS_SET(ret, VICTIM_DEAD))
    ADD_DELETE(rc, DELETE_VICT);
...
int castDeathWave(TBeing* caster, TBeing* victim) {
  ...
  ret = deathWave(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_DEATHWAVE));
