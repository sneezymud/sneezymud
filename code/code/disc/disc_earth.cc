...
int slingShot(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
...
int castSlingShot(TBeing* caster, TBeing* victim) {
  ...
  ret =
    slingShot(caster, victim, level, caster->getSkillValue(SPELL_SLING_SHOT),
      caster->getAdvLearning(SPELL_SLING_SHOT));
...
int graniteFists(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
...
int castGraniteFists(TBeing* caster, TBeing* victim) {
  ...
  ret = graniteFists(caster, victim, level,
    caster->getSkillValue(SPELL_GRANITE_FISTS),
    caster->getAdvLearning(SPELL_GRANITE_FISTS));
...
int castPebbleSpray(TBeing* caster) {
  ...
  ret = pebbleSpray(caster, level, caster->getSkillValue(SPELL_PEBBLE_SPRAY),
    caster->getAdvLearning(SPELL_PEBBLE_SPRAY));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
...
int castLavaStream(TBeing* caster) {
  ...
  ret = lavaStream(caster, level, caster->getSkillValue(SPELL_LAVA_STREAM),
    caster->getAdvLearning(SPELL_LAVA_STREAM));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
...
int meteorSwarm(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  char buf[81];
  ...
  int damage =
    caster->getSkillDam(victim, SPELL_METEOR_SWARM, level, adv_learn);
...
int meteorSwarm(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = meteorSwarm(caster, victim, obj->getMagicLevel(),
...
int castMeteorSwarm(TBeing* caster, TBeing* victim) {
  ...
  ret = meteorSwarm(caster, victim, level,
    caster->getSkillValue(SPELL_METEOR_SWARM),
    caster->getAdvLearning(SPELL_METEOR_SWARM));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
...
int stoneSkin(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_STONE_SKIN)) {
    ...
    aff1.level = level;
    aff1.duration = caster->durationModify(SPELL_STONE_SKIN,
...
int castStoneSkin(TBeing* caster, TBeing* victim) {
  ...
  ret =
    stoneSkin(caster, victim, level, caster->getSkillValue(SPELL_STONE_SKIN));
...
int trailSeek(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_TRAIL_SEEK)) {
    ...
    aff.level = level;
    aff.duration = caster->durationModify(SPELL_TRAIL_SEEK,
...
void trailSeek(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  rc =
    trailSeek(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness());
  if (rc == SPELL_SUCCESS) {
  } else {
  }
}
...
int castTrailSeek(TBeing* caster, TBeing* victim) {
  ...
  ret =
    trailSeek(caster, victim, level, caster->getSkillValue(SPELL_TRAIL_SEEK));
  if (ret == SPELL_SUCCESS) {
  } else {
  }
  return TRUE;
...
int castConjureElemEarth(TBeing* caster) {
  ...
  if ((ret = conjureElemEarth(caster, level,
