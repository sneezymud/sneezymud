...
static void adjustHealHp2(const TBeing* caster, int& hp, int durat) {
  ...
  if (caster->spelltask &&
    ...
    if (durat <= 1) {
      vlogf(LOG_BUG, format("Problem with hitpoint/rounds formula in shaman "
                            "heals, caster is %s") %
                       caster->getName());
      durat = 2;
    }
...
int healingGrasp(TBeing* caster, TBeing* victim, int level, short bKnown,
...
void healingGrasp(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret;
  ret = healingGrasp(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), SPELL_HEALING_GRASP, 0);
...
int castHealingGrasp(TBeing* caster, TBeing* victim) {
  ...
  if (ret == SPELL_SUCCESS) {
    if (victim->getHit() >= victim->hitLimit()) {
      if (caster == victim)
        caster->sendTo(COLOR_SPELLS,
          "<p>You are fully healed so you cease the ritual.<z>\n\r");
      else
        caster->sendTo(COLOR_SPELLS,
          format(
...
int enliven(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_ENLIVEN)) {
    ...
    aff.level = level;
    aff.duration = caster->durationModify(SPELL_ENLIVEN,
