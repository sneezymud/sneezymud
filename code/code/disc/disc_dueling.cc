...
int TBeing::parryWarrior(TBeing* v, TThing* weapon, int* dam, int w_type,
  ...
  if (trance) {
    ...
    //    amt = (int)((float) amt * v->plotStat(STAT_CURRENT, STAT_AGI,
    //    0.80, 1.25, 1.00)); amt = (int)((float) amt *
    //    v->plotStat(STAT_CURRENT, STAT_SPE, 0.80, 1.25, 1.00));
    amt = (int)((float)amt * getSpeMod() *
                getAgiMod());  // does the same thing - fear hobbit trancer.
  } else {
    // 4% base parry chance
    amt += 40;
    strcpy(type, "parry");
