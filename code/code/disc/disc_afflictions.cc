...
int harm(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    act("$N buckles from the pain!", false, caster, nullptr, victim, TO_NOTVICT);
    act("$N buckles from the pain!", false, caster, nullptr, victim, TO_CHAR);
    ...
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        ...
        act("That pain was intended for you!", false, caster, nullptr, victim,
          TO_VICT);
...
int poison(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->isNotPowerful(victim, level, spell, SILENT_YES)) {
    act("You invoke $d to poison $N.", false, caster, nullptr, victim, TO_CHAR);
    act("It looks like you didn't pray hard enough!", false, caster, nullptr,
      victim, TO_CHAR);
  ...
  if (victim->isImmune(IMMUNE_POISON, WEAR_BODY)) {
    act("Your pious poison seems to have no effect on $N!", false, caster, nullptr,
      victim, TO_CHAR);
  ...
  if (victim->affectedBySpell(SPELL_POISON) ||
    ...
    act("You can't poison $N.  $E is already poisoned!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n just tried to poison you again!", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
...
void poison(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    if (caster != victim) {
      ...
      act("You fill $N's veins with poison!", false, caster, nullptr, victim,
        TO_CHAR);
    } else {
    ...
    if (caster != victim) {
      ...
      act(buf, false, caster, nullptr, victim, TO_VICT);
      act("You fill $N's veins with a serious poison!", false, caster, nullptr,
        victim, TO_CHAR);
    } else {
      act("Way to go!  You've poisoned yourself badly!", false, caster, nullptr,
        nullptr, TO_VICT);
    ...
    act("$n seems to have poisoned $mself!", false, caster, nullptr, caster,
      TO_NOTVICT);
    act("Oh no!  You seem to have poisoned yourself!", false, caster, nullptr,
      nullptr, TO_CHAR);
    act("That poison was intended for you!", false, caster, nullptr, victim,
      TO_VICT);
  } else if (IS_SET(ret, SPELL_FAIL)) {
    caster->sendTo("You fail to poison your enemy.\n\r");
    act("$n just failed to poison you!", false, caster, nullptr, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, 0, victim,
      TO_NOTVICT);
  } else {
  }
...
int blindness(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BLINDNESS, SILENT_YES)) {
    act("You invoke $d to blind $N!", false, caster, nullptr, victim, TO_CHAR);
    act("Alas, your prayer is too feeble.", false, caster, nullptr, victim,
      TO_CHAR);
  ...
  if (victim->isAffected(AFF_TRUE_SIGHT) || victim->isAffected(AFF_CLARITY)) {
    act("$N's vision is much too clear to be blinded!", false, caster, nullptr,
      victim, TO_CHAR);
  ...
  if (victim->affectedBySpell(SPELL_BLINDNESS)) {
    act("You can't blind $N since $E is already blind!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n just tried to blind you again!", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
...
void blindness(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS) && !IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    if (caster != victim) {
      act("A thin cloudy film appears over $N's eyes.", false, caster, nullptr,
        victim, TO_CHAR);
      act("A thin cloudy film appears over $N's eyes.", false, caster, nullptr,
        victim, TO_NOTVICT);
      act("A thin cloudy film appears over your eyes.", false, caster, nullptr,
        victim, TO_VICT);
      ...
      act("$n has used $p to blind $mself -- $e's virtually helpless!", false,
        caster, obj, nullptr, TO_ROOM);
    }
    victim->rawBlind(level, duration, save);
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    if (caster != victim) {
      act("A dense dark film appears over $N's eyes.", true, caster, nullptr,
        victim, TO_CHAR);
      act("A dense dark film appears over $N's eyes.", true, caster, nullptr,
        victim, TO_NOTVICT);
      act("A dense dark film appears over your eyes.", false, caster, nullptr,
        victim, TO_VICT);
      ...
      act("A dense dark film appears over your eyes.", true, caster, nullptr, nullptr,
        TO_CHAR);
      act("A dense dark film appears over $n's eyes.", true, caster, nullptr, nullptr,
        TO_ROOM);
      act("You have used $p to blind yourself -- you're virtually helpless!",
        false, caster, obj, nullptr, TO_CHAR);
    ...
    if (IS_SET(ret, SPELL_CRIT_FAIL)) {
      act("$n seems to have blinded $mself with $p!", false, caster, obj, nullptr,
        TO_ROOM);
...
void blindness(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS) && !IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A thin cloudy film appears over $N's eyes.", true, caster, nullptr,
      victim, TO_CHAR);
    act("A thin cloudy film appears over $N's eyes.", true, caster, nullptr,
      victim, TO_NOTVICT);
    act("A thin cloudy film appears over your eyes.", false, caster, nullptr,
      victim, TO_VICT);
    act("$n has blinded $N -- $E's virtually helpless!", true, caster, nullptr,
      victim, TO_NOTVICT);
    act("You have blinded $N -- $E's virtually helpless!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n has blinded you!  You're virtually helpless!", false, caster, nullptr,
      victim, TO_VICT);
    victim->rawBlind(level, duration, save);
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A dense dark film appears over $N's eyes.", true, caster, nullptr, victim,
      TO_CHAR);
    act("A dense dark film appears over $N's eyes.", true, caster, nullptr, victim,
      TO_NOTVICT);
    act("A dense dark film appears over your eyes.", false, caster, nullptr,
    ...
    act("Poor $n can't even invoke a little prayer!", false, caster, nullptr,
      victim, TO_NOTVICT);
    act("Hmm, your attempt to blind $N failed.", false, caster, nullptr, victim,
      TO_CHAR);
    act("$n was trying to harm you with a prayer!", false, caster, nullptr, victim,
      TO_VICT);

    if (IS_SET(ret, SPELL_CRIT_FAIL)) {
      act("$n seems to have blinded $mself!", false, caster, nullptr, nullptr,
        TO_ROOM);
      act("Ack, you seem to have blinded yourself.", false, caster, nullptr, nullptr,
        TO_CHAR);
      caster->rawBlind(level, duration, SAVE_YES);
    }
  }
...
int harmLight(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    act("$n is lightly hurt!", false, victim, nullptr, nullptr, TO_ROOM);
    act("You are lightly hurt!", false, victim, nullptr, nullptr, TO_CHAR);
    if (caster->reconcileDamage(victim, dam, spell) == -1)
      return SPELL_SUCCESS + VICTIM_DEAD;
    return SPELL_SUCCESS;
  } else {
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        ...
        act("$n's injury was intended for you!", false, caster, nullptr, victim,
          TO_VICT);
        if (caster->reconcileDamage(caster, dam, spell) == -1)
          return SPELL_CRIT_FAIL + CASTER_DEAD;
        return SPELL_CRIT_FAIL;
...
int harmCritical(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    act("$n is critically hurt!", false, victim, nullptr, nullptr, TO_ROOM);
    act("You are critically hurt!", false, victim, nullptr, nullptr, TO_CHAR);
    if (caster->reconcileDamage(victim, dam, spell) == -1)
      return SPELL_SUCCESS + VICTIM_DEAD;
    return SPELL_SUCCESS;
  } else {
...
int harmSerious(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    act("$n is seriously hurt!", false, victim, nullptr, nullptr, TO_ROOM);
    act("You are seriously hurt!", false, victim, nullptr, nullptr, TO_CHAR);
    if (caster->reconcileDamage(victim, dam, spell) == -1)
      return SPELL_SUCCESS + VICTIM_DEAD;
    return SPELL_SUCCESS;
  } else {
...
int paralyze(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_PARALYZE, SILENT_YES)) {
    act("Your prayer is feeble and $N is unaffected!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n just tried to paralyze you!  Luckily, $e is too weak.", false,
      caster, nullptr, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  }
  if (victim->isUndead() || victim->isImmune(IMMUNE_PARALYSIS, WEAR_BODY)) {
    act("$d responds to your prayer, but $N does not!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n just tried to paralyze you.  That tickled!", false, caster, nullptr,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->isAffected(AFF_PARALYSIS)) {
    act("Try paralyzing someone who can still move.", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n must really want you to be paralyzed!", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
...
void paralyze(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A thin cloud of icy air has surrounded $N!", false, caster, nullptr,
      victim, TO_NOTVICT);
    act("A thin cloud of icy air has you surrounded you!", false, caster, nullptr,
      victim, TO_VICT);
    act("$N falls down!", false, caster, nullptr, victim, TO_NOTVICT);
    act("$N falls down!", false, caster, nullptr, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", false, caster, nullptr, victim,
      TO_NOTVICT);
    act("$N stops moving -- you have paralyzed $M!", false, caster, nullptr,
      victim, TO_CHAR);
    ...
    act("$n just paralyzed $mself!", false, caster, nullptr, victim, TO_NOTVICT);
    act("Ack!  The affects of the prayer have misfired.", false, caster, nullptr,
      victim, TO_CHAR);
    act("Your limbs freeze in place!", false, caster, nullptr, victim, TO_CHAR);
...
void paralyze(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A thin cloud of icy air surrounds $n!", false, victim, nullptr, nullptr,
      TO_ROOM);
    act("A thin cloud of icy air surrounds you!", false, victim, nullptr, nullptr,
      TO_CHAR);
    act("$N falls down!", false, caster, nullptr, victim, TO_NOTVICT);
    act("$N falls down!", false, caster, nullptr, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", false, caster, nullptr, victim,
  ...
  if (IS_SET(ret, SPELL_CRIT_FAIL)) {
    act("$n just paralyzed $mself!", false, caster, nullptr, victim, TO_NOTVICT);
    act("Ack!  Your prayer backfired on you!", false, caster, nullptr, victim,
      TO_CHAR);
    act("Your limbs freeze in place!", false, caster, nullptr, victim, TO_CHAR);
...
bool TBeing::canBoneBreak(TBeing* victim, silentTypeT silent) {
  ...
  if (!ok) {
    act("$N doesn't have a limb left to break!", false, this, nullptr, victim,
      TO_CHAR);
    act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
    act("$n just tried to break your bones!", false, this, nullptr, victim,
      TO_VICT);
    act("Luckily, you have no limbs to break.", false, this, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, this, nullptr, victim,
      TO_NOTVICT);
    return false;
  }
  if (found) {
    act("$d refuses to break another of $N's limbs!", false, this, nullptr, victim,
      TO_CHAR);
    act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
    act("$n just tried to break another bone!", false, this, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, this, nullptr, victim,
      TO_NOTVICT);
    return false;
...
int boneBreaker(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BONE_BREAKER, SILENT_YES)) {
    act("You invoke $d to break $N's bones!", false, caster, nullptr, victim,
      TO_CHAR);
    act("You did not pray hard enough.", false, caster, nullptr, victim, TO_CHAR);
  ...
  if (victim->isImmune(IMMUNE_BONE_COND, slot)) {
    ...
    act("$n just tried to break your unbreakable bones!", false, caster, nullptr,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
    ...
    switch (critFail(caster, SPELL_BONE_BREAKER)) {
      ...
      case CRIT_F_HITSELF:
        ...
        if (!caster->isImmune(IMMUNE_BONE_COND, slot)) {
          act("Your prayer backfires on you!", false, caster, nullptr, nullptr,
            TO_CHAR);
          ...
          sprintf(buf,
            ...
            limb);
          act(buf, false, caster, nullptr, nullptr, TO_ROOM);
          return SPELL_CRIT_FAIL;
        } else {
          break;
        }
...
int boneBreaker(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    ...
    act("Your prayer fails to break any of $N's bones!", false, caster, nullptr,
      victim, TO_CHAR);
    act("$n's prayer fails to break any of your bones!", false, caster, nullptr,
      victim, TO_VICT);
    act("$n's prayer fails to break any of $N's bones!", false, caster, nullptr,
      victim, TO_NOTVICT);
  } else {
    // ??
  }
...
int bleed(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BLEED, SILENT_YES) ||
    ...
    act("You pray for $N's blood, but you do not pray hard enough.", false,
      caster, nullptr, victim, TO_CHAR);
    act("$n just tried to bleed your bloodless body!", false, caster, nullptr,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (victim->isImmune(IMMUNE_BLEED, WEAR_BODY)) {
    act("You try to bleed $N, but $S veins are like iron!", false, caster, nullptr,
      victim, TO_CHAR);
  ...
  if (slot < MAX_WEAR) {
    act("$N is already bleeding!", false, caster, nullptr, victim, TO_CHAR);
    act("Your prayer goes for naught.", false, caster, nullptr, victim, TO_CHAR);
    act("$n just tried to bleed you again!", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
...
int bleed(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  false, caster, nullptr, victim, TO_VICT); act("What luck!  $p also managed to
  infect it!", false, caster, obj, nullptr, TO_CHAR); if (!victim->isPc()) {
      dynamic_cast<TMonster *>(victim)->addHated(caster);
    }
  } */
  else if (IS_SET(ret, SPELL_SAVE)) {
...
int bleed(TBeing* caster, TBeing* victim) {
  ...
  /* removed crit success in favor of multiple bleed chances
    ...
    act("Oh no!  There are living creatures in the wound!  It's infected!",
  false, caster, nullptr, victim, TO_VICT); act("What luck!  Your spell also
  managed to infect it!", false, caster, nullptr, nullptr, TO_CHAR); if
...
bool TBeing::canWither(TBeing* victim, silentTypeT silent) {
  if (victim->isUndead()) {
    if (!silent) {
      act("Your attempt to wither $N is futile!", false, this, nullptr, victim,
        TO_CHAR);
      act("$n thinks $e can wither you!", false, this, nullptr, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
    return false;
  }
  if (checkForTorment(victim)) {
    if (!silent) {
      act("$d refuses to torment $N any further.", false, this, nullptr, victim,
        TO_CHAR);
      act("$n just tried to wither you!", false, this, nullptr, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
    return false;
  ...
  if (!ok) {
    if (!silent) {
      act("$N has no limbs you can wither!", false, this, nullptr, victim,
        TO_CHAR);
      act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
      act("$n just tried to wither you!", false, this, nullptr, victim, TO_VICT);
      act("Luckily, you have no limbs to wither.", false, this, nullptr, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
  ...
  if (found) {
    if (!silent) {
      act("$d refuses to let you wither another of $N's limbs!", false, this,
        nullptr, victim, TO_CHAR);
      act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
      act("$n just tried to wither you again!", false, this, nullptr, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
...
bool TBeing::canParalyzeLimb(TBeing* victim, silentTypeT silent) {
  if (victim->isUndead()) {
    if (!silent) {
      act("Your attempt to paralyze $N is futile!", false, this, nullptr, victim,
        TO_ROOM);
      act("$n thinks $e can paralyze you!", false, this, nullptr, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
    return false;
  ...
  if (checkForTorment(victim)) {
    if (!silent) {
      act("$d refuses to torment $N any further.", false, this, nullptr, victim,
        TO_CHAR);
      act("$n just tried to paralyze you!", false, this, nullptr, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
    return false;
  ...
  if (!rc) {
    if (!silent) {
      act("$N has no limbs you can paralyze!", false, this, nullptr, victim,
        TO_CHAR);
      act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
      act("$n just tried to paralyze you!", false, this, nullptr, victim, TO_VICT);
      act("Luckily, you have no limbs to paralyze.", false, this, nullptr, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
  ...
  if (IS_SET(rc, 1)) {
    if (!silent) {
      act("$d refuses to let you paralyze another of $N's limbs!", false, this,
        nullptr, victim, TO_CHAR);
      act("Your attack goes for naught.", false, this, nullptr, victim, TO_CHAR);
      act("$n just tried to paralyze you again!", false, this, nullptr, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", false, this, nullptr,
        victim, TO_NOTVICT);
    }
...
int numb(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (checkForTorment(victim)) {
    act("$d refuses to torment $N any further.", false, caster, 0, victim,
      TO_CHAR);
    act("$n just tried to numb you!", true, caster, nullptr, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  }
  ...
  if (!ok) {
    act("$N doesn't have a limb left to numb!", false, caster, nullptr, victim,
      TO_CHAR);
    act("Your attack goes for naught.", false, caster, nullptr, victim, TO_CHAR);
    act("$n just tried to numb you!", false, caster, nullptr, victim, TO_VICT);
    act("Luckily, you have no limbs to numb.", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (found) {
    act("$d refuses to numb another of $N's limbs!", false, caster, nullptr,
      victim, TO_CHAR);
    act("Your attack goes for naught.", false, caster, nullptr, victim, TO_CHAR);
    act("$n just tried to numb you again!", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
...
int disease(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_DISEASE)) {
    ...
    if (!genericDisease(caster, victim, level)) {
      act("$d refuses to further disease $N.", false, caster, nullptr, victim,
        TO_CHAR);
...
int infect(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (victim->isUndead()) {
    ...
    act("$n thinks $e can infect you!", false, caster, nullptr, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  }
  ...
  if (slot < MAX_WEAR) {
    act("$N is already infected.", false, caster, nullptr, victim, TO_CHAR);
    act("$d shows mercy!  Your attack goes for naught.", false, caster, nullptr,
      nullptr, TO_CHAR);
    act("$n just tried to infect you again.", false, caster, nullptr, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", false, caster, nullptr,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
