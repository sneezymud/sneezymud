...
int mysticDarts(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = mysticDarts(caster, victim, obj->getMagicLevel(),
...
int castMysticDarts(TBeing* caster, TBeing* victim) {
  ...
  ret = mysticDarts(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_MYSTIC_DARTS));
...
int stunningArrow(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = stunningArrow(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), 0);
  if (IS_SET(ret, VICTIM_DEAD))
    ADD_DELETE(rc, DELETE_VICT);
  if (IS_SET(ret, CASTER_DEAD))
...
int castStunningArrow(TBeing* caster, TBeing* victim) {
  ...
  ret = stunningArrow(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_STUNNING_ARROW));
...
int castBlastOfFury(TBeing* caster, TBeing* victim) {
  ...
  ret = blastOfFury(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_BLAST_OF_FURY));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
      ...
      if ((caster != tmp_victim) && !tmp_victim->isImmortal()) {
        if (!caster->inGroup(*tmp_victim)) {
          caster->reconcileHurt(tmp_victim,
            discArray[SPELL_COLOR_SPRAY]->alignMod);
          int dam = orig_dam;
...
int castColorSpray(TBeing* caster) {
  ...
  ret = colorSpray(caster, level, bKnown,
    caster->getAdvLearning(SPELL_COLOR_SPRAY));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
    ...
    switch (critSuccess(caster, SPELL_ENERGY_DRAIN)) {
      ...
      case CRIT_S_KILL:
        ...
        if (pers && !save) {
          ...
          pers->setTitle(false);
          vlogf(LOG_MISC,
            format("%s just lost a level from energy drain! :-)") %
              pers->getName());
        }
        break;
      case CRIT_S_NONE:
...
int castEnergyDrain(TBeing* caster, TBeing* victim) {
  ...
  ret = energyDrain(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_ENERGY_DRAIN));
...
int castAcidBlast(TBeing* caster) {
  ...
  ret =
    acidBlast(caster, level, bKnown, caster->getAdvLearning(SPELL_ACID_BLAST));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {}
  }
...
int castAtomize(TBeing* caster, TBeing* victim) {
  ...
  int ret = atomize(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_ATOMIZE));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
  }
  if (IS_SET(ret, VICTIM_DEAD))
...
int animate(TBeing* caster, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_ANIMATE)) {
    ...
    if (!l_boot || !r_boot || !l_sleeve || !r_sleeve || !l_glove || !r_glove ||
        !helm || !jacket || !l_legging || (!r_legging && !paired_leg)) {
    ...
    if (!gol) {
      vlogf(LOG_BUG,
        format("ERROR! spell 'animate' (in code as create_monster) tried to "
               "load mob vnum #%d -- doesn't exist!") %
          Mob::ANIMATION);
    ...
    *caster->roomp += *gol;
    ...
    gol->setLevel(WARRIOR_LEVEL_IND,
      gol->getLevel(WARRIOR_LEVEL_IND) + level / 10 - ((armor + 400) / 30));
    gol->calcMaxLevel();
...
void sorcerersGlobe(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  sorcerersGlobe(caster, victim, obj->getMagicLevel(),
...
int bind(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  aff1.duration =
    caster->durationModify(SPELL_BIND, level * Pulse::UPDATES_PER_MUDHOUR);
    ...
    if (victim != caster) {
      if (!victim->isLucky(caster->spellLuckModifier(SPELL_TELEPORT))) {
        ...
        if (tmons) {
          ...
          victim->sendTo(
            "You feel a strange gut-wrenching, but the effect fades.\n\r");
        return SPELL_SUCCESS;
      }
    }
...
int teleport(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret =
    teleport(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness());
  if (IS_SET(ret, VICTIM_DEAD))
    ADD_DELETE(rc, DELETE_VICT);
  if (IS_SET(ret, CASTER_DEAD))
    ADD_DELETE(rc, DELETE_THIS);
