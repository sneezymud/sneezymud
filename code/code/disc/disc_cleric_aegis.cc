...
void crusade(TBeing* ch) {
  ...
  int duration =
    ch->durationModify(SPELL_CRUSADE, (3 + level) * Pulse::UPDATES_PER_MUDHOUR);
  ...
  ch->sendTo(
    "A <y>golden ray of light<1> shines down from the sky as you call the "
    "crusade.\n\r");
  act(
...
void relive(TBeing* ch, TBeing* vict) {
  ...
  if (ch->bSuccess(ch->getSkillValue(SPELL_RELIVE), ch->getPerc(),
        SPELL_RELIVE)) {
    act(
    ...
    if (victim->affected) {
      for (aff = victim->affected; aff; aff = aff->next) {
        if ((aff->type == AFFECT_DISEASE) &&
            (aff->modifier == DISEASE_EYEBALL)) {
          victim->sendTo(
            "You are blind because you have no eyeballs.  Fix that problem "
            "first.\n\r");
          return SPELL_FAIL;
        }
      }
...
void cureDisease(TBeing* c, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  int ret = cureDisease(c, victim, obj->getMagicLevel(),
...
int curePoison(TBeing* c, TBeing* victim, int level, short learn,
  spellNumT spell) {
  char buf[256];
  affectedData aff;
...
void curePoison(TBeing* c, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  int ret = curePoison(c, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell);
  if (ret == SPELL_SUCCESS) {
  } else if (ret == SPELL_CRIT_FAIL) {
...
int refresh(TBeing* c, TBeing* victim, int level, short learn,
  spellNumT spell) {
  int retCode;
...
void TBeing::removeCurseObj(TObj* target, TMagicItem* obj, spellNumT spell) {
  ...
  ret = removeCurseObj(target, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
  if (ret == SPELL_SUCCESS) {
  } else if (ret == SPELL_FAIL) {
    deityIgnore();
...
int TBeing::removeCurseBeing(TBeing* victim, int level, short learn,
  spellNumT spell) {
  affectedData aff;
  char buf[256];
...
void TBeing::removeCurseBeing(TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret;

  ret = removeCurseBeing(victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell);
...
int armor(TBeing* c, TBeing* victim, int level, short learn, spellNumT spell) {
  ...
  aff.level = level;
  aff.duration = c->durationModify(SPELL_ARMOR,
  ...
  if (c->bSuccess(learn, c->getPerc(), SPELL_SANCTUARY)) {
    ...
    switch (critSuccess(c, SPELL_SANCTUARY)) {
      ...
      case CRIT_S_DOUBLE:
        CS(SPELL_SANCTUARY);
        aff.duration = c->durationModify(SPELL_SANCTUARY,
          ((level <= MAX_MORT) ? 5 : level) * Pulse::UPDATES_PER_MUDHOUR);
        if (!victim->affectJoin(c, &aff, AVG_DUR_NO, AVG_EFF_YES))
          return SPELL_FAIL;
        return SPELL_CRIT_SUCCESS;
      default:
