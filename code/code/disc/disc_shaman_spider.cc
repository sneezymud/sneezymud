...
int transfix(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->affectedBySpell(SKILL_TRANSFIX)) {
    act("$N doesn't look any more transfixed than before.", false, caster, nullptr,
      victim, TO_CHAR);
  ...
  if (!victim->awake()) {
    act("$N looks sound asleep.  Oops!", true, caster, nullptr, victim, TO_CHAR);
    act("Nothing seems to happen.", false, caster, nullptr, nullptr, TO_ROOM);
    return SPELL_FAIL;
  }

  if (victim->fight() || !victim->isDumbAnimal()) {
    act("$N pays no attention to you.", true, caster, nullptr, victim, TO_CHAR);
    act("Nothing seems to happen.", false, caster, nullptr, nullptr, TO_ROOM);
    return SPELL_FAIL;
  }
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SKILL_TRANSFIX) &&
    ...
    act("$N stares transfixed into $n's eyes.", false, caster, nullptr, victim,
      TO_NOTVICT);
    act("$N stares transfixed into your eyes.", false, caster, nullptr, victim,
      TO_CHAR);
    act("Aren't $n's eyes beautiful?", false, caster, nullptr, victim, TO_VICT);
    victim->affectTo(&aff);
    return rc;
  } else {
    if (critFail(caster, SKILL_TRANSFIX)) {
      CF(SKILL_TRANSFIX);
      act("$N turns towards $n, infuriated!", false, caster, nullptr, victim,
        TO_NOTVICT);
      act("$N turns towards you, infuriated!", false, caster, nullptr, victim,
        TO_CHAR);
...
int livingVines(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_LIVING_VINES)) {
    act("$n summons the vines to hold $N!", false, caster, nullptr, victim,
      TO_NOTVICT);
    act("You command the vines to trap $N!", false, caster, nullptr, victim,
      TO_CHAR);
    act("$n summons the vines to ensnare you!", false, caster, nullptr, victim,
      TO_VICT);
    ...
    switch (critFail(caster, SPELL_LIVING_VINES)) {
      ...
      case CRIT_F_HITOTHER:
        ...
        act("$n traps $mself with the vines %s summoned!", false, caster, nullptr,
          nullptr, TO_ROOM);
...
int rootControl(TBeing* caster, TBeing* victim, int, int dam, short bKnown) {
  ...
  if (victim->getPosition() <= POSITION_SLEEPING) {
    act("$N seems to already be on the $g!", false, caster, nullptr, victim,
      TO_CHAR);
    act("Nothing seems to happen.", true, caster, 0, 0, TO_ROOM);
    return SPELL_FAIL;
  }
  ...
  act("Large tree roots start to form in front of $N.", false, caster, nullptr,
    victim, TO_CHAR);
  act("Large tree roots start to form in front of $N.", true, caster, nullptr,
    victim, TO_NOTVICT);
  act("Large tree roots start to form in front of you.", true, caster, nullptr,
    victim, TO_VICT);
  ...
  if (caster->bSuccess(bKnown, SPELL_ROOT_CONTROL)) {
    ...
    act("$n commands the tree roots to trip you!", false, caster, nullptr, victim,
      TO_VICT);
    ...
    if (critFail(caster, SPELL_ROOT_CONTROL) == CRIT_F_HITSELF) {
      ...
      act("The roots turn on you, and cause you to trip!", false, caster, nullptr,
        victim, TO_CHAR);
      ...
      act("The roots you summoned ignore you!", false, caster, nullptr, victim,
        TO_CHAR);
      act("The roots that $n summoned ignore $s commands.", false, caster, nullptr,
        victim, TO_NOTVICT);
      act("The roots ignore $n's request to trip you.", false, caster, nullptr,
        caster, TO_VICT);
      caster->setCharFighting(victim);
      caster->setVictFighting(victim);
      return SPELL_FAIL;
...
int sticksToSnakes(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  *caster->roomp += *snake;
  act("A strange yellow mist appears and suddenly changes into $n!", true,
    snake, nullptr, caster, TO_ROOM);
  act("A strange yellow mist appears and suddenly changes into $n!", true,
    snake, nullptr, caster, TO_CHAR);
...
int sticksToSnakes(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_STICKS_TO_SNAKES, diff,
...
int controlUndead(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_CONTROL_UNDEAD)) {
    ...
    act("You decide to do whatever $e says!", false, caster, nullptr, victim,
      TO_VICT);
    act("$N has become charmed by $n!", false, caster, nullptr, victim,
      TO_NOTVICT);
...
int controlUndead(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_CONTROL_UNDEAD, diff, 1,
...
int clarity(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_CLARITY, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int hypnosis(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->isImmune(IMMUNE_CHARM, WEAR_BODY, level) ||
    ...
    act("You have failed in this important ritual! Your victim is immune!",
      false, caster, nullptr, victim, TO_CHAR, ANSI_RED_BOLD);
    caster->nothingHappens(SILENT_YES);
    act("$n just tried to hypnotize you!", false, caster, nullptr, victim, TO_VICT,
      ANSI_RED_BOLD);
    return SPELL_FAIL;
  }
...
int hypnosis(TBeing* caster, TBeing* victim) {
  ...
  if (!bPassShamanChecks(caster, SPELL_HYPNOSIS, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_HYPNOSIS]->lag;
  diff = discArray[SPELL_HYPNOSIS]->task;

  start_cast(caster, victim, nullptr, caster->roomp, SPELL_HYPNOSIS, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int raze(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (victim->isImmortal()) {
    act("You can't do that to an immortal being.", false, caster, nullptr, victim,
      TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  ...
  if (victim->getImmunity(IMMUNE_ENERGY) >= 100) {
    act("$N is immune to energy rituals!", false, caster, nullptr, victim,
      TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, SPELL_RAZE)) {
    act("$n calls the spirits to erase $N's existance!", false, caster, nullptr,
      victim, TO_NOTVICT);
    act("You call upon the loa to erase any memory of $N!", false, caster, nullptr,
      victim, TO_CHAR);
    ...
    switch (critFail(caster, SPELL_RAZE)) {
      case CRIT_F_HITSELF:
      case CRIT_F_HITOTHER:
        act("$n nearly erases!", false, caster, nullptr, nullptr, TO_ROOM);
        caster->sendTo("ACK!  The loa are extremely angry with you!\n\r");
        act("$n fails in $s ritual!", false, caster, nullptr, victim, TO_VICT);
...
int raze(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_RAZE, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
