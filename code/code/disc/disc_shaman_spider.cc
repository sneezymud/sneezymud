...
int transfix(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (!bPassMageChecks(caster,SKILL_TRANSFIX, victim))
    return SPELL_FAIL;
#else
  if (!caster->useComponent(caster->findComponent(SKILL_TRANSFIX), victim,
        CHECK_ONLY_NO))
    return FALSE;
#endif

  if (victim->affectedBySpell(SKILL_TRANSFIX)) {
    act("$N doesn't look any more transfixed than before.", FALSE, caster, NULL,
      victim, TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SKILL_TRANSFIX) &&
    ...
    aff.modifier = 0;
    aff.duration = caster->durationModify(SKILL_TRANSFIX,
      (level / 10) * Pulse::UPDATES_PER_MUDHOUR);
    aff.bitvector = 0;
    ...
    act("$N stares transfixed into $n's eyes.", FALSE, caster, NULL, victim,
      TO_NOTVICT);
    act("$N stares transfixed into your eyes.", FALSE, caster, NULL, victim,
      TO_CHAR);
...
int livingVines(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->roomp->notRangerLandSector() &&
    ...
    act("Nothing seems to happen.", FALSE, caster, NULL, NULL, TO_ROOM);
    caster->sendTo(
      "You need to be in nature or on land to cast this spell!\n\r");
    return SPELL_FAIL;
  }
  ...
  aff1.duration = caster->durationModify(SPELL_LIVING_VINES,
  ...
  if (caster->bSuccess(bKnown, SPELL_LIVING_VINES)) {
    act("$n summons the vines to hold $N!", FALSE, caster, NULL, victim,
      TO_NOTVICT);
    act("You command the vines to trap $N!", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("$n summons the vines to ensnare you!", FALSE, caster, NULL, victim,
      TO_VICT);
    ...
    switch (critFail(caster, SPELL_LIVING_VINES)) {
      ...
      case CRIT_F_HITOTHER:
        ...
        act("$n traps $mself with the vines %s summoned!", FALSE, caster, NULL,
          NULL, TO_ROOM);
        act("Oops! You trap yourself in the vines you summoned!", FALSE, caster,
          NULL, victim, TO_CHAR);
        act("Hey, $n was trying to trap you with the vines %s summoned!", FALSE,
          caster, NULL, victim, TO_VICT);
...
int rootControl(TBeing* caster, TBeing* victim, int, int dam, short bKnown) {
  ...
  if (victim->getPosition() <= POSITION_SLEEPING) {
    act("$N seems to already be on the $g!", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("Nothing seems to happen.", TRUE, caster, 0, 0, TO_ROOM);
    return SPELL_FAIL;
  }
  ...
  act("Large tree roots start to form in front of $N.", FALSE, caster, NULL,
    victim, TO_CHAR);
  act("Large tree roots start to form in front of $N.", TRUE, caster, NULL,
    victim, TO_NOTVICT);
  act("Large tree roots start to form in front of you.", TRUE, caster, NULL,
    victim, TO_VICT);

  if (caster->bSuccess(bKnown, SPELL_ROOT_CONTROL)) {
    act("$n commands the tree roots to trip $N, causing $M to fall to the $g.",
      FALSE, caster, NULL, victim, TO_NOTVICT);
    act("$n commands the tree roots to trip you!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("You command the tree roots to trip $N, causing $M to fall to the $g.",
    ...
    if (critFail(caster, SPELL_ROOT_CONTROL) == CRIT_F_HITSELF) {
      ...
      act("The roots turn on you, and cause you to trip!", FALSE, caster, NULL,
        victim, TO_CHAR);
      act("The roots that $n summoned turn against $m and cause $m to trip!",
        FALSE, caster, NULL, victim, TO_NOTVICT);
      act("The roots $n summoned to aid $m, trip $m instead!", FALSE, caster,
        NULL, victim, TO_VICT);
      ...
      act("The roots you summoned ignore you!", FALSE, caster, NULL, victim,
        TO_CHAR);
      act("The roots that $n summoned ignore $s commands.", FALSE, caster, NULL,
        victim, TO_NOTVICT);
      act("The roots ignore $n's request to trip you.", FALSE, caster, NULL,
        caster, TO_VICT);
      caster->setCharFighting(victim);
      caster->setVictFighting(victim);
      return SPELL_FAIL;
...
int rootControl(TBeing* caster, TBeing* victim) {
  ...
  int dam = caster->getSkillDam(victim, SPELL_ROOT_CONTROL, level,
    caster->getAdvLearning(SPELL_ROOT_CONTROL));
...
int sticksToSnakes(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if ((caster->followers) && (caster->GetMaxLevel() < GOD_LEVEL1)) {
    for (k = caster->followers; k; k = n) {
      ...
      if (k->follower->getName() == snake->getName()) {
        act("Nothing seems to happen.", FALSE, caster, NULL, NULL, TO_ROOM);
        caster->sendTo(
          "You would have to be immortal to summon another snake!\n\r");
        delete snake;
        snake = NULL;
        return SPELL_FAIL;
      }
  ...
  *caster->roomp += *snake;
  act("A strange yellow mist appears and suddenly changes into $n!", TRUE,
    snake, NULL, caster, TO_ROOM);
  act("A strange yellow mist appears and suddenly changes into $n!", TRUE,
    snake, NULL, caster, TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_STICKS_TO_SNAKES)) {
    switch (critSuccess(caster, SPELL_STICKS_TO_SNAKES)) {
      ...
      case CRIT_S_DOUBLE:
        ...
        act(
          "The snake appears to be particularly strong and particularly angry!",
          TRUE, caster, 0, snake, TO_ROOM);
        act(
          "The snake appears to be particularly strong and particularly angry!",
          TRUE, caster, 0, snake, TO_CHAR);
        lvl *= 2;
        break;
      default:
        if (victim->isLucky(
              caster->spellLuckModifier(SPELL_STICKS_TO_SNAKES))) {
    ...
    act(
      "After following $n, the coiled snake bares its fangs and springs at $N!",
      TRUE, caster, 0, victim, TO_NOTVICT);
    act(
      "After following you, the coiled snake bares its fangs and springs at "
      "$N!",
      TRUE, caster, 0, victim, TO_CHAR);
    act(
      "After following $n, the snake bares its fangs and springs to attack "
      "you!",
    ...
    switch (critFail(caster, SPELL_STICKS_TO_SNAKES)) {
      ...
      case CRIT_F_HITSELF:
        ...
        act("$n loses control of the magic $e has unleashed!", TRUE, caster, 0,
          snake, TO_ROOM);
        act("You lose control of the magic you have unleashed!", TRUE, caster,
          0, snake, TO_CHAR);
    ...
    act("The snake seems to have no interest in $n!", TRUE, caster, 0, snake,
      TO_ROOM);
    caster->sendTo("The snake reverts back into sticks.\n\r");
    act("$N reverts back into the sticks from which it came.", TRUE, caster, 0,
      snake, TO_ROOM);
...
void sticksToSnakes(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  sticksToSnakes(caster, victim, obj->getMagicLevel(),
...
int sticksToSnakes(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_STICKS_TO_SNAKES, diff,
...
int controlUndead(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (!victim->isUndead()) {
    caster->sendTo(
      "Umm...that's not an undead creature. You can't charm it.\n\r");
    act("Nothing seems to happen.", FALSE, caster, NULL, NULL, TO_ROOM);
    return SPELL_FAIL;
  }
  ...
  if ((!victim->isPc() &&
        dynamic_cast<TMonster*>(victim)->Hates(caster, NULL)) ||
      caster->isNotPowerful(victim, level, SPELL_CONTROL_UNDEAD, SILENT_YES)) {
    act("Something went wrong!  All you did was piss $N off!", FALSE, caster,
      NULL, victim, TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_CONTROL_UNDEAD)) {
    ...
    act("You feel an overwhelming urge to follow $n!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("You decide to do whatever $e says!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$N has become charmed by $n!", FALSE, caster, NULL, victim,
      TO_NOTVICT);
...
void controlUndead(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  controlUndead(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness());
}
...
int controlUndead(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_CONTROL_UNDEAD, diff, 1,
...
int clarity(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_CLARITY)) {
    ...
    aff.duration = caster->durationModify(SPELL_CLARITY,
      6 + level / 3 * Pulse::UPDATES_PER_MUDHOUR);
    aff.modifier = 10;
...
int hypnosis(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->isLinkdead()) {
    act("Cant do that to someone who is linkdead.", FALSE, caster, 0, 0,
      TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  ...
  if (victim->isImmune(IMMUNE_CHARM, WEAR_BODY, level) ||
    ...
    victim->failCharm(caster);
    act("You have failed in this important ritual! Your victim is immune!",
  ...
  if (caster->bSuccess(bKnown, SPELL_HYPNOSIS)) {
    ...
    aff.location = APPLY_NONE;
    aff.bitvector = AFF_CHARM;
    aff.duration = caster->durationModify(SPELL_HYPNOSIS,
      2 * level * Pulse::UPDATES_PER_MUDHOUR);
    ...
    act("You have invoked this important ritual incorrectly!", FALSE, caster,
...
void hypnosis(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  if (caster != victim) {
    act("$p attempts to hypnotize $N to your will.", FALSE, caster, obj, victim,
      TO_CHAR, ANSI_RED_BOLD);
    act("$p attempts to hypnotize you to $n's will.", FALSE, caster, obj,
      victim, TO_VICT, ANSI_RED_BOLD);
    act("$p attempts to hypnotize $N.", FALSE, caster, obj, victim, TO_NOTVICT,
      ANSI_RED_BOLD);
  } else {
...
int hypnosis(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_HYPNOSIS, diff, 1, "",
    rounds, caster->in_room, 0, 0, TRUE, 0);
  return TRUE;
}
...
int raze(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  if (victim->isImmortal()) {
    act("You can't do that to an immortal being.", FALSE, caster, NULL, victim,
      TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  ...
  if (victim->getImmunity(IMMUNE_ENERGY) >= 100) {
    act("$N is immune to energy rituals!", FALSE, caster, NULL, victim,
      TO_CHAR);
  ...
  if (caster->bSuccess(bKnown, SPELL_RAZE)) {
    act("$n calls the spirits to erase $N's existance!", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    act("You call upon the loa to erase any memory of $N!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n calls upon the spirits to erase your existance!", FALSE, caster,
      NULL, victim, TO_VICT);
...
int raze(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_RAZE, diff, 1, "",
    rounds, caster->in_room, 0, 0, TRUE, 0);
...
int castRaze(TBeing* caster, TBeing* victim) {
  ...
  int ret =
    raze(caster, victim, level, bKnown, caster->getAdvLearning(SPELL_RAZE));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
  }
  if (IS_SET(ret, VICTIM_DEAD))
...
int raze(TBeing* tMaster, TBeing* tSucker, TMagicItem* tMagItem) {
  ...
  tReturn = raze(tMaster, tSucker, tMagItem->getMagicLevel(),
...
int squish(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_SQUISH, diff, 1, "",
    rounds, caster->in_room, 0, 0, TRUE, 0);
...
int stupidity(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->affectedBySpell(SPELL_STUPIDITY)) {
    act("You sense that $N is already stupid!", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("$n just tried to invoke something on you!", 0, caster, NULL, victim,
      TO_VICT);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FALSE;
  }
...
void stupidity(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A drab olive green aura engulfs $N!", TRUE, caster, NULL, victim,
      TO_NOTVICT);
    act("A drab olive green aura engulfs $N!", TRUE, caster, NULL, victim,
      TO_CHAR);
    act("A drab olive green aura engulfs your head!", FALSE, caster, NULL,
      victim, TO_VICT);
  }
  if (IS_SET(ret, SPELL_SAVE)) {
    act("The fog of stupidity weakens.", FALSE, caster, NULL, victim, TO_CHAR);
    act("The fog of stupidity weakens.", FALSE, caster, NULL, victim, TO_ROOM);
  } else {
  }
  ...
  if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    ...
    if (IS_SET(ret, SPELL_CSUC_DOUBLE)) {
      act("Yep...really stupid....", FALSE, caster, NULL, victim, TO_CHAR);
      act("Yep...really stupid....", FALSE, caster, NULL, victim, TO_ROOM);
    }
  ...
  if (IS_SET(ret, SPELL_CRIT_FAIL) || IS_SET(ret, SPELL_CFAIL_DEFAULT)) {
    act("A drab olive green aura engulfs $n!", TRUE, caster, NULL, victim,
      TO_ROOM);
    act("A drab olive green aura engulfs your head!", TRUE, caster, NULL,
      victim, TO_CHAR);
  } else {
...
void stupidity(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, NULL, caster->roomp, SPELL_STUPIDITY, diff, 1, "",
    rounds, caster->in_room, 0, 0, TRUE, 0);
