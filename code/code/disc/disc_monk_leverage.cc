...
static int hurlHit(TBeing* caster, TBeing* victim, dirTypeT dr) {
  ...
  // default damage
  dam = caster->getSkillDam(victim, SKILL_HURL,
    caster->getSkillLevel(SKILL_HURL), caster->getAdvLearning(SKILL_HURL));

  if (caster->roomp && !caster->roomp->dir_option[dr] &&
      caster->roomp->dir_option[dr]->to_room &&
      !IS_SET(caster->roomp->dir_option[dr]->condition, EXIT_CLOSED) &&
...
int shoulderThrowHit(TBeing* caster, TBeing* victim, int) {
  ...
  int dam = caster->getSkillDam(victim, SKILL_SHOULDER_THROW,
    caster->getSkillLevel(SKILL_SHOULDER_THROW),
    caster->getAdvLearning(SKILL_SHOULDER_THROW));
...
static int defenestrateHit(TBeing* caster, TBeing* victim, int to_room,
  ...
  // default damage
  dam = caster->getSkillDam(victim, SKILL_DEFENESTRATE,
    caster->getSkillLevel(SKILL_DEFENESTRATE),
    caster->getAdvLearning(SKILL_DEFENESTRATE));
  ...
  if (victim->doesKnowSkill(SKILL_COUNTER_MOVE) &&
    ...
    caster->sendTo(COLOR_MOBS,
      format("%s is defenestrated out of the room!\n\r") %
...
int defenestrate(TBeing* caster, TBeing* victim, sstring direction) {
  ...
  TPathFinder path;
  path.setNoMob(false);
  if (path.findPath(caster->in_room, findRoom(window->getTarget())) ==
      DIR_NONE) {
    caster->sendTo(
