...
int plagueOfLocusts(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if ((caster->followers) && (level < GOD_LEVEL1)) {
    for (auto currentFollower = caster->followers; currentFollower;
      currentFollower = currentFollower->next) {
      if (isname("locust", currentFollower->follower->name)) {
        act("Nothing seems to happen.", false, caster, nullptr, nullptr,
          TO_ROOM);
        caster->sendTo(
          "You would have to be a god to summon another locust swarm!\n\r");
        return SPELL_FAIL;
      }
    }
  }
  ...
  act(
    "You hear a loud humming sound as a swarm of locusts descends from above!",
  ...
  */
  int vnumToLoad = level < 26   ? Mob::LOCUSTS25
                   : level < 31 ? Mob::LOCUSTS30
                   : level < 36 ? Mob::LOCUSTS35
                   : level < 41 ? Mob::LOCUSTS40
                                : Mob::LOCUSTS50;
  ...
  */
  ...
  auto wasPrayerSuccessful =
    caster->bSuccess(bKnown, caster->getPerc(), SPELL_PLAGUE_LOCUSTS);
  ...
  if (wasPrayerSuccessful) {
    switch (critSuccess(caster, SPELL_PLAGUE_LOCUSTS)) {
      ...
      case CRIT_S_DOUBLE:
        ...
        act("The swarm seems larger and angrier than expected!", true, caster,
          nullptr, nullptr, TO_ROOM);
        caster->sendTo("The swarm seems larger and angrier than expected!\n\r");
      case CRIT_S_NONE:
        act(
          "Heeding $n's command, the swarm descends on and attacks $N with all "
          "of the fury of $d!",
          true, caster, nullptr, victim, TO_NOTVICT);
        act(
          "Heeding your command, the swarm descends on $N attacking with all "
          "the fury of $d!",
          true, caster, nullptr, victim, TO_CHAR);
        act(
          "Heeding $n's command, the swarm descends on you with all the fury "
          "of $d!",
          true, caster, nullptr, victim, TO_VICT);
    }
  } else {
    switch (critFail(caster, SPELL_PLAGUE_LOCUSTS)) {
      case CRIT_F_NONE:
        caster->sendTo(
          "The swarm circles once but then begins to dissipate into thin "
          "air.\n\r");
        act(
          "The swarm circles once but then begins to dissipate into thin air!",
          true, caster, nullptr, nullptr, TO_ROOM);
        caster->sendTo(
          "Finally, the swarm disappears entirely and the humming stops.\n\r");
        act("Finally, the swarm disappears totally and the humming stops.",
          true, caster, nullptr, nullptr, TO_ROOM);
        return SPELL_FAIL;
      case CRIT_F_HITOTHER:
      case CRIT_F_HITSELF:
        CF(SPELL_PLAGUE_LOCUSTS);
        act("The swarm hums angrily as it attacks $n instead of $s enemy!",
          true, caster, nullptr, nullptr, TO_ROOM);
        act("The swarm of locusts hums angrily as it begins to attack you!",
          true, caster, nullptr, nullptr, TO_CHAR);
    }
  }
  ...
  locusts->setLevel(WARRIOR_LEVEL_IND, (byte)adjustedLevel);
  locusts->setHPLevel(adjustedLevel);
  locusts->setHPFromHPLevel();
  locusts->setACLevel(adjustedLevel);
  locusts->setACFromACLevel();
  locusts->setDamLevel(adjustedLevel);
...
int plagueOfLocusts(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  int rc = 0;
  int ret = plagueOfLocusts(caster, victim, obj->getMagicLevel(),
...
int pillarOfSalt(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  wearSlotT slot = WEAR_NOWHERE;
  char buf[512], limb[256];

  if (!caster->roomp || caster->roomp->isWaterSector() ||
...
int pillarOfSalt(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = pillarOfSalt(caster, victim, obj->getMagicLevel(),
...
int pillarOfSalt(TBeing* caster, TBeing* victim) {
  ...
  ret = pillarOfSalt(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_PILLAR_SALT));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
  } else {
  }
...
namespace {
// Returns DELETE_THIS if victim was deleted by a spec proc during stickIn,
// 0 otherwise.
int brimstoneAsh(TBeing* caster, TBeing* victim) {
  ...
  if (!brimstone) {
    vlogf(LOG_BUG, "brimstoneAsh: read_object returned nullptr for BRIMSTONE_ASH");
    return 0;
  }
  ...
  if (!victim->equipment[slot] && !victim->getStuckIn(slot)) {
    ...
    act(format("A shard of $p embeds itself in your %s!") %
          victim->describeBodySlot(slot),
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    switch (critSuccess(caster, spell)) {
      ...
      case CRIT_S_TRIPLE:
        CS(spell);
        dam *= 3;
        act("The heavens <y>crack<1> and <r>HOT BRIMSTONE<1> rains down on $N!",
      ...
      case CRIT_S_DOUBLE:
        ...
        act(
          ...
          false, caster, nullptr, victim, TO_CHAR);
        act(
          "Thunder <W>CRACKS<z> and <r>flaming hot<1> <y>brimstone<1> rains "
          "down on $N!",
          false, caster, nullptr, victim, TO_VICT);
        ashRc = brimstoneAsh(caster, victim);
        break;
      case CRIT_S_NONE:
        act(
          "You hear <y>thunder<1> and <r>red hot brimstone<1> rains down on "
          "$N!",
          false, caster, nullptr, victim, TO_NOTVICT);
        act(
          "You hear <y>thunder<1> and <r>red hot brimstone<1> rains down on "
    ...
    if (IS_SET_DELETE(ashRc, DELETE_THIS))
      return SPELL_SUCCESS | VICTIM_DEAD;
    ...
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        ...
        act(
          ...
          FALSE, caster, NULL, victim, TO_NOTVICT);
        act(
          "A whining sound issues from above you.  Your skin starts to burn "
          "from "
...
int rainBrimstone(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret = 0;
  int rc = 0;

  ret = rainBrimstone(caster, victim, obj->getMagicLevel(),
...
int rainBrimstone(TBeing* caster, TBeing* victim) {
  ...
  int ret = rainBrimstone(caster, victim, level, bKnown, spell,
...
int curse(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell) {
...
int earthquake(TBeing* caster, int level, short bKnown, spellNumT spell,
  int adv_learn) {
  int rc;
...
int earthquake(TBeing* caster) {
  ...
  int ret =
    earthquake(caster, level, bKnown, spell, caster->getAdvLearning(spell));
  if (ret == SPELL_SUCCESS) {
  } else {
...
int callLightning(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret = 0;
  int rc = 0;

  ret = callLightning(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
  if (IS_SET(ret, VICTIM_DEAD))
    ADD_DELETE(rc, DELETE_VICT);
  if (IS_SET(ret, CASTER_DEAD))
...
int callLightning(TBeing* caster, TBeing* victim) {
  ...
  int ret = callLightning(caster, victim, level, bKnown, spell,
    caster->getAdvLearning(spell));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
...
int spontaneousCombust(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  int rc;
  ...
  int dam =
    caster->getSkillDam(victim, SPELL_SPONTANEOUS_COMBUST, level, adv_learn);
      ...
      if (caster != victim) {
        ...
        act("<R>You are immolated in an enourmous burst of fire!<1>", FALSE,
          caster, NULL, victim, TO_VICT);
                 ...
                 caster->spellLuckModifier(SPELL_SPONTANEOUS_COMBUST))) {
    ...
    switch (critFail(caster, SPELL_SPONTANEOUS_COMBUST)) {
      ...
      case CRIT_F_HITOTHER:
        ...
        act("$n suddenly erupts in a tower of flames!", FALSE, caster, NULL,
          NULL, TO_ROOM);
        act(
          "You are suddenly consumed in a smaller tower of flames, as the Gods "
          "are merciful... for now! <R>FIRE!!!<1>",
          FALSE, caster, NULL, NULL, TO_CHAR);

        // Reducing the damage to avoid one-shotting players at full health (as
        // often)
        dam /= 2;
...
int spontaneousCombust(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = spontaneousCombust(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), 0);
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
  } else {
...
int spontaneousCombust(TBeing* caster, TBeing* victim) {
  ...
  ret = spontaneousCombust(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_SPONTANEOUS_COMBUST));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (IS_SET(ret, SPELL_CRIT_FAIL)) {
    } else {
...
int flamestrike(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  int ret = 0;
...
int flamestrike(TBeing* caster, TBeing* victim) {
  ...
  ret = flamestrike(caster, victim, level, bKnown,
    caster->getAdvLearning(SPELL_FLAMESTRIKE));
