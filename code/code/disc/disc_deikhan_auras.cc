...
// doPojectAuraOfMight is called by the 'aura' command to start the deikhan
// projecting this aura
void doProjectAuraOfMight(TBeing * caster)
{
  if (caster->affectedBySpell(SKILL_AURA_MIGHT)) {
    caster->sendTo("You are already projecting an aura of might.\n\r");
    return;
  }
  ...
  act("You begin to project an aura of might.", TRUE, caster, NULL, NULL, TO_CHAR);

  aff.type      = SKILL_AURA_MIGHT;
  aff.level     = bKnown;
  aff.duration  = 24 * Pulse::UPDATES_PER_MUDHOUR;
  aff.location  = APPLY_NONE;

  caster->affectJoin(caster, &aff, AVG_DUR_NO, AVG_EFF_YES, FALSE);
  caster->addSkillLag(SKILL_AURA_MIGHT, 0);
}
...
int procAuraOfMight(TBeing * caster)
{
  TBeing *tch = NULL;
    ...
    aff.level     = bKnown;
    aff.duration  = Pulse::SPEC_PROCS / 5;
    aff.modifier  = 20;
    ...
    aff2.level     = bKnown;
    aff2.duration  = Pulse::SPEC_PROCS / 5;
    aff2.modifier  = -75;
...
int auraOfMightOnHit(TBeing * caster, TBeing * vict)
{
  ...
  act("Your <r>Martial Aura<z><b> flares and sears $N with <W>Holy Fire<z><b> as you strike.", false, caster, 0, vict, TO_CHAR, ANSI_BLUE);
  act("$n's <r>Martial Aura<z><b> flares and sears you with <W>Holy Fire<z><b>.", false, caster, 0, vict, TO_VICT, ANSI_BLUE);
  act("$n's <r>Martial Aura<z><b> flares and sears $N with <W>Holy Fire<z><b>.", false, caster, 0, vict, TO_NOTVICT, ANSI_BLUE);
...
// doProjectAuraOfRegeneration called by aura command
void doProjectAuraOfRegeneration(TBeing * caster)
{
  if (caster->affectedBySpell(SKILL_AURA_REGENERATION)) {
    caster->sendTo("You are already projecting an aura of regeneration.\n\r");
    return;
  }
  removeAllAuras(caster);
  short bKnown;
  ...
  act("You begin to project an aura of regeneration.", TRUE, caster, NULL, NULL, TO_CHAR);

  aff.type      = SKILL_AURA_REGENERATION;
  aff.level     = bKnown;
  aff.duration  = 24 * Pulse::UPDATES_PER_MUDHOUR;
  aff.location  = APPLY_NONE;

  caster->affectJoin(caster, &aff, AVG_DUR_NO, AVG_EFF_YES, FALSE);
  caster->addSkillLag(SKILL_AURA_REGENERATION, 0);
}
...
{
  if (vict->affectedBySpell(SKILL_AURA_MIGHT)) {
    vict->spellWearOff(SKILL_AURA_MIGHT, SAFE_NO);
    vict->affectFrom(SKILL_AURA_MIGHT);
  }
  if (vict->affectedBySpell(SKILL_AURA_REGENERATION)) {
    vict->spellWearOff(SKILL_AURA_REGENERATION, SAFE_NO);
    vict->affectFrom(SKILL_AURA_REGENERATION);
  }
...
void TBeing::doAura(sstring const& arg)
{
  ...
  if (is_abbrev(cmd, "might")) {
    doProjectAuraOfMight(this);
    return;
  } else if (is_abbrev(cmd, "regeneration")) {
    doProjectAuraOfRegeneration(this);
    return;
  ...
  // TODO: list all auras if nothing is passed in
  sendTo("Sorry not implemented.\n\r");
  return;
