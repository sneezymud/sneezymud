...
int TBeing::doSearch(const char* argument) {
  ...
  if (!*argument) {
    ...
    act("$n begins searching the walls for something.", false, this, 0, 0,
      TO_ROOM);
    start_task(this, nullptr, nullptr, TASK_SEARCH, "", 0, in_room, 1, 0, 4);
  } else {
    for (rc = 0; rc < MAX_DIR; rc++) {
      if (is_abbrev(argument, dirs[rc])) {
        start_task(this, nullptr, nullptr, TASK_SEARCH, "", 0, in_room, 1, rc + 100,
          4);
        return true;
      }
    }
    ...
    if (!strcmp(argument, "ne")) {
      start_task(this, nullptr, nullptr, TASK_SEARCH, "", 0, in_room, 1,
        DIR_NORTHEAST + 100, 4);
      return true;
