...
int astralWalk(TBeing* caster, TBeing* victim) {
  ...
  ret =
    astralWalk(caster, victim, level, caster->getSkillValue(SPELL_ASTRAL_WALK));
...
int castCreateWater(TBeing* caster, TObj* obj) {
  ...
  int ret = createWater(caster, obj, caster->getSkillLevel(spell),
    caster->getSkillValue(spell), spell);
  if (ret == SPELL_SUCCESS) {
...
int createWater(TBeing* caster, TObj* obj, int level, short bKnown,
  spellNumT spell) {
...
void wordOfRecall(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  wordOfRecall(caster, victim, obj->getMagicLevel(),
 ...
 if (!caster->inGroup(*victim) &&
  ...
  if (tmon &&
    ...
    if (mob_index[tmon->getMobIndex()].max_exist < 10) {
      int tDiff =
        ((50 - min((short)10, mob_index[tmon->getMobIndex()].max_exist)) +
      ...
      if (tch->inGroup(*caster) && (tch->getPosition() > POSITION_SLEEPING)) {
        if (isname("Merkaba", name)) {
          switch (::number(1, 20)) {
            ...
            case 7:
              message =
                "You partake of <r>slices of meatloaf drenched in ketchup<z> "
                "with <W>warm milk<z>.\n\rAgain.";
              break;
            case 8:
            case 9:
            case 10:
              message =
                "You feast on a HUGE <G>pollo verde burrito<z> with extra "
                "<g>cilantro<z>.  It's just\n\rtoo much to finish it all!";
              break;
            case 11:
            case 12:
            case 13:
              message =
                "You partake of a heart-warming feast consisting of <K>corned "
                "beef<z>, <g>cabbage<z>, and\n\ra <o>measure of ale<z>!";
              break;
            case 14:
            case 15:
              message =
                "You find your magical meal of <W>icecod<z> and <y>pigsnout<z> "
                "<g>orcfish<z> on a bed of\n\r<g>seaweed<z> with <b>fresh "
                "truffles<z> on the side, accompanied by <c>fresh "
                "spring\n\rwater<z>, very satisfying!";
              break;
            case 16:
            case 17:
              message =
                "You enjoy <r>Steak Diane<z> smothered in <p>Silverstone red "
                "wine<z> topped with\n\r<k>Shiitake mushrooms<z>!";
              break;
            case 18:
              message =
                "You partake of a strengthening repast of <g>dragon haunch<z> "
                "with <k>potatoes<z> and \n\r<g>greens<z>, and <p>spiced red "
                "wine<z>!";
              gain *= 2;
              hitgain *= 2;
              break;
            case 19:
              message =
                "A meal of <P>shrimp<z> and <B>seahorse<z> <G>gumbo<z> now "
                "fills your tummy.  You feel ready to\n\rkill again!";
              gain *= 2;
              hitgain *= 2;
              break;
            case 20:
              message =
                "You feel invigorated by a hearty meal of <c>aarakocra "
                "eggs<z>, <o>fried squash<z>, and\n\r<Y>spinefuit<z>, "
                "accompanied by a nice <y>pear ice wine<z> (completely free of "
                "<K>hobbit\n\rfoot hairs<z>)!";
              gain *= 3;
              hitgain *= 3;
              break;
      ...
      case CRIT_F_HITOTHER:
        for (StuffIter it = caster->roomp->stuff.begin();
          ...
          if (tch->inGroup(*caster) &&
              (tch->getPosition() > POSITION_SLEEPING)) {
            tch->sendTo(
              "You feel weakened! Something went horribly wrong!\n\r");
