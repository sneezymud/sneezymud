...
int voodoo(TBeing* caster, TObj* obj, int level, short bKnown) {
  ...
  *caster->roomp += *mob;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_VOODOO)) {
    ...
    act("You've created a monster; $N hates you!", false, caster, nullptr, mob,
      TO_CHAR);
...
int voodoo(TBeing* caster, TObj* corpse) {
  ...
  start_cast(caster, nullptr, corpse, caster->roomp, SPELL_VOODOO, diff, 1, "",
...
int dancingBones(TBeing* caster, TObj* corpse) {
  ...
  start_cast(caster, nullptr, corpse, caster->roomp, SPELL_DANCING_BONES, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int shieldOfMists(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_SHIELD_OF_MISTS)) {
    ...
    act("<G>$n <G>is enveloped by a thick green mist!<z>", false, victim, nullptr,
      nullptr, TO_ROOM);
    act("<G>You are enveloped by a thick green mist!<z>", false, victim, nullptr,
      nullptr, TO_CHAR);
...
int shieldOfMists(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_SHIELD_OF_MISTS, diff,
...
int enthrallSpectre(TBeing* caster, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_ENTHRALL_SPECTRE)) {
    act("You call upon the powers of your ancestors!", true, caster, nullptr, nullptr,
      TO_CHAR);
    act("$n summons the powers of $s ancestors!", true, caster, nullptr, nullptr,
      TO_ROOM);
    ...
    *caster->roomp += *victim;
    ...
    if (caster->tooManyFollowers(victim, FOL_CHARM)) {
      ...
      act("$N refuses to enter a group the size of $n's!", true, caster, nullptr,
        victim, TO_ROOM);
      act("Your loa is displeased! $N hates you!", false, caster, nullptr, victim,
        TO_CHAR);
    ...
    *caster->roomp += *victim;
    ...
    act("The gods are not pleased! $N hates you!", false, caster, nullptr, victim,
      TO_CHAR);
...
int enthrallSpectre(TBeing* caster) {
  ...
  if (!bPassShamanChecks(caster, SPELL_ENTHRALL_SPECTRE, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_ENTHRALL_SPECTRE]->lag;
  taskDiffT diff = discArray[SPELL_ENTHRALL_SPECTRE]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_ENTHRALL_SPECTRE, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int castEnthrallSpectre(TBeing* caster) {
  ...
  if ((ret = enthrallSpectre(caster, level,
    ...
    act("You feel the ancestors are not pleased.", false, caster, nullptr, nullptr,
      TO_CHAR);
  }
  return true;
}
...
int enthrallGhast(TBeing* caster) {
  ...
  if (!bPassShamanChecks(caster, SPELL_ENTHRALL_GHAST, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_ENTHRALL_GHAST]->lag;
  taskDiffT diff = discArray[SPELL_ENTHRALL_GHAST]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_ENTHRALL_GHAST, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int vampiricTouch(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (victim->isImmortal()) {
    act("You can't touch a god in that manner!", false, caster, nullptr, victim,
      TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  if (1 > victim->getHit()) {
    act("That creature has nothing left for you!", false, caster, nullptr, victim,
      TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  int num =
    ::number(1, ((caster->getSkillValue(SPELL_VAMPIRIC_TOUCH) / 3) * 2));
  int num2 =
    ::number(1, ((caster->getSkillValue(SPELL_VAMPIRIC_TOUCH) / 6) * 5));
  int num3 = ::number(50, 150);

  if (caster->bSuccess(bKnown, SPELL_VAMPIRIC_TOUCH)) {
    act("$N groans in pain as life is drawn from $S body!", false, caster, nullptr,
      victim, TO_NOTVICT);
    act("$N groans in pain as life is drawn from $S body!", false, caster, nullptr,
      victim, TO_CHAR);
    ...
    switch (critFail(caster, SPELL_VAMPIRIC_TOUCH)) {
      ...
      case CRIT_F_HITOTHER:
        ...
        act("$n's body glows a dark, evil-looking red!", false, caster, nullptr,
          nullptr, TO_ROOM);
        act(
...
int vampiricTouch(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_VAMPIRIC_TOUCH, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
...
int lifeLeech(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (victim->getImmunity(IMMUNE_DRAIN) >= 100) {
    act("$N is immune to draining!", false, caster, nullptr, victim, TO_CHAR);
...
int lifeLeech(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_LIFE_LEECH, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
...
int cheval(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_CHEVAL)) {
    ...
    act("$N seems as if $E is possessed!", false, caster, nullptr, victim,
...
int cheval(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_CHEVAL, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int chrism(TBeing* caster, const char* name) {
  if (!bPassShamanChecks(caster, SPELL_CHRISM, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_CHRISM]->lag;
  taskDiffT diff = discArray[SPELL_CHRISM]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_CHRISM, diff, 2, name,
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int castChrism(TBeing* caster, const char* name) {
  ...
  act("$n places $s hands on $s head and howls at the sky.", true, caster, nullptr,
    nullptr, TO_ROOM);
  act(
    "You place your hands on the sides of your head and call unto the loa to "
    "hear your plea.",
    true, caster, nullptr, nullptr, TO_CHAR);
...
int rombler(TBeing* caster, int, short bKnown) {
  ...
  msg = caster->garble(nullptr, msg, Garble::SPEECH_SHOUT, Garble::SCOPE_EVERYONE);
          ...
          if (i->character->doesKnowSkill(SPELL_ROMBLER) ||
            ...
            if (!i->m_bIsClient &&
              ...
              i->clientf(
                ...
                colorString(i->character, i, caster->getName(), nullptr,
                  COLOR_NONE, false) %
                colorString(i->character, i, pgbuff, nullptr, COLOR_NONE, false));
...
int rombler(TBeing* caster, const char* msg) {
  ...
  if (!bPassShamanChecks(caster, SPELL_ROMBLER, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_ROMBLER]->lag;
  diff = discArray[SPELL_ROMBLER]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_ROMBLER, diff, 1, msg,
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int intimidate(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->bSuccess(bKnown, SPELL_INTIMIDATE)) {
    if (victim->isLucky(caster->spellLuckModifier(SPELL_INTIMIDATE)) ||
      ...
      act("You feel intimidated briefly.", false, caster, nullptr, victim, TO_VICT,
        ANSI_YELLOW_BOLD);
    } else {
      act("$N is totally intimidated by $n!", false, caster, nullptr, victim,
        TO_NOTVICT, ANSI_YELLOW_BOLD);
      act("$N is totally intimidated by you.", false, caster, nullptr, victim,
        TO_CHAR, ANSI_YELLOW_BOLD);
      act("$n intimidates you! RUN!!!", false, caster, nullptr, victim, TO_VICT,
        ANSI_YELLOW_BOLD);
    ...
    switch (critFail(caster, SPELL_INTIMIDATE)) {
      ...
      case CRIT_F_HITOTHER:
        ...
        act("$n attempts to flee...from $mself?!? Strange...", false, caster,
          nullptr, nullptr, TO_ROOM, ANSI_YELLOW_BOLD);
        act(
          "Damn! The loa have intimidated you by yourself. Run for your life!",
          false, caster, nullptr, victim, TO_CHAR, ANSI_YELLOW_BOLD);
        act("$n was trying to invoke ritual on you!", false, caster, nullptr,
...
int intimidate(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_INTIMIDATE, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
void senseLifeShaman(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  if (ret == SPELL_SUCCESS) {
    ...
    act("$n's eyes flicker a faint pale blue.", false, victim, nullptr, nullptr,
      TO_ROOM, ANSI_CYAN);
  } else {
    caster->nothingHappens();
  }
...
int senseLifeShaman(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_SENSE_LIFE_SHAMAN, diff,
    1, "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int detectShadow(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_DETECT_SHADOW, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
  ...
  if (caster->bSuccess(bKnown, SPELL_DJALLA)) {
    switch (critSuccess(caster, SPELL_DJALLA)) {
      ...
      case CRIT_S_KILL:
        ...
        act("$n becomes one with the spirits.", false, victim, nullptr, nullptr,
          TO_ROOM, ANSI_GREEN);
        act("You have been greatly blessed with the protection of Djalla!",
          false, victim, nullptr, nullptr, TO_CHAR, ANSI_GREEN);
        break;
      case CRIT_S_NONE:
        act("$n becomes one with the spirits.", false, victim, nullptr, nullptr,
          TO_ROOM, ANSI_GREEN);
...
int djallasProtection(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_DJALLA, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int legbasGuidance(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_LEGBA, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int embalm(TBeing* caster, TObj* corpse) {
  ...
  start_cast(caster, nullptr, corpse, caster->roomp, SPELL_EMBALM, diff, 2, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int embalm(TBeing* caster, TObj* o, int level, short bKnown) {
  ...
  if (!(corpse = dynamic_cast<TBaseCorpse*>(o))) {
    act("$N is not a corpse!  You can only embalm corpses.", false, caster,
      nullptr, o, TO_CHAR);
    act("$n looks momentarily befuddled.", false, caster, nullptr, corpse,
      TO_ROOM);
    return SPELL_FAIL;
  }
  ...
  act("$N stiffens and takes on a rubbery appearance.", false, caster, nullptr,
    corpse, TO_CHAR, ANSI_RED);
  act("The steady march of the flesh devourers has been slowed.", false, caster,
    nullptr, corpse, TO_CHAR, ANSI_RED);
  act("$N stiffens and takes on a rubbery appearance.", false, caster, nullptr,
    corpse, TO_ROOM, ANSI_RED);
  act("The steady march of the flesh devourers has been slowed.", false, caster,
    nullptr, corpse, TO_ROOM, ANSI_RED);

  return SPELL_SUCCESS;
}

int squish(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  int dam = caster->getSkillDam(victim, SPELL_SQUISH, level, adv_learn);

  if (victim->getImmunity(IMMUNE_BONE_COND) >= 100) {
    act("$N is immune to bone ailments!", false, caster, nullptr, victim, TO_CHAR);
    act("$N ignores $n's weak grasp!", false, caster, nullptr, victim, TO_NOTVICT);
    act("$n's ritual fails because of your immunity!", false, caster, nullptr,
      victim, TO_VICT);
    return SPELL_FAIL;
  }
  ...
  if (caster->bSuccess(bKnown, SPELL_SQUISH)) {
    switch (critSuccess(caster, SPELL_SQUISH)) {
      ...
      case CRIT_S_DOUBLE:
        dam *= 2;
        act("Your hands become ENORMOUS and you squish $N's body HARD!", false,
          caster, nullptr, victim, TO_CHAR);
        act(
          "$n's hands somehow become ENORMOUS and $e squishes $N's body REALLY "
          "HARD!",
          false, caster, nullptr, victim, TO_NOTVICT);
        act(
          "$n's hands somehow become ENORMOUS and $e squishes your body REALLY "
          "HARD!",
      ...
      case CRIT_S_NONE:
        ...
        act("$n's hands become enlarged and $e reaches out and squishes $N!",
          false, caster, nullptr, victim, TO_NOTVICT);
        act(
          "$n's hands become enlarged and $e reaches out and squishes your "
          "body!",
          false, caster, nullptr, victim, TO_VICT);
    ...
    if (critFail(caster, SPELL_SQUISH) == CRIT_F_HITSELF) {
      CF(SPELL_SQUISH);
      act(
        "Your hands have a mind of their own and you grab the wrong thing! "
        "OWWW!",
        false, caster, nullptr, victim, TO_CHAR);
      act("$n fondles $mself fondly, and a bit rough...oww!", false, caster,
...
int squish(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_SQUISH, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
...
int castSquish(TBeing* caster, TBeing* victim) {
  ...
  ret = squish(caster, victim, level, caster->getSkillValue(SPELL_SQUISH),
    caster->getAdvLearning(SPELL_SQUISH));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else {
    if (ret == SPELL_CRIT_FAIL) {
    } else {
...
int distort(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_DISTORT, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
...
int soulTwist(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  if (victim->isImmortal()) {
    act("You can't twist a gods soul!", false, caster, nullptr, victim, TO_CHAR);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FAIL;
  }
  if (victim->getImmunity(IMMUNE_DRAIN) >= 100) {
...
int soulTwist(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_SOUL_TWIST, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
...
int enthrallGhoul(TBeing* caster) {
  ...
  if (!bPassShamanChecks(caster, SPELL_ENTHRALL_GHOUL, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_ENTHRALL_GHOUL]->lag;
  taskDiffT diff = discArray[SPELL_ENTHRALL_GHOUL]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_ENTHRALL_GHOUL, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int stupidity(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->affectedBySpell(SPELL_STUPIDITY)) {
    act("You sense that $N is already stupid!", false, caster, nullptr, victim,
      TO_CHAR);
    act("$n just tried to invoke something on you!", 0, caster, nullptr, victim,
      TO_VICT);
    caster->nothingHappens(SILENT_YES);
    return SPELL_FALSE;
  }
  ...
  aff.duration = caster->durationModify(SPELL_STUPIDITY,
    10 * Pulse::UPDATES_PER_MUDHOUR / 2);
...
void stupidity(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret =
    stupidity(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness());

  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A drab olive green aura engulfs $N!", true, caster, nullptr, victim,
      TO_NOTVICT);
    act("A drab olive green aura engulfs $N!", true, caster, nullptr, victim,
      TO_CHAR);
    act("A drab olive green aura engulfs your head!", false, caster, nullptr,
      victim, TO_VICT);
  }
  if (IS_SET(ret, SPELL_SAVE)) {
    act("The fog of stupidity weakens.", false, caster, nullptr, victim, TO_CHAR);
    act("The fog of stupidity weakens.", false, caster, nullptr, victim, TO_ROOM);
  } else {
  }
  ...
  if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    ...
    if (IS_SET(ret, SPELL_CSUC_DOUBLE)) {
      act("Yep...really stupid....", false, caster, nullptr, victim, TO_CHAR);
      act("Yep...really stupid....", false, caster, nullptr, victim, TO_ROOM);
    }
  ...
  if (IS_SET(ret, SPELL_CRIT_FAIL) || IS_SET(ret, SPELL_CFAIL_DEFAULT)) {
    act("A drab olive green aura engulfs $n!", true, caster, nullptr, victim,
      TO_ROOM);
    act("A drab olive green aura engulfs your head!", true, caster, nullptr,
      victim, TO_CHAR);
  } else {
...
void stupidity(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_STUPIDITY, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
...
int castStupidity(TBeing* caster, TBeing* victim) {
  ...
  ret =
    stupidity(caster, victim, level, caster->getSkillValue(SPELL_STUPIDITY));
...
int flatulence(TBeing* caster, int level, short bKnown, int adv_learn) {
  ...
  int dam = caster->getSkillDam(nullptr, SPELL_FLATULENCE, level, adv_learn);

  if (caster->bSuccess(bKnown, SPELL_FLATULENCE)) {
    act("<o>You turn around quickly and pass gas!<1>", false, caster, nullptr,
      nullptr, TO_CHAR);
    act("<o>$n turns around quickly and passes gas!<1>", false, caster, nullptr,
      nullptr, TO_ROOM);
    for (StuffIter it = caster->roomp->stuff.begin();
      ...
      if (!caster->inGroup(*vict) && !(vict->isImmortal()) &&
        ...
        act("$n is choked by the natural gasses!", false, vict, nullptr, nullptr,
          TO_ROOM);
        act("You are choked by the natural gasses!", false, vict, nullptr, nullptr,
          TO_CHAR);
    ...
    if (critFail(caster, SPELL_FLATULENCE)) {
      CF(SPELL_FLATULENCE);
      act("Oh no!!! That one stuck with you!", false, caster, nullptr, nullptr,
        TO_CHAR);
      act("$n chokes on $s own fumes!!", false, caster, nullptr, nullptr, TO_ROOM);
...
int flatulence(TBeing* caster) {
  if (!bPassShamanChecks(caster, SPELL_FLATULENCE, nullptr))
    return false;

  lag_t rounds = discArray[SPELL_FLATULENCE]->lag;
  taskDiffT diff = discArray[SPELL_FLATULENCE]->task;

  start_cast(caster, nullptr, nullptr, caster->roomp, SPELL_FLATULENCE, diff, 1, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
void chaseSpirits(TBeing* caster, TObj* tar_obj, TMagicItem* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("$p chases the spirits from $n...", false, tar_obj, obj, nullptr, TO_ROOM);
  }
}
...
int chaseSpirits(TBeing* caster, TObj* obj) {
  ...
  start_cast(caster, nullptr, obj, caster->roomp, SPELL_CHASE_SPIRIT, diff, 2, "",
    rounds, caster->in_room, 0, 0, true, 0);
  return true;
}
...
int castChaseSpirits(TBeing* caster, TObj* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("You chase away the evil spirits within $N...", false, caster, nullptr,
      obj, TO_CHAR);
    act("$n chases away the evil spirits within $N...", false, caster, nullptr,
      obj, TO_ROOM);
  }
  return ret;
}
...
int chaseSpirits(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  mud_assert(caster != nullptr, "chaseSpirits(): no caster");
  mud_assert(victim != nullptr, "chaseSpirits(): no victim");
...
int chaseSpirits(TBeing* caster, TBeing* victim) {
  ...
  start_cast(caster, victim, nullptr, caster->roomp, SPELL_CHASE_SPIRIT, diff, 1,
    "", rounds, caster->in_room, 0, 0, true, 0);
  return true;
}

int castChaseSpirits(TBeing* caster, TBeing* victim) {
  mud_assert(caster != nullptr, "castChaseSpirits(): no caster");
  mud_assert(victim != nullptr, "castChaseSpirits(): no victim");
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    if (caster != victim) {
      act("You chase away the evil spirits within $N...", false, caster, nullptr,
        victim, TO_CHAR);
      act("$n chases away the evil spirits within you...", false, caster, nullptr,
        victim, TO_VICT);
      act("$n chases away the evil spirits within $N...", false, caster, nullptr,
        victim, TO_NOTVICT);
    } else {
      act("The loa chases away the evil within you...", false, caster, nullptr, 0,
        TO_CHAR);
      act("$n's face glows with much relief.", false, caster, nullptr, 0, TO_ROOM);
    }
    int rc = genericChaseSpirits(caster, victim, level, caster->isImmortal());
    if (IS_SET_DELETE(rc, DELETE_VICT))
      return DELETE_VICT;
...
int genericChaseSpirits(TBeing* caster, TBeing* victim, int,
  ...
  mud_assert(victim != nullptr, "genericChaseSpirits(): no victim");
