...
static void repHealing(TBeing* caster, TBeing* victim) {
  ...
  if (caster != victim) {
    caster->sendTo(COLOR_SPELLS,
      format("%s looks to be at <r>%.1f%c<z> HP.\n\r") % nameBuf %
        victim->getPercHit() % '%');
  }
}
...
static void adjustHealHp(const TBeing* caster, int& hp, int durat) {
  ...
  // ok we have a nice damage modifer, but if we don't modify hp from heal as
  // well, then clerics can heal tanks too fast. the mud is semi balanced as far
  // as heal to 5.1, where our damage constant was .75 as of 5.2 the damage
  // constant is .65 so i'm gonna make the heal constant .75
  ...
  if (caster->spelltask &&
    ...
    hp /= durat;
    hp = max(1, hp);
  } else if (caster->isImmortal() ||
             (caster->spelltask && caster->spelltask->rounds == 0)) {
    ...
    if (durat <= 1) {
      vlogf(LOG_BUG,
        format(
          "Problem with hitpoint/rounds formula in heal spells, caster is %s") %
          caster->getName());
      durat = 2;
    }
    hp /= (durat - 1) * 2;
...
int healLight(TBeing* caster, TBeing* victim, int level, short bKnown,
...
void healLight(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret;
  ret = healLight(caster, victim, obj->getMagicLevel(),
...
int castHealLight(TBeing* caster, TBeing* victim) {
  ...
  int ret = healLight(caster, victim, caster->getSkillLevel(spell),
    caster->getSkillValue(spell), spell, caster->getAdvLearning(spell));
  if (ret == SPELL_SUCCESS) {
    if (victim->getHit() >= victim->hitLimit()) {
      if (caster == victim)
        caster->sendTo(COLOR_SPELLS,
          "<p>You are fully healed so you stop your prayer.<z>\n\r");
      else
        caster->sendTo(COLOR_SPELLS,
          format(
...
int healSerious(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell, int adv_learn) {
  int hp = caster->getSkillDam(victim, spell, level, adv_learn);
  adjustHealHp(caster, hp, discArray[spell]->lag);
...
void healSerious(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret = healSerious(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
  if (ret == SPELL_SUCCESS) {
    repHealing(caster, victim);
  } else {
...
void healCritical(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret;
  ret = healCritical(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
  if (ret == SPELL_SUCCESS) {
    repHealing(caster, victim);
  } else {
...
int heal(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell, int adv_learn) {
  int hp = caster->getSkillDam(victim, spell, level, adv_learn);
  adjustHealHp(caster, hp, discArray[spell]->lag);
...
void heal(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  int ret = heal(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
  if (ret == SPELL_SUCCESS) {
    repHealing(caster, victim);
  } else {
...
int healFull(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
...
void healFull(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  int ret;
  ret = healFull(caster, victim, obj->getMagicLevel(),
...
int castHealFull(TBeing* caster, TBeing* victim) {
  ...
  int ret = healFull(caster, victim, caster->getSkillLevel(spell),
    caster->getSkillValue(spell), caster->getAdvLearning(spell));
  if (ret == SPELL_SUCCESS) {
    if (victim->getHit() >= victim->hitLimit()) {
      if (caster == victim)
        caster->sendTo(COLOR_SPELLS,
...
void healCritSpray(TBeing* caster) {
  ...
  int ret = healCritSpray(caster, level,
    ...
    caster->getAdvLearning(SPELL_HEAL_CRITICAL_SPRAY));
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}
...
void healCritSpray(TBeing* caster, TMagicItem* obj) {
  int ret;
  ret =
    healCritSpray(caster, obj->getMagicLevel(), obj->getMagicLearnedness(), 0);
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}
...
void healFullSpray(TBeing* caster) {
  ...
  int ret =
    healFullSpray(caster, level, caster->getSkillValue(SPELL_HEAL_FULL_SPRAY),
      caster->getAdvLearning(SPELL_HEAL_FULL_SPRAY));
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}
...
void healFullSpray(TBeing* caster, TMagicItem* obj) {
  ...
  ret =
    healFullSpray(caster, obj->getMagicLevel(), obj->getMagicLearnedness(), 0);
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}
...
void knitBone(TBeing* caster, TBeing* victim) {
  ...
  int ret =
    knitBone(caster, victim, level, caster->getSkillValue(SPELL_KNIT_BONE));
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}
...
int clot(TBeing* caster, TBeing* victim, int, short bKnown, spellNumT spell) {
  ...
  // internal bleeding
  if (slot == WEAR_NOWHERE && victim->hasDisease(DISEASE_HEMORRHAGE)) {
    // we mimic the check for cure disease here to make it harder to cure this
    // powerful malady curing internal bleeding is the same difficulty as curing
    // DISEASE_LEPROSY
    int chance = min((caster->getSkillValue(spell) - 50 + 1) * 3,
    ...
    switch (critFail(caster, SPELL_RESTORE_LIMB)) {
      ...
      case CRIT_F_HITSELF:
        ...
        if (victim->isPc()) {
          ...
          } while (num == WEAR_BODY || num == WEAR_HEAD || num == WEAR_NECK ||
                   num == WEAR_BACK);
...
void restoreLimb(TBeing* caster, TBeing* victim) {
  ...
  int ret = restoreLimb(caster, victim, level,
    caster->getSkillValue(SPELL_RESTORE_LIMB));
  if (ret == SPELL_SUCCESS) {
  } else {
...
int sterilize(TBeing* caster, TBeing* victim, int, short bKnown,
  spellNumT spell) {
  char buf[512], limb[256];
  wearSlotT slot;
...
void sterilize(TBeing* caster, TBeing* victim) {
  ...
  int ret =
    sterilize(caster, victim, level, caster->getSkillValue(spell), spell);
  if (ret == SPELL_SUCCESS) {
  } else {
  }
}

void sterilize(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  sterilize(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
}
...
--------------------------------------------------------- */
bool salve_bruise(TBeing* caster, TBeing* victim, byte bKnown,
  spellNumT spell) {
  char buf[256];
...
int salve(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell) {
  int hurting = 0, soothing = 0, fixed = 0;
  wearSlotT slot;
  char buf[256];
  int max_am = spell == SPELL_SALVE ? 1 + (level * number(5, 8))
  ...
  if (spell == SKILL_WOHLIN ||
    ...
    while (fixed < max_am) {
      ...
      if (hurting > 0) {
        ...
        // this shouldn't happen, as getRandomHurtPart should only return
        // hurting parts
        break;
      }
      ...
      if (slot) {
        if (IS_SET(victim->getLimbFlags(slot), PART_BLEEDING) ||
            (!caster->isLucky(levelLuckModifier(victim->GetMaxLevel())) &&
              !::number(0, 2) &&
              (::number(0,
                 victim->plotStat(STAT_CURRENT, STAT_BRA, 50, 100, 75)) >
...
void salve(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  salve(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
}
...
int expel(TBeing* caster, TBeing* victim, int, short bKnown, spellNumT spell) {
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    for (slot = pickRandomLimb();
         ((!victim->slotChance(slot)) || !(o = victim->getStuckIn(slot)));
         slot = pickRandomLimb())
      ;
...
void expel(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  expel(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
}
