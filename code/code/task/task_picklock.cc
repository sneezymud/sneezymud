...
static void pick_pulse(TBeing* ch, TThing* pick) {
  ...
  bool pickable =
    !IS_SET(exit->condition, EXIT_JAMMED) && exit->lock_difficulty < 100;

  if (!pickable && ch->bSuccess(skill, SKILL_PICK_LOCK)) {
    act("This lock is totally impossible to pick.  You give up.", true, ch,
      tool, nullptr, TO_CHAR);
    ch->stopTask();
    return;
  }
  ...
  if (pickable && ch->bSuccess(skill, SKILL_PICK_LOCK)) {
    ...
    sstring msg =
      !exit->keyword.empty()
        ? sstring(format("$n skillfully picks the lock of the %s.") %
                  fname(exit->keyword))
    ...
    ch->gainTaskExp(SKILL_PICK_LOCK, exit->lock_difficulty, lockExp / 100.0,
      true);
    ...
    roomDirData* back;
    if (other &&
        (back = other->dir_option[rev_dir(
           static_cast<dirTypeT>(ch->task->status))]) &&
  ...
  if (!::number(0, 2))
    act("You wiggle $p in the lock, without luck. (yet...)", true, ch, tool,
      nullptr, TO_CHAR);
}

int task_picklock(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*,
  TObj*) {
  ...
  switch (cmd) {
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You are unable to continue lockpicking while under attack!\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST)
