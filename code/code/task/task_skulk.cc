...
int skulk_task(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*, TObj*) {
  ...
  switch (cmd) {
    case CMD_TASK_CONTINUE:
      ...
      // Fatigue check - skulking is tiring
      ch->addToMove(-1);
      if (ch->getMove() < 5) {
      ...
      if (ch->bSuccess(SKILL_SKULK)) {
        ...
        if (ch->task->timeLeft <= 0) {
          ...
          if (ch->task->timeLeft > 5) {
            ...
            act("Your preseence is barely perceptible.", FALSE, ch, 0, 0, TO_CHAR);
          }
        }
      } else {
        CF(SKILL_SKULK);
        // No time reduction on failure
