...
int task_spell_friends(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*,
  ...
  switch (cmd) {
    case CMD_TASK_CONTINUE:
      ...
      switch (ch->task->timeLeft) {
        case 5:
          ...
          //          check for change in position -- use status in bash or
          //          combat where if a successful spell distraction status goes
          //          to > 0 thus in each spell continue, it checks for
          //          distraction set to anything.  The code does a roll for 1.
          //          total fail, 2. time added or nothing then clears
          //          distraction bit. distractions will be bash, or skills or
          //          big w lloping hits
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You are unable to continue spell-casting while under attack!\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST)
