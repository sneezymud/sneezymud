...
int task_search(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*,
  ...
  if (ch->task->flags >= 100)
    eDirection -= 100;
  // Exits are 0, ..., 9.  This value[ch->task->flags] should NEVER get
  // above/below that mark.
  ...
  switch (cmd) {
    case CMD_TASK_CONTINUE:
      if (eDirection == DIR_UP && ch->roomp->getRoomHeight() <= 0) {
        ch->sendTo(
          "You decide to skip the ceiling, seeing that there isn't one.\n\r");
        act(
        ...
        if (ch->task->flags >= 100) {
          ...
          return task_search(ch, cmd, "", pulse, nullptr, nullptr);
                 ...
                 ((IS_SET(fdd->condition, EXIT_SECRET) &&
                    !IS_SET(fdd->condition, EXIT_CLOSED)) ||
                   !IS_SET(fdd->condition, EXIT_SECRET))) {
        ch->sendTo(format("You decide to skip searching %s.  Seeing there is "
                          "an exit there.\n\r") %
      ...
      if (tsSuccess && (fdd = ch->roomp->dir_option[eDirection]) &&
          IS_SET(fdd->condition, EXIT_SECRET) &&
          IS_SET(fdd->condition, EXIT_CLOSED) && !fdd->keyword.empty() &&
          fdd->keyword != "_unique_door_") {
        const char* foundPrint = "Secret %s found %s!\n\r";
        const char* toRoomPrint =
          "$n exclaims, \"Look!  A Secret %s found %s!\"\n\r";
        sstring doorName = fname(fdd->keyword);

        // if the doorname is empty, use a different foundPrint & doorName
        // this is valid if, the door was only opened via room proc, like 'press
        // button' or 'twist lid off'
        if (doorName.empty() || doorName == " ") {
          foundPrint =
            "Secret exit found %s%s, but you can't determine how it is "
            "opened!\n\r";
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You are unable to continue searching while under attack!\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST)
