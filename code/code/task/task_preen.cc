...
int task_preen(TBeing* ch, cmdTypeT cmd, const char* arg, int pulse, TRoom* rp,
  ...
  static const sstring selfPreen[18] = {
    ...
    "You smooth some of your feathers, making them lay flat and shiny.",  // to
                                                                          // self
                                                                          // 1
    "",  // to vict 1
    "$n smoothes some of $s feathers, making them lay flat and shiny.",  // to
                                                                         // room
                                                                         // 1
    "You finish up preening yourself.  You feel nice and clean now.",    // to
                                                                       // self 0
    "",                                           // to vict 0
    "$n finishes up $s preening.",                // to room 0
    "You discard one of your damaged feathers.",  // to self DROP FEATHER
    "",                                           // to vict DROP FEATHER
    "$n discards a damaged feather.  It floats slowly to the $g.",  // to room
                                                                    // DROP
                                                                    // FEATHER
  ...
  static const sstring otherPreen[18] = {
    ...
    "You smooth some of $Ns feathers, making them lay flat and shiny.",  // to
                                                                         // self
                                                                         // 1
    "$n smoothes some of your feathers, making them lay flat and shiny.",  // to
                                                                           // vict
                                                                           // 1
    "$n smoothes some of $Ns feathers, making them lay flat and shiny.",  // to
                                                                          // room
                                                                          // 1
    ...
    "$n plucks a damaged feather from your plumage and discards it.",  // to
                                                                       // vict
                                                                       // DROP
                                                                       // FEATHER
      ...
      if (ch->task->timeLeft <= 0) {
        ...
        if (!target->affectedBySpell(AFFECT_PREENED)) {
          ...
          for (affectedData* hjp = target->affected; hjp; hjp = hjp->next)
            if (hjp->type == AFFECT_PREENED && hjp->location == APPLY_NONE &&
