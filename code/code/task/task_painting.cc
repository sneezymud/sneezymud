...
bool find_paint_target(TBeing* ch, sstring arg, TBeing** tb, TObj** obj,
  ...
  if (arg.empty()) {
    *rp = ch->roomp;
  } else if ((exitp = ch->exitDir(getDirFromChar(arg))) && exitp->to_room &&
             real_roomp(exitp->to_room)) {
...
bool find_paint_supplies(TBeing* ch, TTable** easel, TObj** palette,
  TObj** brushes, TTool** paints, TNote** canvas) {
...
void TBeing::doPaint(sstring arg) {
  ...
  if (!find_paint_target(this, arg, &tb, &obj, &rp)) {
    sendTo(
      "Look, you're no Dali, you better stick to painting things you can "
      "see.\n\r");
    return;
  }
...
int task_painting(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom* rp,
  ...
  switch (cmd) {
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You have not yet mastered the art of fighting while painting.\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST)
