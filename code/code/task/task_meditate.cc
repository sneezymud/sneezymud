...
int task_meditate(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*,
  ...
  switch (cmd) {
      ...
      if (!ch->task->status) {
        if (!ch->roomp->isRoomFlag(ROOM_NO_HEAL)) {
          ...
          if (ch->bSuccess(learn, SKILL_MEDITATE)) {
            ...
            ch->setMana(
              std::min(ch->getMana() + gainAmt, (int)ch->manaLimit()));
          } else {
            // give them resting regen if a fail on meditate
            ch->addToMana(1);
            if (::number(0, 1))
              ch->sendTo(
                "Your meditation has slightly increased your sense of well "
                "being!\n\r");
          }
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You are unable to continue meditation while under attack!\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST) {
