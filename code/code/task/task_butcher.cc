...
const static char* meats[] = {"rib-eye steak", "chuck-eye steak", "skirt steak",
  ...
  "sirloin steak", "tri-tip steak", "chuck steak", "set of ribs",
  "short loin steak", "filet mignon steak"};
...
int TThing::butcherPulse(TBeing* ch, TBaseCorpse* corpse) {
  ...
  if (!steak) {
    ...
    if (robj < 0 || robj >= (signed int)obj_index.size()) {
      vlogf(LOG_BUG, format("butcherPulse: No object (%d) in database!") %
                       Obj::GENERIC_STEAK);
      return false;
    }
    ...
    buf = format("a %s of %s meat") % meats[whichmeat] %
          (tpc ? namebuf : Races[corpse->getCorpseRace()]->getSingularName());
    steak->shortDescr = buf;

    buf = format("A %s of %s meat lies here.") % meats[whichmeat] %
          (tpc ? namebuf : Races[corpse->getCorpseRace()]->getSingularName());
    steak->setDescr(buf);

    *ch += *steak;
    ch->gainTaskExp(SKILL_BUTCHER, corpse->getCorpseLevel(), 1.0, true);
  }
  return FALSE;
}
...
int bareHandsButcherPulse(TBeing* ch, TBaseCorpse* corpse) {
  ...
  if (!steak) {
    ...
    if (robj < 0 || robj >= (signed int)obj_index.size()) {
      vlogf(LOG_BUG,
        format("bareHandsButcherPulse: No object (%d) in database!") %
          Obj::GENERIC_STEAK);
      return false;
    }
    item = read_object(robj, REAL);
    ...
    steak->shortDescr =
      format("a mangled %s of %s meat") % meats[whichmeat] % fromName;
    steak->setDescr(format("A mangled %s of %s meat lies here.") %
                    meats[whichmeat] % fromName);

    *ch += *steak;

    ch->gainTaskExp(SKILL_BUTCHER, corpse->getCorpseLevel(), 1, true);
  }
  return FALSE;
}
...
int task_butchering(TBeing* ch, cmdTypeT cmd, const char*, int pulse, TRoom*,
  ...
  switch (cmd) {
    ...
    case CMD_TASK_FIGHTING:
      ch->sendTo(
        "You are unable to continue butchering while under attack!\n\r");
      ch->stopTask();
      break;
    default:
      if (cmd < MAX_CMD_LIST)
